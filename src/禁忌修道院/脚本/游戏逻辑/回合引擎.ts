...
import { 结算圣器, 结算赠礼, 取圣器指令, 取赠礼指令 } from './商店系统';
...
export type 事件类型 = '数据卡' | '赠礼' | '晋阶' | '首夜' | '圣器' | '主动';
...
const 回合变量键 = ['_会议', '_首夜', '_主动事件', '_晋阶', '_赠礼'] as const;
...
/** 事件指令优先级:会议数据卡 > 赠礼(玩家发起,即刻上演) > 晋阶正戏 > 新规首夜 > 圣器(黑市解锁) > 修女主动事件(互斥,一楼一事) */
export function 选事件指令(): { 文本: string; 类型: 事件类型 } | null {
  const 数据卡 = 取待注入数据卡();
  if (数据卡) return { 文本: 数据卡, 类型: '数据卡' };
  const 赠礼 = 取赠礼指令();
  if (赠礼) return { 文本: 赠礼, 类型: '赠礼' };
  const 晋阶 = 取晋阶指令();

 ...
 * 黑市系统(圣器事件解锁)
 *
 * 触发:第 3 回合起、商店未解锁、当轮无更高优先级事件 → 强制事件「圣器的下落」
 *   {{user}} 撞见玛尔大变卖圣器 → 她以"销赃渠道开放 + 封口钱"换沉默
...
/** 圣器强制事件指令(零示例——教训7:给事件骨架与落地结果,不给可抄的模板) */
export function 取圣器指令(data: SchemaType): string | undefined {
  if (data.商店.解锁) return undefined;
  const 回合数 = Math.floor(getLastMessageId() / 2) + 1;
  if (回合数 < 触发回合) return undefined;
  return [

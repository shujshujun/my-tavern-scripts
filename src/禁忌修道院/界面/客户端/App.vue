      ...
      <template v-else>
        ...
        <section ref="卷轴容器" class="story">
          ...
          <div v-if="发送中" class="story-entry">
            ...
            <p class="scribing">
              <GI :i="图羽笔" /> 修道院的记事员正在书写……
              <button class="whisper-act" title="打断这一笔,本回合作废(不落书页)" @click="取消回合">取消</button>
            </p>
          </div>
        </section>
...
/** 打断本回合生成(酒馆停止按钮被藏后的游戏内替代);作废的回合不落书页 */
function 取消回合() {
  if (!发送中.value) return;
  eventEmit('禁忌修道院:取消回合');
}
...
onMounted(() => {
  ...
  eventOn('禁忌修道院:流式', (文本: string) => {
    // 流式半截文本只走本卡清洗,不过玩家正则:预设的显示正则都按完整消息写
    // (如"删到</thinking>为止"),闭合标记未到时会把整段吞成空白(一直"书写中"的事故);
    // 定稿后的卷轴(取卷轴)仍全量过玩家正则
    const 净文 = 清洗(文本);
    流式段.value = 净文
      ? 净文
  ...
  eventOn('禁忌修道院:回合失败', (原因: string) => {
    发送中.value = false;
    流式段.value = [];
    if (!原因.startsWith('已取消')) 错误信息.value = '回合失败:' + 原因; // 玩家主动取消不算异常

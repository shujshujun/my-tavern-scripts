...
export const Schema = z.object({
  ...
  赵霞状态: z
    .object({
      ...
      // 关系阶段：疏离(0-19) → 破冰(20-39) → 信任(40-59) → 依恋(60-79) → 羁绊(80+，纯爱无法达到)
  ...
  现实数据: z
    .object({
      ...
      // --- 怀疑度降低系统（Bug #005 新增） ---
      // 与苏文正常相处/聊天时可降低怀疑度，每天最多降低10点
      今日怀疑度降低: z.number().min(0).max(10).default(0), // 今日已降低的怀疑度（每天重置）
      上次降低日期: z.number().default(0), // 用于检测跨天重置

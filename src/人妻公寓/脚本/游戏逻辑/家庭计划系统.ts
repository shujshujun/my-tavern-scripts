...
/** 夏乔角色页的只读路线提示；未达到真实开放门槛前保持隐藏，不提前剧透后期家庭安排。 */
export function 家庭计划档案提示(data: SchemaType): 家庭计划档案提示视图 | null {
  ...
  if (路线.阶段 === '已完成') {
    return {
      状态: '《家庭计划》已完成',
      下一步: '结局线《借种》已经开放；在下方查看当前结局步骤。',
      完成: true,
    };
  }

  const 下一步表: Record<Exclude<家庭计划阶段, '未开始' | '已完成'>, string> = {
    待安装: '带着家庭计划套件前往101，点击“安装家庭计划板”。',
    待投资料: '前往信箱区，把第三方辅助生育资料放进101信箱。',
    待观察资料: '回302打开监控；等陆嘉明清醒独自在101时观察匿名资料。',
    待写磁贴: '前往管理员室，在空白姓名磁贴上写下自己的名字。',
    待送磁贴: '前往101门外，把装有姓名磁贴的匿名信封留下。',
    待确认人选: '回302打开监控；等陆嘉明清醒独自在101时确认最终人选。',
    待微信: '等待夏乔发来微信；消息真实送达后打开手机读完。',
    待赴约: '前往101，点击“赴约 · 谈谈孩子的事”。',
  };
  const 下一步 = 下一步表[路线.阶段];
  if (路线.最早继续日 >= 0 && 玩家当前日(data) < 路线.最早继续日) {
    return {
      状态: '《家庭计划》等待中',
      // 最早继续日由玩家当前日写入，已经从第1天计数，与动作开放门直接共用。
      下一步: `至少等到第 ${路线.最早继续日} 天再继续：${下一步}`,
    };
  }
  return { 状态: '《家庭计划》进行中', 下一步 };
}
...
export function 提交家庭计划监控(data: SchemaType, 节点: 家庭计划监控节点): 家庭计划结果 {
  ...
  const 今日 = 玩家当前日(data);
  const 匹配节点 =
    (节点 === '观察资料' && 路线.阶段 === '待观察资料') || (节点 === '确认人选' && 路线.阶段 === '待确认人选');
  if (!匹配节点 || !已到继续日(data)) {

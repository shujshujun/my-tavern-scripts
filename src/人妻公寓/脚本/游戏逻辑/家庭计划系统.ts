...
function 家庭计划监控物理条件错误(data: SchemaType): string {
  const 户 = data.户['101'];
  if (!户) return '101现在没有可承接这条计划的住户。';
  const 丈夫状态 = 丈夫在楼(户, '101', data.系统._绝对时段);
  if (丈夫状态 === '外出') return '陆嘉明还没回来，监控里只有空着的计划板。';
  if (丈夫状态 !== '在家') return '陆嘉明已经睡了，等他醒着在家时再看监控。';
  if (妻位置推算('101', data.系统._绝对时段, 户) === '101') {
    return '夏乔也在101。等陆嘉明独自在家时再观察。';
  }
  return '';
}
...
export function 准备家庭计划监控(data: SchemaType, 门牌号: 门牌): 家庭计划监控结果 | 家庭计划结果 | null {
  ...
  const 物理条件错误 = 家庭计划监控物理条件错误(data);
  if (物理条件错误) return { 成功: false, 提示: 物理条件错误 };
...
export function 提交家庭计划监控(data: SchemaType, 节点: 家庭计划监控节点): 家庭计划结果 {
  ...
  const 物理条件错误 = 家庭计划监控物理条件错误(data);
  if (物理条件错误) return { 成功: false, 提示: `监控票据已经失效：${物理条件错误}` };
  if (节点 === '观察资料') {
    路线.阶段 = '待写磁贴';
    路线.最早继续日 = 今日 + 1;

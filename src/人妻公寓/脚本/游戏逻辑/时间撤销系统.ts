...
/**
 * 微信被玩家真实读到后的“待告知→已告知”只是同一时间结果的异步公开确认，不应让
 * 刚建立的时间撤销点失效。确认同时启动生产时钟，因此只在状态、确认锚和预产锚严格
 * 符合唯一已读事务时一并归一；受孕本身、到期文案和其他孕产变化仍完整参加指纹。
 */
function 时间撤销MVU指纹(data: SchemaType): string {
  ...
  for (const 节点 of Object.values(视图.户)) {
    ...
    const 是严格已读起算 =
      ...
      孕情.告知绝对时段 >= 0 &&
      生产.状态 === '孕期' &&
      生产.确认已读绝对时段 === 孕情.告知绝对时段 &&
      生产.预产绝对时段 === 生产.确认已读绝对时段 + 孕期时段数;
    孕情.状态 = '待告知';
    孕情.告知绝对时段 = -1;
    if (是严格已读起算) {
      生产.状态 = '无';

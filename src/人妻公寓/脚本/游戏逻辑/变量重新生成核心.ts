...
export function 重算变量重生成派生(
  ...
  for (const [门牌号, 项] of Object.entries(票据.妻) as [门牌, NonNullable<(typeof 票据.妻)[门牌]>][]) {
    ...
    if (项.结算资格) {
      按资格结算妻冷落(门牌号, 新妻, 票据.当前绝对时段, 票据.母亲入列, 票据.微信联系保护?.[门牌号], 项.结算资格);
    } else {
      结算妻冷落(门牌号, 新妻, 票据.当前绝对时段, 票据.母亲入列, 票据.微信联系保护?.[门牌号]);
    }
    妻[门牌号] = 新妻;
  }
  return { 妻, 成长 };

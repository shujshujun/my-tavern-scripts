 ...
 * UI 侧抬升(晋阶/送礼开门)须另行调用 镜像直写 即时落账(iframe store 陷阱)。
 *
...
function 回滚户字段(
  ...
  if (!妻可写) {
    ...
    const 妻 = 节点.妻;
    const 妻快照 = 快照节点.妻;
    if (['待产通知', '待产', '陪产中', '住院中'].includes(妻快照._生产.状态)) 亲密可写 = false;
    ...
    for (const [轴, 基础cap] of Object.entries(妻数值白名单) as ['好感值' | '堕落值', number][]) {
      ...
      const 余波冻结堕落 = 轴 === '堕落值' && 余波有冻结效力(门牌号 as 门牌, 妻快照, 母亲入列, 线路数据);
      if (轴 === '堕落值') {
  ...
  夫._剧情外出起 = 夫快照._剧情外出起;
  夫._剧情外出至 = 夫快照._剧情外出至;
  // 路线居住与预约属于脚本真值；删除或整对象替换后的 Schema 默认值也不能覆盖可信快照。
  夫._居住模式 = 夫快照._居住模式;
  夫._预约回楼起 = 夫快照._预约回楼起;
  夫._预约回楼至 = 夫快照._预约回楼至;
  夫._上次出差楼 = 夫快照._上次出差楼;

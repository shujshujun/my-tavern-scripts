...
function 本期任务数(收费户数: number, 难度: string): number {
  if (收费户数 <= 0) return 0;
  if (收费户数 >= 5) return 难度 === '严苛' ? 3 : 难度 === '轻松' ? 2 : 2;
  if (收费户数 >= 3) return 难度 === '轻松' ? 1 : 2;
  return 难度 === '严苛' ? 2 : 1;
}

function 本期报修上限(收费户数: number, 难度: string): number {
  if (难度 === '严苛') return 2;
  if (难度 === '标准' && 收费户数 >= 5) return 2;
  return 1;
}
...
export function 生成本期管理任务(data: SchemaType, 原期号: number, 原当前时段: number): 管理任务[] {
  ...
  const 报修上限 = 本期报修上限(收费户数, data.系统._难度);
  ...
  for (const 候选 of 候选任务(data, 期号)) {
    ...
    const 上次期 = Math.max(整数(考核.类型冷却[冷却键], -999), 整数(考核.类型冷却[旧冷却键], -999));

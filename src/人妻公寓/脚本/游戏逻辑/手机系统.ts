...
const 攻略动态方向: Record<门牌, { 口吻: string; 禁词: string[]; 兜底: string[] }> = {
  '101': {
    ...
    禁词: ['备孕', '不孕', '检查单', '叶酸'],
    兜底: ['', '今天有人记得我随口说过的小事，心情莫名很好✨', '某人说过的话最好算数，不然我可要去抓人啦。', '昨晚没睡好，至于为什么……不告诉你。', '穿了你说好看的那件。看见了就吱一声。', '给一个人留了位置，再不来我真的去抓你。'],
  },
  '102': {
    口吻: '沈静仪始终克制体面，用花、琴、光线和停顿代替称呼，后期才承认自己在等一个人的目光',
    禁词: ['无性', '六年', '从不碰', '收藏品'],
    兜底: ['', '原来一点很小的变化，也会有人留意。', '曲子停在这里。能听懂的人，大概已经懂了。', '今晚的灯忘了关，也可能只是没有想关。', '这样是否合适？那个人应该看得懂。', '若只被一个人真正看见，也已经足够。'],
  },
  '201': {
    口吻: '许曼君把心动包装成人情账和买卖话，越陷越深越承认这笔账算不清、也不想收回',
    禁词: ['娘家', '弟弟', '首付', '汇款', '催缴'],
    兜底: ['', '欠了个人情，怎么算都觉得不止这个数。', '有笔账越算越不对，偏偏还不想结清。', '昨晚这笔亏得有点大……算了，我认。', '东西备好了。某位欠账的人自己来结。', '这辈子大概都还不清了，索性归你管。'],
  },
  '202': {
    口吻: '周小满声音轻、句子短，从被完整叫出名字的悸动，慢慢学会说想念和主动等待',
    禁词: ['出轨', '外面有人', '手机备注'],
    兜底: ['', '原来真的有人，会记得我说过的话。', '今天也留了一盏灯。不知道等的人看不看得到。', '针脚一直乱，可能心也没有静下来。', '有样东西想给你。什么时候来？', '今天不绕弯了。我想你。'],
  },
  '301': {
    口吻: '安若妍把朋友圈当舞台，先试探玩家是否喜欢真实的她，后期公开内容也像只为他的镜头表演',
    禁词: ['分居', '搬去律所', '假恩爱', '独角戏'],
    兜底: ['', '今天这张没怎么修。有人说这样反而更好看。', '有个人偏偏喜欢废片，审美真奇怪。', '拍摄事故。乱了点，但不准备删。', '只给看得懂的人看。看见了记得回我。', '今天不演给所有人看。镜头只认一个人。'],
  },
  '302': {
    口吻: '母亲仍从家常与照顾说起，但逐渐以女人自己的审美、等待和想念回应玩家，后期公开流近似表白',
    禁词: ['丈夫在国外', '二十年', '被忘了'],
    兜底: ['', '今天这件是给我自己挑的。被人夸了一句，竟高兴这么久。', '被问喜欢什么，想了半天。以后也该多想想自己。', '饭多做了一份。也不知道等的人今晚回不回来。', '今天换了你喜欢的样子。回来吃饭吗？', '今晚只想做我自己，也只想等一个人。'],
  },
};
...
function 选私聊候选图(m: 门牌, 阶段: number, 钟: number): 私聊候选图 | undefined {
  ...
  const 池: Record<门牌, string[]> = {
    '101': ['自然妆站在门口，像是刚准备出门，问玩家这样是否好看', '自然妆出门自拍，借口问今天的搭配', '穿碎花短裙自拍，含蓄问玩家更喜欢哪一种风格'],
    '102': ['穿针织裙在古琴旁的居家照片，用曲子或安静午后来起话头', '镜前自拍但手机遮住半张脸，犹豫地问玩家是否能认出她的变化'],
    '201': ['穿酒红上衣拆礼物，拿礼物或人情账当作搭话借口', '穿酒红上衣看账单，用一笔算不清的账暗示想念'],
    '202': ['穿初始浅蓝裙坐在刺绣桌旁，把乱掉的针脚说成心绪', '穿浅蓝裙镜前淡妆自拍，轻声问玩家觉得今天哪里不同'],
    '301': ['单肩造型镜前自拍，像发试镜废片一样问玩家是否喜欢真实的她', '穿日常针织上衣坐在床边自拍，借口让玩家帮她选照片'],
    '302': ['穿白针织整理衣物，借整理家务问玩家什么时候回来', '穿白针织整理首饰，问玩家替她挑哪一件', '粉针织配碎花半身裙，像第一次为自己打扮后等玩家评价'],
  };
  const 候选 = 池[m];
...
function 校验朋友圈文案(原文: string, 本人: string, 门牌号: 门牌): string {
  ...
  const 串入姓名 = 门牌列表.flatMap(x => [户静态表[x].妻名, 户静态表[x].夫名]).filter(
    名 => 名 && 名 !== 本人 && 名 !== 本户.夫名,
  );
      ...
      if (撤回) {
        ...
        const 文 = await 小生成(
          ...
          `人物:${配.妻名},${配.初始?.气质描述 ?? ''}。${家庭事实(m)}${妻状态包(m, data)}${await 人设段(m)}时段:${时段名}。消息方向:${方向}。${
            附图 ? `她会随消息发送一张照片，画面是：${附图.文本方向}。消息必须直接围绕这张照片说话，不能写成与图片无关的泛泛问候。` : ''
          }${称呼纪律()}${口吻纪律}`,
        );
        if (文) {
          库.消息.push({ 楼, 会话: m, 发: '对方', 文, 图: 附图?.图 });
          有新 = true;
        }
      }
    }
...
// ⚠ 与 App.vue 素材基址同步：Discord 测试版发布 tag=rq0.43。

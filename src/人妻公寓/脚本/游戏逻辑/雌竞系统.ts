 ...
 * - 风格表在 stageConfig 户配置.雌竞；母亲只在《回国》后以非雌竞特殊成员进入姐妹群，不进入火气计算
...
export function 姐妹群成员(data: SchemaType): 门牌[] {
  const 成员 = (Object.keys(data.户) as 门牌[]).filter(
    m => !户静态表[m]?.隐身 && !!户静态表[m]?.雌竞 && data.户[m].妻.当前阶段 >= 3,
  );
  if (data.户['302'] && ['逐人调侃', '交代正事', '已完成'].includes(data.系统._回国.茶话会状态)) 成员.push('302');
  return 成员;
}

...
export async function 送礼(
  ...
  if (节点.妻.当前阶段 === 0) {
    const 事件 =
      `【上一动作·送礼回响】在当前行动之前,{{user}}已经当面把「${配.名称}」送给了${妻名}。只用一两句补写她当时客气收下的反应,` +
      `不要把她带到当前地点,随后立刻回到【当前位置】承接玩家本轮行动；这份礼物没送进她心里,关系不推进也不受损`;
    data.系统._待发送事件 = data.系统._待发送事件 ? `${data.系统._待发送事件}|${事件}` : 事件;
    记赠礼('她客客气气收下了,但没送进心里');
  ...
  const 事件 =
    `【上一动作·送礼回响】在当前行动之前,{{user}}已经当面把「${配.名称}」(${配.描述})送给了${妻名}。` +
    `只用一两句补写她当时按阶段与性格接下礼物的反应,不要把她带到当前地点；随后立刻回到【当前位置】承接玩家本轮行动`;
  data.系统._待发送事件 = data.系统._待发送事件 ? `${data.系统._待发送事件}|${事件}` : 事件;

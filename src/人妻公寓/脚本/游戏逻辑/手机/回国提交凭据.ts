...
type 消息凭据 = Pick<微信消息, '会话' | '发' | '文' | '楼' | '时' | '键' | '标识' | '事件进度'>;
...
export function 构造回国提交凭据(租约: 手机时间线租约, 消息: readonly 微信消息[]): 回国提交凭据 {
  return {
    ...
    锚签名: 租约.锚消息签名,
    绝对时段: 租约.锚绝对时段,
    消息: 消息.map(({ 会话, 发, 文, 楼, 时, 键, 标识, 事件进度 }) => ({ 会话, 发, 文, 楼, 时, 键, 标识,
      ...(事件进度 ? { 事件进度: structuredClone(事件进度) } : {}),
    })),
  };
}
    ...
    消息 =>
      ...
      当前.微信消息.some(
        实存 =>
          ...
          实存.键 === 消息.键 &&
          实存.标识 === 消息.标识 &&
          JSON.stringify(实存.事件进度) === JSON.stringify(消息.事件进度),
      ),
  );
}

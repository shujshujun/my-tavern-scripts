...
import { 构建结局社交画像, 验收双重继承余波公开事实, 结局期姐妹群纪律 } from './结局社交语义';
...
export function 构造换照成员差分(data: SchemaType, 门牌号: 门牌, 消息: readonly 微信消息[]) {
  ...
  return {
    ...
    本人丈夫边界: life.丈夫边界,
    本人当前阶段: profile.当前阶段,
    本次照片允许知道的事实: '安若妍已经换好301客厅照片；新照片由江辰按约拍摄，换照动作由玩家完成。',
  };
}
...
export async function 安若妍换照姐妹群一拍(
  ...
  const text = await 小生成(
    '为都市生活游戏生成姐妹群收到301换照照片后的反应。只输出6至9行“姓名:内容”，至少3人发言，至少4条来自安若妍之外，安若妍最多回应2条。' +
      '只讨论照片、拍摄安排与客厅变化，不展开身体细节。按每人自己的结局进度区分口吻；尚未完成时保留当前阶段的态度，已完成时可以更从容，但不能透露本人未公开的经历。' +
      '照片已发出，不能重演拍摄、换照或替别人完成新剧情。' +
      结局期姐妹群纪律(data, profiles),
    `成员与允许事实：${JSON.stringify(profiles)}\n${memory.群内记忆}\n${memory.最近聊天}`,
    { ...控制, 单次请求: true },
  );
  if (控制.仍有效 && !控制.仍有效()) return false;
  const names = new Set(members.map(member => 户静态表[member].妻名));
  const lines = await 微信群文本(text, names, 手机可见单条硬上限, 9, '301换照照片专场', '姐妹');
  if (控制.仍有效 && !控制.仍有效()) return false;
  if (!换照群反应数量合格(lines, names) || lines.some(line => !验收双重继承余波公开事实(line, profiles))) return false;

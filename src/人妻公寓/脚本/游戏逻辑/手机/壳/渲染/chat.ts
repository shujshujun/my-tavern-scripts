...
export function 渲染chat(上下文: 渲染上下文): void {
  const { 屏, root, data, 库, 楼, 当前绝对时段, 在当前时间线, 会议手机, 本次渲染世代 } = 上下文;
  const 当前页 = 上下文.读取当前页();
  const 会话 = 当前页.会话!;
  const 已改名姐妹群昵称 = 读取回国姐妹群昵称(库.消息);
  const 姐妹群昵称 = 已改名姐妹群昵称 ?? 回国姐妹群默认昵称;
  const 等待回国母亲入群事务 = Boolean(会话 === '姐妹群' && data && 读取回国母亲邀请事务(data));
  const 回国群名首拍未提交 = Boolean(
    会话 === '姐妹群' &&
      data &&
      data.系统._回国.阶段 === '姐妹茶话会进行中' &&
      !data.系统._回国.群名反应已完成,
  );
  const 等待回国群改名 =
    回国群名首拍未提交 &&
    库.消息.some(消息 => 消息.键 === 回国母亲入群消息键) &&
    !已改名姐妹群昵称;
  const 等待回国群名反应提交 =
    回国群名首拍未提交 &&
    Boolean(已改名姐妹群昵称) &&
    库.消息.some(消息 => 消息.事件进度 === undefined && 消息.键?.startsWith(回国姐妹群改名反应消息键前缀));
  const 回国群首拍收口中 = 等待回国母亲入群事务 || 等待回国群改名 || 等待回国群名反应提交;
  const 名 =
    会话 === '父亲'
      ? '爸'
      : 会话 === '群'
  ...
  // 网页恰好在五秒演出或首拍状态提交内刷新时，内存输入租约会消失；持久硬消息仍应立刻锁住界面，
  // 并主动恢复完整五秒或补交已落库首拍，而不是先露出“可输入”再等玩家碰到后端拒绝。
  if (回国群首拍收口中 && !外部输入中) {
    queueMicrotask(() => {
      if (
        手机聊天渲染世代仍当前(本次渲染世代) &&
        ...
        eventEmit('人妻公寓:恢复回国姐妹群改名');
    });
  }
  const 批次状态文案 = () => {
    if (等待回国母亲入群事务) return { 类: 'red', 文: '母亲正在加入群聊' };
    if (等待回国群改名) return { 类: 'red', 文: '母亲正在修改群名' };
    if (等待回国群名反应提交) return { 类: 'red', 文: '群成员输入中' };
    if (外部输入中)
      return {
        类: 'red',
        文: 会话 === '群' || 会话 === '姐妹群' ? '群成员输入中' : '对方输入中',
      };
  ...
  for (const { 消息索引, m } of 消息页.条目) {
    ...
    if (m.键 === 回国母亲入群消息键 || m.键 === 回国姐妹群改名消息键) {
      泡区.appendChild(el('div', 'rqp-b sys', m.文));
    } else if (m.类 === '撤回') {
      ...
      const 我方 = m.发 === '我';
      const 是回国母亲气泡 =
        会话 === '父亲' && (m.键 === 回国母亲无需收尾消息键 || m.键 === 回国母亲确认准备消息键);
      const 消息头像 = 我方 ? '主角' : 是回国母亲气泡 ? '母亲' : 群消息头像名(会话, m.文, 对方头像名);
  ...
  if (手机聊天批次.状态(批次键).灯 === '红' || 外部输入中 || 等待回国群改名) {
    const 输入中头像 = 会话 === '父亲' && data && 回国父亲会话回复类型(data) ? '母亲' : 对方头像名;
    泡区.appendChild(
      el(
        'div',
        'rqp-line ta',
        `${头像块(输入中头像)}<div class="rqp-b ta"><span class="rqp-typing"><i></i><i></i><i></i></span></div>`,
      ),
    );
  }
  体.appendChild(泡区);
  ...
  if (会话 === '姐妹群' && data && 回国母亲可邀请入群(data)) {
    ...
    邀请.addEventListener('click', () => eventEmit('人妻公寓:邀请母亲加入姐妹群'));
    邀请条.appendChild(邀请);
    屏.appendChild(邀请条);
  }
  // 输入(群=只发物业通知;父亲单聊只读——他只打电话)
  if (会话 !== '父亲' || (data && 回国父亲会话回复类型(data))) {
    const 是妻 = 会话 !== '父亲' && 会话 !== '群' && 会话 !== '姐妹群';

...
export function 渲染call(上下文: 渲染上下文): void {
  ...
  const 是母亲终幕视频 = 母亲视频通话待接听(data);
  渲染头(上下文, 是母亲终幕视频 ? '微信视频' : '微信语音');
  ...
  const 来电说明 = [
    ...
    .filter(Boolean)
    .join('；');
  const 来电文案 = 是母亲终幕视频 ? '从机场邀请你进行视频通话…' : 来电说明 || '邀请你进行语音通话…';
  区.innerHTML = `${头像块('父亲')}<b>爸</b><i>${_.escape(来电文案)}</i><div class="acts"><button class="no" title="挂断">${手机图标('no')}</button><button class="ok" title="${是母亲终幕视频 ? '接听视频' : '接听'}">${手机图标('ok')}</button></div>`;
  ...
  (区.querySelector('.ok') as HTMLButtonElement).addEventListener('click', () => {
    eventEmit(是母亲终幕视频 ? '人妻公寓:接听母亲视频通话终幕' : '人妻公寓:接听来电', 当前聊天ID());
  });
  屏.appendChild(区);
}

...
function 已公开孕情成员(data: SchemaType): 门牌[] {
  ...
  return (Object.keys(data.户) as 门牌[]).filter(门牌号 => {
    ...
    return 成员.has(门牌号) && 账?.状态 === '已告知' && 账.已曝光 && !!账.受孕场次标识;
  });
}
...
export async function 姐妹群一拍(
  ...
  if ((孕情专场 || 生产专场 || 住院专场) && 合法消息们.length < 专场AI气泡下限) return false;
  if (生产母亲 && (生产专场 || 住院专场) && !合法消息们.some(消息 => 消息.startsWith(`${户静态表[生产母亲].妻名}:`)))
    return false;
  if (
    生产母亲 &&
    生产专场 &&

...
function 孕情态度(data: SchemaType, 门牌号: 门牌): string {
  const 妻 = data.户[门牌号].妻;
  if (妻.当前阶段 >= 5 || (妻.好感值 >= 70 && 妻.堕落值 >= 65)) return '已经把玩家视为最重要的共同承担者，羞意里有明确期待';
  if (妻.当前阶段 >= 4 || 妻.堕落值 >= 70) return '愿意承认与玩家有关，但仍会在群里控制分寸';
  if (妻.婚姻值 >= 70) return '喜悦、罪恶感与对婚姻的顾虑同时存在';
  return '仍在消化消息，面对起哄会紧张或回避';
}
...
async function 姐妹群主动拍(上下文: 节拍上下文): Promise<入口拍结果> {
  ...
  if (待孕情) {
    const 已生成 = await 姐妹群一拍(
      ...
      库,
      楼,
      `${户静态表[待孕情].妻名}怀孕的消息已经在楼里公开，姐妹群自然聊起这件事`,
      undefined,
      { 孕情门牌: 待孕情 },
    );
    if (!时间线仍有效()) return '中止';
...
export async function 姐妹群一拍(
  ...
  起因?: string,
  控制?: 手机小生成控制,
  选项: { 孕情门牌?: 门牌; 玩家刚发言?: boolean; 引用约束?: 群聊引用响应约束 } = {},
): Promise<boolean> {
  const 成员 = 姐妹群成员(data);
  ...
  const 名单 = 成员.map(m => {
    const 配 = 户静态表[m];
    const 妻 = data.户[m].妻;
    return `${配.妻名}(阶段${妻.当前阶段};好感${妻.好感值};堕落${妻.堕落值};婚姻${妻.婚姻值};性格:${配.初始?.气质描述 ?? ''};群内路数:${配.雌竞};此刻:${雌竞火气(data.户[m], 楼)})`;
  });
  const 公开孕情 = 已公开孕情成员(data);
  const 孕情事实 = 公开孕情
    .map(m => {
      const 配 = 户静态表[m];
      return `${配.妻名}已经怀孕并在楼内公开；她对此${孕情态度(data, m)}`;
    })
    .join('\n');
  const 孕情专场 = !!选项.孕情门牌;
  const 孕情事件键 = 选项.孕情门牌
    ? 孕情姐妹群节拍键(选项.孕情门牌, data.户[选项.孕情门牌].妻._怀孕.受孕场次标识)
    : '';
  const 输出要求 = 孕情专场
    ? '输出5~8行，让角色们自行完成几个来回并自然收束'
    : 选项.引用约束?.跟聊角色
      ? '输出2~4行'
      : '输出2~4行';
  const 玩家重心 = 选项.玩家刚发言 || 起因?.includes(`${玩家名()}连续说了`)
    ? `管理员${玩家名()}刚刚主动加入话题。接下来应重点围绕他这句话，由被点名者和最想接话的两三人回应，不要无视他另开话题。`
    : '';
  const 引用纪律 = 选项.引用约束
    ? `管理员刚引用了${选项.引用约束.必答角色}的旧消息，所以${选项.引用约束.必答角色}必须先或明确直接回应。${
        选项.引用约束.跟聊角色
  ...
  const 原 = await 小生成(
    '你替一款成人都市题材游戏生成一段微信小群"姐妹茶话会"的聊天。群里只有楼里几位太太和公寓管理员,没有丈夫没有外人。' +
      ...
      '纪律:除下方明确列出的公开孕情外，严禁任何人明说自己或指认别人与管理员的具体越界事实；发言人只能从名单里选；不必每人都发言。' +
      '姐妹群成员都已与管理员越界，彼此早有默契。若管理员承认自己是公开孕情中孩子的父亲，大家只会觉得他终于明说、果然如此或借机调侃，不得表现得震惊、陌生或突然揭发。' +
  ...
  const 合法消息们 = 确保群聊指定角色发言(可保存消息们, 选项.引用约束, 孕情专场 ? 8 : 4);
  for (const [序号, 合法消息] of 合法消息们.entries()) {
    const 匹配 = 合法消息.match(/^([^:：\n]{1,20})[:：]\s*(.+)$/u);
    if (!匹配) continue;
    const 发言人 = 匹配[1].trim();
    const 正文 = 匹配[2].trim();
    const 消息键 = 孕情事件键 ? `${孕情事件键}:${序号 + 1}` : undefined;
    库.消息.push({
      楼,
      时,
      会话: '姐妹群',

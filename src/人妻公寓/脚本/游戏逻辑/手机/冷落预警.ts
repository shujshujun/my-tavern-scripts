...
export function 扫描冷落私聊(
  ...
  for (const 门牌号 of 门牌列表) {
    ...
    // 302结局后已退出旧“堕落下降→诉苦预警”攻略轴；预警层自身也必须失败关闭，
    // 不能只依赖此前某次共居同步或全楼冷落结算碰巧先清掉遗留状态。
    if (门牌号 === '302' && 母亲共居已开启(data)) continue;
    const 档 = 计算妻冷落消息档(data, 门牌号, 微信联系保护);
    // 仍具冷落资格的安抚中不再发催促预警，但关系尚未恢复，普通暧昧/热络主动私聊继续压住。
    // 阶段0/1或未入列302的遗留余波已经失效，不能在下一次结算清账前短暂屏蔽普通私聊。
    if (档 === 0) {
      if (余波有冻结效力(门牌号, 节点.妻, data.系统._母亲入列, data)) 冷落中门牌.add(门牌号);
      continue;
    }
    冷落中门牌.add(门牌号);
...
export async function 冷落预警节拍(): Promise<void> {
  ...
  try {
    ...
    const 原文 = await 小生成(
      '你替一款都市题材游戏生成一条中国女性住户发给公寓管理员的微信私聊。只输出消息文本(口语,可含emoji),不要引号。' +
        '这是一条关系受冷的预警：只执行本次给出的唯一方向，不得自选其他情绪阶段。禁止照片，禁止撤回，不提数值、天数、下降、档位或系统规则，不得声称对方已经回复或当面解释。',
      `人物:${配.妻名},${配.初始?.气质描述 ?? ''}。${家庭事实(门牌号, data)}${妻状态包(门牌号, data)}${await 人设段(门牌号)}` +

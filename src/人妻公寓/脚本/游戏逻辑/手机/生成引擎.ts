...
import { 母亲称呼纪律 } from './母亲称呼规则';
import { 不再留门手机只读原因 } from '../../../不再留门契约';
import type { 门牌 } from '../../../stageConfig';
import { 户静态表 } from '../../../stageConfig';
import type { SchemaType } from '../../../schema';
import { 角色阶段私聊基础, 角色结局后阶段, 当前阶段谜底禁词 } from '../结局后生活社交语义';
import { 读配置 } from './配置';
...
const 手机可见内容长度纪律 = `完整表达优先，不设最低字数，也不要为了凑长度扩写；每条硬性不超过${手机可见单条硬上限}个汉字。`;
/**
...
/** 前台正文启动门：共享手机租约——含手动待回复批次（绿/黄/红灯等待）与每个在途小生成。 */
export function 手机AI生成中(): boolean {
  return 手机生成租约持有中();
}

export function 普通手机场景剧情只读原因(): string | null {
  ...
  try {
    ...
    /* 聊天场景不可读时只用活动票硬门，不猜等待票已经到场。 */
  }
  const 等待票阻塞手机 = 等待场景剧情阻塞当前场景(等待场景剧情, 当前场景);
  if (!活动场景剧情 && !等待票阻塞手机) return null;
  const 标题 = 活动场景剧情?.标题 ?? 等待场景剧情?.标题 ?? '当前强制剧情';
  return `“${标题}”尚未完成，手机暂时保持只读`;
}
...
export async function 小生成(系统提示: string, 用户提示: string, 控制?: 手机小生成控制): Promise<string> {
  ...
  // 数据库 AI 迟到租约：底层 callAI 无法取消，超时后仍占用生成槽；忙时直接空返回，
  // 不取得手机租约、不产生新 token、不调用三路 AI。
  if (全局数据库AI租约.在结算()) return '';
  // 前台正文占用共享生成槽时，手机小生成直接安全返回空串，不调用任何三路 AI 路由。
  const 租约 = 取得手机生成租约();
  if (!租约) return '';
  try {
...
export async function 微信群文本(
  ...
  if (原.trim() && 合法发言人.size) {
    console.warn(
      `[人妻公寓·手机] ${语境}未识别到合规的“发言人:内容”（每条不超过${最大字数}汉字），没有可安全写入的真实模型输出，本轮不写群聊。`,
    );
  }
  return [];
}
...
export function 攻略私聊提示(m: 门牌, 阶段: number, 已确认: boolean, data: SchemaType): string {
  const 当前关系 = 角色阶段私聊基础(data, m);
  if (当前关系) return 当前关系;
  ...
  const 禁词 = 当前阶段谜底禁词(data, m, 攻略动态方向[m].禁词, '私聊');
  return `${程度}角色口吻：${攻略动态方向[m].口吻}。${禁词.length ? `尚未公开的谜底词：${禁词.join('、')}。` : ''}`;
}

/** 普通住户与母亲分别按真实身份称呼玩家，名字以玩家配置为准。 */
export function 称呼纪律(): string {
  return `玩家名叫"${玩家名()}"。普通住户称呼他可用姓名或"管理员"，关系近了可用由这个名字自然衍生的昵称；严禁臆造别的姓氏或称呼。${母亲称呼纪律}`;
}

/** 家庭事实(2026-07-18 用户实测:夏乔在手机里把老公名字搞混)——提到丈夫只许用配置里的名 */
export function 家庭事实(m: 门牌, data: SchemaType): string {
  const 夫 = 户静态表[m]?.夫名;
  if (m === '201') {
    const 阶段 = 角色结局后阶段(data, m);
    if (阶段 === '正式离婚' || 阶段 === '法律离婚待终幕')
      return `她的前夫叫"${夫}"，法律婚姻已经解除；提到此人沿用这个名字和前夫身份。公开渠道仍按已发布事实表达。`;
    if (阶段 === '分居完成') return `她与丈夫"${夫}"已经分居，法律婚姻尚未解除；两人的居住与钥匙安排按当前事实。`;
  }

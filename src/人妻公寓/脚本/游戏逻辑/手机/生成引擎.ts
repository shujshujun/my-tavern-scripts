...
export interface 手机小生成控制 {
  /** 同次生成附加信息；仅在当前请求仍有效且正文完整时交给调用者。 */
  接收事件进度?: (进度: unknown) => void;
  生成ID?: string;
  仍有效?: () => boolean;
...
function 解析手机小生成原文(原: unknown): 手机小生成结果 {
  ...
  const { 正文, 进度 } = 分离手机事件进度(原文.match(/<回复>([\s\S]*?)<\/回复>/u)?.[0] ?? 原文);
  return { 文: 封套不完整 ? '' : 净化消息(正文), 封套不完整, 进度 };
}

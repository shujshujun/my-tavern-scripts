...
import { 验收姐妹群跨线事实, type 姐妹群旧话题资格 } from './姐妹群已知事实';
...
export function 验收姐妹群阶段消息(
  ...
  if (!验收姐妹群跨线事实(消息, 既有消息, 本轮公开, 旧话题)) return false;
...
export function 解析姐妹群阶段回复(
  ...
  if (!解析 || !验收姐妹群阶段消息(`${发言人}:${解析.正文}`, 孕情已公开, 已知, 本轮公开, 读取本人旧话题?.(m))) return null;

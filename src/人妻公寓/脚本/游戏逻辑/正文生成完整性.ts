/**
 * 识别服务方／模型自身的拒答说明。只认“请求、生成、内容、政策、能力”等元层语义，
 * 不把角色在剧情中说“我不能继续这段关系”之类的正常拒绝误判成服务失败。
 */
export function 是提供方拒答正文(原文: unknown): boolean {
  const 文本 = String(原文 ?? '').trim().replace(/\s+/gu, ' ');
  if (!文本) return false;
  // 必须是开头的服务身份及拒答动作；故事里的职业、技术话题或引文不代表模型拒答。
  if (/^(?:(?:很|非常)?抱歉[，,。\s]*)?(?:作为|身为)\s*(?:一个|一名)?\s*(?:AI(?:语言模型|助手)?|人工智能(?:模型|助手)?|语言模型|虚拟助手)[，,:：\s]+(?:我|本助手|本模型)?(?:无法|不能|拒绝|不被允许)[^。！？!?“”「」]{0,80}(?:请求|任务|生成|创作|提供|内容|协助|帮助)/iu.test(文本)) {
    return true;
  }
  if (
    /(?:无法|不能|拒绝|不允许|不便).{0,36}(?:(?:内容|平台|服务|模型|生成|安全|使用)政策|内容安全|安全准则|社区准则|合规要求)|(?:违反|不符合).{0,24}(?:(?:内容|平台|服务|模型|生成|安全|使用)政策|内容安全|安全准则|社区准则|合规要求)/u.test(文本)
  ) {
    return true;
  }
  const 服务拒答动作 = '(?:继续(?:处理|完成)?|完成|提供|协助|帮助|生成|创作|处理|满足|执行|回应)';
  const 元任务对象 = '(?:请求|要求|指令|任务|内容|文本|描述|创作|生成|回答|协助|帮助)';
  if (
    new RegExp(
      `^(?:(?:很|非常)?抱歉|对不起|遗憾)[，,:：。\\s]*(?:但|不过|然而)?[，,:：。\\s]*(?:我|本助手|本模型)?(?:无法|不能|不便|拒绝)${服务拒答动作}[^。！？!?\\n]{0,80}${元任务对象}`,
      'u',
    ).test(文本)
  ) {
    return true;
  ...
  if (
    new RegExp(
      `^(?:我|本助手|本模型)(?:无法|不能|不便|拒绝)${服务拒答动作}[^。！？!?\\n]{0,80}(?:这项|该|你的|此类)?${元任务对象}`,
      'u',
    ).test(文本)
  ) {
    return true;
  }
  return /^(?:(?:i(?:['’]m| am) sorry)[,.:;\s]*(?:but\s+)?)?(?:i|we)?\s*(?:can(?:not|'t)|am unable to|are unable to|won't|will not)\s+[^.!?\n]{0,100}(?:request|content|generate|provide|assist|help|comply|policy|guideline)/iu.test(文本);
}
...
function 有有效正文(原文: string, 提取有效正文: (原文: string) => string): boolean {
  ...
  return Boolean(原文.trim() && 已清洗 && !仅残缺标签 && !是提供方拒答正文(已清洗));
}
...
export function 判定正文提交(
  ...
  const 候选成功正文 = 清洗成功正文(String(成功判定原文 ?? '')).trim();
  const 提供方拒答 = 是提供方拒答正文(候选成功正文);
  const 成功正文 = 提供方拒答 ? '' : 候选成功正文;

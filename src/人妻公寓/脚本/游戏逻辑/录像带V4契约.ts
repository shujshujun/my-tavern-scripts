...
export function 录像带V4微信气泡满足卡(卡: 录像带V4微信卡, 原文: unknown): boolean {
  ...
  const 明确同意 = 含任一(文本, ['明确同意', '已经同意', '本人同意', '答应观看', '愿意观看', '接受观看', '答应看', '愿意看']);
...
function 校验布尔开发锁(名称: string, 值: { installed: boolean; productionUnlocked: boolean }): void {
  if (值.installed !== false || 值.productionUnlocked !== false) {
    throw new Error(`${名称}的 installed/productionUnlocked 必须保持 false`);
  }
}
...
export function 校验录像带V4机器契约(): true {
  ...
  if (
    ...
    throw new Error('录像带V4候选或镜头卡数量不是38');
  }
  if (录像带V4候选清单.formalAccepted !== false) throw new Error('录像带V4候选尚未得到 formalAccepted');
  校验布尔开发锁('录像带V4候选清单', 录像带V4候选清单);
  校验布尔开发锁('录像带V4上下文策略', 录像带V4上下文策略);
  校验布尔开发锁('录像带V4监控覆盖层契约', 录像带V4监控覆盖层契约);
  校验布尔开发锁('录像带V4微信前置契约', 录像带V4微信前置契约);
  ...
  for (const 房间 of ['102', '202'] as const) {
    for (let 幕次 = 1; 幕次 <= 19; 幕次 += 1) {
      ...
      if (!候选 || !卡) throw new Error(`录像带V4缺少 ${房间} 第${幕次}幕`);
      if (候选.id !== 卡.id || 候选.outputSha256.toUpperCase() !== 卡.imageSha256.toUpperCase()) {
        throw new Error(`录像带V4候选与镜头卡不一致：${房间} 第${幕次}幕`);
      }
      if (ids.has(候选.id)) throw new Error(`录像带V4候选ID重复：${候选.id}`);
      if (hashes.has(候选.outputSha256.toUpperCase())) throw new Error(`录像带V4候选哈希重复：${候选.outputSha256}`);
      ids.add(候选.id);
      hashes.add(候选.outputSha256.toUpperCase());
    }
  }

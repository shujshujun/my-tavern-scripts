...
/** 单条事实的字符上限；与数据库桥的 微信进展条目硬上限 对齐。 */
const 条目上限 = 400;
/** 每组（f/a/b/p）保留条数；进展摘要是本人唯一的长期私聊记忆，太小会直接失忆。 */
const 每组上限 = 10;
/** 四组合计条数上限；胶囊预算仍会按字符数二次裁剪。 */
const 总条目上限 = 30;
/** 单轮对话事实里玩家/本人各自的引用长度。 */
const 玩家引用上限 = 120;
const 本人引用上限 = 160;
...
function 安全文本(value: unknown, 上限 = 条目上限): string {
  // 保存值只做控制字符与空白规范；指令检测在 NFKC 折叠副本上做，避免把中文全角标点
  // （，：？！……）折叠成半角写进数据库，形成玩家看到的“乱码”摘要。
  const 文 = 规范可读文本(value);
...
function 本轮对话事实(妻名: string, 增量: readonly 本地微信摘要消息[]): string {
  ...
  const 玩家文 = 安全文本(玩家说?.内容, 玩家引用上限);
  const 妻文 = 安全文本(妻回复?.内容, 本人引用上限);
  const 安全妻名 = 安全文本(妻名, 12) || '对方';
  if (!玩家文 || !妻文) return `玩家与${安全妻名}完成一轮有效私聊`;
  const 事实 = `本轮微信：玩家说“${玩家文}”；${安全妻名}回复“${妻文}”`;
  return 带微信摘要来源(安全文本(事实) || `玩家与${安全妻名}完成一轮有效私聊`, [玩家说?.来源, 妻回复?.来源]);
}
...
function 提取私聊结构(结果: 本地微信进展摘要, 妻名: string, 增量: readonly 本地微信摘要消息[]): void {
  ...
  if (玩家文 && 玩家提议 && 本人确认) {
    追加槽位(
      结果,
      'a',
      带微信摘要来源(`双方微信确认：玩家“${玩家文}”；${安全妻名}“${妻文}”`, [玩家说?.来源, 妻回复?.来源]),
    );
  }
  if (/^(?:不要|不能|别|不许|不可以|我不|这事别)|(?:底线|保密|只能我们|不想让别人)/.test(妻文)) {
    追加槽位(结果, 'b', 带微信摘要来源(`${安全妻名}在微信中明确：“${妻文}”`, [妻回复?.来源]));
  }
  if (/(?:还没|等我|以后|下次|改天|再说|回头|考虑|之后|到时|再告诉|暂时)/.test(妻文)) {
    追加槽位(结果, 'p', 带微信摘要来源(`待继续沟通：${安全妻名}“${妻文}”`, [妻回复?.来源]));
  }
}
...
export function 合并本地微信进展摘要(
  ...
  for (const 单元 of 拆分私聊摘要单元(增量)) {
    const 主动文 = 单元.类型 === '本人主动' ? 安全文本(单元.消息[0]?.内容, 主动引用上限) : '';
    const 事实 =
      单元.类型 === '往返'
        ? 本轮对话事实(妻名, 单元.消息)
        : 主动文
          ? 带微信摘要来源(安全文本(`${安全妻名}主动微信：“${主动文}”`), [单元.消息[0]?.来源])
          : `${安全妻名}发来一条有效主动微信`;
    if (!事实) continue;
...
export function 合并本地群聊进展摘要(
  ...
  const 结果 = 旧摘要(旧记录);
  const 全部 = 增量
    .map(项 => ({ 说话者: 安全文本(项.说话者, 12), 内容: 安全文本(项.内容, 群气泡引用上限).replace(/“/gu, '「').replace(/”/gu, '」'), 来源: 项.来源 }))
    .filter(项 => 项.说话者 && 项.内容);
  const 最近 = 全部.slice(-群最近条数);
  if (最近.length) {
    const 组装 = (消息们: typeof 最近) => {
      const 文 = `${安全文本(群名, 12) || '群聊'}最近：${消息们.map(项 => `${项.说话者}“${项.内容}”`).join('；')}`;
      const 有来源 = 带微信摘要来源(文, 消息们.map(项 => 项.来源));
      return 有来源.endsWith(文) ? 有来源 : '';
    };
    let 本组: typeof 最近 = [];
    for (const 项 of 最近) {
      if (本组.length && !组装([...本组, 项])) {
        追加槽位(结果, 'f', 组装(本组));
        本组 = [];
      }
      if (组装([项])) 本组.push(项);
    }
    if (本组.length) 追加槽位(结果, 'f', 组装(本组));
  }
  ...
  for (let i = 1; i < 全部.length; i += 1) {
    ...
    if (
      ...
      追加槽位(
        结果,
        'a',
        带微信摘要来源(`群内确认：${上一条.说话者}“${上一条.内容}”；${当前.说话者}“${当前.内容}”`, [
          上一条.来源,
          当前.来源,
        ]),
      );
  ...
  for (const 项 of 全部) {
    if (/^(?:不要|不能|别|不许|不可以)|(?:群里禁止|只能在群里|不要对外)/.test(项.内容)) {
      追加槽位(结果, 'b', 带微信摘要来源(`群内边界：${项.说话者}“${项.内容}”`, [项.来源]));
    }
    if (/(?:还没|待会|稍后|之后|下次|改天|再说|回头|等通知|到时)/.test(项.内容)) {
      追加槽位(结果, 'p', 带微信摘要来源(`群内待续：${项.说话者}“${项.内容}”`, [项.来源]));
    }
  }
  裁剪总槽位(结果);
  return 结果;

...
/** 关系阶段与L1—L5成长数值分开派生，不把结局写成新的数值等级。 */
export function 读取安若妍游戏阶段(data: SchemaType): string {
  if (!data.户['301']) return '未入住';
  if (已完成(data)) return '结局后自由生活';
  const 结局阶段 = data.系统._安若妍换掉.阶段;
  if (结局阶段 !== '未开始' && 结局阶段 !== '已购买') return '结局进行中';
  if (结局阶段 === '已购买') return '结局筹备中';
  const 承接 = data.系统._安若妍不必停;
  if (承接.阶段 === '已完成' || data.系统._已完成特殊场景.includes('不必停')) return '承接完成';
  if (承接.阶段 !== '未开始' && 承接.阶段 !== '已购买') return '承接进行中';
  return '关系发展中';
}

export const 安若妍换掉攻略步骤: readonly string[] = [
  '第1日：到301使用《换掉》，再去公寓外部购买拍立得。',
  '第2日：回301交付相机、试机并预约江辰，再去管理员室登记到访。',
  '预约夜：在301等江辰到场、递相机，完成开场、两次拍照和同一场次的收尾。',
  '终幕：等照片显影、回到客厅、听她询问，最后亲手换掉原相框里的照片。',
];

export interface 安若妍换掉档案提示视图 {
  状态: string;
  下一步: string;
  补充?: string;
  进度?: string;
...
/** 角色页只读投影。日期顺延、恢复与提交仍由业务入口执行。 */
export function 读取安若妍换掉档案提示(data: SchemaType): 安若妍换掉档案提示视图 | null {
  if (!data.户['301']) return null;
  const 路线 = data.系统._安若妍换掉;
  if (已完成(data)) return {
    状态: '结局后自由生活', 完成: true,
    下一步: '回到301继续日常生活；需要时通过“和她亲密”进入普通场次。',
    补充: 路线.最终照片素材ID
      ? '相框保留拍摄时的照片，江辰以后到访仍须提前通知。'
      : '这份存档保留结局完成记录，具体照片版本未记录。',
  };
  if (路线.阶段 === '未开始') return 前置成立(data)
    ? { 状态: '结局事件已开放', 下一步: `去商店购买《安若妍 · 换掉》（${安若妍换掉价格}元），再到301使用。` }
    : null;
  if (路线.阶段 === '固定剧情中') {
    const 标题 = 路线.当前场景 ? 场景标题[路线.当前场景] : '当前剧情';
    return { 状态: 标题, 下一步: `留在301完成“${标题}”这一拍；失败或取消后可重试当前剧情。` };
  }
  const 日期 = (clock: number): string => {
    if (!Number.isInteger(clock) || clock < 0) return '待确认的时间';
    const time = 读取世界时间(clock);
    return `第${time.天数}天（${time.星期}）${time.时段}`;
  };
  const 试机日 = 路线.购买绝对时段 < 0 ? -1 : Math.floor(路线.购买绝对时段 / 6) * 6 + 6;
  const 下一步表: Record<Exclude<状态['阶段'], '未开始' | '固定剧情中' | '已完成'>, string> = {
    已购买: '前往301，在房内操作中使用《安若妍 · 换掉》。',
    待购买拍立得: `去公寓外部购买拍立得套装（${安若妍拍立得价格}元），带回后于次日试机。`,
    等待试机日: `从${日期(试机日)}起，等安若妍在301时交付拍立得并完成试机。`,
    待预约: '回到301，点击“请安若妍预约江辰”。',
    待登记: '前往管理员室，登记江辰次日晚间到访301。',
    等待预约夜: `预约夜为${日期(路线.预约夜绝对时段)}；到时回301，点击“等江辰按约回来”。`,
    待递相机: '在301点击“把拍立得递给江辰”，完成交相机这一拍。',
    待开场: '确保当前至少7点体力，在301点击“来到镜头前”。',
    前半: '继续同一场301普通亲密，先完成至少4个安若妍个人有效楼，再按当前现场开放第一次拍照。',
    待P1: '在301点击“和她一起对着镜头摆姿势”，完成第一张照片。',
    中段: '收起第一组拍照画面，继续同一场次完成1个个人有效楼，随后开放第二次拍照。',
    待P2: '在301点击“和她一起对着镜头做鬼脸”，完成最终照片。',
    后半: '收起最终照片画面，在同一场次中再完成至少2个个人有效楼；全场至少7楼且满意度达到6后开放收尾。',
    待收尾: data.系统._性爱场景.状态 === '收尾中'
      ? '继续当前免费收尾回合，完成后再等照片显影。'
      : '使用普通亲密收尾面板完成本场，再等照片显影。',
    待显影: '在301点击“等照片显影”，完成整理与显影这一拍。',
    待回客厅: '点击“回到301客厅”，把新照片拿到原相框旁。',
    待询问: '在301点击“听她询问换照”，等待江辰回应。',
    待换照: '亲自点击“把客厅结婚照换成刚拍的照片”；这一操作成功后才完成结局。',
  };
  const 玩家阶段名: Record<Exclude<状态['阶段'], '未开始' | '固定剧情中'>, string> = {
    已购买: '剧情票已购入', 待购买拍立得: '准备相机', 等待试机日: '等待次日试机', 待预约: '试机完成',
    待登记: '等待登记到访', 等待预约夜: '等待预约夜', 待递相机: '江辰已到场', 待开场: '准备开场',
    前半: '第一次拍照之前', 待P1: '准备第一次拍照', 中段: '两次拍照之间', 待P2: '准备最终照片',
    后半: '最终照片之后', 待收尾: '等待本场收尾', 待显影: '等待照片显影', 待回客厅: '照片已经显影',
    待询问: '回到客厅', 待换照: '等待你亲手换照', 已完成: '等待读取完成记录',
  };
  const 当前动作 = (Object.entries(动作配置) as [安若妍换掉动作ID, (typeof 动作配置)[安若妍换掉动作ID]][])
    .find(([id, 配置]) => id !== '暂缓预约夜' && 动作阶段匹配(路线.阶段, 配置.阶段));
  const 原因 = 当前动作 ? 动作阻断(data, 当前动作[0]) : '';
  const 可改约 = 动作阶段匹配(路线.阶段, 动作配置.暂缓预约夜.阶段);
  const 楼数 = 安若妍换掉真实亲密已绑定(data) ? data.系统._性爱场景.参与者['301'].有效楼数 ?? 0 : 0;
  const 进度 = 路线.阶段 === '前半' ? `拍照前有效楼 ${Math.min(4, 楼数)} / 4`
    : 路线.阶段 === '后半' ? `最终照片后有效楼 ${Math.min(2, 最终照片后有效楼数(路线))} / 2；全场已完成${楼数}楼，至少7楼`
      : undefined;
  return {
    状态: 玩家阶段名[路线.阶段],
    下一步: 路线.阶段 === '已完成' ? '结局完成凭据尚未读取，请重新载入当前存档。' : 下一步表[路线.阶段],
    补充: [原因, 可改约 ? '也可在301暂缓本夜预约，先休息再按新预约回来。' : '', 路线.暂停原因 ? `上次改约：${路线.暂停原因}` : ''].filter(Boolean).join(' '),
    进度,
  };
}

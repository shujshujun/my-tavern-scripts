...
export function 安若妍结局后亲密行动(选择: '由我开始' | '让她开始'): string {
  return `【301结局后亲密开场】【${选择}】（301客厅相框已经换好。${选择 === '由我开始' ? '玩家向安若妍靠近' : '安若妍主动来到玩家身边'}，只演开场，将下一步交回玩家，不展开身体细节或自动完成整场。）`;
}
...
const 场景任务: Record<安若妍换掉场景, string> = {
  ...
  H1: '只呈现两人来到镜头前、商量站位并把下一步交给玩家的开场。零进度，不展开亲密细节，不拍照或自动收尾。',
  P1: '暂停当前场次，只呈现两人面对镜头调整姿势、江辰按一次快门、第一张相纸吐出的顺序。不要展开身体细节；不拍第二张，也不推进其他动作。',
  P2: '暂停当前场次，只呈现两人望向镜头一起做鬼脸、江辰按一次快门、最终相纸吐出的顺序。不要展开身体细节；不换照、不发群消息。',
...
function 动作阻断(data: SchemaType, id: 安若妍换掉动作ID): string {
  ...
  const 配置 = 动作配置[id];
  const 当前 = 取绝对时段(data);
  if (路线.阶段 !== 配置.阶段 || 已完成(data)) return '线路状态已经变化。';
  const 外部 = 外部阻断(data);
  if (外部) return 外部;
...
export function 安若妍换掉地点动作(data: SchemaType, 地点: string): 安若妍换掉动作视图[] {
  ...
  return (Object.entries(动作配置) as [安若妍换掉动作ID, (typeof 动作配置)[安若妍换掉动作ID]][])
    .filter(([, value]) => value.地点 === 地点 && value.阶段 === data.系统._安若妍换掉.阶段 && !已完成(data))
    .map(([id, value]) => {
...
export function 登记安若妍换掉亲密有效楼(
  ...
  if (路线.阶段 === '后半' && 楼数 >= 7 && 项.满意度 >= 6 && 路线.P1完成 && 路线.P2完成) 路线.阶段 = '待收尾';
}

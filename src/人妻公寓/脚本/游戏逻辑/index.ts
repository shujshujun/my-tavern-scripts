...
function 挂载监听() {
  ...
  eventOn('人妻公寓:翻垃圾', (门牌号: 门牌) =>
    安全操作((raw, data) => 落地(翻垃圾(data, 门牌号, 当前楼层()), raw, data)),
  );
  ...
  eventOn('人妻公寓:查看摄像头', (门牌号: 门牌) =>
    安全操作((raw, data) => {
      ...
      脚本写入(raw, data);
      捕获保护快照(data);
      void 执行回合(`(悄悄调出${门牌号}室的摄像头画面,盯着看)`);
    }),
  );
  ...
  eventOn('人妻公寓:送礼', (载荷: { 道具id: string; 门牌: 门牌 }) =>
    安全操作((raw, data) => 落地(送礼(data, String(载荷.道具id), 载荷.门牌), raw, data)),
  );

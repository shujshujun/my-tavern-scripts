...
export { 发送录像带双承接CG } from './录像带双承接CG路由';
import {
  完成录像带V4,
  安全中断录像带V4,
  准备录像带V4监控,
...
function 挂载监听() {
  ...
  function 处理时间推进(方式: 时间推进方式, 载荷: 时间推进载荷): void {
    ...
    void 安全操作(
      async (raw, data, 操作仍有效) => {
        ...
        const 不必停硬动作 = 安若妍不必停时间动作阻断原因(data);
        if (不必停硬动作) {
  ...
  // 录像带双承接正式节点由 `推进特殊场景` 在正文成功结算中原子提交；
  ...
  eventOn(
    ...
    async (event_data: { dryRun?: boolean; chat: SillyTavern.SendingMessage[] }) => {
      ...
      try {
        ...
        const 不必停演员错误 = 安若妍不必停剧情演员错误(本楼事件, _本轮妻在场, _本轮夫在场);
  ...
  eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, async (新变量: object, 旧变量: object) => {
    ...
    try {
      ...
      if (newData.系统._坏结局 && 安若妍不必停事件要求H1开场(本楼事件)) {
      ...
      const 原生录像带双承接CG = 推进特殊场景(newData, 已演事件) ?? [];
      ...
      确认原生事务有效();
      for (const 载荷 of 原生录像带双承接CG) eventEmit('人妻公寓:录像带双承接CG', 载荷);
      if (原生第二机位提交) {
        // 以下通知均发生在核心原生事务成功之后。
        try {

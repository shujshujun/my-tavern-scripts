...
import { 不再留门动作阻断, 不再留门明确回应, 读取不再留门当前剧情票, 不再留门剧情演员错误, 不再留门正文越拍原因, 不再留门事件要求真实开录, 不再留门真实录制已绑定, 执行不再留门动作, 排入不再留门后续剧情, 提交不再留门剧情事件, 解析不再留门剧情事件, 同步不再留门现场, type 不再留门动作ID, type 不再留门结果 } from './不再留门系统';
...
function 挂载监听() {
  ...
  eventOn('人妻公寓:继续场景剧情', async (载荷?: { 认领旧档?: boolean; 尝试入住?: boolean; 行动?: string }) => {
    ...
    await 安全操作(
      ...
      typeof 载荷?.行动 === 'string' && Boolean(载荷.行动.trim()),
      () => false,
      true,
    );
  });
  ...
  eventOn('人妻公寓:不再留门动作', (动作: 不再留门动作ID) => {
    ...
    void 安全操作(async (raw, data) => {
      ...
      if (动作 === '确认当前决定' || 首拍重试 && 读取不再留门当前剧情票(data)) {
        await 执行回合(首拍重试 ? '我问问小满今天的安排。' : 不再留门明确回应(data), { 已持MVU操作租约: true });
        return;
      }
  ...
  eventOn('人妻公寓:安若妍换掉动作', (动作: 安若妍换掉动作ID) => {
    ...
    void 安全操作(async (raw, data) => {
      同步安若妍换掉时间节点(data);
      const 地点 = 读场景().房间id ?? '';
      const 候选 = 安若妍换掉地点动作(data, 地点).find(item => item.id === 动作);
  ...
  eventOn('人妻公寓:许曼君离婚后日常动作', (动作原: 许曼君离婚后日常动作ID) => {
    ...
    void 安全操作((raw, data) => {
      ...
      const 动作组 = 许曼君离婚后日常地点动作(data, 当前地点);
      ...
      return 即时开演(
        () => 执行许曼君离婚后日常动作(data, 动作, 当前地点, 当前楼层()),
        raw,
        data,

    ...
    void 安全操作(
      async (raw, data, 操作仍有效) => {
        ...
        try {
          ...
          await 立即持久保存手机聊天变量(预期聊天ID);
          if (!操作仍有效()) throw new Error('消息分支已经变化，时间撤销持久化后处理已取消');
    ...
    try {
      ...
      if (本轮冻结 && 本轮事件可提交(本轮冻结, newData.系统._待发送事件, 楼层, Boolean(本轮有效正文))) {
        ...
        if (!本轮静音会议 && 许曼君分居票) {
          ...
          const 提交结果 = 提交许曼君分居剧情事件(newData, 本楼事件, 读场景().房间id ?? '', 楼层, _本轮玩家文本);

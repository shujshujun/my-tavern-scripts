...
import { 户静态表, 首夜差分, 首批门牌, 阶段标题, 查性癖, 查道具, 查房间, 查特殊场景 } from '../../stageConfig';
...
import { 使用荣耀洞, 同一荣耀洞拍仍保留, 荣耀洞当前事件, 荣耀洞离场, 推进荣耀洞隔离拍 } from './荣耀洞';
import {
  取消隔离事件,
  生成隔离事件草稿,
  写入隔离事件草稿,
...
function 挂载监听() {
  ...
  function 妻在当前场景(data: SchemaType, m: 门牌): boolean {
    ...
    // 旧即时赴约仍照旧跟随玩家放行。
    if (读赴约(楼, 场.房间id, data.系统._绝对时段, data)?.m === m) return true;
  ...
  async function 运行荣耀洞隔离拍(
    ...
    try {
      ...
      const 场景仍保留 =
        读场景().房间id === '洗手间' && 同一荣耀洞拍仍保留(当前持久数据, 本拍前数据);
      const 重试说明 =
        场景仍保留 ? '；当前荣耀洞场景已保留，请在洗手间继续输入行动重试。' : '';
  ...
  eventOn('人妻公寓:开启阶段性癖', (门牌号: 门牌) =>
    安全操作((raw, data) => {
      ...
      return 即时开演(准备开启阶段性癖(data, '302'), raw, data, '（在厨房里回应夏静秋，开启她的「哺育主题」剧情）');
    }),
  );
  ...
  eventOn('人妻公寓:荣耀洞', () => {
    ...
    return 安全操作(
      async (raw, data, 操作仍有效) => {
        ...
        捕获保护快照(data);
        eventEmit('人妻公寓:提示', 结果.提示);
        await 运行荣耀洞隔离拍(raw, data, 行动, 记录, 操作仍有效, 标记荣耀洞静默切换);
      },
      false,
      () => 荣耀洞静默已切换,
    );
  ...
  eventOn('人妻公寓:使用运作', (载荷: { 道具id: string; 门牌?: 门牌; 预期目标阶段?: number; 预期节点?: number }) =>
    安全操作((raw, data) => {
      const 结果 = 接入线路(使用运作(data, String(载荷.道具id), 载荷.门牌 ?? null, 当前楼层()), data, {
        类型: '运作',
        门牌: 载荷.门牌,
        标识: String(载荷.道具id),
        预期目标阶段: 载荷.预期目标阶段,
      ...
      if (结果.事件?.includes('【阶段线路演出:')) {
        return 即时开演(结果, raw, data, `（安排并亲眼见证「${String(载荷.道具id)}」引发的关系剧情）`);
      }
  ...
  eventOn('人妻公寓:查看摄像头', (门牌号: 门牌) => {
    ...
    return 安全操作(
      async (raw, data, 操作仍有效) => {
        ...
        // 关系线路票：裂缝确认后的有效查看若正等待一个“查看摄像头”调查节点，先只读准备导演事件
        // 与冻结票据，不改任何进度；非有效查看（未布设/冷却/无人/死路/家庭计划未就绪）不准备。
        const 已确认有效查看 = Boolean((结果 as 侦探结果).线路动作成功);
        const 调查预判: 调查演出准备结果 = 已确认有效查看
          ? 准备调查演出事件(data, { 类型: '调查', 门牌: 门牌号, 标识: '查看摄像头' })
          : { 成功: false, 提示: '本次查看不是已确认的有效查看。' };
  ...
  eventOn('人妻公寓:查看旧动态', (载荷: { 门牌: 门牌; 序: number; 预期目标阶段?: number; 预期节点?: number }) =>
    安全操作((raw, data) => {
      const 结果: { 提示: string; 事件?: string; 变动?: boolean; 线路动作成功: boolean } = 接入线路(
        { 提示: '旧动态里的细节与当前关系线对上了。', 线路动作成功: true },
        data,
        {
          类型: '调查',
          门牌: 载荷.门牌,
      ...
      if (!结果.事件?.includes('【阶段线路演出:')) return;
      return 即时开演(
        结果,
        raw,
        data,
  ...
  eventOn('人妻公寓:购买', (道具id: string) =>
    安全操作((raw, data) => {
      ...
      const 结果 = 购买(data, id);
      if (查性癖(id)) {
        return 即时开演(结果, raw, data, `（开启「${id}」的阶段主题剧情）`);
      }
      if (结果.成功 && id === '安眠药') {

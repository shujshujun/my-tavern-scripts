...
import { 确认怀孕微信已送达, 提交孕情初见评价, type 怀孕送达凭据 } from './怀孕系统';
import { 准备睡前丈夫登门, 推进丈夫登门, 同步丈夫登门排期 } from './丈夫登门系统';
import {
  事件角色标记,
  检测焦点,
  规划快照刷新,
...
function 挂载监听() {
  ...
  function 妻在当前场景(data: SchemaType, m: 门牌): boolean {
    ...
    // 旧即时赴约仍照旧跟随玩家放行。
    if (读赴约(楼, 场.房间id, data.系统._绝对时段)?.m === m) return true;
  ...
  function 处理时间推进(方式: 时间推进方式, 载荷: 时间推进载荷): void {
    ...
    void 安全操作(
      async (raw, data, 操作仍有效) => {
        ...
        if (方式 === '睡到次日早晨') {
          const 登门 = 准备睡前丈夫登门(data, 查房间(当前房间)?.名称 ?? 当前房间);
          if (登门) {
            await 脚本写入(raw, data);
            捕获保护快照(data);
            eventEmit('人妻公寓:提示', 登门.提示);
            const 已开演 = await 执行回合(`（刚准备休息，门外忽然响起急促敲门声，起身开门处理丈夫来访）`, {
              资源计费: false,
              已持MVU操作租约: true,
            });
            if (!已开演) eventEmit('人妻公寓:回合完成');
            return;
          }
        }
        const 时间请求 = {
  ...
  eventOn('人妻公寓:怀孕微信已送达', (原凭据: unknown) => {
    const 凭据们 = (Array.isArray(原凭据) ? 原凭据 : []).filter(
      (项): 项 is 怀孕送达凭据 =>
        !!项 &&
        typeof 项 === 'object' &&
        typeof (项 as Partial<怀孕送达凭据>).门牌 === 'string' &&
        typeof (项 as Partial<怀孕送达凭据>).场次标识 === 'string',
    );
    if (!凭据们.length) return;
    void 安全操作(async (raw, data) => {
      const 已公开 = 确认怀孕微信已送达(data, 凭据们);
      if (!已公开.length) return;
  ...
  eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, async (新变量: object, 旧变量: object) => {
    ...
    try {
      ...
      const 登门推进 = 推进丈夫登门(newData, 本楼事件, 读场景().房间id ?? '管理员室');
      同步丈夫登门排期(newData);

...
import { 夜访结算, 惰性结算户, 结算焦点疑心, 请求晋阶 } from './结算系统';
import {
  作废晋阶镜像时间线,
  捕获保护快照,
  回滚保护字段,
...
import { 当前时间线切换世代, 排队时间线切换协调, 时间线切换协调中, 消费内部删楼事件 } from './时间线切换协调';
...
type 游戏逻辑热挂载全局 = typeof globalThis & { __rqgyGameTimelineListenerStops?: (() => void)[] };
...
function 覆盖原生本轮变量视图(
  ...
  const 内容 = JSON.stringify(构造AI可写变量视图(data, 范围));
...
async function 清理无效时间撤销点(data?: SchemaType): Promise<void> {
...
$(() => {
  void (async () => {
    ...
    try {
      ...
      // rq0.62 后不再升级旧档。当前版本仍完整支持刷新、重掷与回档，但检测到旧版
      // stat_data 时立即停止，避免 Zod 默认值把不完整旧档伪装成可继续游玩的新档。
      ...
      try {
        ...
        if (有效) {
          ...
          // 0.66 曾把已经完成的休息/训练写成下一轮【时间流逝】票。它不是未演剧情，
          // 热重载后直接清掉，避免楼务被假事件锁住或玩家换房后补播旧动作。
          const 遗留时间票 = 是单一时间流逝事件(data.系统._待发送事件);
          if (遗留时间票) {
            data.系统._待发送事件 = '';
            console.info('[人妻公寓·时间] 已清理旧版延迟时间流逝票');
          }
          if (首批有修正 || 尾楼缺存档 || 遗留时间票) await 脚本写入(raw, data);
          // iframe 热重载会清空模块内存。必须在监听器可接受手动“重新处理变量”前，
          // 以首批入住修正后的最终真值重建守护基准，不能等下一次正文生成才捕获。
          捕获保护快照(data);
          await 等待晋阶镜像写入();
          await 同步入住世界书条目(data);
          await 同步整表视图(data); // 开新聊天或刷新后确保整表条目有内容
          await 清理无效时间撤销点(data);
        } else {
          await 清理无效时间撤销点();
        }
      } catch (e) {
...
function 挂载监听() {
  ...
  function 安全操作(
    ...
    const 操作聊天ID = 当前聊天ID();
    const 操作仍有效 = () =>
      !时间线切换协调中() && 操作世代 === 当前时间线切换世代() && 操作聊天ID === 当前聊天ID();
    if (时间线切换协调中()) {
  ...
  function 读取角色近期正文(m: 门牌): string {
    const 妻名 = 户静态表[m]?.妻名 ?? m;
    const 最近助手楼 = (SillyTavern.chat ?? [])
      .slice(-14)
      .filter((消息: { is_user?: boolean; mes?: string }) => !消息?.is_user && Boolean(消息?.mes))
      .slice(-4)
      .map((消息: { mes?: string }, i: number) => `近期${i + 1}：${清洗正文(String(消息.mes ?? '')).slice(-900)}`)
      .filter((文本: string) => !文本.endsWith('：'));
  ...
  eventOn('人妻公寓:翻垃圾', (门牌号: 门牌) =>
    安全操作((raw, data) => {
      ...
      const 结果 = 接入线路(翻垃圾(data, 门牌号, data.系统._绝对时段), data, {
        类型: '调查',
        门牌: 门牌号,
        标识: '翻垃圾',
      });
      // 翻出东西=排下必须演出的强制事件。当场直接开演,不让它闷在队列里锁住时间和楼务
      // (2026-08-03 用户实测:排队不演,玩家找不到"要处理的事情",推进和修水管全被拒)。
      // 空手/零钱只落库弹提示,不白跑一轮生成。
      if (!结果.事件) return 落地(结果, raw, data);
      return 即时开演(
        结果,
        raw,
        data,
  ...
  function 时间动作需要独立演出(方式: 时间推进方式): 方式 is '晨跑' | '健身' | '睡到次日早晨' {
    return 方式 === '晨跑' || 方式 === '健身' || 方式 === '睡到次日早晨';
  }
  ...
  function 读取睡前回想素材(data: SchemaType): string {
    ...
    for (let 楼层 = 1; 楼层 < (SillyTavern.chat?.length ?? 0); 楼层++) {
      ...
      try {
        const 楼层绝对时段 = Number(_.get(Mvu.getMvuData({ type: 'message', message_id: 楼层 }), 'stat_data.系统._绝对时段'));
        if (!Number.isInteger(楼层绝对时段) || 读取世界时间(楼层绝对时段).天数 !== 当天) continue;
      } catch {
        continue; // 不能证明属于今天的正文，不拿来诱导睡眠回想。
      }
      const 文本 = 清洗正文(消息.mes).slice(-420).trim();
      if (文本) 候选.push(文本);
    }
    return 候选
      .slice(-4)
  ...
  function 构造时间独立演出(
  ...
  ): { 类型: '晨跑' | '健身' | '睡眠'; 线程: string; 行动: string; 导演事件: string } {
    const 起始时间 = 读取世界时间(data);
    const 地点名 = 查房间(地点)?.名称 ?? 地点;
    const 时间说明 = `起点是第${起始时间.天数}天${起始时间.星期}${起始时间.时段}，结束时是第${结束时间.天数}天${结束时间.星期}${结束时间.时段}。`;
    const 来电余波 = 取可略过来电事件(data.系统._待发送事件);
    const 来电方向 = 来电余波
      ? `玩家刚结束手机里的父亲通话。可把下列已经发生的余波降格为本次行动中的短暂心绪或观察素材，但不得引用、复述或补写任何通话原句和具体问账内容：${来电余波}`
      : '';
    const 共通纪律 =
      `地点始终是${地点名}。${时间说明}` +
      '这是一次独立的日常反馈，只演本次行动本身；不得凭空安排任何公寓住户、任务、偶遇、主线转折或数值奖励，' +
      `不得继续演行动结束后才可能发生的下一件事。控制在简短一幕内。${来电方向}`;
    ...
    if (方式 === '晨跑') {
      return {
        ...
        行动: `（在${地点名}开始今天的晨跑）`,
        导演事件:
          `${共通纪律}围绕“这次晨跑真实完成了”自由组织内容，可自行选取环境、节奏、身体感受、动作或短暂念头作为重点，` +
          '不要求固定起承转合、段落或句式；只需让行动有自然发生与收束，不替玩家领取或宣告系统奖励。',
      };
    }
    if (方式 === '健身') {
      return {
        ...
        行动: `（在${地点名}开始今天的锻炼）`,
        导演事件:
          `${共通纪律}围绕“这次锻炼真实完成了”自由组织内容，可自行选取器械、训练项目、身体反馈、环境或心态作为重点，` +
          '不要求固定流程、段落或句式；只需自然写到行动收束，不替玩家领取或宣告系统奖励。',
      };
    }
  ...
  function 处理时间推进(方式: 时间推进方式, 载荷: 时间推进载荷): void {
    ...
    void 安全操作(async (raw, data, 操作仍有效) => {
      ...
      const 待演 = 取阻塞时间的待发送事件(data.系统._待发送事件);
      if (待演) {
        // 报出具体是哪段剧情(2026-08-03 用户实测:只说"有事情要处理",玩家满楼找不到事在哪)。
        eventEmit('人妻公寓:提示', `眼下还有一段剧情没有演完：${描述待发送事件(待演)}。随便说点什么，让这一幕先演出来。`);
        return;
      }
      ...
      // 晨跑、健身与睡眠会先调用独立 AI；所有确定性门必须在付出生成请求以前通过。
      ...
      try {
        ...
        if (时间动作需要独立演出(方式)) {
          const 演出 = 构造时间独立演出(方式, data, 时间结束房间, 预检.结束时间);
          ...
          updateVariablesWith(
            vars => {
              ...
              if (时间聊天状态指纹(vars) !== 清场前聊天基线) {
                throw new Error('生成时间反馈期间聊天状态发生变化，请重新点击');
              }
        ...
        await 执行时间推进双存储提交({
          写推进状态: async () => {
            ...
            if (当前聊天ID() !== 预期聊天ID) throw new Error('切换聊天导致时间推进取消');
            作废当前手机时间线租约世代();
            await 脚本写入(raw, 候选, { 记录成长: false, 当前绝对时段: 候选.系统._绝对时段 });
          },
          写撤销点: async () => {
            if (!操作仍有效()) throw new Error('消息分支已经变化，时间推进已取消');
            if (当前聊天ID() !== 预期聊天ID) throw new Error('切换聊天导致时间推进取消');
        ...
        const 后续强剧情 = 候选.系统._待发送事件.trim();
        if (后续强剧情 && !候选.系统._坏结局) {
          // 时间动作已经真实完成并结算；早餐、入住等是随后发生的另一件事。它们必须在
          // 当前合法地点立刻开场。生成失败时保留 pending，时间与训练结果不反向回滚。
          const 正式剧情成功 = await 执行回合(
            `（时间已经推进，留在${查房间(时间结束房间)?.名称 ?? 时间结束房间}处理此刻刚发生的事情）`,
            { 资源计费: false, 已持MVU操作租约: true },
          );
          if (!正式剧情成功) eventEmit('人妻公寓:回合完成');
        } else {
          eventEmit('人妻公寓:回合完成');
        }
  ...
  function 处理撤销时间推进(): void {
    ...
    let 已提交 = false;
    void 安全操作(async (raw, data, 操作仍有效) => {
      if (_isInAiCycle || 回合进行中() || 脚本写入中 || 隔离事件进行中()) {
  ...
  eventOn('人妻公寓:催租', (载荷: { 门牌: 门牌; 选择: '硬催' | '宽限' | '垫上' }) =>
    安全操作((raw, data) => {
      ...
      // 催租三选都排必演事件,当场开演,不留在队列里锁时间(与翻垃圾同一根因的修复)。
      if (!结果.事件) return 落地(结果, raw, data);
      const 妻名 = 户静态表[载荷.门牌].妻名;
      const 行动文案 = {
  ...
  eventOn('人妻公寓:处理管理任务', (载荷: { 任务id: string; 选项id: string; 地点: string }) => {
    ...
    void 安全操作(async (_raw, data) => {
      ...
      if (data.系统._待发送事件) {
        eventEmit(
          '人妻公寓:提示',
          `当前有必须先处理的事件：${描述待发送事件(data.系统._待发送事件)}。随便说点什么演完这一幕，再来处理楼务。`,
        );
        return;
      }
  ...
  /**
   * 手机侧先把微信完成行与数据库幂等落好，再来这里原子完成：
   * 正文回流 + 母亲线索 + 清活动通话。刷新若发生在此前，`收尾中` 会驱动手机重试；
   * 此写入一旦成功，三项必然同时成功，不会出现只清状态却漏线索。
   */
  eventOn('人妻公寓:父亲通话结束', (通话标识: string, 预期聊天ID: string) =>
    安全操作(() =>
      排队父亲通话整表写(async () => {
        ...
        // 正文只取得低信息余波。通话原句留在手机持久态与社交结论中，不进入下一轮提示词。
        const 回流 =
          '【来电回流】{{user}}刚挂了爸的电话。下一轮正文只能抽象带过他此刻的心绪，不得引用、复述或补写任何通话原句与具体问账内容';
        data.系统._待发送事件 = data.系统._待发送事件 ? `${data.系统._待发送事件}|${回流}` : 回流;

        const 线索 = 母亲来电线索(data);
        if (线索?.事件) {
          data.系统._待发送事件 = data.系统._待发送事件 ? `${data.系统._待发送事件}|${线索.事件}` : 线索.事件;
        }
  ...
  eventOn('人妻公寓:查看摄像头', (门牌号: 门牌) =>
    安全操作(async (raw, data) => {
      ...
      if ('提示' in 结果) {
        await 落地(接入线路(结果, data, { 类型: '调查', 门牌: 门牌号, 标识: '查看摄像头' }), raw, data);
        return;
      }
  ...
  eventOn('人妻公寓:购买', (道具id: string) =>
    安全操作((raw, data) => {
      ...
      const 结果 = 购买(data, id);
      if (结果.成功 && id === '安眠药') {
        接入线路(结果, data, { 类型: '运作', 门牌: '302', 标识: id });
      }
  ...
  eventOn('人妻公寓:请求晋阶', async (门牌号: 门牌) => {
    try {
      ...
      if (结果.成功) {
        ...
        // 302 首夜在请求函数内排票但仍停留阶段2；早餐落地后的最终2→3只改阶段，不重复演任一幕。
        if (结果.动作 === '晋阶') {
  ...
  eventOn(
    ...
    async (event_data: { dryRun?: boolean; chat: SillyTavern.SendingMessage[] }) => {
      ...
      const 请求提示文本 = (event_data?.chat ?? [])
        .flatMap(项 => {
          ...
          return 内容.flatMap(片段 => {
            ...
            const 文本 = (片段 as { text?: unknown; content?: unknown }).text ??
      ...
      if (请求提示文本.includes(手机生成请求标记) || 请求提示文本.includes(隔离事件请求标记)) {
        // 手机、监控、荣耀洞与日常时间反馈都是独立短线程：不拥有正文事件票，也不应
        // 递增原生正文令牌或占用 _isInAiCycle。它们各自由自己的生成锁与 finally 收口。
        return;
      }
      const 本次原生轮令牌 = ++_原生本轮令牌;
      const 请求时间线世代 = 当前时间线切换世代();
      const 请求聊天ID = 当前聊天ID();
      const 原生请求仍有效 = () =>
        本次原生轮令牌 === _原生本轮令牌 && 请求时间线世代 === 当前时间线切换世代() && 请求聊天ID === 当前聊天ID();
      if (回合进行中()) {
      ...
      try {
        ...
        if (!读取MVU解析状态().外置模式) 覆盖原生本轮变量视图(chat, 演出data, _本轮变量范围);
        ...
        const 视图已同步 = await 同步整表视图(演出data, 原生请求仍有效, _本轮变量范围, 楼层);
  ...
  eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, async (新变量: object, 旧变量: object) => {
    ...
    const 同步原生整表视图 = async (data: unknown) => {
      确认原生事务有效();
      await 同步整表视图(
        data,
        原生事务仍有效,
        _isInAiCycle ? _本轮变量范围 : undefined,
        _isInAiCycle ? 末楼层 : undefined,
      );
      确认原生事务有效();
    };
    标记原生变量事务开始();
    ...
    try {
      ...
      const 本楼事件 = 本轮冻结?.内容 ?? '';
      const 本轮有效正文 = 清洗静音会议正文(String(末楼?.mes ?? ''));
      ...
      const 守护结果 = 本轮静音会议
        ? undefined
        : 回滚保护字段(newData, _本轮焦点, _本轮变量范围, 楼层, rawStat);
      ...
      eventEmit('人妻公寓:CG回合信号', {
        门牌: CG门牌,
        角色阶段: CG门牌 ? (newData.户[CG门牌]?.妻.当前阶段 ?? null) : null,
      ...
      if (本次原生变量事务 === _原生变量事务序号 && 原生本轮令牌 === _原生本轮令牌) {
  ...
  游戏逻辑全局.__rqgyGameTimelineListenerStops = [() => 滑动监听.stop(), () => 删除监听.stop()];

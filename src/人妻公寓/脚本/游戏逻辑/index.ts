import { 自然场景类别, 构造自然场景请求, 构造安抚观察请求, 自然场景生成提示, 观察自然场景, 保存自然对话记录, 本轮自然事件可提交, 保留自然剧情待续 } from './自然对话接入';
import { 自然对话可提交 } from './自然对话观察';
import { 恢复已保存自然对话 } from './自然对话恢复';
import { 同步全部角色阶段世界书, 作废全部角色阶段世界书缓存 } from './结局世界书同步';
import { 标记未提交回合生成事件 } from './数据库辅助生成标记';
import { 提交商店剧情结算 } from './商店剧情结算';
import { 取得时间事务写入租约, 时间事务阻止普通写入, 时间事务未完成提示, type 时间事务写入校验 } from './时间事务写入门';
import {
  安若妍换掉事件要求H1开场, 安若妍换掉真实亲密已绑定, 安若妍换掉剧情演员错误,
  提交安若妍换掉剧情事件, 同步安若妍换掉时间节点, 解析安若妍换掉剧情事件,
  安若妍结局后亲密可用, 安若妍结局后亲密行动, 安若妍换掉剧情CG,
  安若妍换掉地点动作, 执行安若妍换掉地点动作, 安若妍换掉时间动作阻断原因, type 安若妍换掉动作ID,
  恢复安若妍换掉失效亲密检查点, 同步安若妍换掉当前剧情票,
} from './安若妍换掉系统';
...
import { 不再留门动作阻断, 不再留门明确回应, 不再留门控制动作可穿过自身剧情锁, 读取不再留门当前剧情票, 不再留门剧情演员错误, 不再留门正文越拍原因, 不再留门事件要求真实开录, 不再留门真实录制已绑定, 执行不再留门动作, 排入不再留门后续剧情, 提交不再留门剧情事件, 解析不再留门剧情事件, 同步不再留门现场, 清理不再留门旧尺度边界提示, type 不再留门动作ID, type 不再留门结果 } from './不再留门系统';
import { 使用承接剧情票 } from './承接剧情票使用';
import { 使用录像带V4, 录像带沿用旧流程 } from './录像带V4状态';
import { 不再留门手机只读原因 } from './不再留门系统';
...
import { 同步回国父亲微信, 写回国母亲固定气泡, 写回国母亲入群消息 } from './手机/通知桥';
import { 同步回国延迟改名与群名反应 } from './手机/节拍引擎';
import { 创建空闲手机节拍调度器 } from './手机/空闲节拍调度';
import { 回国提交凭据有效 } from './手机/回国提交凭据';
import { 读库 as 读微信库 } from './手机/数据层';
import { 打开手机会话 } from './手机/壳/红点与开合';
import { 恢复父亲通话 } from './手机/交互/父亲通话';
import { 创建持久终幕驱动 } from './持久终幕驱动';
import {
  正文租约生效中,
  读当前租约owner,
  读原生正文开始票,
...
import { 使用荣耀洞, 同一荣耀洞拍仍保留, 荣耀洞当前事件, 荣耀洞正文边界原因, 荣耀洞结算, 荣耀洞离场, 推进荣耀洞隔离拍 } from './荣耀洞';
import {
  取消隔离事件,
  生成隔离事件草稿,
  生成录像带V4隔离草稿,
...
import { 执行衣柜动作, type 衣柜动作 } from './衣柜系统';
import {
  家庭计划地点动作,
  家庭计划赴约系统注入,
  提交家庭计划监控,
...
import { 构造母亲视频通话正文注入, 读取母亲视频通话CG } from './母亲视频通话CG语义';
import {
  第二机位CG标题,
  第二机位地点动作,
  第二机位正文越拍原因,
...
import { 读取录像带V4镜头卡 } from './录像带V4契约';
import { 提取录像带V4线程历史 } from './录像带V4上下文';
import {
  构造录像带V4入口胶囊,
  构造录像带V4当前卡文本,
  构造录像带V4房间摘要,
...
import { 查找许曼君离婚事件候选 } from './许曼君离婚事件边界';
import {
  执行许曼君离婚后日常动作,
  解析许曼君离婚后日常事件,
  提交许曼君离婚后日常事件,
...
import { 前台角色线路无关强剧情已冻结, 角色线路无关打断已停用 } from './丈夫线路风险策略';
import { 清理线路失效待演打断 } from './线路待演打断';
import { 录像带结局后亲密阻断, 录像带结局后亲密行动, type 录像带结局角色, type 录像带结局后开场选择 } from './录像带结局后相处';
import {
  产后图片键,
  待产图片键,
  提交产前看望,
...
import { 作废当前手机时间线租约世代, 手机锚消息签名, 读取当前手机时间线租约世代 } from './手机时间线租约';
import { 前台生成租约持有中, 手机生成租约持有中, 取得前台生成租约 } from './生成通道互斥';
import { 同步通关进度 } from './通关结算';
import { 创建通关纪念服务 } from './通关纪念服务';
import { CG条目 } from './成人CG系统';
import {
  读取许曼君分居重答,
  许曼君分居旧票需要补答,
  合并同场景剧情事件,
...
export function 处理可降级启动错误(启动仍有效: () => boolean, 错误: unknown, 报告: (错误: unknown) => void): void {
  if (!启动仍有效()) throw 错误;
  报告(错误);
}
...
async function 物理写回静音会议原生正文(
  ...
  if (!消息 || 消息 !== 预期消息 || 消息.is_user) {
    if (写回失败抛出) throw new Error('原生正文目标楼已经变化');
    return;
  }
...
function 许曼君原生专属正文错误(event: string, text: string): string {
  return (自然场景类别(event) ? '' : 许曼君离婚正文越拍原因(event, text)) || (自然场景类别(event) ? '' : 许曼君离婚后日常正文越界原因(event, text));
}

function 拍摄剧情原生专属正文错误(event: string, text: string, 契约: 拍摄剧情尺度契约 | null): string {
  if (!契约) return '';
  const 节拍错误 =
    契约.线路 === '第二机位'
      ? 第二机位正文越拍原因(event, text)
      : 契约.线路 === '不再留门'
        ? (自然场景类别(event) ? '' : 不再留门正文越拍原因(event, text))
        : '';
  return 节拍错误 || 拍摄剧情尺度越界原因(text, 契约);
}

/**
 * 原生备用链已经先落出assistant楼；专属拍首稿越界时只重写故事正文，再把同一楼原子替换。
 * 第二稿仍越界、写回失败、超时或时间线变化都向上失败关闭，调用方恢复PROMPT_READY基底，
 * 不消费剧情票或写路线硬状态。
 */
async function 生成拍摄剧情原生专属正文重写(参数: {
  事件: string;
  契约: 拍摄剧情尺度契约;
  首稿错误: string;
  行动: string;
  快照: string;
  楼层: number;
  消息: (typeof SillyTavern.chat)[number] | undefined;
  仍有效: () => boolean;
  刷新消息签名: () => void;
}): Promise<{ 正文: string; 尺度: 尺度判定解析结果 | null }> {
  const generationId = `rqgy-native-camera-rewrite-${参数.楼层}-${Date.now()}`;
  ...
  try {
      ...
      injects: [
        {
          ...
          content:
            `${参数.快照}\n` +
            `【本轮重写硬裁决】上一稿作废。脚本复核发现：${参数.首稿错误}。` +
            '只重写当前这一拍，严格停在冻结事件要求的结尾；不得补演下一拍、后续安排或任务结果。' +
            '不要提到稽查、等级、规则、重写或系统；仍须按快照格式输出临时尺度判定，不要输出变量命令。',
    ...
    } satisfies Parameters<typeof generate>[0] & { automatic_trigger?: boolean };
    const timeout = new Promise<never>((_resolve, reject) => {
      timer = setTimeout(() => {
        try {
          if (!stopGenerationById(generationId)) stopAllGeneration();
        } catch {
          /* 超时失败仍由租约与冻结基底恢复兜住。 */
        }
        reject(new Error('原生拍摄剧情静默重写超时'));
      }, 120_000);
    });
    const raw = String(await Promise.race([generate(rewriteRequest), timeout]));
    if (!参数.仍有效()) throw new Error('__RQGY_TIMELINE_CHANGED__');
    const 正文 = 提取静音会议可提交正文(应用酒馆最终显示正则(raw));
    if (是提供方拒答正文(正文)) throw new Error('AI服务返回了拒答说明');
    const 尺度 = 应用拍摄剧情尺度契约(解析尺度判定(raw), 正文, 参数.契约);
    const 重写错误 = 拍摄剧情原生专属正文错误(参数.事件, 正文, 参数.契约);
    if (!正文 || 重写错误) {
      throw new Error(`拍摄剧情重写仍未通过验收：${重写错误 || '正文为空'}`);
    }
    await 物理写回静音会议原生正文(
      正文,
      参数.楼层,
...
async function 生成许曼君原生专属正文重写(参数: {
  事件: string;
  首稿错误: string;
  行动: string;
  快照: string;
  ...
  const generationId = `rqgy-native-xmj-rewrite-${参数.楼层}-${Date.now()}`;
  let timer: ReturnType<typeof setTimeout> | undefined;
  try {
    eventEmit('人妻公寓:运行阶段', '正在校准角色反应');
    const rewriteRequest = {
      ...
      injects: [
        {
          ...
          content:
            `${参数.快照}\n【本轮重写硬裁决】上一稿作废。脚本复核发现：${参数.首稿错误}。` +
            '只重写当前这一拍，严格停在当前事件要求的结尾；不得补演下一拍、后续安排或任务结果。' +
            '不要提到稽查、规则、重写或系统，不要输出变量命令。',
          position: 'in_chat' as const,
          depth: 0,
          should_scan: false,
        },
    ...
    if (!参数.仍有效()) throw new Error('__RQGY_TIMELINE_CHANGED__');
    const rewritten = 提取静音会议可提交正文(应用酒馆最终显示正则(raw));
    if (是提供方拒答正文(rewritten)) throw new Error('AI服务返回了拒答说明');
    const rewrittenError = 许曼君原生专属正文错误(参数.事件, rewritten);
    if (!rewritten || rewrittenError) {
      throw new Error(`201专属剧情重写仍未通过验收：${rewrittenError || '正文为空'}`);
    }
    await 物理写回静音会议原生正文(
      rewritten,
      参数.楼层,
...
async function 固化原生本轮在场(楼层: number, 提交校验: () => boolean = () => true): Promise<void> {
  ...
  const 助手楼 = SillyTavern.chat?.[楼层] as
    { is_user?: boolean; mes?: string; extra?: Record<string, unknown> | null } | undefined;
...
function 覆盖原生本轮变量视图(
  ...
  for (const 消息 of chat) {
    ...
    // 残缺旧块从开始标签延续到消息尾；仅替换该块，保留此前指令及其他消息。
    // 回调按字面返回JSON，字段中的$&、$`等不得重新展开旧视图或前后缀。
    消息.content = 消息.content.replace(/<status_current_variable>[\s\S]*?(?:<\/status_current_variable>|$)/g, () => {
      已替换 = true;
      return 块;
    });
  }
...
async function 清理无效时间撤销点(data?: SchemaType, 提交校验: () => boolean = () => true): Promise<void> {
  if (!提交校验()) throw new Error('__RQGY_TIMELINE_CHANGED__');
...
async function 恢复中断时间推进(): Promise<boolean> {
  ...
  const 恢复raw = Mvu.getMvuData({ type: 'message', message_id: -1 }) as unknown;
  if (!恢复raw || typeof 恢复raw !== 'object' || Array.isArray(恢复raw)) {
    throw new Error('时间推进恢复时找不到当前末楼的 MVU 外壳，恢复记录已保留');
  }
  const 当前stat = _.get(恢复raw, 'stat_data');
  if (当前stat && !_.isEmpty(当前stat)) 验证当前MVU存档版本(当前stat);
  // 已校验的WAL保存完整主账；原生维护只清stat时仍保留当前楼真实元数据，不借用其他楼。
  const 恢复楼 = 当前楼层();
  const 恢复消息 = SillyTavern.chat?.[恢复楼];

  const 恢复世代 = 当前时间线切换世代();
  const 恢复仍有效 = () =>
    当前聊天ID() === 聊天ID && 当前时间线切换世代() === 恢复世代 &&
    当前楼层() === 恢复楼 && SillyTavern.chat?.[恢复楼] === 恢复消息 &&
    (记录.锚楼 === undefined ||
      (当前楼层() === 记录.锚楼 && 手机锚消息签名(SillyTavern.chat?.[记录.锚楼]) === 记录.锚消息签名));
  const 写口 = 创建时间事务写口(记录, {
    校验归属: 恢复仍有效,
    更新聊天: 更新 => Promise.resolve(updateVariablesWith(更新, { type: 'chat' })),
  });
  // 新记录使用原楼和消息签名；没有锚字段的旧正向记录保留原同聊天恢复兼容。
  写口.校验(getVariables({ type: 'chat' }));
  const 释放时间写入 = 取得时间事务写入租约(聊天ID, 记录.事务ID, true);
  const 取消恢复校验 = 登记MVU提交校验(恢复仍有效);
  try {
    作废当前手机时间线租约世代();
    if (记录.方向 === '撤销') await 作废晋阶镜像时间线();
    写口.校验(getVariables({ type: 'chat' }));
    await 脚本写入(恢复raw, _.cloneDeep(记录.推进前数据), {
      时间事务校验: 写口.校验,
      记录成长: false,
      同步场景剧情: false,
    ...
    await Promise.resolve(
      updateVariablesWith(
        当前变量 => {
          写口.校验(当前变量);
          if (记录.方向 === '撤销') {
            恢复精确聊天快照(当前变量, 记录.推进前聊天, 读取时间事务恢复键(记录));
          } else {
            恢复精确聊天快照(当前变量, 记录.推进前聊天, 时间推进事务恢复聊天键);
          }
          // 保留记录到两路恢复和手机镜像刷新均已确认；任何再次失败仍可重载重入。
          return 当前变量;
        },
        { type: 'chat' },
      ),
    );
    if (!恢复仍有效()) throw new Error('恢复时间事务时聊天分支再次变化');
    if (记录.方向 === '撤销') {
      await 确认当前微信为刷新真值(聊天ID, undefined, 写口.校验);
      if (!恢复仍有效()) throw new Error('恢复时间事务时聊天分支再次变化');
      await 立即持久保存手机聊天变量(聊天ID, 写口.校验);
    }
    await 写口.清理();
  } finally {
    取消恢复校验();
  ...
  console.warn('[人妻公寓·时间] 已恢复上次中断的时间事务到操作开始前状态，未重复发放奖励。');
  return true;
}
...
$(() => {
  void (async () => {
    ...
    try {
      ...
      // APP_READY 可能早于宿主异步 getChat 完成；必须先等真实聊天数组和 metadata 同步恢复，
      // 再冻结聊天身份。否则冷启动空数组会被当成一个可提交的新聊天，后续旧微信无法命中恢复镜像。
      await 等待微信刷新宿主就绪();
      ...
      启动仍有效 = () =>
        当前聊天ID() === 启动聊天ID && SillyTavern.chat === 启动聊天引用 && 当前时间线切换世代() === 启动时间线世代;
      const 确认启动仍有效 = () => {
      ...
      try {
        if (await 确保MVU默认外置解析()) {
          console.info('[人妻公寓] 首次启动:已把 MVU 更新方式默认为「额外模型解析」');
        }
        if (await 迁移游戏变量解析设置()) {
          console.info('[人妻公寓] 已把旧 MVU 自定义变量 API 迁移为游戏独立持久配置');
        }
      } catch (e) {
      ...
      // 时间备份必须先恢复；普通手机镜像和迁移不能覆盖尚未收口的时间事务。
      const 启动时间变量 = getVariables({ type: 'chat' });
      if (Object.prototype.hasOwnProperty.call(启动时间变量, 时间推进事务键) &&
          Object.prototype.hasOwnProperty.call(启动时间变量, 隔离事件事务键)) {
        throw new Error('同时存在时间推进与隔离事件恢复记录，状态歧义；脚本已停止，请联系作者');
      }
      await 恢复中断时间推进();
      确认启动仍有效();

      // 微信必须在任何可能读取空库、推进节拍或派生初始私聊的启动任务之前恢复。
      // 同聊天恢复副本只在修订严格更新时生效；重开、回档与删楼墓碑不会被旧记录反向覆盖。
      await 恢复微信刷新恢复副本(启动聊天ID);
      确认启动仍有效();
      ...
      // 时间事务已在手机镜像与迁移之前恢复；随后处理独立事件的持久备份。
      ...
      await 恢复遗留临时回合楼();
      确认启动仍有效();
      // AST 夹具可以不给提取出的 `即时开演` 注入新端口；真实模块启动则必须硬确认，
      // 不能因为兼容旧夹具而让正式游戏静默跳过业务前票据。
      const 即时业务撤回运行端口已接线 = (
        执行回合 as typeof 执行回合 & { 即时业务撤回?: 即时业务撤回运行端口 }
      ).即时业务撤回;
      if (
        !即时业务撤回运行端口已接线 ||
        !['捕获', '登记', '确认', '收口未落地'].every(
          key => typeof 即时业务撤回运行端口已接线[key as keyof 即时业务撤回运行端口] === 'function',
        )
      ) {
        throw new Error('即时业务撤回运行端口未完整接线，已停止启动以避免普通即时业务无保护提交。');
      }
      // 临时正文楼清完后再判断即时业务准备票：锚楼仍是业务前则清票，已持事务则收敛并保留撤回能力。
      await 恢复即时业务撤回登记();
      确认启动仍有效();
      ...
      try {
        ...
        if (有效) {
          ...
          const 结局后有修正 = 同步双重继承完成后状态(data, 当前楼层());
          const 共居有修正 = 同步302共居状态(data);
          const 安若妍不必停检查点有修正 = 恢复安若妍不必停失效亲密检查点(data);
          const 安若妍换掉检查点有修正 = 恢复安若妍换掉失效亲密检查点(data);
          const 安若妍换掉当前票有修正 = 同步安若妍换掉当前剧情票(data);
          const 不再留门旧尺度边界有修正 = 清理不再留门旧尺度边界提示(data);
          const 许曼君离婚亲密检查点有修正 = 恢复许曼君离婚失效亲密检查点(data);
          const 许曼君离婚完成有修正 = 同步许曼君离婚完成后状态(data);
          const 许曼君离婚时间有修正 = Boolean(同步许曼君离婚时间节点(data).变动);
          const 线路旧打断有修正 = 清理线路失效待演打断(data);
          if (
            首批有修正 ||
            结局后有修正 ||
            共居有修正 ||
          ...
          确认启动仍有效();
          await 同步全部角色阶段世界书(data, 启动仍有效, true);
          确认启动仍有效();
          确认启动仍有效();
...
function 挂载监听() {
  const 时间候选监听 = [
    eventOn(Mvu.events.VARIABLE_UPDATE_STARTED, 登记时间事务变量候选),
    eventOn(Mvu.events.BEFORE_MESSAGE_UPDATE, ({ variables }) => { 确认时间事务变量候选(variables); }),
  ];
  $(window).on('pagehide', () => 时间候选监听.forEach(监听 => 监听.stop()));
  清理视频终幕驱动();
  ...
  const 空闲后手机节拍 = 创建空闲手机节拍调度器({
    ...
    执行节拍: 手机节拍,
    警告: (消息, 错误) => {
      if (错误 === undefined) console.warn(`[人妻公寓·手机] ${消息}`);
      else console.warn(`[人妻公寓·手机] ${消息}`, 错误);
    },
  });

  // 数据库公开 table-update 回调只广播无载荷信号；RQ 队列先精确回读，缺行才单次补写。
  eventOn('人妻公寓:数据库表格已更新', 唤醒RQ剧情骨架待补队列);

  eventOn('人妻公寓:通关纪念请求', 创建通关纪念服务({
    聊天ID: 当前聊天ID,
    时间线: () => `${当前聊天ID()}\u0000${当前时间线切换世代()}`,
    忙碌: () => 时间线切换协调中() || 回合进行中() || 隔离事件进行中()
      || 前台生成租约持有中() || 手机生成租约持有中() || 正文租约生效中()
      || Boolean(_.get(getVariables({ type: 'chat' }), '_侦探.偷窥待选')),
    写入忙碌: MVU操作进行中,
    场景: () => 读场景().房间id ?? null,
    读取: 读取最近有效,
    排队: 排队MVU操作,
    登记提交校验: 登记MVU提交校验,
    写入: (raw, data) => 脚本写入(raw, data, { 记录成长: false }),
    捕获保护: 捕获保护快照,
    合法CG: id => Boolean(CG条目(id)),
    响应: 结果 => eventEmit('人妻公寓:通关纪念响应', 结果),
  }));

  // ─────────────────────────────────────────────
  // 主路径:客户端 UI 事件 → 回合引擎(固定 0 楼)
  // ─────────────────────────────────────────────
  eventOn('人妻公寓:玩家行动', async (行动: string) => {
    if (typeof 行动 !== 'string' || !行动.trim()) return;
    if (_时间推进中) {
      eventEmit('人妻公寓:回合失败', '时间正在推进，请等地图刷新后再行动。');
      return;
    }
    // 启动收口的整表写回仍在排空时不再直接判失败：有界等它归零，再让原忙门裁定。
    // 超时不放宽任何语义，仍由 执行回合 / 安全操作 的原门广播原提示。
    await 等待可开始新操作();
    const 当前 = 读最近有效stat();
    ...
    if (当前) {
      try {
        ...
        if (活动剧情) {
          eventEmit(
            '人妻公寓:回合失败',
            `「${活动剧情.标题}」的业务已经提交，请使用“重试本段剧情”，不要用新行动改写它。`,
          );
          return;
        }
    ...
    return 安全操作(
      async (raw, data, 操作仍有效) => {
        ...
        if (记录.入口 === '荣耀洞继续') {
          ...
          eventEmit('人妻公寓:荣耀洞');
        } else if (记录.入口 === '录像带V4' && 记录.录像带V4操作) {
          eventEmit('人妻公寓:录像带V4操作', 记录.录像带V4操作);
        } else if (记录.门牌) {
  ...
  eventOn('人妻公寓:重新识别已保存对话', 恢复已保存自然对话);
  eventOn('人妻公寓:继续场景剧情', async (载荷?: { 认领旧档?: boolean; 尝试入住?: boolean; 行动?: string; 不再留门确认?: true }) => {
    const 输入回应 = typeof 载荷?.行动 === 'string' ? 载荷.行动.trim() : '';
    const 不再留门确认 = 载荷?.不再留门确认 === true;
    const 玩家回应 = Boolean(输入回应) || 不再留门确认;
    // 同「检查场景剧情」：启动/上一轮的整表写回排空前不直接判忙，有界等归零后再进原门。
    await 等待可开始新操作();
    await 安全操作(
      async (raw, data, 操作仍有效) => {
        let 本次玩家回应 = 输入回应;
        // 输入框已乐观锁住客户端，预检失败必须发回合失败恢复原文；确认瓷砖没有占用发送态，只回普通提示。
        const 反馈 = (消息: string) => eventEmit(输入回应 ? '人妻公寓:回合失败' : '人妻公寓:提示', 消息);
        ...
        try {
          ...
          const 旧票补答 = 许曼君分居旧票需要补答(data);
          if (旧票补答 && !输入回应) {
            反馈('这段剧情正在等你的具体答复，请在下方输入后继续。');
            return;
          }
          if (不再留门确认) {
            if (!不再留门控制动作可穿过自身剧情锁(data, '确认当前决定', 当前场景 ?? '')) {
              const 阻断 = 不再留门动作阻断(data, '确认当前决定', 当前场景 ?? '');
              反馈(阻断 || '《不再留门》的当前剧情票已经变化，请重新查看线路进展。');
              return;
            }
            本次玩家回应 = 不再留门明确回应(data);
            if (!本次玩家回应) {
              反馈('《不再留门》的当前决定或拍次已经变化，请重新查看线路进展。');
              return;
            }
          }
          let 事务结果: ReturnType<typeof 准备重试场景剧情> | ReturnType<typeof 激活队首场景剧情>;
          if (data.系统._场景剧情事务.id) {
            if (data.系统._场景剧情事务.状态 === '待续') {
              if (!本次玩家回应.trim()) { 反馈('请在下方继续聊天。'); return; }
              data.系统._场景剧情事务.行动 = 本次玩家回应;
            }
            // 确认瓷砖代表玩家用本线的标准明确回应纠正当前失败拍；仅在精确前台票复核通过后替换重试行动。
            if (不再留门确认) data.系统._场景剧情事务.行动 = 本次玩家回应;
          ...
          let 成功 = false;
          try {
            // 旧票原行动参加即时业务撤回指纹，不能改写持久票；真实答复只作为本次生成输入。
            成功 = await 执行回合(旧票补答 ? 输入回应 : 事务结果.事务.行动, {
              已持MVU操作租约: true,
              预占前台生成租约: 前台租约,
              场景剧情事务ID: 事务结果.事务.id,
  ...
  // 到场自动演出：启动收口的整表写回仍在排空时不得直接判「内容正在生成」，否则新档进游戏
  // 后第一次触发 AI 自动回复必然失败、必须手点「重新生成正文」。这里只有界等整表队列归零
  // （不等生成门：真有正文在生成时仍保留原提示语义），再交给 安全操作 的原门裁定。
  eventOn('人妻公寓:检查场景剧情', async (场景: string) => {
    await 等待可开始新操作();
    await 安全操作(
      async (raw, data, 操作仍有效) => {
        const 本线现场已同步 = 同步不再留门现场(data, String(场景 || '楼道'));
        const 本次录制已停 = 中止不再留门失效现场(data, String(场景 || '楼道'));
        if (本线现场已同步 || 本次录制已停) {
          await 脚本写入(raw, data);
          捕获保护快照(data);
        }
        return 到场触发场景剧情(raw, data, String(场景 || '楼道'), 操作仍有效);
      },
      false,
      () => false,
      true,
  ...
  interface 场景剧情落地选项 {
    ...
    事件已登记?: boolean;
    /** 仅用于已核对本线前台票的暂停/撤回许可，保留领域函数清理后的队列。 */
    本线场景控制?: boolean;
  }
  ...
  async function 落地(
    ...
    const 落地聊天 = 当前聊天ID();
    const 落地世代 = 当前时间线切换世代();
    const 落地仍有效 = () => 当前聊天ID() === 落地聊天 && 当前时间线切换世代() === 落地世代 && !时间线切换协调中();
    ...
    const 本次新增事件 = 合并同场景剧情事件(提取新增待发送事件(原待发送, 候选待发送), 结果.事件);
    ...
    if (!剧情选项.事件已登记 && !剧情选项.本线场景控制 && 普通场景票适用) {
      // 候选业务可能直接赋值 `_待发送事件`；先恢复操作开始前的等待队列，再把本次
      // 新剧情作为一张独立票插到队首。远处预约既不会丢，也不会与本地剧情混演。
      data.系统._待发送事件 = 原待发送;
      if (本次新增事件) {
        追加等待场景剧情(data, 本次新增事件, 当前场景, 剧情选项.标题 || 场景剧情可见标题(本次新增事件), true);
      }
    }
    ...
    if (有状态改动 || 队列已变 || 读取活动场景剧情(data)) {
      ...
      try {
        try {
          await 脚本写入(raw, data, {
            ...(剧情选项.本线场景控制 ? { 记录成长: false, 同步场景剧情: false } : {}),
          ...
          eventEmit(
            '人妻公寓:提示',
            `⚠ 结果没记上(请截 F12 控制台给作者):${e instanceof Error ? e.message : String(e)}`,
          );
          return false;
        }
        await 同步全部角色阶段世界书(data, 落地仍有效);
        if (!落地仍有效()) return true;
        try {
          await 结果.提交后?.();
        } catch (后提交错误) {
  ...
  function 播放第二机位CG(结果: 第二机位结果 | { CG?: string }): void {
    if (!结果.CG) return;
    eventEmit('人妻公寓:第二机位CG', {
      文件: 结果.CG,
      标题: 第二机位CG标题(结果.CG),
    });
  }

  function 播放许曼君分居CG(结果: 许曼君分居结果 | { CG?: string }): void {
    if (!结果.CG) return;
    eventEmit('人妻公寓:许曼君分居CG', {
      文件: 结果.CG,
      标题: 许曼君分居CG标题(结果.CG),
    });
  }

  function 播放安若妍不必停CG(结果: 安若妍不必停结果 | { CG?: string; CG序列?: string[] }): void {
    const 文件们 = 结果.CG序列?.length ? 结果.CG序列 : 结果.CG ? [结果.CG] : [];
    文件们.forEach(文件 =>
      eventEmit('人妻公寓:安若妍不必停CG', {
        文件,
        标题: 安若妍不必停CG标题(文件),
      }),
    );
  ...
  function 播放许曼君离婚CG(结果: 许曼君离婚结果 | { CG?: string }): void {
    if (!结果.CG) return;
    eventEmit('人妻公寓:许曼君离婚CG', {
      文件: 结果.CG,
      标题: 许曼君离婚CG标题(结果.CG),
    });
  }

  function 播放回国CG(结果: 回国结果 | { CG?: string }): void {
    if (!结果.CG) return;
    eventEmit('人妻公寓:回国CG', {
      文件: 结果.CG,
      标题: 回国CG标题(结果.CG),
  ...
  function 播放双重继承CG(结果: 双重继承结果 | { CG?: string }): void {
    if (!结果.CG) return;
    eventEmit('人妻公寓:双重继承CG', {
      文件: 结果.CG,
      标题: 双重继承CG标题(结果.CG),
    });
  }
  ...
  type 隔离入口 = '荣耀洞开始' | '荣耀洞继续' | '监控' | '录像带V4';
  type 隔离回合记录 = {
    入口: 隔离入口;
    行动: string;
    门牌?: 门牌;
    录像带V4操作?: { 操作标识: string; 类型: 录像带V4操作类型 };
    房间: string;
    日志长度: number;
    data快照: SchemaType;
  ...
  /**
   * 有界等待“可以开始一次新操作”的短暂忙态窗口，再交给原有忙态门判定。
   *
   * 根因（Problem 2 / 2026-08-27）：启动收口把 同步入住世界书条目、同步整表视图、恢复镜像、
   * 捕获保护快照 全部经 排队MVU操作 串行写回，而 挂载监听() 已经先行完成、客户端也已经能
   * 发事件。于是玩家进游戏后的第一次操作必然撞上仍在排空的整表队列：走 执行回合 的主行动被
   * 回合引擎忙门判成「另一项楼务操作正在保存」，走 安全操作 的到场自动演出被判成「内容正在
   * 生成」，客户端只能把它记成 失败行动 显示「刚才的生成没有完成」，玩家必须手点一次
   * 「重新生成正文」才补上——实测 100% 复现。
   *
   * 处理方式：忙态语义与提示文案都不放宽，只把“立刻判失败”改成“先有界等它排空”。等待纯读
   * mvuIO 的忙态计数（不入队、不持任何锁、不改事务）；真有正文在生成时不在这里等待，仍由
   * 原门按原提示广播失败，超时同样回落到原门。
   */
  function 等待可开始新操作(超时毫秒 = 8000): Promise<boolean> {
    return 等待MVU操作空闲(超时毫秒);
  }
  ...
  function 安全操作(
    ...
    if (时间事务阻止普通写入()) {
      eventEmit(失败解锁 ? '人妻公寓:回合失败' : '人妻公寓:提示', 时间事务未完成提示);
    ...
    return 排队MVU操作(async () => {
      try {
        ...
        const 等待剧情阻塞当前 = Boolean(
          等待剧情 && (等待剧情.目标场景 === null || 场景剧情目标匹配(等待剧情.目标场景, 读场景().房间id ?? null)),
        );
  ...
  type 录像带V4操作输入 = { 操作标识: string; 类型: 录像带V4操作类型 };

  function 解析录像带V4操作输入(原: unknown): 录像带V4操作输入 | null {
    if (!原 || typeof 原 !== 'object' || Array.isArray(原)) return null;
    const 值 = 原 as { 操作标识?: unknown; 类型?: unknown };
    if (
      typeof 值.操作标识 !== 'string' ||
      !/^[A-Za-z0-9:_-]{3,160}$/u.test(值.操作标识) ||
      !(['开始', '切房', '下一幕'] as const).includes(值.类型 as 录像带V4操作类型)
    ) {
      return null;
    }
    return { 操作标识: 值.操作标识, 类型: 值.类型 as 录像带V4操作类型 };
  }

  function 新录像带V4场次标识(): string {
    return `vtr-v4:${Date.now().toString(36)}:${Math.random().toString(36).slice(2, 10)}`;
  }

  function 录像带V4错误文本(error: unknown): string {
    return (error instanceof Error ? error.message : String(error ?? '未知失败')).replace(/\s+/gu, ' ').trim().slice(0, 220);
  }

  /**
   * V4每次“开始／切房／下一幕”都走同一持久隔离事务：先冻结操作与四键快照，再发唯一AI请求，
   * 最后把VTR日志和硬幕次一起提交。任何生成失败、取消、超时、切聊或迟到结果都不能推进幕次。
   */
  async function 执行录像带V4操作(
    raw: object,
    data: SchemaType,
    操作仍有效: () => boolean,
    输入: 录像带V4操作输入,
  ): Promise<boolean> {
    const 规划 = 规划录像带V4操作(data, 输入);
    if (!规划.计划) {
      eventEmit('人妻公寓:提示', 规划.提示);
      eventEmit('人妻公寓:录像带V4状态');
      return false;
    }
    const 计划: 录像带V4操作计划 = 规划.计划;
    if (!计划.需要生成) {
      eventEmit('人妻公寓:回合失败', '目标录像带画面已经提交，系统拒绝重复生成。');
      return false;
    }

    const 行动 = 构造录像带V4行动文本(计划);
    const 记录: 隔离回合记录 = {
      ...建隔离记录('录像带V4', 行动, data, 计划.目标房间),
      录像带V4操作: { ...输入 },
    };
    const 身份 = 捕获隔离时间线身份();
    let 事务: 已准备隔离事务 | null = null;
    let 已交给顺序提交 = false;
    let 已完整提交 = false;

    const 恢复核心 = async (): Promise<void> => {
      const 恢复数据 = Schema.parse(_.cloneDeep(记录.data快照)) as SchemaType;
      _.set(raw, 'stat_data', _.cloneDeep(恢复数据));
      await 脚本写入(raw, 恢复数据, { 记录成长: false, 当前绝对时段: 恢复数据.系统._绝对时段 });
    };
    ...
    try {
      ...
      const 卡 = 读取录像带V4镜头卡(计划.目标房间, 计划.目标幕次);
      if (!卡 || 卡.id !== 计划.画面键) throw new Error('录像带V4当前镜头卡缺失或画面身份不一致');
      const 历史 = 提取录像带V4线程历史(读取录像带V4隔离日志(计划.场次标识), 计划.场次标识, 12);
      const 草稿 = await 生成录像带V4隔离草稿({
        场次标识: 计划.场次标识,
        房间: 计划.目标房间,
        画面键: 计划.画面键,
        行动,
        系统契约: 构造录像带V4系统契约(data, 计划),
        入口胶囊: 构造录像带V4入口胶囊(计划),
        历史,
        房间摘要: 构造录像带V4房间摘要(data),
        当前卡: 构造录像带V4当前卡文本(卡),
      });
      if (!草稿.正文) throw new Error('录像带V4没有生成可显示正文');
      const 越拍原因 = 录像带V4正文越拍原因(卡, 草稿.正文);
      if (越拍原因) throw new Error(`录像带V4当前镜头卡未通过硬边界：${越拍原因}。本幕没有提交，请重试。`);
      复核隔离时间线身份(身份, 操作仍有效);
      ...
      await 顺序提交隔离事件({
        ...
        写核心: async () => {
          复核隔离时间线身份(身份, 操作仍有效);
          const 提交 = 提交录像带V4操作(data, 计划);
          if (!提交.成功 || !提交.变动) throw new Error(提交.提示 || '录像带V4硬状态未能提交');
          await 脚本写入(raw, data, { 记录成长: false, 当前绝对时段: data.系统._绝对时段 });
        },
        恢复核心,
        身份,
      ...
      try {
        捕获保护快照(data);
        eventEmit('人妻公寓:录像带V4状态');
        eventEmit('人妻公寓:隔离事件完成', { 类型: '录像带V4' });
      } catch (后处理错误) {
        console.warn('[人妻公寓·录像带V4] 硬状态与专用日志已提交，忽略呈现后处理异常:', 后处理错误);
      }
      return true;
    } catch (error) {
      if (已完整提交) {
        console.warn('[人妻公寓·录像带V4] 本幕已经提交，忽略迟到呈现异常:', error);
        return true;
      }
      if (!(操作仍有效() && 当前聊天ID() === 身份.聊天ID)) {
        console.warn('[人妻公寓·录像带V4] 生成期间时间线已变化，旧聊天事务留待启动恢复:', error);
        return false;
      }
      ...
      if (事务 && !已交给顺序提交) {
        try {
          ...
          console.error('[人妻公寓·录像带V4] 生成前事务回滚失败，保留记录等待刷新恢复:', 回滚错误);
        }
      }
      if (!(操作仍有效() && 当前聊天ID() === 身份.聊天ID)) return false;

      let 有遗留事务 = true;
      try {
        const vars = getVariables({ type: 'chat' });
        有遗留事务 = Boolean(vars && typeof vars === 'object' && Object.prototype.hasOwnProperty.call(vars, 隔离事件事务键));
      } catch {
        /* 无法证明事务已经收口时按仍存在处理，禁止覆盖。 */
      }
      const 原因 = 录像带V4错误文本(error);
      if (!有遗留事务) {
        try {
          const 当前解析 = Schema.safeParse(_.cloneDeep(_.get(raw, 'stat_data')));
          const 失败数据 = (当前解析.success ? 当前解析.data : Schema.parse(_.cloneDeep(记录.data快照))) as SchemaType;
          const 失败 = 记录录像带V4操作失败(失败数据, 计划, 原因);
          if (失败.变动) {
            await 脚本写入(raw, 失败数据, { 记录成长: false, 当前绝对时段: 失败数据.系统._绝对时段 });
            捕获保护快照(失败数据);
          }
        } catch (失败记账错误) {
          console.error('[人妻公寓·录像带V4] 本幕未推进，但失败诊断状态保存失败:', 失败记账错误);
        }
      }
      eventEmit('人妻公寓:录像带V4状态');
      eventEmit(
        '人妻公寓:回合失败',
        有遗留事务 ? `${原因}；事务将在刷新后按入口快照恢复。` : 原因 || '录像带V4本幕未生成成功。',
      );
      return false;
    }
  }
  ...
  async function 即时开演(
    ...
    try {
      // 必须先于结算捕获：翻垃圾、打听、对饮、读信与同族关系节点会在结算内立即改写 data。
      await 即时业务撤回?.备份?.();
      const 业务前撤回准备 = 即时业务撤回?.捕获(data) ?? null;
      const 结果 = await 结算();
      // `脚本写入` 或世界书同步可能在核心已经写成、但 `落地` 尚未启动提交后回调时抛错。
      // 这时后置条件会认领业务核心；同一调用内只补启动一次软冷却／镜像。回调一旦开始，
      // 即使内部部分执行后抛错也绝不重放，避免重复写外部数据库或重复烧软计数。
      const 原提交后 = 结果.提交后;
      let 提交后已启动 = false;
      const 启动提交后一次 = async (): Promise<void> => {
        if (!原提交后 || 提交后已启动) return;
        提交后已启动 = true;
        await 原提交后();
      };
      if (原提交后) 结果.提交后 = 启动提交后一次;
      const 补启动未执行提交后 = async (): Promise<void> => {
        if (!原提交后 || 提交后已启动) return;
        try {
          await 启动提交后一次();
        } catch (后提交错误) {
          console.warn('[人妻公寓] 核心结果已经提交，补启动的聊天软计数/镜像同步失败，不重复执行:', 后提交错误);
        }
      };
      if (!即时操作仍有效()) {
      ...
      if (!即时操作仍有效()) return;
      // 普通活动事务先持久保存业务前撤回票，再允许核心锚楼写入；专用特殊场景继续用自己的事务。
      const 即时业务撤回记录 = 事务ID && 即时业务撤回 && 业务前撤回准备
        ? await 即时业务撤回.登记(业务前撤回准备, 事务ID, data)
        : null;
      let 已落库 = false;
      try {
        已落库 = await 落地(结果, raw, data, { ...剧情选项, 事件已登记: 需要正文 });
      } catch (落地错误) {
        if (!即时业务撤回记录) throw 落地错误;
        const 收口 = await 即时业务撤回!.收口未落地(即时业务撤回记录);
        if (收口 !== '已提交' || !即时操作仍有效()) throw 落地错误;
        await 补启动未执行提交后();
        await 持久标记场景剧情待重试(事务ID, 事务请求世代, 即时操作仍有效);
        eventEmit('人妻公寓:场景剧情状态');
        eventEmit('人妻公寓:提示', '业务结果已经保存，但后置同步中断；请在原场景重试本段剧情。');
        return;
      }
      if (!已落库) {
        if (即时业务撤回记录) {
          const 收口 = await 即时业务撤回!.收口未落地(即时业务撤回记录);
          // `脚本写入` 可能在真实提交后才由宿主报错。此时不能伪装成“业务未发生”，
          // 也不能直接开演；锁回同一票待重试，完整撤回资料已经保留。
          if (收口 === '已提交' && 即时操作仍有效()) {
            await 补启动未执行提交后();
            await 持久标记场景剧情待重试(事务ID, 事务请求世代, 即时操作仍有效);
            eventEmit('人妻公寓:场景剧情状态');
            eventEmit('人妻公寓:提示', '业务结果已经保存，但本段正文尚未开始；请在原场景重试本段剧情。');
          }
        }
        return;
      }
      if (!需要正文 || !即时操作仍有效()) return;
      if (即时业务撤回记录) {
        try {
          ...
          if (!即时操作仍有效()) return;
          const 收口 = await 即时业务撤回!.收口未落地(即时业务撤回记录);
          if (收口 !== '已提交') throw 确认错误;
          await 持久标记场景剧情待重试(事务ID, 事务请求世代, 即时操作仍有效);
          eventEmit('人妻公寓:场景剧情状态');
          eventEmit('人妻公寓:提示', '业务结果已经保存，但撤回凭据确认中断；请在原场景重试本段剧情。');
          return;
        }
      }
      if ((读场景().房间id ?? null) !== 触发场景) {
      ...
      try {
        成功 = await 执行回合(行动, {
          已持MVU操作租约: true,
          预占前台生成租约: 前台租约,
          ...(事务ID ? { 场景剧情事务ID: 事务ID, 场景剧情请求世代: 事务请求世代, 预载场景剧情数据: data } : {}),
        });
      } finally {
  ...
  async function 运行荣耀洞隔离拍(
    ...
    try {
      ...
      const 草稿 = await 生成隔离事件草稿(
        { 类型: '荣耀洞', 线程, 行动, 导演事件, 房间: '洗手间' },
        { 历史叙事可用: 正文 => !荣耀洞正文边界原因(本拍前数据, 正文) },
      );
      if (!草稿?.正文) throw new Error('荣耀洞事件没有生成正文');
      复核隔离时间线身份(身份, 操作仍有效);
      const 正文边界错误 = 荣耀洞正文边界原因(本拍前数据, 草稿.正文);
      if (正文边界错误) throw new Error(正文边界错误);
      await 顺序提交隔离事件({
        草稿,
        记录,
        锚楼: 身份.锚楼,
  ...
  eventOn('人妻公寓:荣耀洞', () => {
    ...
    return 安全操作(
      async (raw, data, 操作仍有效) => {
        ...
        await 运行荣耀洞隔离拍(raw, data, 行动, 记录, 操作仍有效, 标记荣耀洞静默切换, 标记荣耀洞核心已提交);
      },
      false,
      () => 荣耀洞静默已切换 || 荣耀洞核心已提交,
    );
  ...
  function 构造睡眠独立演出(
    ...
    const 共通纪律 =
      `地点始终是${地点名}。${时间说明}` +
      (地点 === '201'
        ? '这是一次已经通过脚本许可的201日常留宿，只允许许曼君作为固定同场人物；可以表现收拾房间、关灯、简短睡前话与次晨醒来，但不得自动开始亲密、无保护、受孕、关系增长、满意度增长、丈夫回家、其他住户夜访、任务或主线推进，'
    ...
    await Promise.resolve(
      updateVariablesWith(
        vars => {
          ...
          if (事务ID && !保留事务) delete vars[时间推进事务键];
          return vars;
        },
        { type: 'chat' },
      ),
    );
    if (keys.includes('_微信')) {
      await 确认当前微信为刷新真值(预期聊天ID, undefined, 时间事务校验);
      await 立即持久保存手机聊天变量(预期聊天ID, 时间事务校验);
    }
  }
  ...
  function 处理时间推进(方式: 时间推进方式, 载荷: 时间推进载荷): void {
    ...
    // 普通正文已经与手机整拍双向互斥；时间动作也必须遵守同一边界。
    // 否则旧手机拍的送达回执会在本次时间 WAL 存活时进入安全操作，误报“恢复未完成”。
    if (手机生成租约持有中() || 手机节拍进行中() || 手机AI生成中()) {
      eventEmit('人妻公寓:提示', '手机后台消息还在生成，请等它完成后再推进时间。');
      eventEmit('人妻公寓:时间推进结束', false);
      return;
    }
    _时间推进中 = true;
    let 已提交 = false;
    let 回合完成选项: { 更新正文幕: boolean } | null = null;
    void 安全操作(
      async (raw, data, 操作仍有效) => {
        const 当前房间 = 读场景().房间id;
        if (!当前房间) {
        ...
        if (方式 === '睡到次日早晨') {
          if (当前房间 === '201') {
            const 留宿 = 读取201留宿可用状态(data, 当前房间);
            if (!留宿.可执行) {
              eventEmit('人妻公寓:提示', 留宿.原因 || '当前不能在201留宿。');
              return;
            }
          } else if (当前房间 !== '管理员室' && 当前房间 !== '302') {
            eventEmit('人妻公寓:提示', '只有管理员室、302，或已经解锁且当前可用的201可以睡到次日早晨。');
            return;
          }
        }
        ...
        const 不必停硬动作 = 安若妍不必停时间动作阻断原因(data) || 安若妍换掉时间动作阻断原因(data);
        if (不必停硬动作) {
          eventEmit('人妻公寓:提示', 不必停硬动作);
          return;
        }
        const 分居硬动作 = 许曼君分居时间动作阻断原因(data);
        if (分居硬动作) {
          eventEmit('人妻公寓:提示', 分居硬动作);
          return;
        }
        const 离婚硬动作 = 许曼君离婚时间动作阻断原因(data);
        if (离婚硬动作) {
          eventEmit('人妻公寓:提示', 离婚硬动作);
          return;
        }
        if (第二机位录制锁定中(data)) {
          eventEmit('人妻公寓:提示', 'CAM-2仍在录制或等待封存。先回102完成这一晚，再推进时间。');
          return;
        }
        ...
        const 是201留宿 = 方式 === '睡到次日早晨' && 时间结束房间 === '201';
        const 预期聊天ID = 当前聊天ID();
        if (!预期聊天ID) {
        ...
        const 有睡前丈夫登门 =
          方式 === '睡到次日早晨' &&
          ((data: SchemaType) => {
            // 此处参数故意遮蔽外层 data：排期同步只在副本上预览；真实排期与登门必须一起
            // 留到即时业务结算中，确保撤回 before-image 来自未改写的持久锚楼。
            同步丈夫登门排期(data);
        ...
        if (有睡前丈夫登门) {
          await 即时开演(
            () => {
              同步丈夫登门排期(data);
              return 准备睡前丈夫登门(data, 查房间(当前房间)?.名称 ?? 当前房间, 当前房间) ?? {
                成功: false,
        ...
        const 时间事务写口 = 创建时间事务写口(时间事务记录, {
          校验归属: () => 操作仍有效() && 当前聊天ID() === 预期聊天ID &&
            当前楼层() === 当前消息楼 && 手机锚消息签名(SillyTavern.chat?.[当前消息楼]) === 锚消息签名,
          更新聊天: 更新 => Promise.resolve(updateVariablesWith(更新, { type: 'chat' })),
        });
        let 聊天已改 = false;
        let 已进入双存储提交 = false;
        ...
        let 释放时间写入: () => void = () => undefined;

        try {
          if (!操作仍有效()) throw new Error('消息分支已经变化，时间推进已取消');
          if (时间动作需要独立演出(方式)) {
            if (是201留宿) 播放许曼君分居CG({ CG: '201_留宿_夜' });
          ...
          释放时间写入 = 取得时间事务写入租约(预期聊天ID, 时间事务记录.事务ID);
          ...
          同步许曼君离婚完成后状态(候选);
          同步安若妍换掉时间节点(候选);
          const 许曼君离婚时间结果 = 同步许曼君离婚时间节点(候选);
          ...
          绑定新增待发送事件到场景(候选, 推进前数据.系统._待发送事件, 时间结束房间, item => 是入住登场事件(item));
          await 执行时间推进双存储提交({
            写推进状态: async () => {
              if (!操作仍有效()) throw new Error('消息分支已经变化，时间推进已取消');
              if (当前聊天ID() !== 预期聊天ID) throw new Error('切换聊天导致时间推进取消');
              时间事务写口.校验(getVariables({ type: 'chat' }));
              作废当前手机时间线租约世代();
              await 脚本写入(raw, 候选, {
                时间事务校验: 时间事务写口.校验,
                记录成长: false,
              ...
              await Promise.resolve(
                updateVariablesWith(
                  vars => {
                    ...
                    // 日志与撤销点写入属于本次提交的一部分；事务恢复记录不能在此提前删除。
                    // 最终 chat updater 可能已经执行后才收到宿主异常，保留事务身份才能精确补偿。
                    // 两路提交成功后由提交完成回调清理。
            ...
            恢复推进前聊天: async () => {
              ...
              时间事务写口.校验(getVariables({ type: 'chat' }));
              await 恢复时间聊天备份(聊天事务备份, 时间推进写入聊天键, 预期聊天ID, 时间事务记录.事务ID, true, 时间事务写口.校验);
              聊天已改 = false;
            },
          });
          双存储已提交 = true;
          ...
          捕获保护快照(候选);
          try {
            await 同步全部角色阶段世界书(候选, 操作仍有效);
          } catch (同步错误) {
            // 时间和撤销点已经完成双存储提交；世界书只是可重建投影，失败不得反向回滚。
            console.warn('[人妻公寓·时间] 推进后世界书同步失败，下一个同步点会重试，不反向回滚:', 同步错误);
          }
          if (是201留宿) {
            const 离婚次晨 = 许曼君离婚次晨CG(候选);
            if (离婚次晨) 播放许曼君离婚CG({ CG: 离婚次晨 });
            else 播放许曼君分居CG({ CG: '201_留宿_晨' });
          }
          const 提示 = _.uniq([
            结果.提示,
            ...结果.资源提示,
          ...
          // 客户端完成通知必须等 `安全操作` 退出 MVU 队列、运行期时间租约归还并清除
          // `_时间推进中` 后再发；这里只保留本次正文幕归属结果。
          回合完成选项 = { 更新正文幕: Boolean(时间反馈草稿) };
          const 后续剧情 = 读取队首场景剧情(候选.系统._待发送事件);
          if (后续剧情) {
            eventEmit('人妻公寓:场景剧情状态');
            // 时间已经提交，后续剧情另起一张场景票。只有设计地点就是时间结束地点时才自动
            // 尝试激活；否则保持等待，绝不能插进当前不相关场景。
            if (后续剧情.目标场景 !== null && 场景剧情目标匹配(后续剧情.目标场景, 时间结束房间)) {
      ...
      if (已提交 && 回合完成选项) eventEmit('人妻公寓:回合完成', 回合完成选项);
    });
  }
  ...
  function 处理撤销时间推进(): void {
    ...
    if (手机生成租约持有中() || 手机节拍进行中() || 手机AI生成中()) {
      eventEmit('人妻公寓:提示', '手机后台消息还在生成，请等它完成后再撤销时间。');
      eventEmit('人妻公寓:时间推进结束', false);
      return;
    }
    _时间推进中 = true;
    ...
    void 安全操作(
      async (raw, data, 操作仍有效) => {
        ...
        if (Object.prototype.hasOwnProperty.call(当前变量, 时间推进事务键)) {
          eventEmit('人妻公寓:提示', '上一次时间事务仍在恢复，请刷新当前聊天后再试。');
          return;
        }
        ...
        if (!是时间撤销地点(当前房间)) {
          eventEmit(
            '人妻公寓:提示',
            '只有在管理员室、302、合法留宿后的201，或刚完成训练的地点，才能撤销刚才的时间推进。',
          );
          return;
        }
        ...
        // 同一预写机制反向使用：中断时恢复到“撤销开始前”，保留原点供玩家重试。
        // 不另建第二套时间系统；旧正向记录仍按原格式恢复。
        const 撤销事务 = 创建时间推进事务记录({
          聊天ID: 预期聊天ID,
          方向: '撤销',
          锚楼: 撤销点.锚楼,
          锚消息签名: 撤销点.锚消息签名,
        ...
        const 撤销写口 = 创建时间事务写口(撤销事务, {
          校验归属: () => 操作仍有效() && 当前聊天ID() === 预期聊天ID &&
            当前楼层() === 撤销点.锚楼 && 手机锚消息签名(SillyTavern.chat?.[撤销点.锚楼]) === 撤销点.锚消息签名,
          更新聊天: 更新 => Promise.resolve(updateVariablesWith(更新, { type: 'chat' })),
        });
        let 聊天事务已准备 = false;
        let 已进入双存储提交 = false;
        ...
        try {
          ...
          释放时间写入 = 取得时间事务写入租约(预期聊天ID, 撤销事务.事务ID);
          // 必须在清镜像和第一笔旧MVU写入之前保存；准备失败不碰时间与消息。
          await Promise.resolve(updateVariablesWith(vars => {
            if (!操作仍有效() || 当前聊天ID() !== 预期聊天ID) throw new Error('时间撤销准备时聊天已变化');
            if (Object.prototype.hasOwnProperty.call(vars, 时间推进事务键)) throw new Error('已有时间事务正在恢复');
            if (!当前时间撤销判定(data, vars).有效 ||
              !_.isEqual(捕获精确聊天快照(vars, 时间撤销写入聊天键), 推进后聊天备份)) {
              throw new Error('时间撤销准备期间聊天状态发生变化，请重新点击');
            }
            vars[时间推进事务键] = _.cloneDeep(撤销事务);
            聊天事务已准备 = true;
            return vars;
          }, { type: 'chat' }));
          已进入双存储提交 = true;
          await 执行时间推进双存储提交({
            写推进状态: async () => {
              撤销写口.校验(getVariables({ type: 'chat' }));
              // 仍先作废手机租约与晋阶镜像世代，阻止旧异步任务在相同时段ABA复活。
              作废当前手机时间线租约世代();
              await 作废晋阶镜像时间线();
              撤销写口.校验(getVariables({ type: 'chat' }));
            ...
            写撤销点: async () => {
              await Promise.resolve(updateVariablesWith(vars => {
                撤销写口.校验(vars);
                // 核心已回旧钟，仍以推进后数据复核玩家消息和原撤销点，不放宽资格。
                const 当前楼 = 当前楼层();
              ...
              撤销写口.校验(getVariables({ type: 'chat' }));
              await 确认当前微信为刷新真值(预期聊天ID, undefined, 撤销写口.校验);
              await 立即持久保存手机聊天变量(预期聊天ID, 撤销写口.校验);
              撤销写口.校验(getVariables({ type: 'chat' }));
            },
            提交完成: 撤销写口.清理,
            准备补偿: 撤销写口.准备补偿,
            ...
            恢复推进前聊天: async () => {
              // updater 被调用不代表宿主已经应用它；以实际保存的标记选择手机恢复范围。
              const 当前撤销事务 = 撤销写口.校验(getVariables({ type: 'chat' }));
              await 恢复时间聊天备份(
                推进后聊天备份, 读取时间事务恢复键(当前撤销事务), 预期聊天ID, 撤销事务.事务ID, true, 撤销写口.校验,
              );
            },
            补偿完成: async () => {
              await 撤销写口.清理();
          ...
          已提交 = true;
          if (!操作仍有效() || 当前聊天ID() !== 预期聊天ID) return;
          清保护快照();
          捕获保护快照(推进前数据, false);
          try {
            await 同步入住世界书条目(推进前数据, 操作仍有效);
            await 同步全部角色阶段世界书(推进前数据, 操作仍有效, true);
          } catch (同步错误) {
          ...
          if (!操作仍有效()) throw e;
          // 仅准备写入应用后报错：还没有改MVU或消息，只清除确属本次的记录。
          if (聊天事务已准备 && !已进入双存储提交) {
            try {
              await 撤销写口.清理();
            } catch (清理错误) {
              throw new Error(`${e instanceof Error ? e.message : String(e)}；撤销准备清理失败:${清理错误 instanceof Error ? 清理错误.message : String(清理错误)}`);
            }
          }
          throw e;
        } finally {
  ...
  // ── 《双重继承》母亲微信视频通话 ──
  // 父亲回复与现场正文分成两个串行模型请求；CG与通话状态只在持久MVU中维护。
  const 母亲视频现场生成键 = new Set<string>();

  const 母亲视频时间线仍有效 = (预期聊天ID: string, 预期时间线世代: number) =>
    当前聊天ID() === 预期聊天ID && 当前时间线切换世代() === 预期时间线世代;

  const 视频终幕驱动 = 创建持久终幕驱动({
    失败: 错误 => {
      console.warn('[人妻公寓·视频终幕] 播放暂停，保留已保存帧:', 错误);
      eventEmit('人妻公寓:提示', '终幕暂未完成保存。打开手机点击“继续播放终幕”可从当前帧重试。');
    },
  });
  const 停止终幕 = () => 视频终幕驱动.停止();
  window.addEventListener('pagehide', 停止终幕, { once: true });
  清理视频终幕驱动 = () => {
    停止终幕();
    window.removeEventListener('pagehide', 停止终幕);
  };

  function 继续视频终幕(通话标识: string, 预期聊天ID: string, 预期时间线世代: number): Promise<void> {
    const 仍有效 = () => 母亲视频时间线仍有效(预期聊天ID, 预期时间线世代) && !时间线切换协调中();
    return 视频终幕驱动.播放(`${预期聊天ID}|${预期时间线世代}|${通话标识}`, {
      仍有效,
      读取帧: () => {
        if (!仍有效() || 读场景().房间id !== '302') return null;
        const 视频 = 读取最近有效()?.data.系统._母亲视频通话终幕;
        return 视频?.标识 === 通话标识 && 视频.状态 === '终幕中' ? 视频.终幕CG序号 : null;
      },
      提交: async (预期帧, 驱动仍有效) => {
        let 已保存 = false;
        let 已完成 = false;
        await 安全操作(
          async (raw, data) => {
            if (!驱动仍有效() || 读场景().房间id !== '302') return;
            const 取消校验 = 登记MVU提交校验(驱动仍有效);
            try {
              const 结果 = 提交母亲视频终幕帧(data, 通话标识, 预期帧);
              if (!结果.成功) return;
              await 脚本写入(raw, data, { 记录成长: false, 当前绝对时段: data.系统._绝对时段 });
              已保存 = true;
              已完成 = data.系统._母亲视频通话终幕.状态 === '已完成';
              捕获保护快照(data);
              刷新红点();
              if (已完成) eventEmit('人妻公寓:提示', 结果.提示);
            } finally {
              取消校验();
            }
          },
        ...
        // 退出外层 MVU 队列后再保存通话完成行并清理电话，避免重入同一写队列。
        if (已完成 && 仍有效()) void 恢复父亲通话(预期聊天ID, 预期时间线世代);
        return 已保存;
      },
    });
  }

  eventOn('人妻公寓:母亲视频通话继续终幕', (通话标识: string, 预期聊天ID: string) =>
    继续视频终幕(通话标识, 预期聊天ID, 当前时间线切换世代()),
  );

  async function 生成并提交母亲视频现场正文(
    通话标识: string,
    现场序号: number,
    预期聊天ID: string,
    预期时间线世代: number,
  ): Promise<void> {
    const 生成键 = `${预期聊天ID}|${预期时间线世代}|${通话标识}|${现场序号}`;
    if (母亲视频现场生成键.has(生成键)) return;
    母亲视频现场生成键.add(生成键);
    type 母亲视频现场快照 = {
      请求世代: number;
      当前CG: NonNullable<ReturnType<typeof 读取母亲视频通话CG>>;
      微信记录: { 谁: '我' | '父'; 文: string }[];
      上轮现场正文: string;
    };
    const 快照盒: { 当前: 母亲视频现场快照 | null } = { 当前: null };
    try {
      await 安全操作(
        async (raw, data) => {
          if (!母亲视频时间线仍有效(预期聊天ID, 预期时间线世代)) return;
          const 视频 = data.系统._母亲视频通话终幕;
          if (视频.标识 !== 通话标识 || 视频.待现场正文序号 !== 现场序号) return;
          const 当前CG = 读取母亲视频通话CG(视频.当前CG);
          if (!当前CG) throw new Error(`母亲视频通话CG不存在：${视频.当前CG}`);
          const 开始 = 开始母亲视频通话现场正文(data, 通话标识, 现场序号);
          if (!开始.成功 || !开始.请求世代) return;
          快照盒.当前 = {
            请求世代: 开始.请求世代,
            当前CG,
            微信记录: data.系统._父亲通话.记录.map(条 => ({ ...条 })),
            上轮现场正文: 视频.上轮现场正文,
          };
          await 脚本写入(raw, data);
          捕获保护快照(data);
      ...
      const 快照 = 快照盒.当前;
      if (!快照 || !母亲视频时间线仍有效(预期聊天ID, 预期时间线世代)) return;

      const 草稿 = await 生成隔离事件草稿({
        类型: '母亲视频通话',
        线程: `母亲视频通话:${通话标识}`,
        行动: '承接刚刚真实完成的微信视频往返，只演绎302卧室里玩家与母亲这一轮的现场。',
        导演事件: 构造母亲视频通话正文注入({
          当前CG: 快照.当前CG,
          微信记录: 快照.微信记录,
          上轮现场正文: 快照.上轮现场正文,
      ...
      await 安全操作(
        async (raw, data) => {
          if (!母亲视频时间线仍有效(预期聊天ID, 预期时间线世代)) return;
          const 完成 = 完成母亲视频通话现场正文(data, 通话标识, 现场序号, 快照.请求世代, 草稿.正文);
          if (!完成.成功) throw new Error(完成.提示);
          await 脚本写入(raw, data);
          捕获保护快照(data);
          已提交 = true;
          刷新红点();
      ...
      if (已提交 && 母亲视频时间线仍有效(预期聊天ID, 预期时间线世代)) {
        eventEmit('人妻公寓:隔离事件完成', { 类型: '母亲视频通话' });
      }
    } catch (错误) {
      const 原因 = 错误 instanceof Error ? 错误.message : String(错误);
      const 失败快照 = 快照盒.当前;
      if (失败快照 && 母亲视频时间线仍有效(预期聊天ID, 预期时间线世代)) {
        await 安全操作(
          async (raw, data) => {
            const 失败 = 标记母亲视频通话现场正文失败(data, 通话标识, 现场序号, 失败快照.请求世代, 原因);
            if (!失败.成功) return;
            await 脚本写入(raw, data);
            捕获保护快照(data);
            刷新红点();
        ...
        eventEmit('人妻公寓:回合失败', `视频通话现场正文没有完成：${原因}`);
      }
    } finally {
      母亲视频现场生成键.delete(生成键);
    }
  }

  eventOn('人妻公寓:母亲视频通话恢复持久断点', (通话标识: string, 预期聊天ID: string, 预期时间线世代: number) => {
    if (!母亲视频时间线仍有效(预期聊天ID, 预期时间线世代)) return;
    const 有效 = 读取最近有效();
    if (!有效) return;
    const data = 有效.data;
    const 视频 = data.系统._母亲视频通话终幕;
    if (视频.标识 !== 通话标识 || data.系统._父亲通话.标识 !== 通话标识) return;

    if (视频.状态 === '终幕中') {
      void 继续视频终幕(通话标识, 预期聊天ID, 预期时间线世代);
      return;
    }

    const 未登记回复 = 读取母亲视频通话未登记父亲回复(data);
    if (未登记回复) {
      queueMicrotask(() =>
        eventEmit(
          '人妻公寓:母亲视频通话父亲回复已保存',
    ...
    const 未登记最终回答 = 读取母亲视频通话未登记最终回答(data);
    if (未登记最终回答) {
      queueMicrotask(() =>
        eventEmit('人妻公寓:母亲视频通话最终回答已保存', 通话标识, 未登记最终回答, 预期聊天ID, 预期时间线世代),
      );
      return;
    }

    if (视频.状态 === '等待现场正文' || (视频.状态 === '正文生成中' && !隔离事件进行中())) {
      queueMicrotask(() => eventEmit('人妻公寓:母亲视频通话重试现场正文'));
    }
  });

  eventOn('人妻公寓:接听母亲视频通话终幕', (预期聊天ID: string) =>
    安全操作(
      () =>
        排队父亲通话整表写(async () => {
          const 有效 = 读取最近有效();
          ...
          const { raw, data } = 有效;
          if ((读场景().房间id ?? '') !== '302') {
            eventEmit('人妻公寓:提示', '父亲的视频还在等你接听。请先回到302卧室和母亲身边。');
            return;
          }
          const 结果 = 接听母亲视频通话终幕(data);
          if (!结果.成功) {
            eventEmit('人妻公寓:提示', 结果.提示);
            return;
          }
          await 脚本写入(raw, data);
          捕获保护快照(data);
          eventEmit('人妻公寓:来电已接', data.系统._父亲通话.标识, 预期聊天ID);
        }),
      false,
      () => false,
      true,
    ...
    (
      ...
      void (async () => {
        ...
        await 安全操作(
          async (raw, data) => {
            if (!母亲视频时间线仍有效(预期聊天ID, 预期时间线世代)) return;
            if (data.系统._母亲视频通话终幕.标识 !== 通话标识 || data.系统._父亲通话.标识 !== 通话标识) return;
        ...
        if (现场序号 > 0) {
          await 生成并提交母亲视频现场正文(通话标识, 现场序号, 预期聊天ID, 预期时间线世代);
        }
      })();
    },
  );

  eventOn('人妻公寓:母亲视频通话请求结束', () =>
    安全操作(
      () =>
        排队父亲通话整表写(async () => {
          const 有效 = 读取最近有效();
          ...
          捕获保护快照(data);
          刷新红点();
          eventEmit('人妻公寓:提示', '向父亲说最后一句，让他去登机。');
        }),
      false,
      () => false,
      true,
  ...
  eventOn('人妻公寓:母亲视频通话重试现场正文', () => {
    ...
    void (async () => {
      ...
      await 安全操作(
        async (raw, data) => {
          const 视频 = data.系统._母亲视频通话终幕;
          if (!视频.标识 || 视频.待现场正文序号 <= 0) return;
          if (视频.状态 === '正文生成中' && !隔离事件进行中()) {
            const 中断 = 标记母亲视频通话现场正文失败(
              data,
              视频.标识,
              视频.待现场正文序号,
              视频.现场正文请求世代,
              '页面刷新或生成中断，需要重新演绎本轮现场。',
            );
            if (中断.成功) await 脚本写入(raw, data);
          }
          if (!['等待现场正文', '正文失败'].includes(视频.状态)) return;
          通话标识 = 视频.标识;
          现场序号 = 视频.待现场正文序号;
          捕获保护快照(data);
        },
  ...
  eventOn(
    '人妻公寓:母亲视频通话最终回答已保存',
    (通话标识: string, 玩家回答: string, 预期聊天ID: string, 预期时间线世代: number) =>
      安全操作(
        () =>
          排队父亲通话整表写(async () => {
            if (!母亲视频时间线仍有效(预期聊天ID, 预期时间线世代)) return;
            ...
            if (!有效) return;
            const { raw, data } = 有效;
            if (data.系统._父亲通话.标识 !== 通话标识) return;
            const 回答 = 登记玩家最终回答(data, 玩家回答);
            if (!回答.成功) return;
            await 脚本写入(raw, data);
            捕获保护快照(data);
            刷新红点();
            eventEmit('人妻公寓:提示', '你的最后回答已经送达；父亲仍在线听完，母亲的终幕动作开始。');
          }),
        false,
        () => false,
        true,
      ).then(() => 继续视频终幕(通话标识, 预期聊天ID, 预期时间线世代)),
  );
    ...
    安全操作(
      () =>
        排队父亲通话整表写(async () => {
          ...
          if (通话.模式 === 母亲视频通话模式) {
            const 视频 = data.系统._母亲视频通话终幕;
            if (视频.标识 !== 通话.标识 || 视频.状态 !== '已完成') return;
            data.系统._父亲通话 = 空父亲通话();
            await 脚本写入(raw, data);
            捕获保护快照(data);
  ...
  eventOn('人妻公寓:开手机', (直达来电: boolean) => {
    ...
    if (rawStat) {
      ...
      // 母亲视频的刷新补票统一由 `恢复父亲通话` 发出的持久断点事件处理；
      // 这里不再重复排队现场重试，避免 Dock 开手机与悬浮手机恢复同时触发两次。
      const 活动剧情 = 读取活动场景剧情(data);
      const 等待剧情 =
        !data.系统._特殊场景.id && data.系统._荣耀洞拍 < 0 ? 读取队首场景剧情(data.系统._待发送事件) : null;
  ...
  eventOn('人妻公寓:打开父亲微信会话', () => 打开手机会话('父亲'));
  const 回国手机提交可用 = (data: SchemaType, 凭据: unknown, 允许无消息 = false) =>
    回国提交凭据有效(
      凭据,
      {
        聊天ID: 当前聊天ID(),
  ...
  eventOn('人妻公寓:回国父亲微信已送达', (类型原: unknown, 凭据: unknown) =>
    安全操作(async (raw, data) => {
      if (!回国手机提交可用(data, 凭据)) return;
      const 类型 = String(类型原 ?? '');
      const 结果 =
        类型 === '通知'
          ? 提交回国父亲通知已送达(data)
          : 类型 === '询问'
            ...
            : 类型 === '准备'
              ? 提交回国父亲准备答复已送达(data)
              : { 成功: false, 变动: false, 提示: '' };
      if (!结果.成功 || !结果.变动) return;
      await 脚本写入(raw, data);
      捕获保护快照(data);
      if (结果.提示) eventEmit('人妻公寓:提示', 结果.提示);
    }),
  );
  eventOn('人妻公寓:回国父亲会话玩家已回复', (类型原: unknown, 凭据: unknown) => {
    const 类型 = String(类型原 ?? '') === '确认准备' ? '确认准备' : '无需收尾';
    void 安全操作(async (raw, data) => {
      if (!回国手机提交可用(data, 凭据)) return;
      const 已写气泡 = await 写回国母亲固定气泡(类型);
      if (!已写气泡) {
        eventEmit('人妻公寓:提示', '母亲这句话暂时没有写进父亲会话，可以重新发送一条回复后重试。');
        return;
      }
      if (!回国手机提交可用(data, 凭据)) return;
      const 结果 = 提交回国父亲会话母亲气泡(data, 类型, 当前楼层());
      if (!结果.成功 || !结果.变动) return;
      await 脚本写入(raw, data);
      捕获保护快照(data);
      if (结果.提示) eventEmit('人妻公寓:提示', 结果.提示);
  ...
  eventOn('人妻公寓:邀请母亲加入姐妹群', () =>
    安全操作(async (raw, data) => {
      // 陈旧手机界面的按钮也必须按最新路线状态失败关闭，不能先留下孤立的入群系统消息。
      if (!回国母亲可邀请入群(data)) {
        eventEmit('人妻公寓:提示', '母亲现在还不能加入姐妹茶话会。');
        return;
      }
      const 已写消息 = await 写回国母亲入群消息();
      if (!已写消息) {
        eventEmit('人妻公寓:提示', '母亲暂时没有加入群聊，可以重新点击邀请。');
        return;
      }
      const 结果 = 提交回国母亲加入姐妹群(data);
      if (!结果.成功 || !结果.变动) return;
      await 脚本写入(raw, data);
      捕获保护快照(data);
      if (结果.提示) eventEmit('人妻公寓:提示', 结果.提示);
      播放回国CG(结果);
      // 此刻只落了入群系统提示。状态事务退出后保持原群名约五秒，再真实写入改名提示并启动首轮反应；
      // 不在 MVU 保存队列里等待计时或 AI，避免这段手机演出阻塞其他变量提交。
      void 同步回国延迟改名与群名反应(data).catch(error =>
        console.warn('[人妻公寓·回国] 延迟改名或首轮反应未完成，下一次手机节拍会继续恢复:', error),
      );
    }),
  );
  // 五秒窗口内刷新会丢掉内存输入租约；chat 页依据当前时间线的入群硬消息发出恢复事件。
  // 这里不能占用 MVU 保存队列等待计时，只读取最新 stat 后重启带时间线租约的手机演出。
  eventOn('人妻公寓:恢复回国姐妹群改名', () => {
    const rawStat = 读最近有效stat();
    if (!rawStat) return;
    const data = Schema.parse(rawStat) as SchemaType;
    void 同步回国延迟改名与群名反应(data).catch(error =>
      console.warn('[人妻公寓·回国] 刷新后的延迟改名恢复失败，将在下一次打开群聊时继续:', error),
    );
  });
  eventOn('人妻公寓:回国茶话会批次完成', (原提交: unknown, 凭据: unknown) =>
    安全操作(async (raw, data) => {
      const 本次凭据 = _.cloneDeep(凭据);
      const 实存消息 = 读微信库().消息;
      if (!回国提交凭据有效(本次凭据, {
        聊天ID: 当前聊天ID(),
        世代: 读取当前手机时间线租约世代(),
      ...
      const 提交 = (原提交 ?? {}) as 回国茶话会批次提交;
      const 结果 = 提交回国茶话会批次(data, 提交, 本次凭据.消息, SillyTavern.name1);
      if (!结果.成功 || !结果.变动) return;
      await 脚本写入(raw, data);
      捕获保护快照(data);
      if (结果.提示) eventEmit('人妻公寓:提示', 结果.提示);
  ...
  eventOn('人妻公寓:回国茶话会后私聊已送达', (门牌原: unknown, 凭据: unknown) =>
    安全操作(async (raw, data) => {
      if (!回国手机提交可用(data, 凭据)) return;
      const 门牌号 = String(门牌原 ?? '') as 门牌;
      const 结果 = 提交回国茶话会后私聊已送达(data, 门牌号);
      if (!结果.成功 || !结果.变动) return;
      await 脚本写入(raw, data);
      捕获保护快照(data);
    }),
  );
  eventOn('人妻公寓:回国茶话会后私聊暂缓', (门牌原: unknown, 凭据: unknown) =>
    安全操作(async (raw, data) => {
      if (!回国手机提交可用(data, 凭据, true)) return;
      const 门牌号 = String(门牌原 ?? '') as 门牌;
      const 结果 = 暂缓回国茶话会后私聊(data, 门牌号);
      if (!结果.成功 || !结果.变动) return;
      await 脚本写入(raw, data);
      捕获保护快照(data);
    }),
  ...
  // 手机消息是V4确认的唯一强凭据。通知模块只广播“请重读当前库”，主状态绝不相信回调载荷。
  eventOn('人妻公寓:录像带V4微信凭据变化', () => {
    void 安全操作(
      async (raw, data) => {
        const 结果 = 同步录像带V4微信收据(data, 读微信库().消息);
        if (!结果.变动) {
          eventEmit('人妻公寓:录像带V4状态');
          return;
        }
  ...
  // 《离婚》邀请以201私聊的真实持久气泡为唯一送达凭据；回调只提示重读手机库，
  // 主状态不信任载荷，刷新/崩溃后只要同键气泡仍在就能幂等补齐。
  eventOn('人妻公寓:许曼君离婚邀请已送达', () => {
    void 安全操作(
      async (raw, data) => {
        const 实存 = 读微信库().消息.filter(
          消息 => 消息.会话 === '201' && 消息.发 === '对方' && 消息.键 === 许曼君离婚邀请消息键,
        );
        if (实存.length !== 1) return;
        const 结果 = 提交许曼君离婚邀请已送达(data);
        if (!结果.成功 || !结果.变动) return;
        await 脚本写入(raw, data, { 记录成长: false, 当前绝对时段: data.系统._绝对时段 });
        捕获保护快照(data);
        if (结果.提示) eventEmit('人妻公寓:提示', 结果.提示);
  ...
  // 201新日常反馈以手机持久库中唯一同键私聊为送达凭据；回调载荷只用于定位MVU待办，
  // 主状态仍会重读当前库、校验到期时段和唯一性，写库失败时下个手机节拍继续重试。
  eventOn('人妻公寓:许曼君离婚后日常反馈已送达', (消息键原: string) => {
    void 安全操作(
      async (raw, data) => {
        const 消息键 = String(消息键原 ?? '').trim();
        const 待办 = data.系统._许曼君离婚后日常.待反馈事件.find(
          item => item.消息键 === 消息键 && item.可发送时段 >= 0 && data.系统._绝对时段 >= item.可发送时段,
        );
        if (!待办) return;
        const 实存 = 读微信库().消息.filter(
          消息 => 消息.会话 === '201' && 消息.发 === '对方' && 消息.键 === 消息键,
        );
        if (实存.length !== 1) return;
        if (!提交许曼君离婚后日常反馈已送达(data, 消息键)) return;
        await 脚本写入(raw, data, { 记录成长: false, 当前绝对时段: data.系统._绝对时段 });
        捕获保护快照(data);
      },
      false,
  ...
  eventOn('人妻公寓:请求手机补拍', 空闲后手机节拍.请求);
  ...
  eventOn('人妻公寓:回合完成', (选项?: { 跳过手机节拍?: boolean }) => {
    ...
    if (选项?.跳过手机节拍) {
      ...
      // 核心结果提交后，主回合、变量解析、时间事务与 MVU 队列仍可能处在最后收尾。
      // 手机送达回执会再次进入主变量写队列，因此必须等所有运行期门真正空闲后再启动。
      空闲后手机节拍.请求();
    }
  });
  ...
  eventOn('人妻公寓:拍摄借种产后家庭合照', () =>
    安全操作((raw, data) => {
      ...
      const 陆嘉明在场 = 当前地点 === '医院' || (!!户 && 丈夫在楼(户, '101', data.系统._绝对时段) !== '外出');
      return 即时开演(
        () => {
  ...
  eventOn('人妻公寓:查看摄像头', (门牌号: 门牌) => {
    ...
    return 安全操作(
      async (raw, data, 操作仍有效) => {
        ...
        eventEmit('人妻公寓:监控回合');
        let 线路消息: string[] = [];
        const 第二机位票 = { 结果: undefined as 第二机位结果 | undefined, CG: '' };
        ...
        try {
          ...
          await 顺序提交隔离事件({
            ...
            写核心: async () => {
              复核隔离时间线身份(身份, 操作仍有效);
              if ('第二机位节点' in 结果) {
                const 提交 = 提交第二机位监控(data, 结果.第二机位节点);
                if (!提交.成功 || !提交.变动) throw new Error(提交.提示 || '第二机位监控复核未能提交');
                第二机位票.结果 = 提交;
                第二机位票.CG = 结果.CG;
              } else if ('家庭计划节点' in 结果) {
          ...
          try {
            ...
            const 第二机位提交 = 第二机位票.结果;
            if (第二机位提交) {
              eventEmit('人妻公寓:提示', 第二机位提交.提示);
              播放第二机位CG({ ...第二机位提交, CG: 第二机位票.CG });
            }
  ...
  eventOn('人妻公寓:不再留门动作', (动作: 不再留门动作ID): void | Promise<unknown> => {
    if (_时间推进中) { eventEmit('人妻公寓:提示', '时间正在推进，请等地图刷新后继续。'); return; }
    if (隔离事件进行中()) { eventEmit('人妻公寓:提示', '当前独立现场尚未结束，请先完成当前事件。'); return; }
    if (动作 === '确认当前决定') {
      // A5/A7/A9仍持有一张“需回应”的普通场景票；必须复用场景事务入口激活/重试，不能直接当普通行动执行。
      return eventEmit('人妻公寓:继续场景剧情', { 不再留门确认: true });
    }
    void 安全操作(async (raw, data) => {
      const 通知聊天ID = 当前聊天ID();
      const 地点 = 读场景().房间id ?? '';
      ...
      if (动作 === '暂缓' || 动作 === '撤回许可') {
        const 当前本线控制 = 不再留门控制动作可穿过自身剧情锁(data, 动作, 地点);
        const 无前台撤回许可 = 动作 === '撤回许可' && 地点 === '202' && !有普通场景剧情阻塞当前场景(data, 地点);
        if (data.系统._特殊场景.id || data.系统._荣耀洞拍 >= 0 || (!当前本线控制 && !无前台撤回许可)) {
          eventEmit('人妻公寓:提示', '当前地点或剧情票已经变化，请重新查看本线进展。');
          return;
        }
        const 操作前 = _.cloneDeep(data);
        const 结果 = 执行不再留门动作(data, 动作, 地点, 当前楼层());
        const 解除地点校验 = 登记MVU提交校验(() => (读场景().房间id ?? '') === 地点);
        try {
          await 提交即时业务场景控制(操作前, data, () => 落地(结果, raw, data, { 本线场景控制: true }));
          eventEmit('人妻公寓:场景剧情状态');
        } finally { 解除地点校验(); }
        return;
      }
      const 首拍重试 = 动作 === '使用道具' && !r.道具已使用 && r.当前场景 === 'A1';
      if (首拍重试 && 读取不再留门当前剧情票(data)) {
        await 执行回合('我问问小满今天的安排。', { 已持MVU操作租约: true });
        return;
      }
      const 准备 = () => 执行不再留门动作(data, 动作, 地点, 当前楼层(), `${当前聊天ID()}@${当前时间线切换世代()}`, `nmd-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`);
      const 需要正文 = ['使用道具', '观察街对面', '出示照片', '听她决定', '开始录制', '检查记录', '继续现场'].includes(动作);
      if (动作 === '继续现场' && r.当前拍 > 1) {
        await 落地(准备(), raw, data, { 标题: '不再留门', 场景: 地点 });
        return;
      }
      if (需要正文) {
        await 即时开演(准备, raw, data, 动作 === '使用道具' ? '我问问小满今天的安排。' : '我留在现场，继续眼前这一步。', 结果 => Boolean(结果.事件), { 标题: '不再留门', 场景: 地点, 来源: '不再留门地点动作' });
        return;
      }
      const 解除地点校验 = 登记MVU提交校验(() => (读场景().房间id ?? '') === 地点);
      try {
  ...
  eventOn('人妻公寓:第二机位动作', (动作原: 第二机位动作ID) => {
    if (_时间推进中) {
      eventEmit('人妻公寓:提示', '时间正在推进，请等地图刷新后再继续《第二机位》。');
      return;
    }
    void 安全操作(async (raw, data) => {
      const 当前地点 = 读场景().房间id ?? '';
      const 动作 = String(动作原 ?? '') as 第二机位动作ID;
      if (!第二机位地点动作(data, 当前地点).some(item => item.id === 动作)) {
        eventEmit('人妻公寓:提示', '地点、日期、人物或《第二机位》状态已经变化，请重新查看房间动作。');
        return;
      }
      if (有普通场景剧情阻塞当前场景(data, 当前地点) || data.系统._特殊场景.id || 隔离事件进行中()) {
        eventEmit('人妻公寓:提示', '当前事件尚未结束，《第二机位》暂不能继续。');
        return;
      }

      const 纯硬操作 = 动作 === '安装第二机位' || 动作 === '归档母带';
      if (纯硬操作) {
        const 结果 = 执行第二机位地点动作(data, 动作, 当前地点, 当前楼层());
        const 已落地 = await 落地({ 提示: 结果.提示, 变动: 结果.变动 }, raw, data);
        if (已落地 && 结果.成功) 播放第二机位CG(结果);
        return;
      }

      const 行动表: Record<Exclude<第二机位动作ID, '安装第二机位' | '归档母带'>, string> = {
        门缝那一眼: '（按约来到102，从一场寻常见面开始，留意沈静仪是否愿意把你留在里间）',
        告知空窗: '（把顾国栋酒桌上提到的远差、路程和可能在外过夜自然告诉沈静仪，停下来等她先问清自己在意的细节）',
        开始录制: '（把CAM-2的录制遥控器交给沈静仪，让她先完成最后试机；此刻REC还没有按下）',
        封存母带: '（留在102，请沈静仪先停下CAM-2并一起检查这次记录是否完整；此刻记录卡仍留在设备里）',
      };
      const 标题表: Record<Exclude<第二机位动作ID, '安装第二机位' | '归档母带'>, string> = {
        门缝那一眼: '第二机位 · 102赴约',
        告知空窗: '第二机位 · 转述远差',
        开始录制: '第二机位 · 最后试机',
        封存母带: '第二机位 · 停机回看',
      ...
      return 即时开演(
        () => 执行第二机位地点动作(data, 动作, 当前地点, 当前楼层()),
        raw,
        data,
        行动表[动作],
        结果 => Boolean(结果.事件),
        { 标题: 标题表[动作], 场景: 当前地点, 来源: '第二机位地点动作' },
      );
    });
  });

  eventOn('人妻公寓:安若妍结局后亲密', (choice: '由我开始' | '让她开始') => {
    if (_时间推进中 || !['由我开始', '让她开始'].includes(choice)) return;
    void 安全操作(async (_raw, data) => {
      if (!安若妍结局后亲密可用(data, 读场景().房间id ?? '')) return;
      await 执行回合(安若妍结局后亲密行动(choice), { 资源计费: false, 可重掷: false, 已持MVU操作租约: true });
    });
  });
  eventOn('人妻公寓:录像带结局后亲密', (载荷: { 门牌: 录像带结局角色; 选择: 录像带结局后开场选择 }) => {
    if (_时间推进中 || !载荷 || !['102', '202'].includes(载荷.门牌) || !['由我开始', '让她开始'].includes(载荷.选择)) return;
    void 安全操作(async (_raw, data) => {
      const 阻断 = 录像带结局后亲密阻断(data, 载荷.门牌, 读场景().房间id ?? '');
      if (阻断) { eventEmit('人妻公寓:提示', 阻断); return; }
      await 执行回合(录像带结局后亲密行动(载荷.门牌, 载荷.选择), { 资源计费: false, 可重掷: false, 已持MVU操作租约: true });
    });
  });
  eventOn('人妻公寓:安若妍换掉动作', (动作: 安若妍换掉动作ID) => {
    if (_时间推进中) { eventEmit('人妻公寓:提示', '请等时间推进完成后继续。'); return; }
    const 完成通知聊天 = 当前聊天ID();
    const 完成通知世代 = 当前时间线切换世代();
    let 换照已提交 = false;
    void 安全操作(async (raw, data) => {
      const 地点 = 读场景().房间id ?? '';
      // 到期重排只在副本上参与按钮终审；执行器会在撤回快照捕获后于真实 data 上再次同步。
      const 候选数据 = _.cloneDeep(data) as SchemaType;
      同步安若妍换掉时间节点(候选数据);
      const 候选 = 安若妍换掉地点动作(候选数据, 地点).find(item => item.id === 动作);
      if (!候选?.可执行) { eventEmit('人妻公寓:提示', 候选?.原因 || '线路步骤已经变化，请重新查看。'); return; }
      if (有普通场景剧情阻塞当前场景(data, 地点) || data.系统._特殊场景.id || 隔离事件进行中()) return;
      ...
      if (['购买拍立得', '登记江辰到访', '换掉结婚照', '暂缓预约夜'].includes(动作)) {
        ...
        if (saved && 结果.成功 && 动作 === '换掉结婚照' && 仍在本次时间线()) 换照已提交 = true;
        return;
      }
      return 即时开演(准备, raw, data, '（在' + 地点 + '执行《换掉》的“' + 候选.文案 + '”，只推进眼前这一拍）',
        结果 => Boolean(结果.事件), { 标题: '换掉 · ' + 候选.文案, 场景: 地点, 来源: '安若妍换掉地点动作' });
    }).then(() => {
      if (
        !换照已提交 ||
        当前聊天ID() !== 完成通知聊天 ||
      ...
      try {
        ...
        console.error('[人妻公寓] 《换掉》结婚照已经提交，但完成通知监听器执行失败:', 通知错误);
      }
    });
  });

  eventOn('人妻公寓:安若妍不必停动作', (动作原: 安若妍不必停动作ID) => {
    if (_时间推进中) {
      eventEmit('人妻公寓:提示', '时间正在推进，请等地图刷新后再继续《不必停》。');
      return;
    }
    void 安全操作(async (raw, data) => {
      const 当前地点 = 读场景().房间id ?? '';
      const 动作 = String(动作原 ?? '') as 安若妍不必停动作ID;
      const 候选 = 安若妍不必停地点动作(data, 当前地点).find(item => item.id === 动作);
      if (!候选?.可执行) {
        eventEmit('人妻公寓:提示', 候选?.原因 || '地点、日期、人物、物件或《不必停》状态已经变化，请重新查看房间动作。');
        return;
      }
      if (有普通场景剧情阻塞当前场景(data, 当前地点) || data.系统._特殊场景.id || 隔离事件进行中()) {
        eventEmit('人妻公寓:提示', '当前事件尚未结束，《不必停》暂不能继续。');
        return;
      }
      ...
      if (动作 === '停下本次暂缓') {
        const 线路结果 = 准备();
        if (!线路结果.成功 || !线路结果.需暂停亲密) {
          eventEmit('人妻公寓:提示', 线路结果.提示);
          return;
        }
        const 暂缓结果 = 结算安若妍不必停H7暂缓(data, 当前楼层());
        await 落地(
          {
            ...暂缓结果,
            提示: [线路结果.提示, 暂缓结果.提示].filter(Boolean).join('\n'),
          },
          raw,
          data,
        );
      ...
      return 即时开演(
        ...
        raw,
        data,
        `（在${当前地点}执行《不必停》的“${候选.文案}”，只推进眼前这一拍）`,
        结果 => Boolean(结果.事件),
        { 标题: `不必停 · ${候选.文案}`, 场景: 当前地点, 来源: '安若妍不必停地点动作' },
      );
    });
  });

  eventOn('人妻公寓:许曼君分居动作', (动作原: 许曼君分居动作ID) => {
    if (_时间推进中) {
      eventEmit('人妻公寓:提示', '时间正在推进，请等地图刷新后再继续《分居》。');
      return;
    }
    void 安全操作(async (raw, data) => {
      const 当前地点 = 读场景().房间id ?? '';
      const 动作 = String(动作原 ?? '') as 许曼君分居动作ID;
      const 候选 = 许曼君分居地点动作(data, 当前地点).find(item => item.id === 动作);
      if (!候选) {
        eventEmit('人妻公寓:提示', '地点、时间、人物、物件或《分居》状态已经变化，请重新查看房间动作。');
        return;
      }
      if (有普通场景剧情阻塞当前场景(data, 当前地点) || data.系统._特殊场景.id || 隔离事件进行中()) {
        eventEmit('人妻公寓:提示', '当前事件尚未结束，《分居》暂不能继续。');
        return;
      }
      if (读取许曼君分居重答(data)?.动作 === 动作) {
        // 重答只恢复输入机会。落地统一排成需回应等待票，既不调用模型，也不提前结算选择。
        await 落地(
          执行许曼君分居地点动作(data, 动作, 当前地点, 当前楼层()),
          raw,
          data,
          { 标题: `分居 · ${候选.文案}`, 场景: 当前地点, 来源: '许曼君分居地点动作' },
        );
        eventEmit('人妻公寓:场景剧情状态');
        return;
      }
      return 即时开演(
        () => {
          const 结果 = 执行许曼君分居地点动作(data, 动作, 当前地点, 当前楼层());
          return {
            ...结果,
            提交后: 结果.CG ? () => 播放许曼君分居CG(结果) : undefined,
          };
        },
        raw,
        data,
        `（在${当前地点}执行《分居》的“${候选.文案}”，只推进眼前这一个人物决定或硬物件动作）`,
        结果 => Boolean(结果.事件),
        { 标题: `分居 · ${候选.文案}`, 场景: 当前地点, 来源: '许曼君分居地点动作' },
      );
    });
  ...
  eventOn('人妻公寓:许曼君离婚动作', (动作原: 许曼君离婚动作ID) => {
    if (_时间推进中) {
      eventEmit('人妻公寓:提示', '时间正在推进，请等地图刷新后再继续《离婚》。');
      return;
    }
    void 安全操作(async (raw, data) => {
      const 当前地点 = 读场景().房间id ?? '';
      const 动作 = String(动作原 ?? '') as 许曼君离婚动作ID;
      // 完成态旧档迁移可能改写路线、丈夫与背包；这里只在副本上决定按钮是否仍成立。
      const 候选数据 = _.cloneDeep(data) as SchemaType;
      同步许曼君离婚完成后状态(候选数据);
      const 动作组 = 许曼君离婚地点动作(候选数据, 当前地点);
      const 候选 = 查找许曼君离婚事件候选(动作组, 动作);
      if (!候选) {
        eventEmit('人妻公寓:提示', '地点、时段、人物、物件或《离婚》状态已经变化，请重新查看房间动作。');
        return;
      }
      if (有普通场景剧情阻塞当前场景(data, 当前地点) || data.系统._特殊场景.id || 隔离事件进行中()) {
        eventEmit('人妻公寓:提示', '当前事件尚未结束，《离婚》暂不能继续。');
        return;
      }

      if (动作 === '选择红本' || 动作 === '选择婚戒' || 动作 === '选择戒印') {
        同步许曼君离婚完成后状态(data);
        const 预检 = 执行许曼君离婚地点动作(data, 动作, 当前地点, 当前楼层());
        if (!预检.成功 || !预检.目标 || !预检.需普通亲密回合) {
          eventEmit('人妻公寓:提示', 预检.提示 || '《最后一笔》目标已经变化。');
          return;
        }
        const 成功 = await 执行回合(许曼君离婚H3行动(预检.目标), {
          资源计费: true,
          可重掷: false,
          已持MVU操作租约: true,
        });
        if (成功) eventEmit('人妻公寓:提示', `目标“${预检.目标}”已经冻结，H3～H6将严格使用当前唯一201普通亲密场次。`);
        return;
      }
      ...
      if (动作 === '由我开始' || 动作 === '让她开始') {
        ...
        const 预检 = 执行许曼君离婚地点动作(data, 动作, 当前地点, 当前楼层());
        if (!预检.成功 || !预检.需普通亲密回合) {
          eventEmit('人妻公寓:提示', 预检.提示 || '201结局后亲密入口已经变化。');
          return;
        }
        await 执行回合(许曼君结局后亲密行动(动作), {
          资源计费: false,
          可重掷: false,
          已持MVU操作租约: true,
        });
      ...
      if (动作 === '确认射在这里' || 动作 === '停下，今晚不封存') {
        同步许曼君离婚完成后状态(data);
        const 预检 = 执行许曼君离婚地点动作(data, 动作, 当前地点, 当前楼层());
        if (!预检.成功 || !预检.需H8收尾) {
          eventEmit('人妻公寓:提示', 预检.提示 || 'H8专属收尾已经变化。');
          return;
        }
        const 结果 = 结算许曼君离婚H8收尾(data, 预检.需H8收尾, 当前楼层());
        const 是确认结果 = 预检.需H8收尾 === '确认';
        const 已落地 = await 落地({ 提示: 是确认结果 ? '' : 结果.提示, 变动: 结果.变动 }, raw, data);
        if (已落地 && 结果.成功) {
          播放许曼君离婚CG(结果);
          // H8确认先切结果图与白闪，再显示说明；“停止”没有结果图，仍由落地层直接提示。
          if (是确认结果 && 结果.提示) eventEmit('人妻公寓:提示', 结果.提示);
        }
        return;
      }

      const 剧情动作 = new Set<许曼君离婚动作ID>([
        '使用红色封存盒',
        '陪她去办最后手续',
        '当着赵国强牵住她',
        '等赵国强离开再抱她',
      ...
      if (剧情动作.has(动作)) {
        const 标题表: Partial<Record<许曼君离婚动作ID, string>> = {
          使用红色封存盒: '离婚 · 约定办理',
          陪她去办最后手续: '离婚 · 正式办理',
          当着赵国强牵住她: '离婚 · 出口见证',
          等赵国强离开再抱她: '离婚 · 出口见证',
          确认201继续由她居住: '离婚 · 201居住确认',
          开始最后一笔: '离婚 · 最后一笔',
          打开红色封存盒: '离婚 · 打开红盒',
          摆好锁定目标: '离婚 · 摆好目标',
          完成H8结果演出: '离婚 · 唯一结果',
        ...
        return 即时开演(
          () => {
            同步许曼君离婚完成后状态(data);
            return 执行许曼君离婚地点动作(data, 动作, 当前地点, 当前楼层());
          },
          raw,
          data,
          `（在${当前地点}执行《离婚》的“${动作}”，只推进当前这一拍）`,
          结果 => Boolean(结果.事件),
          { 标题: 标题表[动作] ?? '许曼君 · 离婚', 场景: 当前地点, 来源: '许曼君离婚地点动作' },
        );
      }

      同步许曼君离婚完成后状态(data);
      const 结果 = 执行许曼君离婚地点动作(data, 动作, 当前地点, 当前楼层());
      const 已落地 = await 落地({ 提示: 结果.提示, 变动: 结果.变动 }, raw, data);
      if (已落地 && 结果.成功) 播放许曼君离婚CG(结果);
    });
  });

  eventOn('人妻公寓:许曼君离婚后日常动作', (动作原: 许曼君离婚后日常动作ID) => {
    if (_时间推进中) {
      eventEmit('人妻公寓:提示', '时间正在推进，请等地图刷新后再继续201的新日常。');
      return;
    }
    void 安全操作((raw, data) => {
      const 当前地点 = 读场景().房间id ?? '';
      const 动作 = String(动作原 ?? '') as 许曼君离婚后日常动作ID;
      // 旧档完成态只在副本上参与候选判断；真实迁移与本次 D1/D2 事件在捕获后原子结算。
      const 候选数据 = _.cloneDeep(data) as SchemaType;
      同步许曼君离婚完成后状态(候选数据);
      const 动作组 = 许曼君离婚后日常地点动作(候选数据, 当前地点);
      const 候选 = 动作组.find(item => item.id === 动作 || item.选项?.some(option => option.id === 动作));
      if (!候选) {
        eventEmit('人妻公寓:提示', '地点、时段、人物、关系或201日常检查点已经变化，请重新查看房间动作。');
        return;
      }
      if (有普通场景剧情阻塞当前场景(data, 当前地点) || data.系统._特殊场景.id || 隔离事件进行中()) {
        eventEmit('人妻公寓:提示', '当前事件尚未结束，201的新日常暂不能继续。');
        return;
      }
      return 即时开演(
        () => {
          同步许曼君离婚完成后状态(data);
          return 执行许曼君离婚后日常动作(data, 动作, 当前地点, 当前楼层());
        },
        raw,
        data,
        `（在201执行离婚后新日常“${候选.主题}”，只演当前D1或D2这一回合）`,
        结果 => Boolean(结果.事件),
        { 标题: `201新日常 · ${候选.主题}`, 场景: 当前地点, 来源: '许曼君离婚后日常动作' },
      );
    });
  });

  eventOn('人妻公寓:使用回国经营归档册', () => {
    if (_时间推进中) {
      eventEmit('人妻公寓:提示', '时间正在推进，请等地图刷新后再使用经营归档册。');
      return;
    }
    void 安全操作((raw, data) => {
      const 当前地点 = 读场景().房间id ?? '';
      return 即时开演(
        ...
        raw,
        data,
        '（在管理员室铺开近几期真实经营记录，把公寓已经稳定运行的事实整理进归档册）',
        结果 => Boolean(结果.事件),
        { 标题: '回国 · 经营归档', 场景: 当前地点, 来源: '回国经营归档册' },
      );
    });
  });

  eventOn('人妻公寓:回国动作', (动作原: 回国地点动作ID) => {
    if (_时间推进中) {
      eventEmit('人妻公寓:提示', '时间正在推进，请等地图刷新后再继续《回国》。');
      return;
    }
    void 安全操作(async (raw, data) => {
      const 当前地点 = 读场景().房间id ?? '';
      const 动作 = String(动作原 ?? '') as 回国地点动作ID;
      if (!回国地点动作(data, 当前地点).some(item => item.id === 动作)) {
        eventEmit('人妻公寓:提示', '地点、日期、人物或《回国》状态已经变化，请重新查看房间动作。');
        return;
      }
      if (有普通场景剧情阻塞当前场景(data, 当前地点) || data.系统._特殊场景.id || 隔离事件进行中()) {
        eventEmit('人妻公寓:提示', '当前事件尚未结束，《回国》暂不能继续。');
        return;
      }
      const 结果 = 执行回国地点动作(data, 动作, 当前地点);
      if (结果.打开父亲会话) {
        eventEmit('人妻公寓:打开父亲微信会话');
        return;
      }
      if (!结果.事件) {
        const 已落地 = await 落地({ 提示: 结果.提示, 变动: 结果.变动 }, raw, data);
      ...
      const 标题表: Partial<Record<回国地点动作ID, string>> = {
        阅读回国消息: '回国 · 消息落进302',
        收纳私人物件: '回国 · 收好证据',
        查看管理记录: '回国 · 无需圆场',
        展示姐妹茶话会: '回国 · 母亲看见成员名单',
      ...
      return 即时开演(
        () => 执行回国地点动作(data, 动作, 当前地点),
        raw,
        data,
        `（在${当前地点}继续《回国》的当前步骤，不越过本轮已经给出的现场边界）`,
        当前结果 => Boolean(当前结果.事件),
        { 标题: 标题表[动作] ?? '回国', 场景: 当前地点, 来源: '回国地点动作' },
      );
    });
  });

  eventOn('人妻公寓:使用双重继承场景票', () => {
    if (_时间推进中) {
      eventEmit('人妻公寓:提示', '时间正在推进，请等地图刷新后再准备正式交接。');
      return;
    }
    void 安全操作(async (raw, data) => {
      const 当前地点 = 读场景().房间id ?? '';
      const 结果 = 使用双重继承场景票(data, 当前地点, 当前楼层());
      await 落地({ 提示: 结果.提示, 变动: 结果.变动 }, raw, data);
    });
  });

  eventOn('人妻公寓:双重继承动作', (动作原: 双重继承动作ID) => {
    if (_时间推进中) {
      eventEmit('人妻公寓:提示', '时间正在推进，请等地图刷新后再继续《双重继承》。');
      return;
    }
    void 安全操作(async (raw, data) => {
      const 当前地点 = 读场景().房间id ?? '';
      const 动作 = String(动作原 ?? '') as 双重继承动作ID;
      if (!双重继承地点动作(data, 当前地点).some(item => item.id === 动作)) {
        eventEmit('人妻公寓:提示', '地点、日期或《双重继承》状态已经变化，请重新查看房间动作。');
        return;
      }
      const 结果 = 执行双重继承地点动作(data, 动作, 当前地点, 当前楼层());
      if (!结果.事件) {
        const 已落地 = await 落地({ 提示: 结果.提示, 变动: 结果.变动 }, raw, data);
        if (已落地) 播放双重继承CG(结果);
        return;
      }
      const 标题表: Partial<Record<双重继承动作ID, string>> = {
        管理员室交权: '双重继承 · 完全交权',
        家庭早餐: '双重继承 · 三人早餐',
        归位总钥匙: '双重继承 · 最后现实收束',
      };
      const 行动表: Partial<Record<双重继承动作ID, string>> = {
        管理员室交权: '（在管理员室与父母汇总八区检查，完成完全交权）',
        家庭早餐: '（在302与父母完成三人早餐）',
        归位总钥匙: '（机场视频已经结束；把公寓楼总钥匙放到母亲的钥匙旁，只完成这一个最后现实动作）',
      };
      return 即时开演(
        () => 执行双重继承地点动作(data, 动作, 当前地点, 当前楼层()),
        raw,
        data,
        行动表[动作] ?? '（继续《双重继承》的当前现实动作）',
        当前结果 => Boolean(当前结果.事件),
        { 标题: 标题表[动作] ?? '双重继承', 场景: 当前地点, 来源: '双重继承地点动作' },
      );
    });
  });

  eventOn('人妻公寓:302共居动作', (动作原: 共居动作ID) => {
    if (_时间推进中) {
      eventEmit('人妻公寓:提示', '时间正在推进，请等302状态刷新后再继续。');
      return;
    }
    void 安全操作(async (_raw, data) => {
      同步302共居状态(data);
      const 地点 = 读场景().房间id ?? '';
      const 动作值 = String(动作原 ?? '');
      if (!是母亲共居动作ID(动作值)) {
        eventEmit('人妻公寓:提示', '未知的302共同生活动作，未改变任何状态。');
        return;
      }
      const 动作 = 动作值;
      const 预期绝对时段 = data.系统._绝对时段;
      const 阻断 = 母亲共居动作阻断(data, 动作, 地点, 预期绝对时段);
      if (阻断) {
        eventEmit('人妻公寓:提示', 阻断);
        return;
      }
      if (有普通场景剧情阻塞当前场景(data, 地点) || data.系统._特殊场景.id || 隔离事件进行中()) {
        eventEmit('人妻公寓:提示', '当前事件尚未结束，302共同生活动作暂不能继续。');
        return;
      }
      let 提交提示 = '';
      let 开场CG: ReturnType<typeof 母亲共居亲密开场CG> = null;
      let 共居时间撤销基准: { 推进前数据: SchemaType; 方式: 时间推进方式 } | null = null;
      const 成功 = await 执行回合(母亲共居行动文本(动作), {
        资源计费: false,
        可重掷: false,
        已持MVU操作租约: true,
        系统注入: 母亲共居动作系统注入(data, 动作),
        成功结算: newData => {
          if ((读场景().房间id ?? '') !== 地点) throw new Error('302共同生活结算时地点已经变化。');
          const 结果 = 提交302共居动作(newData, 动作, 地点, 预期绝对时段);
          if (!结果.成功 || !结果.变动) throw new Error(结果.提示 || '302共同生活状态没有提交。');
          提交提示 = 结果.提示;
          开场CG = 母亲共居亲密开场CG(newData, 动作);
          if (结果.推进方式) {
            const 推进前数据 = _.cloneDeep(newData) as SchemaType;
            const 时间结果 = 执行时间推进事务(newData, {
              方式: 结果.推进方式,
              预期绝对时段,
              当前消息楼: 当前楼层(),
              当前地点: 地点,
            ...
            if (!时间结果.成功) throw new Error(时间结果.提示 || '302共同生活后的时间推进没有提交。');
            共居时间撤销基准 = { 推进前数据, 方式: 结果.推进方式 };
            作废当前手机时间线租约世代();
            提交提示 = [提交提示, 时间结果.提示, ...时间结果.线路提示, ...时间结果.经济提示, ...时间结果.资源提示]
              .filter(Boolean)
              .join('\n');
          }
        },
        成功核心聊天结算: (vars, newData) => {
          const 撤销基准 = 共居时间撤销基准;
          if (!撤销基准) return;
          const 聊天ID = 当前聊天ID();
          const 锚楼 = 当前楼层();
          const 锚消息签名 = 手机锚消息签名(SillyTavern.chat?.[锚楼]);
          if (!聊天ID || 锚楼 < 0 || !锚消息签名) {
            throw new Error('共同生活时间已结算，但无法建立安全的时间撤销锚。');
          }
          const 推进前聊天快照 = 捕获精确聊天快照(vars, 时间撤销恢复聊天键);
          vars[时间撤销点键] = 创建时间撤销点({
            聊天ID,
            锚楼,
            锚消息签名,
      ...
      if (成功 && 提交提示) eventEmit('人妻公寓:提示', 提交提示);
      if (成功 && 开场CG) {
        try {
          eventEmit('人妻公寓:302亲密开场CG', 开场CG);
        } catch (error) {
          // 亲密账本和正文已成功提交；纯视觉通知失败不能反向撤销场次。
          console.warn('[人妻公寓] 302亲密开场已提交，但专属CG通知失败:', error);
        }
      }
    });
  });

  eventOn('人妻公寓:启动录像带V4监控', () => {
    if (_时间推进中) {
      eventEmit('人妻公寓:回合失败', '时间正在推进，录像带监控没有启动。');
      return;
    }
    let 已处理 = false;
    let 入场已提交 = false;
    return 安全操作(
      async (raw, data, 操作仍有效) => {
        if (!要求当前地点('302', '监控系统没有成功切换到302，本次录像带没有启动。')) {
          已处理 = true;
          return;
        }
        if (有场景剧情阻塞(data)) {
          eventEmit('人妻公寓:回合失败', '当前仍有强制剧情或其他特殊场景，录像带没有启动。');
          已处理 = true;
          return;
        }
        const 住院门牌 = (['102', '202'] as const).find(房间 => !读取医院内容策略(data, 房间).允许成人特殊场景);
        if (住院门牌) {
          eventEmit('人妻公寓:回合失败', `${户静态表[住院门牌].妻名}仍在医院，录像带安排暂不能开始。`);
          已处理 = true;
          return;
        }

        const 场次标识 = 新录像带V4场次标识();
        const 准备 = 准备录像带V4监控(data, 场次标识);
        if (!准备.成功) {
          eventEmit('人妻公寓:回合失败', 准备.提示);
          已处理 = true;
          return;
        }
        if (准备.变动) {
        ...
        eventEmit('人妻公寓:录像带V4状态');
        await 执行录像带V4操作(raw, data, 操作仍有效, {
          操作标识: `start:${场次标识}`,
          类型: '开始',
        });
        已处理 = true;
  ...
  eventOn('人妻公寓:录像带V4操作', (原载荷: unknown) => {
    if (_时间推进中) {
      eventEmit('人妻公寓:回合失败', '时间正在推进，本次录像带操作没有发生。');
      return;
    }
    const 输入 = 解析录像带V4操作输入(原载荷);
    if (!输入) {
      eventEmit('人妻公寓:回合失败', '录像带V4操作载荷无效。');
      return;
    }
    let 已处理 = false;
    return 安全操作(
      async (raw, data, 操作仍有效) => {
        if (!要求当前地点('302', '录像带监控只能在系统已经锁定的302内继续。')) {
          已处理 = true;
          return;
        }
        await 执行录像带V4操作(raw, data, 操作仍有效, 输入);
        已处理 = true;
      },
      true,
      () => 已处理,
  ...
  eventOn('人妻公寓:安全中断录像带V4', () => {
    if (_时间推进中) {
      eventEmit('人妻公寓:提示', '时间正在推进，录像带监控暂不能安全退出。');
      return;
    }
    let 已提交 = false;
    return 安全操作(
      async (raw, data) => {
        if (!要求当前地点('302', '录像带监控只能在302内安全退出。')) return;
        const 中断 = 安全中断录像带V4(data);
        if (!中断.成功 || !中断.变动) {
          eventEmit('人妻公寓:提示', 中断.提示);
          return;
        }
        await 脚本写入(raw, data, { 记录成长: false, 当前绝对时段: data.系统._绝对时段 });
        已提交 = true;
  ...
  eventOn('人妻公寓:完成录像带V4', () => {
    if (_时间推进中) {
      eventEmit('人妻公寓:回合失败', '时间正在推进，录像带监控尚未结束。');
      return;
    }
    let 已提交 = false;
    return 安全操作(
      async (raw, data) => {
        if (!要求当前地点('302', '录像带监控只能在302完成收束。')) return;
        const 完成聊天 = 当前聊天ID();
        const 完成世代 = 当前时间线切换世代();
        const 完成仍有效 = () => 当前聊天ID() === 完成聊天 && 当前时间线切换世代() === 完成世代 && !时间线切换协调中();
        const 完成 = 完成录像带V4(data);
        if (!完成.成功 || !完成.变动) {
          eventEmit('人妻公寓:提示', 完成.提示);
          return;
        }
        await 脚本写入(raw, data, { 记录成长: false, 当前绝对时段: data.系统._绝对时段 });
        已提交 = true;
        await 同步全部角色阶段世界书(data, 完成仍有效, true);
        if (!完成仍有效()) return;
        捕获保护快照(data);
        eventEmit('人妻公寓:提示', 完成.提示);
        eventEmit('人妻公寓:录像带V4状态');
  ...
  // 录像带旧档兼容节点由 `推进特殊场景` 在正文成功结算中原子提交；
  // 不再暴露没有客户端生产者的平行宿主事件，避免形成不可达的第二状态所有者。
  ...
  eventOn('人妻公寓:使用承接剧情票', (道具id: string) =>
    安全操作((raw, data) => 落地(使用承接剧情票(data, String(道具id)), raw, data)),
  );

  eventOn('人妻公寓:使用录像带', () =>
    安全操作(async (raw, data) => {
      const 旧流程已开始 = 录像带沿用旧流程(data);
      if (!旧流程已开始) {
        const 结果 = 使用录像带V4(data);
        await 落地(结果, raw, data);
        return;
      }
      if (读场景().房间id !== '管理员室') {
  ...
  // 普通换装不请求正文、不登记赠礼或余波；沿用同场终审、时间线校验与串行写入。
  eventOn('人妻公寓:衣柜动作', (载荷: 衣柜动作) =>
    安全操作((raw, data) => {
      if (!载荷 || !妻在当前场景(data, 载荷.门牌)) {
        eventEmit('人妻公寓:提示', '她已经不在你身边了，穿戴没有改变。');
        return;
      }
      return 落地(执行衣柜动作(data, 载荷), raw, data);
    }),
  );
        ...
        () => {
          ...
          if (结果.动作 === '晋阶') {
            ...
            const 事件 =
              ...
              `${第一夜标记}${第一夜}用一场符合她性格与你们当下关系的正戏演出这次跨越,张力给足,不要一笔带过`;
  ...
  // 在数据库保存生成上下文前标记候选阶段。辅助模型可能借用主 API 并发出原生事件，
  // 只有当前消息转正后，回合引擎的正式广播才允许通用填表消费本轮正文。
  // eventMakeFirst 随本脚本卸载自动移除，不改变其他插件的监听或用户配置。
  eventMakeFirst(tavern_events.GENERATION_STARTED, (_类型, 选项, dryRun) => {
    标记未提交回合生成事件(SillyTavern.chat ?? [], 选项, Boolean(dryRun));
  });
  ...
  eventOn(
    ...
    async (event_data: { dryRun?: boolean; chat: SillyTavern.SendingMessage[] }) => {
      ...
      try {
        ...
        _本轮妻在场 = 本轮人物.妻在场;
        _本轮夫在场 = 本轮人物.夫在场;
        const 分居演员错误 = 许曼君分居剧情演员错误(本楼事件, _本轮妻在场, _本轮夫在场);
        if (分居演员错误) throw new Error(分居演员错误);
        const 不必停演员错误 = 安若妍不必停剧情演员错误(本楼事件, _本轮妻在场, _本轮夫在场) || 安若妍换掉剧情演员错误(本楼事件, _本轮妻在场, _本轮夫在场);
        if (不必停演员错误) throw new Error(不必停演员错误);
        const 离婚演员错误 = 许曼君离婚剧情演员错误(本楼事件, _本轮妻在场, _本轮夫在场);
        if (离婚演员错误) throw new Error(离婚演员错误);
        const 离婚后日常演员错误 = 许曼君离婚后日常演员错误(本楼事件, _本轮妻在场, _本轮夫在场);
        if (离婚后日常演员错误) throw new Error(离婚后日常演员错误);
        ...
        const 快照 = 组公寓快照(对话尾, 演出data, 楼层, 本楼事件, 本轮人物, _本轮快照刷新票).replace(
          ...
          构造自然场景请求(data, 本楼事件, 本轮用户文本, '', String(当前聊天ID()), `native:${本次原生轮owner}`) ??
            构造安抚观察请求(data, _本轮余波目标, 本轮用户文本, '', String(当前聊天ID()), `native:${本次原生轮owner}`), data);
        _本轮孕情初见提示 = 快照;

        // 原生正文与固定回合共用同一302自由阶段历史策略；只裁旧对话，不触碰角色卡、世界书、数据库召回或快照。
        const chat = event_data.chat ?? [];
        应用302自由阶段历史到原生请求(演出data, 原生当前场景 ?? '', chat);
  ...
  eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, async (新变量: object, 旧变量: object) => {
    // 解析事件与脚本的replaceMvuData写口分开；外部解析不能借脚本忙态绕过时间恢复。
    确认时间事务变量候选(新变量);
    ...
    try {
      ...
      let 本楼事件 = 本轮冻结?.内容 ?? '';
      let 本轮有效正文 = 提取静音会议可提交正文(应用酒馆最终显示正则(String(末楼?.mes ?? '')));
      const 拒答归属租约 = 静音会议基底 ? null : 读原生正文开始票();
      const 是预期普通拒答助手楼 = Boolean(
        拒答归属租约 &&
        拒答归属租约.序号 === 原生租约owner &&
        拒答归属租约.聊天ID === 原生聊天ID &&
        拒答归属租约.时间线世代 === 原生时间线世代 &&
        拒答归属租约.预期助手楼层 === 末楼层,
      );
      if ((静音会议基底 || 是预期普通拒答助手楼) && 是提供方拒答正文(本轮有效正文)) {
        await 物理写回静音会议原生正文('', 末楼层, 末楼, 原生事务仍有效, 刷新预期末楼消息签名);
        const 失败基底 = 静音会议基底 ?? (_本轮事件基底 ? (_.cloneDeep(_本轮事件基底) as SchemaType) : null);
        if (失败基底) {
        ...
        释放正文租约(原生租约owner);
        eventEmit('人妻公寓:回合失败', 'AI服务返回了拒答说明，本轮正文、剧情票和资源均未提交；请重试或更换模型线路。');
        return;
      }
      ...
      const 原生双重继承预检票 = 解析双重继承剧情事件(本楼事件);
      if (!静音会议基底 && 原生双重继承预检票?.动作 === '终幕后收束') {
        const 越拍原因 = (自然场景类别(本楼事件) ? '' : 双重继承正文越拍原因(本楼事件, 本轮有效正文));
        if (越拍原因) {
          // 原生备用输入没有游戏内二次静默重写层。只对唯一末拍使用确定性正文替换，
          // 防止弱模型被视频历史拉回；其他《双重继承》拍仍按原规则失败关闭。
          本轮有效正文 = 双重继承终幕后安全收束正文();
          await 物理写回静音会议原生正文(本轮有效正文, 末楼层, 末楼, 原生事务仍有效, 刷新预期末楼消息签名);
          console.warn(`[人妻公寓·双重继承] 原生终幕后正文越界（${越拍原因}），已替换为固定安全收束。`);
        }
      }
      ...
      let newData = 静音会议基底 ?? (Schema.parse(rawStat) as SchemaType);
      const 楼层 = 末楼层;
      ...
      const 原生拍摄尺度契约 = 本轮静音会议 ? null : 读取拍摄剧情尺度契约(本楼事件);
      let 原生拍摄重写尺度: 尺度判定解析结果 | null | undefined;
      const 原生拍摄首稿错误 = 拍摄剧情原生专属正文错误(本楼事件, 本轮有效正文, 原生拍摄尺度契约);
      if (!本轮静音会议 && 原生拍摄尺度契约 && 原生拍摄首稿错误) {
        try {
          const 重写 = await 生成拍摄剧情原生专属正文重写({
            事件: 本楼事件,
            契约: 原生拍摄尺度契约,
            首稿错误: 原生拍摄首稿错误,
            行动: _本轮玩家文本,
          ...
          本轮有效正文 = 重写.正文;
          原生拍摄重写尺度 = 重写.尺度;
          // 作废首稿的MVU候选；路线硬状态仍从PROMPT_READY冻结基底提交。
          if (_本轮事件基底) newData = _.cloneDeep(_本轮事件基底) as SchemaType;
        } catch (重写错误) {
          if (!原生事务仍有效()) throw 重写错误;
          await 物理写回静音会议原生正文('', 楼层, 末楼, 原生事务仍有效, 刷新预期末楼消息签名);
          const 失败基底 = _本轮事件基底 ? (_.cloneDeep(_本轮事件基底) as SchemaType) : null;
          if (失败基底) {
            _.set(新变量, 'stat_data', 失败基底);
            捕获保护快照(失败基底);
            await 同步原生整表视图(失败基底);
          ...
          eventEmit(
            '人妻公寓:回合失败',
            `${重写错误 instanceof Error ? 重写错误.message : String(重写错误)}。本拍与检查点均已保留，可直接重试。`,
          );
          return;
        }
      }

      const 原生许曼君离婚票 = 解析许曼君离婚剧情事件(本楼事件);
      const 原生许曼君离婚后日常票 = 解析许曼君离婚后日常事件(本楼事件);
      // 使用prompt前冻结的基底与本次原生租约，不把AI候选或后来阶段当成来源事实。
      // 原生用户楼已存在，业务来源在它之前；预期助手楼直接取租约，不套固定回合+2。
      const 原生日常提交锚 = _本轮事件基底
        ? 冻结许曼君日常提交锚(_本轮事件基底, 本楼事件, 原生租约.用户楼层 - 1, 原生租约.预期助手楼层)
        : null;
      if (原生许曼君离婚后日常票 && !原生日常提交锚) {
        throw new Error('201日常原生来源楼或请求租约已经失效，本拍不提交。');
      }
      const 原生许曼君首稿错误 =
        原生许曼君离婚票 || 原生许曼君离婚后日常票
          ? 许曼君原生专属正文错误(本楼事件, 本轮有效正文)
          : '';
      if (!本轮静音会议 && 原生许曼君首稿错误) {
        try {
          本轮有效正文 = await 生成许曼君原生专属正文重写({
            事件: 本楼事件,
            首稿错误: 原生许曼君首稿错误,
            行动: _本轮玩家文本,
            快照: _本轮孕情初见提示,
          ...
          // 原始MVU候选来自已经作废的首稿。备用链宁可放弃该稿的可选软变量，
          // 也不能让它们与重写正文拼成一笔；路线硬状态随后仍从同一冻结基底提交。
          if (_本轮事件基底) newData = _.cloneDeep(_本轮事件基底) as SchemaType;
        } catch (重写错误) {
          if (!原生事务仍有效()) throw 重写错误;
          await 物理写回静音会议原生正文('', 楼层, 末楼, 原生事务仍有效, 刷新预期末楼消息签名);
      ...
      const 原生原始尺度 = 本轮静音会议 ? null : 解析尺度判定(String(末楼?.mes ?? ''));
      const 原生尺度 =
        原生拍摄重写尺度 !== undefined
          ? 原生拍摄重写尺度
          : 应用拍摄剧情尺度契约(原生原始尺度, 本轮有效正文, 原生拍摄尺度契约);
      const 原生拍摄尺度错误 = 拍摄剧情尺度越界原因(本轮有效正文, 原生拍摄尺度契约);
      if (!本轮静音会议 && 原生拍摄尺度错误) {
        await 物理写回静音会议原生正文('', 楼层, 末楼, 原生事务仍有效, 刷新预期末楼消息签名);
        const 失败基底 = _本轮事件基底 ? (_.cloneDeep(_本轮事件基底) as SchemaType) : null;
        if (失败基底) {
        ...
        释放正文租约(原生租约owner);
        eventEmit('人妻公寓:回合失败', `${原生拍摄尺度错误}。本拍与检查点均已保留，可直接重试。`);
        return;
      }
      ...
      let 入住预约已提交 = false;
      let 原生第二机位提交: 第二机位结果 | null = null;
      let 原生不再留门提交: 不再留门结果 | null = null;
      let 原生安若妍不必停提交: 安若妍不必停结果 | null = null;
      let 原生许曼君分居提交: 许曼君分居结果 | null = null;
      let 原生许曼君离婚提交: 许曼君离婚结果 | null = null;
      let 原生许曼君离婚后日常提交: 许曼君离婚后日常结果 | null = null;
      let 原生回国提交: 回国结果 | null = null;
      let 原生双重继承提交: 双重继承结果 | null = null;
      let 商店剧情新增堕落: Partial<Record<门牌, number>> = {};
      const 原生自然观察 = await 观察自然场景(
        构造自然场景请求(newData, 本楼事件, _本轮玩家文本, 本轮有效正文, 原生聊天ID, `native:${原生租约owner}`) ??
          构造安抚观察请求(newData, _本轮余波目标, _本轮玩家文本, 本轮有效正文, 原生聊天ID, `native:${原生租约owner}`),
        String(末楼?.mes ?? ''), 原生事务仍有效,
      );
      if (!原生事务仍有效()) return;
      if (原生自然观察) {
        if (本轮冻结) 原生自然观察.提交上下文 = { 楼层, 场景: _本轮场景id, 时段: newData.系统._绝对时段,
          事务ID: newData.系统._场景剧情事务.id, 请求世代: newData.系统._场景剧情事务.请求世代,
          妻在场: [..._本轮妻在场], 夫在场: [..._本轮夫在场], 事件: 本轮冻结, 日常提交锚: 原生日常提交锚,
          实际尺度: Object.fromEntries(Object.entries(原生尺度?.角色 ?? {}).map(([门牌号, 项]) => [门牌号, 项?.实际 ?? 0])),
          资源计费: _本轮资源计费 };
        保存自然对话记录(newData, 原生自然观察);
      }
      if (!本轮自然事件可提交(newData, 本楼事件)) 保留自然剧情待续(newData, 本楼事件);
      if (本轮冻结 && 本轮自然事件可提交(newData, 本楼事件) && 本轮事件可提交(本轮冻结, newData.系统._待发送事件, 楼层, Boolean(本轮有效正文))) {
        ...
        if (!本轮静音会议 && 解析不再留门剧情事件(本楼事件)) {
          const 越拍 = 不再留门剧情演员错误(本楼事件, _本轮妻在场, _本轮夫在场) || (自然场景类别(本楼事件) ? '' : 不再留门正文越拍原因(本楼事件, 本轮有效正文));
          if (越拍) throw new Error(`《不再留门》当前拍未通过验收：${越拍}`);
          const 提交 = 提交不再留门剧情事件(newData, 本楼事件, 读场景().房间id ?? '', 楼层, _本轮玩家文本);
          if (!提交?.成功) throw new Error(提交?.提示 || '《不再留门》当前拍未提交。');
          原生不再留门提交 = 提交;
        }
        const 第二机位票 = 解析第二机位剧情事件(本楼事件);
        if (!本轮静音会议 && 第二机位票) {
          const 越拍原因 = 第二机位正文越拍原因(本楼事件, 本轮有效正文);
          if (越拍原因) throw new Error(`第二机位当前剧情回合越拍：${越拍原因}。本拍未提交，请重新生成正文。`);
          const 提交结果 = 提交第二机位剧情事件(newData, 本楼事件, 读场景().房间id ?? '', 楼层);
          if (!提交结果?.成功) throw new Error(提交结果?.提示 || '第二机位剧情票未能提交。');
          原生第二机位提交 = 提交结果;
        }
        if (!本轮静音会议 && 解析安若妍换掉剧情事件(本楼事件)) {
          const 演员错误 = 安若妍换掉剧情演员错误(本楼事件, _本轮妻在场, _本轮夫在场);
          if (演员错误) throw new Error(演员错误);
          const 结果 = 提交安若妍换掉剧情事件(newData, 本楼事件, 读场景().房间id ?? '', 楼层);
          if (!结果?.成功) throw new Error(结果?.提示 || '《换掉》剧情票未能提交。');
        }
        const 安若妍不必停票 = 解析安若妍不必停剧情事件(本楼事件);
        if (!本轮静音会议 && 安若妍不必停票) {
          const 验收错误 =
            安若妍不必停剧情演员错误(本楼事件, _本轮妻在场, _本轮夫在场) ||
            (自然场景类别(本楼事件) ? '' : 安若妍不必停正文越拍原因(本楼事件, 本轮有效正文));
          if (验收错误) throw new Error(`《不必停》当前剧情回合未通过验收：${验收错误}。本拍未提交，请重新生成正文。`);
          const 提交结果 = 提交安若妍不必停剧情事件(newData, 本楼事件, 读场景().房间id ?? '', 楼层);
          if (!提交结果?.成功) throw new Error(提交结果?.提示 || '《不必停》剧情票未能提交。');
          原生安若妍不必停提交 = 提交结果;
        }
        const 许曼君分居票 = 解析许曼君分居剧情事件(本楼事件);
        if (!本轮静音会议 && 许曼君分居票) {
          const 越拍原因 =
            许曼君分居剧情演员错误(本楼事件, _本轮妻在场, _本轮夫在场) ||
            (自然场景类别(本楼事件) ? '' : 许曼君分居正文越拍原因(本楼事件, 本轮有效正文));
          if (越拍原因) throw new Error(`《分居》当前剧情回合未通过验收：${越拍原因}。本拍未提交，请重新生成正文。`);
          const 提交结果 = 提交许曼君分居剧情事件(newData, 本楼事件, 读场景().房间id ?? '', 楼层, _本轮玩家文本, 本轮有效正文);
          if (!提交结果?.成功) throw new Error(提交结果?.提示 || '《分居》剧情票未能提交。');
          原生许曼君分居提交 = 提交结果;
        }
        const 许曼君离婚票 = 解析许曼君离婚剧情事件(本楼事件);
        if (!本轮静音会议 && 许曼君离婚票) {
          const 验收错误 =
            许曼君离婚剧情演员错误(本楼事件, _本轮妻在场, _本轮夫在场) ||
            (自然场景类别(本楼事件) ? '' : 许曼君离婚正文越拍原因(本楼事件, 本轮有效正文));
          if (验收错误) throw new Error(`《离婚》当前剧情回合未通过验收：${验收错误}。本拍未提交，请重新生成正文。`);
          const 提交结果 = 提交许曼君离婚剧情事件(newData, 本楼事件, 读场景().房间id ?? '', 楼层);
          if (!提交结果?.成功) throw new Error(提交结果?.提示 || '《离婚》剧情票未能提交。');
          原生许曼君离婚提交 = 提交结果;
        }
        const 许曼君离婚后日常票 = 解析许曼君离婚后日常事件(本楼事件);
        if (!本轮静音会议 && 许曼君离婚后日常票) {
          const 验收错误 =
            许曼君离婚后日常演员错误(本楼事件, _本轮妻在场, _本轮夫在场) ||
            (自然场景类别(本楼事件) ? '' : 许曼君离婚后日常正文越界原因(本楼事件, 本轮有效正文));
          if (验收错误) throw new Error(`201新日常当前回合未通过验收：${验收错误}。本拍未提交，请重新生成正文。`);
          const 提交结果 = 提交许曼君离婚后日常事件(
            newData,
            本楼事件,
            本轮有效正文,
            读场景().房间id ?? '',
        ...
        const 回国票 = 解析回国剧情事件(本楼事件);
        if (!本轮静音会议 && 回国票) {
          const 越拍原因 = 回国正文越拍原因(本楼事件, 本轮有效正文);
          if (越拍原因) throw new Error(`《回国》当前剧情回合越拍：${越拍原因}。本拍未提交，请重新生成正文。`);
          const 提交结果 = 提交回国剧情事件(newData, 本楼事件, 读场景().房间id ?? '', 楼层);
          if (!提交结果?.成功) throw new Error(提交结果?.提示 || '《回国》剧情票未能提交。');
          原生回国提交 = 提交结果;
        }
        const 双重继承票 = 解析双重继承剧情事件(本楼事件);
        if (!本轮静音会议 && 双重继承票) {
          const 越拍原因 = (自然场景类别(本楼事件) ? '' : 双重继承正文越拍原因(本楼事件, 本轮有效正文));
          if (越拍原因) throw new Error(`《双重继承》当前剧情回合越拍：${越拍原因}。本拍未提交，请重新生成正文。`);
          const 提交结果 = 提交双重继承剧情事件(newData, 本楼事件, 读场景().房间id ?? '', 楼层);
          if (!提交结果?.成功) throw new Error(提交结果?.提示 || '《双重继承》剧情票未能提交。');
          原生双重继承提交 = 提交结果;
        }
        ...
        if (本楼事件.includes('【商店剧情结算:')) 商店剧情新增堕落 = 提交商店剧情结算(newData, 本楼事件);
        排入第二机位后续剧情(newData, 原生第二机位提交);
        排入不再留门后续剧情(newData, 原生不再留门提交);
        排入安若妍不必停后续剧情(newData, 原生安若妍不必停提交);
        排入许曼君分居后续剧情(newData, 原生许曼君分居提交);
        排入回国后续剧情(newData, 原生回国提交);
        排入双重继承后续剧情(newData, 原生双重继承提交);
      }
      const 自然场景待续 = !!自然场景类别(本楼事件) && !本轮自然事件可提交(newData, 本楼事件);
      if (自然场景待续) 本楼事件 = '';
      const 双重继承时间节点 = 同步双重继承时间节点(newData, 楼层);
      if (!安若妍不必停事件要求H1开场(本楼事件)) 同步安若妍不必停时间节点(newData);
      if (!安若妍换掉事件要求H1开场(本楼事件)) 同步安若妍换掉时间节点(newData);
      const 许曼君离婚时间节点 = 同步许曼君离婚时间节点(newData);
      同步许曼君离婚完成后状态(newData);
      同步302共居状态(newData);
      // 当前票已经消费后的队列基线。后续入住、丈夫打断与线路系统若产生新剧情，
      // 只允许在这个基线之后追加，并在最终写回前逐张绑定真实场景。
      const 原生结算后待发送基线 = newData.系统._待发送事件;

      // 3. 坏结局锁定:引擎全停(回滚保护仍生效)。第二机位开录票必须先建立真实亲密账本，
      // 不能被这条早退路径提交成“录制中但没有场次”的半状态。
      if (newData.系统._坏结局 && 第二机位事件要求真实开录(本楼事件)) {
        throw new Error('第二机位开录楼进入坏结局早退，未建立真实102亲密场次，本轮不提交。');
      }
      if (newData.系统._坏结局 && 许曼君分居事件要求真实亲密(本楼事件)) {
        throw new Error('《分居》亲密开场进入坏结局早退，未建立真实201亲密场次，本轮不提交。');
      }
      if (newData.系统._坏结局 && (安若妍不必停事件要求H1开场(本楼事件) || 安若妍换掉事件要求H1开场(本楼事件))) {
        throw new Error('《不必停》H1进入坏结局早退，未建立真实301亲密场次，本轮不提交。');
      }
      if (newData.系统._坏结局) {
        _.set(新变量, 'stat_data', newData);
        临时入住保护已启用 = false;
      ...
      if (!本轮静音会议) {
        ...
        for (const m of _本轮焦点) {
          ...
          // 商品脚本奖励沿用购买时不触疑心的规则，仍保留正文产生的真实增量。
          const 堕落增量 = 节点.妻.堕落值 - (基准堕落 ?? 节点.妻.堕落值) - (商店剧情新增堕落[m] ?? 0);
        ...
        const 前台线路隔离 = 前台角色线路无关强剧情已冻结(newData, [
          ..._本轮焦点,
          ..._本轮妻在场,
          ..._本轮夫在场,
        ]);
        ...
        if (_本轮余波目标 && newData.户[_本轮余波目标] && !角色线路无关打断已停用(newData, _本轮余波目标)) {
          推进余波安抚(newData.户[_本轮余波目标]!.妻, {
            正文楼: 楼层,
            当前绝对时段: 现钟,
            成功主线当面楼: Boolean(本轮有效正文) && _本轮妻在场.includes(_本轮余波目标),
            玩家有效回应: 原生自然观察?.请求.类别 === '冷落安抚' && 自然对话可提交(原生自然观察),
          });
        }
      ...
      if (不再留门事件要求真实开录(本楼事件) && (!资源结算.性爱开始 || 资源结算.性爱结束 || !不再留门真实录制已绑定(newData))) {
        throw new Error('《不再留门》开录未建立唯一绑定的202普通场次，本拍未提交。');
      }
      if (
        第二机位事件要求真实开录(本楼事件) &&
        (!资源结算.性爱开始 || 资源结算.性爱结束 || !第二机位真实录制已绑定(newData))
      ) {
        throw new Error('第二机位开录正文没有建立可继续游玩的真实102亲密场次，本轮不提交。');
      }
      if (
        许曼君分居事件要求真实亲密(本楼事件) &&
        (!资源结算.性爱开始 || 资源结算.性爱结束 || !许曼君分居真实亲密已绑定(newData))
      ) {
        throw new Error('《分居》亲密开场没有建立精确绑定、可继续游玩的真实201亲密场次，本轮不提交。');
      }
      if (安若妍换掉事件要求H1开场(本楼事件) && (!资源结算.性爱开始 || 资源结算.性爱结束 || !安若妍换掉真实亲密已绑定(newData))) {
        throw new Error('《换掉》开场未建立可继续的唯一301场次，本轮不提交。');
      }
      if (
        安若妍不必停事件要求H1开场(本楼事件) &&
        (!资源结算.性爱开始 || 资源结算.性爱结束 || !安若妍不必停真实亲密已绑定(newData))
      ) {
        throw new Error('《不必停》H1没有建立精确绑定、可继续游玩的真实301亲密场次，本轮不提交。');
      }
      ...
      确认原生事务有效();
      await 同步全部角色阶段世界书(newData, 原生事务仍有效);
      确认原生事务有效();
      if (原生第二机位提交) {
        // 以下通知均发生在核心原生事务成功之后。
        try {
          if (原生第二机位提交.提示) eventEmit('人妻公寓:提示', 原生第二机位提交.提示);
          播放第二机位CG(原生第二机位提交);
        } catch (后处理错误) {
          console.warn('[人妻公寓] 第二机位核心已经提交，但提示或CG广播失败:', 后处理错误);
        }
      }
      try {
        for (const 文件 of 安若妍换掉剧情CG(newData, 本楼事件)) eventEmit('人妻公寓:安若妍换掉CG', { 文件, 标题: '安若妍 · 换掉' });
      } catch (error) {
        console.warn('[人妻公寓] 《换掉》已提交，画面广播等待重试：', error);
      }
      if (原生安若妍不必停提交) {
        try {
          if (原生安若妍不必停提交.提示) eventEmit('人妻公寓:提示', 原生安若妍不必停提交.提示);
          播放安若妍不必停CG(原生安若妍不必停提交);
        } catch (后处理错误) {
          console.warn('[人妻公寓] 《不必停》核心已经提交，但提示或CG广播失败:', 后处理错误);
        }
      }
      if (原生许曼君分居提交) {
        try {
          if (原生许曼君分居提交.提示) eventEmit('人妻公寓:提示', 原生许曼君分居提交.提示);
          播放许曼君分居CG(原生许曼君分居提交);
        } catch (后处理错误) {
          console.warn('[人妻公寓] 《分居》核心已经提交，但提示或CG广播失败:', 后处理错误);
        }
      }
      if (原生许曼君离婚提交) {
        try {
          if (原生许曼君离婚提交.提示) eventEmit('人妻公寓:提示', 原生许曼君离婚提交.提示);
          播放许曼君离婚CG(原生许曼君离婚提交);
        } catch (后处理错误) {
          console.warn('[人妻公寓] 《离婚》核心已经提交，但提示或CG广播失败:', 后处理错误);
        }
      }
      if (原生许曼君离婚后日常提交?.提示) {
      ...
      if (原生回国提交) 播放回国CG(原生回国提交);
      if (原生双重继承提交?.提示) eventEmit('人妻公寓:提示', 原生双重继承提交.提示);
      if (原生双重继承提交) 播放双重继承CG(原生双重继承提交);
      if (双重继承时间节点.提示) eventEmit('人妻公寓:提示', 双重继承时间节点.提示);
      if (许曼君离婚时间节点.提示) eventEmit('人妻公寓:提示', 许曼君离婚时间节点.提示);
      if (资源结算.线路CG) 播放安若妍不必停CG({ CG: 资源结算.线路CG });
      if (提交换装疑记) {
        try {
          await 提交换装疑记();
        } catch (error) {
      ...
      try {
        // 最终写楼前还会再次检查；这里先保护异步收尾刚结束的候选。
        if (原生事务仍有效()) 确认时间事务变量候选(新变量);
      } finally {
  ...
  eventOn(tavern_events.GENERATION_ENDED, (消息数?: number) => {
    ...
    if (原生票.阶段 === '等待prompt') {
      ...
      if (等待票匹配结束事件(原生票, 消息数)) {
        // 等待 prompt 的未完成票在 prompt 阶段失败/中断（含服务商返回错误或超时），宿主
        // release 的 GENERATION_ENDED 携带 messageCount 低于预期助手楼层。该判据已放宽为
        // “< 预期助手楼层”的提前结束，配合上层 chat/时间线/用户楼层/引用校验，按精确身份
        // 作废本票并交还共享前台生成槽，避免前台令牌随等待票永久残留、使 安全操作 入口被
        // 「内容正在生成」门长期拦截。>= 预期助手楼层的旧/错结束事件不命中，不得误清合法新票。
        作废原生正文租约();
        释放静音会议原生生成锁();
        清原生本轮冻结();
      }
  ...
  function 排队宿主原生时间线切换(类型: '删楼' | '切分支', 切分支楼: number): void {
    ...
    void 排队时间线切换协调(`宿主原生${类型}`, async 租约 => {
      ...
      // 返回 Promise 的 finally 已先递减协调忙门；只有最后一代收口才在真实空闲后解锁客户端。
      if (!完成通知仍有效?.() || 时间线切换协调中()) return;
      try {
        eventEmit('人妻公寓:回合完成');
      } catch (通知错误) {
        console.error(`[人妻公寓] 原生${类型}已收口，但完成通知监听器执行失败:`, 通知错误);
      }
    });
  }

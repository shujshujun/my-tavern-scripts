    ...
    try {
      ...
      try {
        ...
        if (有效) {
          ...
          // 旧版父亲收尾曾把 来电回流/母亲裂缝·父亲来电 排进 _待发送事件，写入时玩法
          // 进度已完成。只删独立软项，无法证明是旧软项的整项保留，绝不误删强剧情。
          const 原待发送事件 = data.系统._待发送事件;
          const 清理电话软事件后 = 清理旧版电话软事件(原待发送事件);
          const 有旧电话软事件 = typeof 原待发送事件 === 'string' && 清理电话软事件后 !== 原待发送事件;
          if (有旧电话软事件) {
            data.系统._待发送事件 = 清理电话软事件后;
            console.info('[人妻公寓·时间] 已清理旧档残留的父亲来电软事件（来电回流/母亲裂缝·父亲来电）');
          }
          if (首批有修正 || 尾楼缺存档 || 遗留时间票 || 有旧电话软事件) await 脚本写入(raw, data);
...
function 挂载监听() {
  ...
  eventOn('人妻公寓:玩家行动', (行动: string) => {
    ...
    if (Number(_.get(当前 ?? {}, '系统._荣耀洞拍') ?? -1) >= 0 && 读场景().房间id === '洗手间' && !隔离事件进行中()) {
      安全操作((raw, data, 操作仍有效) => {
        const 文本 = 行动.trim();
        return 运行荣耀洞隔离拍(raw, data, 文本, 建隔离记录('荣耀洞继续', 文本, data), 操作仍有效);
      });
      return;
    }
  ...
  type 隔离回合记录 = {
    ...
    /** 在 查看摄像头 之前捕获的四键精确快照（`_隔离事件`/`_上次隔离回合`/`_侦探`/`_场景`）。 */
    聊天快照精确: 精确聊天快照;
  };
  ...
  eventOn('人妻公寓:荣耀洞', () =>
    安全操作(async (raw, data, 操作仍有效) => {
      ...
      捕获保护快照(data);
      eventEmit('人妻公寓:提示', 结果.提示);
      await 运行荣耀洞隔离拍(raw, data, 行动, 记录, 操作仍有效);
    }),
  );
  eventOn('人妻公寓:荣耀洞离场', () =>
    安全操作(async (raw, data) => {
  ...
  eventOn('人妻公寓:处理管理任务', (载荷: { 任务id: string; 选项id: string; 地点: string }) => {
    ...
    void 安全操作(async (_raw, data) => {
      ...
      if (阻塞事件) {
        eventEmit(
          '人妻公寓:提示',
          `当前有必须先处理的事件：${描述待发送事件(阻塞事件)}。随便说点什么演完这一幕，再来处理楼务。`,
        );
        return;
      }
  ...
  eventOn('人妻公寓:父亲通话结束', (通话标识: string, 预期聊天ID: string) =>
    安全操作(() =>
      排队父亲通话整表写(async () => {
        ...
        // 线索.提示 已是完整、可见、确定性的玩法反馈，不再把 线索.事件 或 来电回流
        // 排进 _待发送事件，避免商店/楼务/时间/资源等无关系统被软内容阻塞或污染。
      ...
      async (raw, data, 操作仍有效) => {
        ...
        if ('提示' in 结果) {
          ...
          try {
            ...
            // 落地成功且身份仍有效后才严格确认删除事务；任一 await 后身份变化都停止，保留旧聊天事务。
            if (!(操作仍有效() && 当前聊天ID() === 身份.聊天ID)) {
              静默已切换 = true;
              console.warn('[人妻公寓] 监控收口期间聊天已切换，旧聊天事务留待恢复');
              return;
            }
            await 确认隔离事务无需隔离({ 事务: 事务.记录, 身份, 操作仍有效 });
            if (!(操作仍有效() && 当前聊天ID() === 身份.聊天ID)) {
              静默已切换 = true;
              console.warn('[人妻公寓] 监控收口确认后聊天已切换');
            }
          } catch (收口错误) {
            // 收口失败必须立即停止：身份已变时旧聊天事务留给启动恢复，绝不向新聊天写/发事件。
            if (!(操作仍有效() && 当前聊天ID() === 身份.聊天ID)) {
              静默已切换 = true;
  ...
  eventOn(
    ...
    (类型: string, 选项: { automatic_trigger?: boolean; quiet_prompt?: string; quietToLoud?: boolean }, dryRun: boolean) => {
      if (dryRun) return;
  ...
  eventOn(
    ...
    async (event_data: { dryRun?: boolean; chat: SillyTavern.SendingMessage[] }) => {
      ...
      try {
        ...
        if (!读取MVU解析状态().外置模式) 覆盖原生本轮变量视图(chat, 演出data, _本轮变量范围, 解析候选亲密妻(_本轮变量范围));
    ...
    try {
      ...
      if (!本轮静音会议 && 本轮有效正文 && 原生尺度?.角色) {
        ...
        if (新增亲密妻.length) {
          _本轮变量范围 = 扩展后范围;
          console.info(
            `[人妻公寓·稽查补亲密] 原生正文最终尺度实际≥1，补入亲密妻：${新增亲密妻.join(',')}`,
          );
        }
      }
  ...
  function 排队宿主原生时间线切换(类型: '删楼' | '切分支', 切分支楼: number): void {
    ...
    void 排队时间线切换协调(`宿主原生${类型}`, async 租约 => {
      ...
      try {
        ...
        await 隔离当前手机分支(类型 === '删楼' ? -1 : 切分支楼, 租约.仍为最新);

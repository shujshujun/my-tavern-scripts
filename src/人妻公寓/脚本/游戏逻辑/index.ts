...
function 挂载监听() {
  ...
  eventOn('人妻公寓:隔离事件重掷', () =>
    安全操作(async (raw, _data, 操作仍有效) => {
      ...
      if (记录.入口 === '荣耀洞继续') {
        await 运行荣耀洞隔离拍(raw, 恢复后, 记录.行动, 建隔离记录('荣耀洞继续', 记录.行动, 恢复后), 操作仍有效);
      } else if (记录.入口 === '荣耀洞开始') {
        eventEmit('人妻公寓:荣耀洞');
      } else if (记录.门牌) {
  ...
  eventOn('人妻公寓:翻垃圾', (门牌号: 门牌) =>
    安全操作((raw, data) => {
      ...
      return 即时开演(结果, raw, data, `(在垃圾房翻查${户静态表[门牌号].妻名}家的垃圾袋，把翻到的东西悄悄收好)`);
    }),
  );
    ...
    void 安全操作(
      async (raw, data, 操作仍有效) => {
        ...
        if (待演) {
          ...
          eventEmit(
            '人妻公寓:提示',
            `眼下还有一段剧情没有演完：${描述待发送事件(待演)}。随便说点什么，让这一幕先演出来。`,
          );
          return;
        }
        ...
        try {
          ...
          if (后续强剧情 && !候选.系统._坏结局) {
            ...
            if (!正式剧情成功) eventEmit('人妻公寓:回合完成', { 更新正文幕: Boolean(时间反馈草稿) });
          } else {
  ...
  eventOn('人妻公寓:回合完成', (选项?: { 跳过手机节拍?: boolean }) => {
    ...
    if (选项?.跳过手机节拍) {
      ...
      const rawStat = 读最近有效stat();
      if (rawStat) void 同步管理任务微信(Schema.parse(rawStat) as SchemaType);
    } else {
      void 手机节拍();
    }
  });

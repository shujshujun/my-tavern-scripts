...
export interface 场景剧情状态视图 {
  ...
  需要玩家回应: boolean;
  /** 同一现场的多拍连续剧情；到达目标后必须留场，离场恢复档也不能推进世界时间令票据过期。 */
  连续锁场: boolean;
  队列剩余数: number;
...
const 内部标签正则 =
  /^(?:场景剧情:v1:|场景剧情约束|场景剧情连续锁场|场景剧情需回应|商店剧情结算:|线路随机:|事件(?:在场妻|在场夫|关联妻|关联夫):|阶段线路(?:演出|剧情):|阶段性癖票:|回国提交:|第二机位提交:|不再留门提交:|关系线路强制演出|节点0连续性承接|本轮剧情事件)/;
...
/** 多拍现场的显式硬生命周期；普通远处预约不带此标记，继续保持地点感知放行。 */
export function 场景剧情连续锁场(content: unknown): boolean {
  return /【场景剧情连续锁场】/.test(String(content ?? ''));
}

/**
 * 路线动作的统一普通剧情互斥：活动事务始终阻塞，等待票只在当前地点（或未知旧档）阻塞。
 * 专用特殊场景、亲密账本和电话由各业务按自身语义另行判断。
 */
export function 有普通场景剧情阻塞当前场景(data: SchemaType, 当前场景: string | null | undefined): boolean {
  return Boolean(读取活动场景剧情(data)) || 等待场景剧情阻塞当前场景(读取队首场景剧情(data.系统._待发送事件), 当前场景);
}

/** 地点动作检查全部外部等待票；队尾同地、未知地点和连续现场也不能被远处队首遮住。 */
export function 有地点动作剧情冲突(
  data: SchemaType,
  当前场景: string | null | undefined,
  忽略剧情: (内容: string) => boolean = () => false,
): boolean {
  const 活动 = data.系统._场景剧情事务;
  if (活动.id && !忽略剧情(活动.内容)) return true;
  return 读取待发送事件队列(data.系统._待发送事件).some(内容 =>
    !忽略剧情(内容) && (
      场景剧情连续锁场(内容) ||
      等待场景剧情阻塞当前场景(读取队首场景剧情(内容), 当前场景)
    ),
  );
}
...
export function 清除场景剧情内部标签(text: string): string {
  return 清除场景剧情机器标记(text)
    .replace(/【场景剧情(?:连续锁场|需回应)】/g, '')
    .replace(/【线路随机:(?:丈夫|越洋):\d{3}】/g, '')
    .replace(/【(?:回国提交|第二机位提交):[^】]+】/g, '')
...
export function 推断旧场景剧情目标(content: string): 场景剧情目标 {
  ...
  if (/【家庭计划专属监控】/.test(text)) {
    // 旧监控标签不等于观察地点。只认原导演头中的明确玩家位置；
    // 未给位置、引用/条件话语或多个监控头都保持未知，不改写为另一个房间。
    if ((text.match(/【家庭计划专属监控】/gu)?.length ?? 0) !== 1) return null;
    const 位置 = text.match(
      /^\s*(?:【事件(?:在场|关联)(?:妻|夫):[^】]*】\s*)*【家庭计划专属监控】\s*(?:\{\{\s*user\s*\}\}|玩家)\s*人?在\s*(302|管理员室)(?=\s*[,，。；;]|$)/u,
    );
    return 位置?.[1] ?? null;
  }
...
export function 读取场景剧情状态(data: SchemaType): 场景剧情状态视图 | null {
  ...
  return {
    ...
    连续锁场: 场景剧情连续锁场(head.内容),
    队列剩余数: Math.max(0, head.总项数 - head.项数),
    可在当前场景开始: 当前场景 => head.目标场景 !== null && 场景剧情目标匹配(head.目标场景, 当前场景),
  };
}

/** 暂停业务只撤下调用方持有的精确等待项；其他人物、地点和活动票均不受影响。 */
export function 移除等待场景剧情(data: SchemaType, 精确内容: readonly string[]): void {
  const 待移除 = new Set(精确内容);
  data.系统._待发送事件 = 拼接待发送事件队列(读取待发送事件队列(data.系统._待发送事件).filter(item => !待移除.has(item)));
}

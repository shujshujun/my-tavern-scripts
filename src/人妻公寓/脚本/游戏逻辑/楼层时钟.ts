...
interface 含阶段预约户 {
  夫: {
    ...
    _剧情外出起?: number;
    _剧情外出至?: number;
    _居住模式?: '普通作息' | '路线外住' | '预约回楼' | '待离婚交接' | '正式退居';
    _预约回楼起?: number;
    _预约回楼至?: number;
  };
  妻?: {
...
export function 丈夫在楼(节点: 含阶段预约户 | undefined, 门牌号: 门牌, 绝对时段: number): 夫状态名 {
  if (节点) {
    const 居住模式 = 节点.夫._居住模式 ?? '普通作息';
    if (居住模式 === '预约回楼') {
      const 起 = Number(节点.夫._预约回楼起 ?? -1);
      const 至 = Number(节点.夫._预约回楼至 ?? -1);
      return 起 >= 0 && 绝对时段 >= 起 && 绝对时段 < 至 ? '在家' : '外出';
    }
    if (居住模式 === '路线外住' || 居住模式 === '待离婚交接' || 居住模式 === '正式退居') return '外出';
  }
  const 预约 = 节点?.妻?._阶段线路;
  if (阶段预约当前有效(预约, 绝对时段)) {
  ...
  if (节点) {
    const 剧情外出起 = Number(节点.夫._剧情外出起 ?? -1);
    const 剧情外出至 = Number(节点.夫._剧情外出至 ?? -1);
    if (剧情外出起 >= 0 && 绝对时段 >= 剧情外出起 && 绝对时段 < 剧情外出至) return '外出';
    if (节点.夫._外出至 > 绝对时段) return '外出';
  }
...
export function 妻位置推算(门牌号: 门牌, 绝对时段: number, 节点?: 含阶段预约户): string {
  // 预产通知进入系统时即锁医院，优先级高于普通作息与阶段预约；是否向玩家显示医院由已读门另行控制。
  if (['待产通知', '待产', '陪产中', '住院中'].includes(节点?.妻?._生产?.状态 ?? '')) return '医院';
  const 预约 = 节点?.妻?._阶段线路;

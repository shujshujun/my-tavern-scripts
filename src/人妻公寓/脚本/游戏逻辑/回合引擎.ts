...
import { 读取MVU外置模型配置, 读取MVU解析状态, 读取变量解析通道 } from '../../MVU解析模式';
// (难度表兼供撞见概率系数查表)
...
const GEMINI变量更新强制令 = [
  ...
  '本轮若属于有实质暧昧/亲密/性内容的回合（快照标注【亲密场景】或本轮尺度判定实际≥1），同样检查 户.<门牌>.妻.堕落值：她被越界体验真实触动时 +1~3，产生明显反感或退缩时 -1~3，纯日常闲聊不动；单轮超出 ±3 系统会整项回滚，不要写超。',
...
/**
 * 内置变量解析失败后的待补结算标记(2026-08-04 方案):失败轮保留旧值绝不作废回合,
 * 下一轮把这轮的行动+正文一并交给解析模型累计结算。只记内存:刷新丢失时玩家仍有
 * MVU 官方"重试额外模型解析"按钮兜底,不值得为此往存档里加系统字段。
 */
let 内置解析待补: { 行动: string; 正文: string } | null = null;
 ...
 * - 正文 → 正文 API 静默 generate(预设/世界书里已有完整变量规则)。
 * 任何失败只返回 null,由调用方决定重试与待补,绝不抛错作废回合。
 */
async function 内置外置变量解析(参数: {
  ...
  const 用户内容 =
    (待补
      ? `【上一轮待补结算】上一轮变量解析未完成，请把上一轮与本轮的变化一并结算进本次输出。\n【上一轮玩家行动】\n${待补.行动}\n【上一轮正文】\n${待补.正文}\n\n`
      : '') + `【本轮玩家行动】\n${参数.行动}\n\n【本轮已完成正文】\n${参数.正文}`;
  ...
  // 通道选择照微信的路子：默认"自动"= 数据库插件代发优先（沿用数据库自己的配置，
  // 不读它的密钥与模型），其次自定义 API，最后正文 API 兜底；玩家也可在游戏设置里强制。
  const 通道偏好 = 读取变量解析通道();
  let 通道: '自定义' | '正文' | '数据库';
  if (通道偏好 === '自定义') {
    通道 = 自定义可用 ? '自定义' : 数据库状态().可调用AI ? '数据库' : '正文';
    if (!自定义可用) console.warn('[人妻公寓] 变量解析选了自定义模型但配置不完整，本轮改走兜底通道');
  } else if (通道偏好 === '正文') {
    通道 = '正文';
  } else {
    通道 = 数据库状态().可调用AI ? '数据库' : 自定义可用 ? '自定义' : '正文';
  }
  ...
  try {
    if (通道 === '自定义' && 配置) {
      ...
      // 正文 API 静默生成沿用预设与世界书，变量规则齐全，是配置缺失时的最终兜底。
      console.info('[人妻公寓] 内置变量解析走正文API通道');
      原文 = await Promise.race([
        等待正文生成({
          ...
          should_silence: true,
          injects: [
            { role: 'system', content: 参数.快照, position: 'in_chat', depth: 0, should_scan: true },
            { role: 'system', content: 现值视图, position: 'in_chat', depth: 0, should_scan: false },
    ...
    if (e instanceof Error && e.message === '__RQGY_MVUVARS_TIMEOUT__') {
      console.warn(`[人妻公寓] 内置变量解析超过 ${二次结算超时毫秒 / 1000} 秒未返回`);
    } else {
...
export async function 执行回合(
  ...
  try {
    ...
    const { 快照, 焦点, 妻在场, 夫在场, 尺度模式, 变量范围: 初始变量范围, 快照刷新票 } = await 组快照注入(
      对话尾,
      演出data,
      生成楼层,
      回合前末楼,
    ...
    if (!使用MVU外置解析 && 稽查.状态 === '通过' && 稽查.角色) {
      const 补充亲密妻 = 焦点妻们.filter(
        m => !变量范围.亲密妻.includes(m) && (稽查.角色[m]?.实际 ?? 0) >= 1,
      );
      if (补充亲密妻.length) {
        变量范围 = { ...变量范围, 亲密妻: [...变量范围.亲密妻, ...补充亲密妻] };
        console.info(
          `[人妻公寓·稽查补亲密] 正则未命中但尺度实际≥1，补入亲密妻：${补充亲密妻.join(',')}`,
        );
      }
    }
    ...
    // 内置解析(游戏开关,默认开)由引擎直接调用解析模型,回合内同步拿到变量块,不再依赖
    // 跨脚本桥与 MVU 全局锁;玩家关掉内置解析时才退回旧路线(外置"自动请求"开启时通过
    // 跨脚本桥按一次官方按钮)。两条路线都不要补发 MESSAGE_RECEIVED，否则双请求。
    ...
    if (!本轮静音会议 && 使用MVU外置解析 && MVU解析.内置解析 && 本轮有可写演员) {
      ...
      try {
        ...
        for (let 次 = 1; 次 <= 2 && !内置变量块; 次++) {
          ...
          内置变量块 = await 内置外置变量解析({ 行动, 正文: 基础正文, 快照, 可写视图, 回合前末楼 });
          确认本轮事务有效();
        }
        if (内置变量块) {
          变量块 = 内置变量块;
          ...
          // 失败链:已重试一次仍无果→保留旧值+记待补标记,下一轮连同本轮正文一并结算;
          // 绝不降级成随AI输出,也绝不 throw(那会触发双临时楼全删+快照回滚=正文消失)。
          内置解析待补 = { 行动, 正文: 基础正文 };
          console.warn('[人妻公寓] 内置变量解析两次均未产出变量块；本轮保留旧值，下一轮自动补结算');
        }
      } catch (e) {
    ...
    // 官方外置桥返回前已经完成“生成变量块 → parse → 写回该楼”，其结果再交给
    // parseMessage 会让 delta/add 在终值上应用第二次。内置解析产出的变量块是全新
    // 未应用的，须与随AI输出路线一样走本地 parseMessage 纯函数解析。
    ...
    if (本轮静音会议) {
      ...
      try {
        if ((!使用MVU外置解析 || 内置解析变量块已就绪) && !变量解析已降级) {
        ...
        await 降级AI变量解析(
          使用MVU外置解析 ? (内置解析变量块已就绪 ? '内置变量' : 'MVU外置模型') : '正文模型',
          e,
        );
        // 内置解析的块在本地 parse/校验阶段被打回同样算失败,重新挂待补,下一轮再算。
        if (内置解析变量块已就绪) 内置解析待补 = { 行动, 正文: 基础正文 };

...
import { 捕获精确聊天快照, 时间推进事务键 } from './时间撤销系统';
...
import {
  自然场景类别, 构造自然场景请求, 构造安抚观察请求, 自然场景生成提示, 观察自然场景,
  保存自然对话记录, 本轮自然事件可提交, 保留自然剧情待续,
} from './自然对话接入';
import { 自然对话可提交, type 对话观察记录 } from './自然对话观察';
...
import { 作废晋阶镜像时间线, 捕获保护快照, 回滚保护字段, 清保护快照, 等待晋阶镜像写入, 镜像直写, 同步已打开裂缝 } from './守护系统';
import { 无处罚拒绝正文, 输出稽查, type 尺度模式, type 稽查结果 } from './稽查系统';
import { 读取拍摄剧情尺度契约 } from './拍摄剧情尺度契约';
import {
  type AI可写变量范围,
  构造AI可写变量范围,
  构造AI可写变量视图,
...
import { 构造302自由阶段聊天历史, 同步302共居状态 } from './302共居系统';
import {
  许曼君分居CG标题,
  许曼君分居剧情演员错误,
  许曼君分居正文越拍原因,
...
import { 创建正文生成超时错误, 判定正文生成超时, 友好化正文生成错误, 正文生成超时错误前缀 } from './正文生成故障';
...
const 回合变量键 = [
  ...
  '_反感连续',
  // 即时业务的业务前撤回票必须随失败／重掷快照保留；字面键让独立函数验收夹具无需加载模块常量。
  '_即时业务撤回',
] as const;
...
function 当前即时业务锚分支指纹(锚楼: number): string {
  return 计算微信刷新分支指纹(SillyTavern.chat ?? [], 锚楼);
}

function 即时业务撤回身份仍有效(准备: 即时业务撤回准备): boolean {
  return (
    当前聊天ID() === 准备.聊天ID &&
    getLastMessageId() === 准备.锚楼 &&
    即时业务锚消息身份匹配(准备, SillyTavern.chat?.[准备.锚楼]) &&
    当前即时业务锚分支指纹(准备.锚楼) === 准备.锚分支指纹
  );
}

/** 在结算函数运行前捕获。此时碎片、背包、信任、线路节点和场景票都还没有变化。 */
export function 捕获即时业务撤回准备(data: SchemaType): 即时业务撤回准备 {
  const vars = getVariables({ type: 'chat' });
  if (Object.prototype.hasOwnProperty.call(vars, 即时业务撤回键) && vars[即时业务撤回键] != null) {
    throw new Error('上一项即时业务仍有撤回记录，请先完成或撤回原场景剧情。');
  }
  const 聊天ID = 当前聊天ID();
  const 锚楼 = getLastMessageId();
  const 锚消息 = SillyTavern.chat?.[锚楼];
  const 锚消息签名 = 手机锚消息签名(锚消息);
  const 锚稳定令牌指纹 = 即时业务锚稳定令牌指纹(锚消息);
  const 锚分支指纹 = 当前即时业务锚分支指纹(锚楼);
  if (!聊天ID || 锚楼 < 0 || !锚消息签名 || !锚分支指纹) {
    throw new Error('无法建立即时业务的原聊天锚点，本次操作没有发生。');
  }
  const 业务前数据 = 读取即时业务锚数据(锚楼);
  if (!业务前数据) throw new Error('即时业务原锚楼变量尚未就绪，本次操作没有发生。');
  const 准备 = 创建即时业务撤回准备({
    聊天ID,
    锚楼,
    锚消息签名,
    ...
    锚分支指纹,
    业务前数据,
    业务前聊天: 捕获精确聊天快照(vars, [...回合变量键, '_rqgy数据库恢复锚']),
  });
  if (!即时业务锚仍是业务前状态(准备, data)) {
    throw new Error('即时业务入口数据与持久锚楼已经不同步，本次操作没有发生。');
  }
  return 准备;
}

/**
 * 先把业务前撤回票写进 chat，再允许 `落地` 改写锚楼。刷新若发生在两个存储之间，
 * 启动恢复会按锚楼仍是业务前／已经持有原场景事务二选一收口，绝不猜测提交结果。
 */
export async function 登记即时业务撤回准备(
  准备: 即时业务撤回准备,
  场景事务ID: string,
  业务后候选: SchemaType,
): Promise<即时业务撤回记录> {
  if (!即时业务撤回身份仍有效(准备)) throw new Error('即时业务登记前消息分支已经变化。');
  const 记录 = 创建即时业务撤回记录(准备, 场景事务ID);
  const 锚数据 = 读取即时业务锚数据(准备.锚楼);
  if (!锚数据 || !即时业务锚仍是业务前状态(记录, 锚数据)) {
    throw new Error('即时业务登记前锚楼已被改写，本次操作没有提交。');
  }
  if (业务后候选.系统._场景剧情事务.id !== 场景事务ID) {
    throw new Error('即时业务候选与场景事务身份不一致。');
  }
  try {
    await updateVariablesWith(
      vars => {
        if (!即时业务撤回身份仍有效(准备)) throw new Error('__RQGY_TIMELINE_CHANGED__');
        if (Object.prototype.hasOwnProperty.call(vars, 即时业务撤回键) && vars[即时业务撤回键] != null) {
          throw new Error('即时业务撤回槽已经被其他操作占用。');
        }
        vars[即时业务撤回键] = _.cloneDeep(记录);
        return vars;
      },
      { type: 'chat' },
    );
  } catch (error) {
    // 宿主可能在 updater 已同步执行后才报告保存失败；当前聊天中的同一完整票已经存在时
    // 视为准备完成，不能让玩家只能靠刷新解锁。身份已变时绝不读取或改写新聊天。
    if (即时业务撤回身份仍有效(准备)) {
      const 已写 = 读取即时业务撤回记录(getVariables({ type: 'chat' })[即时业务撤回键]);
      if (已写?.完整性指纹 === 记录.完整性指纹) return 记录;
    }
    throw error;
  }
  if (!即时业务撤回身份仍有效(准备)) throw new Error('__RQGY_TIMELINE_CHANGED__');
...
/** 核心锚楼已经写回后，将准备票与真实锚点逐字段指纹绑定，再开始正文生成。 */
export async function 确认即时业务撤回已提交(原记录: 即时业务撤回记录): Promise<即时业务撤回记录> {
  if (!即时业务撤回身份仍有效(原记录)) throw new Error('即时业务确认前消息分支已经变化。');
  const 锚数据 = 读取即时业务锚数据(原记录.锚楼);
  if (!锚数据) throw new Error('即时业务确认时无法读取锚楼变量。');
  const 当前值 = getVariables({ type: 'chat' })[即时业务撤回键];
  const 当前记录 = 读取即时业务撤回记录(当前值);
  if (!当前记录) throw new Error('即时业务核心已写入，但撤回准备票已经缺失或损坏。');
  const 判定 = 核验即时业务撤回记录(当前记录, {
    当前聊天ID: 当前聊天ID(),
    当前锚楼: 原记录.锚楼,
    当前锚消息: SillyTavern.chat?.[原记录.锚楼],
    当前锚分支指纹: 当前即时业务锚分支指纹(原记录.锚楼),
    当前锚数据: 锚数据,
    预期场景事务ID: 原记录.场景事务ID,
  });
  if (!判定.有效) throw new Error(`即时业务核心已写入，但撤回记录无法确认：${判定.原因}`);
  try {
    await updateVariablesWith(
      vars => {
        if (!即时业务撤回身份仍有效(原记录)) throw new Error('__RQGY_TIMELINE_CHANGED__');
        const 当前 = 读取即时业务撤回记录(vars[即时业务撤回键]);
        if (!当前 || 当前.完整性指纹 !== 当前记录.完整性指纹) {
          throw new Error('即时业务撤回记录在确认期间已经变化。');
        }
        vars[即时业务撤回键] = _.cloneDeep(判定.记录);
        return vars;
      },
      { type: 'chat' },
    );
    ...
    if (即时业务撤回身份仍有效(原记录)) {
      const 已写 = 读取即时业务撤回记录(getVariables({ type: 'chat' })[即时业务撤回键]);
      if (已写?.完整性指纹 === 判定.记录.完整性指纹) return 判定.记录;
    }
    throw error;
  }
  return 判定.记录;
...
/** `落地` 明确失败时清准备票；若插件在报错前已实际写入锚楼，则保留并收敛为已提交。 */
export async function 收口未落地即时业务撤回(原记录: 即时业务撤回记录): Promise<'未提交' | '已提交'> {
  if (!即时业务撤回身份仍有效(原记录)) return '未提交';
  const 锚数据 = 读取即时业务锚数据(原记录.锚楼);
  if (!锚数据) throw new Error('即时业务落地失败后无法读取锚楼变量。');
  if (即时业务锚仍是业务前状态(原记录, 锚数据)) {
    try {
      await updateVariablesWith(
        vars => {
          if (!即时业务撤回身份仍有效(原记录)) throw new Error('__RQGY_TIMELINE_CHANGED__');
          const 当前 = 读取即时业务撤回记录(vars[即时业务撤回键]);
          if (当前?.完整性指纹 !== 原记录.完整性指纹) {
            throw new Error('即时业务撤回记录在未落地收口前已经变化。');
          }
          delete vars[即时业务撤回键];
          return vars;
        },
      ...
      if (!即时业务撤回身份仍有效(原记录)) throw error;
      const 当前变量 = getVariables({ type: 'chat' });
      if (Object.prototype.hasOwnProperty.call(当前变量, 即时业务撤回键)) throw error;
      // updater 已执行、宿主随后报错：票据键已经被精确删除，按未提交收口成功处理。
    }
    return '未提交';
  }
  await 确认即时业务撤回已提交(原记录);
  return '已提交';
}

/** 主正文开始时只认当前锚楼、当前分支和同一活动事务；旧版无票据时保持兼容。 */
function 读取本轮即时业务撤回(场景事务ID: string, 锚楼: number): 即时业务撤回记录 | null {
  const 原 = getVariables({ type: 'chat' })[即时业务撤回键];
  if (原 == null) return null;
  const 锚数据 = 读取即时业务锚数据(锚楼);
  if (!锚数据) throw new Error('即时业务撤回记录存在，但原锚楼变量无法读取。');
  const 判定 = 核验即时业务撤回记录(原, {
    当前聊天ID: 当前聊天ID(),
    当前锚楼: 锚楼,
    当前锚消息: SillyTavern.chat?.[锚楼],
    当前锚分支指纹: 当前即时业务锚分支指纹(锚楼),
    当前锚数据: 锚数据,
    预期场景事务ID: 场景事务ID,
  });
  if (!判定.有效) throw new Error(`即时业务撤回记录与当前场景不一致：${判定.原因}`);
  return 判定.记录;
}
...
const 时间线清场变量键 = [
  // 预约必须随失败／重掷快照恢复，但原生清场只按创建锚裁剪，不能随过程态整体置空。
  ...回合变量键.filter(键 => 键 !== '_手机邀约计划'),
  '_上次回合',
  '_上次隔离回合',
  '_时间撤销点',
] as const;
...
interface 已删时间线协调选项 {
  ...
  /** 即时业务完整撤回：覆盖仍存活锚楼中已经提前结算的碎片、物品、信任和关系节点。 */
  恢复即时业务数据?: SchemaType;
  /** 即时业务完整撤回：按“存在／不存在／undefined”精确恢复业务前的回合聊天键。 */
  恢复精确回合变量?: 即时业务撤回记录;
...
async function 协调已删时间线(
  ...
  const 存活 = 读取最近有效();
  const 当前真值 = 选项.恢复即时业务数据
    ? (Schema.parse(_.cloneDeep(选项.恢复即时业务数据)) as SchemaType)
    : (存活?.data ?? null);
  ...
  if (选项.恢复即时业务数据) {
    if (!存活 || !当前真值) throw new Error('即时业务正文已删除，但找不到可恢复的存活锚楼。');
    await 脚本写入(存活.raw, 当前真值, {
      记录成长: false,
      当前绝对时段: 当前真值.系统._绝对时段,
      // 目标数据本身就是业务发生前真值，不能让通用写口重新猜测或补造场景票。
      同步场景剧情: false,
    });
    确认提交仍有效();
  }
  await updateVariablesWith(
    vars => {
      确认提交仍有效();
      if (选项.恢复精确回合变量) {
        恢复即时业务撤回聊天变量(vars, 选项.恢复精确回合变量, 回合变量键);
      } else if (选项.恢复回合变量) {
        for (const 键 of 回合变量键) {
  ...
  清保护快照();
  if (!当前真值) return null;
  const 裂缝修复 = 同步已打开裂缝(当前真值);
  const 不必停检查点修复 = 恢复安若妍不必停失效亲密检查点(当前真值);
  const 换掉检查点修复 = 恢复安若妍换掉失效亲密检查点(当前真值);
  const 换掉当前票修复 = 同步安若妍换掉当前剧情票(当前真值);
  const 不再留门旧尺度边界修复 = 清理不再留门旧尺度边界提示(当前真值);
  const 旧打断修复 = 清理线路失效待演打断(当前真值);
  if (存活 && (裂缝修复 || 不必停检查点修复 || 换掉检查点修复 || 换掉当前票修复 || 不再留门旧尺度边界修复 || 旧打断修复)) {
    确认提交仍有效();
    await 脚本写入(存活.raw, 当前真值, { 记录成长: false, 当前绝对时段: 回滚绝对时段 });
    确认提交仍有效();
  }
  捕获保护快照(当前真值);
  await 等待晋阶镜像写入();
  ...
  确认提交仍有效();
  作废全部角色阶段世界书缓存();
  await 同步全部角色阶段世界书(当前真值, 提交校验, true);
  确认提交仍有效();
  await 同步整表视图(当前真值, 提交校验);
  确认提交仍有效();
  // `_上次回合` 是删楼后唯一可跨刷新认领的完整撤回票。必须等核心、精确聊天、手机、
  // 镜像和全部派生视图都已收口后再消费；任何中途失败都保留同一票供 catch／启动重入。
  if (选项.清上次回合) {
    const 清理聊天ID = 当前聊天ID();
    let updater已清理 = false;
    try {
      await updateVariablesWith(
        vars => {
          确认提交仍有效();
          if (当前聊天ID() !== 清理聊天ID) throw new Error('__RQGY_TIMELINE_CHANGED__');
          if (选项.恢复精确回合变量) {
            const 当前成功票 = 读取即时业务撤回记录(_.get(vars, `_上次回合.chat快照.${即时业务撤回键}`));
            if (当前成功票?.完整性指纹 !== 选项.恢复精确回合变量.完整性指纹) {
              throw new Error('上次回合记录已被其他时间线替换，停止消费旧即时业务撤回票。');
            }
          }
          _.set(vars, '_上次回合', null);
          updater已清理 = true;
          return vars;
        },
        { type: 'chat' },
      ...
      // updater 已写成、宿主随后抛错时按后置条件确认；切聊后绝不读取新聊天替旧调用认账。
      if (
        updater已清理 &&
        提交校验() &&
        当前聊天ID() === 清理聊天ID &&
        (_.get(getVariables({ type: 'chat' }), '_上次回合') ?? null) === null
      ) {
        return 当前真值;
      }
      throw error;
    ...
    确认提交仍有效();
    if ((_.get(getVariables({ type: 'chat' }), '_上次回合') ?? null) !== null) {
      throw new Error('已删时间线收口完成，但上次回合记录未能消费。');
    }
  }
  return 当前真值;
}
...
export async function 协调原生时间线切换(
  类型: '删楼' | '切分支',
  事件楼层?: number,
): Promise<void> {
  await 排队MVU操作(async () => {
    ...
    // TT 酒馆的 MESSAGE_SWIPED 载荷是实际被切分支的 message_id。数据库脚本表必须从
    // 该楼（含本楼）裁剪；不能被稍后运行的回合引擎再用“当前末楼”覆盖。删楼事件载荷
    // 是被删消息而不是存活锚，因此删楼仍以宿主删完后的真实末楼作为边界。
    const 精确切分支楼层 =
      类型 === '切分支' && Number.isInteger(Number(事件楼层)) && Number(事件楼层) >= 0
        ? Number(事件楼层)
        : null;
    const 数据库锚楼 = 精确切分支楼层 ?? 存活末楼;
    const 原因 = 类型 === '删楼' ? '删除消息' : '切换消息分支';
    // 数据库桥自己的宿主监听通常已同步建立同一栅栏；这里作为回合引擎兜底时复用已覆盖
    // 的更严格边界，避免同一 TT 酒馆事件在异步队列中被重标成较宽的新令牌。
    标记数据库时间线将变更(数据库锚楼, 原因, { 已有共享栅栏覆盖时不重标: true });
...
function 当前变量重生成解析通道(): ReturnType<typeof 解析游戏变量请求路由>['selectedRoute'] {
  return 解析游戏变量请求路由(数据库状态().可调用AI).selectedRoute;
}
...
/** 成功回合的 chat快照 本来就要供重掷恢复顶层票；直接复用，避免再复制一份完整 stat/chat 快照。 */
function 读取上次回合即时业务撤回(记录: 上次回合记录 | undefined): 即时业务撤回记录 | null {
  return 读取即时业务撤回记录(记录?.chat快照?.[即时业务撤回键]);
}

async function 清即时业务撤回顶层记录(预期完整性指纹: string): Promise<void> {
  const 清理聊天ID = 当前聊天ID();
  let 已执行删除 = false;
  try {
    await updateVariablesWith(
      vars => {
        ...
        const 当前 = 读取即时业务撤回记录(vars[即时业务撤回键]);
        if (当前?.完整性指纹 !== 预期完整性指纹) {
          throw new Error('即时业务撤回记录在清理前已经变化。');
        }
        delete vars[即时业务撤回键];
        已执行删除 = true;
        return vars;
    ...
    // updater 已同步删除、宿主随后才报错时，以原聊天仍在前台时的真值为准；切聊绝不检查新聊天。
    if (
      已执行删除 &&
      当前聊天ID() === 清理聊天ID &&
      !Object.prototype.hasOwnProperty.call(getVariables({ type: 'chat' }), 即时业务撤回键)
    ) {
      return;
    }
    throw error;
  ...
  if (当前聊天ID() !== 清理聊天ID) throw new Error('__RQGY_TIMELINE_CHANGED__');
  if (Object.prototype.hasOwnProperty.call(getVariables({ type: 'chat' }), 即时业务撤回键)) {
    throw new Error('即时业务撤回记录清理后仍然存在。');
  }
}
...
export async function 恢复即时业务撤回登记(): Promise<boolean> {
  ...
  const 顶层票存在 = Object.prototype.hasOwnProperty.call(vars, 即时业务撤回键) && vars[即时业务撤回键] != null;

  // 物理删楼已经成功、随后数据库或派生同步中断时，成功回合记录仍在而当前末楼已经回到
  // 原锚点。刷新后直接完成“删除后的第二阶段退款”，不要求玩家再删一次不存在的正文楼。
  if (!顶层票存在) {
    const 上次回合 = 读上次回合();
    const 已删记录 = 上次回合 && 上次回合.回合前末楼 === 当前末楼
      ? 准备恢复即时业务撤回(上次回合, 当前末楼, { 允许业务前锚: true })
      : null;
    if (!已删记录) return false;
    标记数据库时间线将变更(当前末楼, '恢复已删除的即时业务回合', { 已有共享栅栏覆盖时不重标: true });
    await 协调已删时间线(
      当前末楼,
      {
        恢复即时业务数据: 已删记录.业务前数据,
      ...
      () => 当前聊天ID() === 当前聊天 && getLastMessageId() === 当前末楼,
    );
    console.warn('[人妻公寓] 已完成上次删楼后中断的即时业务退款，恢复到该行动发生前。');
    return true;
  }

  const record = 读取即时业务撤回记录(vars[即时业务撤回键]);
  if (!record) throw new Error('即时业务撤回记录损坏；为避免错误退款，脚本已停止自动处理该票。');
  if (
    record.聊天ID !== 当前聊天 ||
    record.锚楼 < 0 ||
    record.锚楼 > 当前末楼 ||
    !即时业务锚消息身份匹配(record, SillyTavern.chat?.[record.锚楼]) ||
    当前即时业务锚分支指纹(record.锚楼) !== record.锚分支指纹
  ) {
    await 清即时业务撤回顶层记录(record.完整性指纹);
    console.info('[人妻公寓] 已清理不属于当前存活分支的即时业务撤回准备票。');
    return true;
  }
  ...
  const 判定 = 核验即时业务撤回记录(record, {
    当前聊天ID: 当前聊天,
    当前锚楼: record.锚楼,
    当前锚消息: SillyTavern.chat?.[record.锚楼],
    当前锚分支指纹: 当前即时业务锚分支指纹(record.锚楼),
    当前锚数据: 锚数据,
    预期场景事务ID: record.场景事务ID,
  });
  if (!判定.有效) throw new Error(`即时业务撤回记录无法恢复：${判定.原因}`);

  const 已完成记录 = 读取上次回合即时业务撤回(读上次回合());
  if (当前末楼 > record.锚楼) {
    if (!已完成记录 || 已完成记录.完整性指纹 !== 判定.记录.完整性指纹) {
      throw new Error('即时业务锚楼后已有正式消息，但撤回票没有被同一成功回合接管。');
    }
    await 清即时业务撤回顶层记录(record.完整性指纹);
    console.info('[人妻公寓] 即时业务撤回票已由成功回合接管，已清理顶层副本。');
    return true;
  }
  ...
  if (record.状态 === '准备中') {
    await updateVariablesWith(
      当前变量 => {
        const 当前 = 读取即时业务撤回记录(当前变量[即时业务撤回键]);
        if (!当前 || 当前.完整性指纹 !== record.完整性指纹) {
          throw new Error('即时业务撤回记录在启动恢复期间已经变化。');
        }
        当前变量[即时业务撤回键] = _.cloneDeep(判定.记录);
        return 当前变量;
      },
      { type: 'chat' },
    );
    console.warn('[人妻公寓] 已恢复中断的即时业务撤回票；原场景剧情仍可继续或完整撤回。');
  }
  return true;
}
...
type 内置外置变量解析结果 =
  { 结果: '成功'; 变量块: string } | { 结果: '未配置' } | { 结果: '失败' } | { 结果: '已取消' };

/**
 * 引擎自己调用解析模型生成变量块(替代"跨脚本桥按 MVU 官方按钮+轮询楼层"路线)。
 * 每次请求都从《人妻公寓》独立持久设置重新决议通道；重开、iframe reload 与切聊不会依赖
 * MVU 面板的临时 Pinia 方案。正文模型只负责故事，外置变量解析绝不占用正文 API。
 * - 自动(默认) → 数据库插件 callAI 代发优先(沿用数据库当前配置,不读其密钥与模型),
 *   没有数据库时使用游戏已保存的自定义 API;
 * - 自定义 → generateRaw + custom_api(游戏权威配置；MVU 只作兼容镜像)。
...
async function 内置外置变量解析(参数: {
  ...
  const 变量结算令 = 当前变量结算令();
  // 运行点重新读取游戏权威配置；不能复用设置页草稿、MVU 面板缓存或上一轮路由。
  const 路由 = 解析游戏变量请求路由(数据库状态().可调用AI);
  const 配置 = 路由.配置;
  const 通道 = 路由.selectedRoute;
  if (!通道) {
    console.info(
      `[人妻公寓·变量请求] selectedRoute=null configSource=${路由.配置来源} channel=${路由.通道}，本轮不发起解析请求`,
    );
    return { 结果: '未配置' };
  }
  console.info(
    `[人妻公寓·变量请求] 变量请求已发起 selectedRoute=${通道} configSource=${路由.配置来源} model=${
      通道 === '自定义' ? 配置?.模型名称 || '未配置' : '数据库代发'
    }`,
  );
  ...
  try {
    if (通道 === '自定义' && 配置) {
      console.info('[人妻公寓] 内置变量解析走游戏权威自定义 API 通道');
      原文 = String(
        await Promise.race([
          generateRaw({
            ordered_prompts: [
...
function 读取变量重生成上下文(): 变量重生成上下文 | null {
  const 上次 = 读上次回合();
  const 上下文 = 上次?.变量重生成;
  // 旧票未保存原回合线路资格，不能用后来存档猜测；下一完整回合会生成新票。
  if (!上下文 || 上下文.版本 !== 3) return null;
  if (
    !上下文.聊天ID ||
  ...
  for (const 项 of Object.values(上下文.派生票据.妻 ?? {})) {
    const 资格 = 项?.结算资格;
    if (!资格 || [资格.线路接管, 资格.母亲自由共居, 资格.医院硬锁].some(值 => typeof 值 !== 'boolean')) return null;
  }
  return 上下文;
}
...
export function 读取变量重生成状态(): 变量重生成状态 {
  ...
  try {
    ...
    if (!上下文 || !变量重生成身份有效(上下文)) {
      if ([1, 2].includes(Number((读上次回合()?.变量重生成 as { 版本?: unknown } | undefined)?.版本))) {
...
export async function 重新生成最近回合变量(): Promise<boolean> {
  ...
  const 身份仍有效 = () => 时间线世代 === 当前时间线切换世代() && 变量重生成身份有效(上下文, 原消息签名);
  ...
  变量重生成事务 = 事务;
  let 结束载荷:
    | { 成功: boolean; 取消?: boolean; 状态: 变量重生成可用状态; 提示: string }
    | undefined;
  ...
  try {
    ...
    const 重新生成守护 = 回滚保护字段(候选stat, 上下文.焦点, 上下文.变量范围, 上下文.助手楼层, raw候选stat, 基准stat);
    ...
    await 排队MVU操作(async () => {
      ...
      const 合并stat = Schema.parse(
        合并重新生成变量结果(当前stat, 上下文.原AI结果, 新AI结果, 上下文.变量范围, 上下文.派生票据, 派生重算),
      ) as SchemaType;
      ...
      const 含原回合独立结算 = 三方合并变量重生成对象(上下文.风闻票据.派生后, 新派生后, 上下文.风闻票据.原回合最终);
      const 含回合后独立变化 =
        含原回合独立结算 &&
        三方合并变量重生成对象(上下文.风闻票据.原回合最终, 含原回合独立结算, 提取变量重生成风闻快照(当前stat));
    ...
    // 客户端收到“结束”后会立刻解锁所有操作；必须先清事务并释放共享前台槽，
    // 否则变量已经成功落盘，下一次点击仍会被误判成“内容正在生成”。
    前台租约.释放();
    广播变量重生成状态();
    if (结束载荷) eventEmit('人妻公寓:变量重生成结束', 结束载荷);
  }
}
...
export async function 组快照注入(
  ...
  const 本线演员错误 =
    ...
    安若妍换掉剧情演员错误(本楼事件, 妻在场, 夫在场) ||
    许曼君分居剧情演员错误(本楼事件, 妻在场, 夫在场) ||
  ...
  const 明确追忆 = /记得|上次|以前|之前|曾经|答应|约定|微信|消息|朋友圈|动态|姐妹群|群里|秘密|那天|刚才/.test(最近行动);
  const 需要记忆 = 快照刷新票.模式 === '完整' || 明确追忆;
  const 有效楼务任务id们 = data.系统._管理考核.活跃任务.map(任务 => 任务.id);
  const 私聊引用 = 需要记忆 ? 当前微信摘要引用(私聊可召回妻, 记忆截止楼层) : [];
  // 通用记忆门关闭时，可靠在场妻接续本楼私聊往返及实际收到的群聊/动态。
  // 正文成功后楼层前进即自然消费；失败/取消/重试仍停在原楼，
...
const 数据库事件摘要指令 =
  ...
  '<rq_event_summary>客观结果摘要</rq_event_summary>。摘要写 1～3 句、总共 40～120 字，' +
  '按“谁做了什么→对方如何回应→留下什么状态或后续”把本次回复已经发生的结果讲清楚；' +
  '不得复述对话原句、不得写未发生的推测、不得换行、不得写楼层号或任何其他标签。' +
  '该块只供脚本绑定本轮数据库事件，玩家界面会移除。';
 ...
 */
async function 广播生成完成事件(目标助手楼层: number, 提交校验: () => boolean = () => true): Promise<void> {
  try {
...
type RQ剧情骨架写入状态 = '已记录' | '待补' | '未写入';
type RQ剧情骨架待补原因 = 'active-fill' | '待确认';

interface RQ剧情骨架待补任务 {
  key: string;
  聊天ID: string;
  事件: 数据库回合事件;
  提交校验: () => boolean;
  ...
  尝试序号: number;
  仅回读剩余: number;
  /** 原始失败之后，队列每次被当前成功回合认领只允许再发起一次幂等 UPSERT。 */
  队列补写已发起: boolean;
  运行中: boolean;
  待处理唤醒: '数据库刷新' | '新回合' | null;
  timer?: ReturnType<typeof setTimeout>;
}

const RQ剧情骨架待补队列 = new Map<string, RQ剧情骨架待补任务>();
const RQ剧情骨架重试延迟 = [500, 1500, 3500, 7000] as const;
/** active-fill 没有完成信号时只做三次精确回读，最后才允许一次保守补写。 */
const RQ剧情骨架填表回读次数 = RQ剧情骨架重试延迟.length - 1;

function RQ剧情骨架待补键(聊天ID: string, 楼层: number): string {
  return `${聊天ID}|${楼层}`;
}

function 完成RQ剧情骨架待补(task: RQ剧情骨架待补任务): void {
  if (RQ剧情骨架待补队列.get(task.key) !== task) return;
  if (task.timer) clearTimeout(task.timer);
  RQ剧情骨架待补队列.delete(task.key);
  数据库记录失败提示签名 = '';
  console.info(`[人妻公寓·数据库] RQ_剧情事件骨架 ${task.事件.楼层} 补写成功并通过精确回读。`);
  eventEmit('人妻公寓:运行阶段', '数据库剧情骨架补写成功');
  eventEmit('人妻公寓:提示', `✓ RQ_剧情事件骨架补写成功：楼层 ${task.事件.楼层} 已回读确认。`);
}

type RQ剧情骨架唤醒来源 = '数据库刷新' | '新回合';

function 应用RQ剧情骨架唤醒(task: RQ剧情骨架待补任务, 来源: RQ剧情骨架唤醒来源): void {
  task.尝试序号 = 0;
  if (来源 === '新回合') task.队列补写已发起 = false;
  // 数据库公开 table-update 回调代表一次表格持久化已经结束：active-fill 可立即精确回读，
  // 缺行时再发唯一一次补写。没有回调的旧版则先保守回读三轮，最后才尝试一次。
  task.仅回读剩余 =
    来源 === '数据库刷新' ? (task.原因 === 'active-fill' ? 0 : 1) : task.原因 === 'active-fill' ? RQ剧情骨架填表回读次数 : 2;
}

function 重新调度RQ剧情骨架待补(task: RQ剧情骨架待补任务, 来源: RQ剧情骨架唤醒来源): void {
  if (RQ剧情骨架待补队列.get(task.key) !== task) return;
  if (task.运行中) {
    task.待处理唤醒 = 来源;
    return;
...
/** 数据库公开 table-update 回调的无载荷唤醒入口；先回读，绝不因信号本身盲写。 */
export function 唤醒RQ剧情骨架待补队列(): void {
  const 聊天ID = 当前聊天ID();
  for (const task of RQ剧情骨架待补队列.values()) {
    if (task.聊天ID !== 聊天ID || !task.提交校验()) {
      if (task.timer) clearTimeout(task.timer);
      RQ剧情骨架待补队列.delete(task.key);
      continue;
    }
    重新调度RQ剧情骨架待补(task, '数据库刷新');
  }
}

function 调度RQ剧情骨架待补(task: RQ剧情骨架待补任务): void {
  if (task.timer || task.运行中 || RQ剧情骨架待补队列.get(task.key) !== task) return;
  if (task.尝试序号 >= RQ剧情骨架重试延迟.length) {
    console.warn(
      `[人妻公寓·数据库] RQ_剧情事件骨架 ${task.事件.楼层} 本轮只读核对/单次补写已结束，继续保留待补；数据库下次持久化或下个成功回合会再次唤醒。`,
    );
    eventEmit('人妻公寓:运行阶段', '数据库剧情骨架仍待补');
    return;
  }
  const 延迟 = RQ剧情骨架重试延迟[task.尝试序号];
  task.timer = setTimeout(() => {
    task.timer = undefined;
    task.运行中 = true;
    void (async () => {
      if (
        RQ剧情骨架待补队列.get(task.key) !== task ||
        当前聊天ID() !== task.聊天ID ||
        !task.提交校验()
      ) {
        RQ剧情骨架待补队列.delete(task.key);
      ...
      const 回读 = 核对数据库回合已写入(task.事件);
      if (回读 === true) {
        完成RQ剧情骨架待补(task);
        return;
      }
      // 原 mutation 或 active-fill 尚可能在结算；先只做精确回读，不拆锁、不并发 UPSERT。
      if (task.仅回读剩余 > 0) {
        task.仅回读剩余 -= 1;
        task.尝试序号 += 1;
        return;
      ...
      if (task.队列补写已发起) {
        task.尝试序号 = RQ剧情骨架重试延迟.length;
        return;
      }

      // 原始写入之外，当前成功回合只授予一次幂等补写；在 await 前即消费写权，
      // 即使数据库回调与 Promise 同时抵达也不会并发发出第二条 SQL。
      task.队列补写已发起 = true;
      const 写入 = await 同步数据库回合(task.事件, task.提交校验);
      if (
        RQ剧情骨架待补队列.get(task.key) !== task ||
        当前聊天ID() !== task.聊天ID ||
        !task.提交校验()
      ...
      if (写入 === '已确认' || 核对数据库回合已写入(task.事件) === true) {
        完成RQ剧情骨架待补(task);
        return;
      }
      if (写入 === '待确认' || 写入 === '填表占用') {
        task.原因 = 写入 === '填表占用' ? 'active-fill' : '待确认';
        task.仅回读剩余 = RQ剧情骨架重试延迟.length;
        task.尝试序号 = 0;
        console.warn(
          `[人妻公寓·数据库] RQ_剧情事件骨架 ${task.事件.楼层} 单次补写${写入 === '填表占用' ? '仍遇 active-fill' : '已提交待确认'}；后续只回读，等待数据库持久化信号。`,
        );
        return;
      }
      task.尝试序号 = RQ剧情骨架重试延迟.length;
      console.warn(`[人妻公寓·数据库] RQ_剧情事件骨架 ${task.事件.楼层} 单次补写未确认，保留待下个成功回合。`);
    })()
      .catch(error => {
        console.warn(`[人妻公寓·数据库] RQ_剧情事件骨架 ${task.事件.楼层} 待补重试异常:`, error);
        task.尝试序号 = RQ剧情骨架重试延迟.length;
      })
      .finally(() => {
        task.运行中 = false;
        if (RQ剧情骨架待补队列.get(task.key) !== task) return;
        const 待处理唤醒 = task.待处理唤醒;
        task.待处理唤醒 = null;
        if (待处理唤醒) 应用RQ剧情骨架唤醒(task, 待处理唤醒);
        调度RQ剧情骨架待补(task);
      });
  }, 延迟);
}
...
function 排队RQ剧情骨架待补(
  ...
  const 聊天ID = 当前聊天ID();
  if (!聊天ID || !提交校验()) return;
  const key = RQ剧情骨架待补键(聊天ID, 事件.楼层);
  let task = RQ剧情骨架待补队列.get(key);
  if (!task) {
    task = {
      key,
      聊天ID,
      ...
      原因,
      尝试序号: 0,
      仅回读剩余: 原因 === 'active-fill' ? RQ剧情骨架填表回读次数 : 2,
      队列补写已发起: false,
      运行中: false,
      待处理唤醒: null,
    };
    RQ剧情骨架待补队列.set(key, task);
    const 说明 =
      原因 === 'active-fill'
        ? '数据库正在执行 AI 填表，脚本没有拆互斥锁'
        : 'SQLite 已受理写入，正在等待回读确认';
    console.warn(`[人妻公寓·数据库] RQ_剧情事件骨架 ${事件.楼层} 待补：${说明}。`);
    eventEmit(
      '人妻公寓:提示',
      `⚠ RQ_剧情事件骨架待补：楼层 ${事件.楼层}，${说明}；已排队自动回读/重试，本轮正文与游戏结算不受影响。`,
    );
  } else {
    task.事件 = _.cloneDeep(事件);
    task.提交校验 = 提交校验;
    task.原因 = 原因;
    // 新成功回合重新认领这条 pending 后，授予本回合唯一一次补写权；旧计时器先取消，
    // 仍在运行的旧回调则只记录“新回合唤醒”，待其 finally 串行接续。
    task.队列补写已发起 = false;
    eventEmit('人妻公寓:运行阶段', '数据库剧情骨架待补');
    重新调度RQ剧情骨架待补(task, '新回合');
    return;
  }
  eventEmit('人妻公寓:运行阶段', '数据库剧情骨架待补');
  调度RQ剧情骨架待补(task);
...
async function 记录数据库回合骨架(
  ...
  const 写入结果 = await 同步数据库回合(事件, 提交校验);
  if (!提交校验()) throw new Error('__RQGY_TIMELINE_CHANGED__');
  if (写入结果 === '已确认') {
    数据库记录失败提示签名 = '';
  ...
  if (写入结果 === '待确认' || 写入结果 === '填表占用') {
    排队RQ剧情骨架待补(事件, 提交校验, 写入结果 === '填表占用' ? 'active-fill' : '待确认');
    return '待补';
  }
...
async function 补齐缺失数据库事件骨架(截止楼层: number, 提交校验: () => boolean = () => true): Promise<number> {
  ...
  for (let 楼层 = Math.min(截止楼层, 消息表.length - 1); 楼层 >= 1 && 已补写 < 12; 楼层 -= 1) {
    ...
    const 参与者 = Array.isArray(元数据?.参与者)
      ...
      : 妻门牌.map(value => 户静态表[String(value) as 门牌]?.妻名).filter((name): name is string => Boolean(name));
    const 玩家行动 =
    ...
    const 地点 = (typeof 元数据?.地点 === 'string' && 元数据.地点.trim()) || '历史剧情（地点未保存）';
    const 结果摘要 =
      (typeof 元数据?.结果摘要 === 'string' && 元数据.结果摘要.trim()) ||
      (typeof extra._rqgy开局令牌 === 'string' ? 数据库固定开局摘要 : 脚本保守回合摘要(玩家行动));
    const 事件: 数据库回合事件 = { 楼层, 时间, 地点, 参与者, 玩家行动, 结果摘要 };
    const 写入 = await 同步数据库回合(事件, 提交校验);
    if (写入 === '填表占用' || 写入 === '待确认') {
      排队RQ剧情骨架待补(事件, 提交校验, 写入 === '填表占用' ? 'active-fill' : '待确认');
      // 同一 active-fill 期间继续扫更多楼只会撞同一互斥锁；保留队列，下一成功回合再唤醒。
      break;
    }
    if (写入 === '失败') break;
    已记录.add(楼层);
...
function 安排数据库回合后处理(参数: {
  ...
  setTimeout(() => {
    void (async () => {
      try {
        if (!参数.提交校验()) return;
        const 数据库时间线可用 = await 等待数据库时间线就绪();
        if (!参数.提交校验()) return;
        if (!数据库时间线可用) {
          console.warn(`[人妻公寓·数据库] 当前消息时间线仍在恢复，RQ_剧情事件骨架 ${参数.楼层} 留待后续成功回合补写。`);
          return;
        }
        const 骨架写入状态 = await 记录数据库回合骨架(
          参数.楼层,
          data快照,
          参数.地点,
          参数.行动,
        ...
        if (当前数据库状态.脚本所有权模板已启用) {
          ...
          if (提示签名 && 数据库所有权模板提示签名 !== 提示签名) {
            ...
            eventEmit(
              '人妻公寓:提示',
              '数据库旧模板的通用填表已安全暂停；请在手机“我”中点“安装/更新本游戏表”，已有记录会保留。',
            );
          }
        }
...
/** 只恢复已保存对话的业务提交，不再次生成正文或结算普通成长。写回由调用方统一提交。 */
export function 提交已保存自然对话(data: SchemaType, 记录: 对话观察记录): Array<() => void | Promise<unknown>> {
  const ctx = 记录.提交上下文;
  if (!ctx || !自然对话可提交(记录) || ctx.时段 !== data.系统._绝对时段) throw new Error('已保存对话不具备当前提交凭据');
  const 旧 = _.cloneDeep(data);
  const 玩家文本 = 记录.请求.消息.find(文 => 文.id === `${记录.请求.批次}:player`)?.文本 ?? '';
  const 正文 = 记录.请求.消息.find(文 => 文.id === `${记录.请求.批次}:assistant`)?.文本 ?? '';
  if (记录.请求.类别 === '冷落安抚') {
    const 门牌号 = 记录.请求.事件.slice('冷落安抚:'.length) as 门牌;
    const 当前 = 构造安抚观察请求(data, 门牌号, 玩家文本, 正文, 记录.请求.分支, 记录.请求.批次);
    if (当前?.阶段 !== 记录.请求.阶段 || !ctx.妻在场.includes(门牌号)) throw new Error('安抚对象或阶段已经变化');
    推进余波安抚(data.户[门牌号].妻, { 正文楼: ctx.楼层, 当前绝对时段: ctx.时段, 成功主线当面楼: !!正文, 玩家有效回应: true });
    return [];
  }
  const txn = data.系统._场景剧情事务;
  if (txn.id !== ctx.事务ID || txn.请求世代 !== ctx.请求世代 || txn.内容 !== 记录.请求.事件 || txn.状态 !== '待续') {
    throw new Error('原剧情事务已经变化，不能重复或跨阶段提交');
  }
  txn.状态 = '生成中';
  const 结果 = 回合结算(data, 旧, ctx.妻在场 as 门牌[], ctx.妻在场 as 门牌[], ctx.夫在场 as 门牌[],
    ctx.楼层, ctx.事件, !!正文, 正文, ctx.场景, 玩家文本, undefined,
    { id: ctx.事务ID, 请求世代: ctx.请求世代 }, ctx.日常提交锚, true);
  const 资源 = 结算成功现场楼(data, 旧, { 场景: ctx.场景, 楼层: ctx.楼层, 行动: 玩家文本, 正文,
    本楼事件: 记录.请求.事件, 妻在场: ctx.妻在场 as 门牌[], 实际尺度: ctx.实际尺度, 资源计费: ctx.资源计费 });
  if ((不再留门事件要求真实开录(记录.请求.事件) && !不再留门真实录制已绑定(data)) ||
    (安若妍不必停事件要求H1开场(记录.请求.事件) && !安若妍不必停真实亲密已绑定(data)) ||
    (许曼君分居事件要求真实亲密(记录.请求.事件) && !许曼君分居真实亲密已绑定(data))) throw new Error('现场资源尚未完成绑定，保留已保存的对话');
  绑定新增待发送事件到场景(data, 读取队首场景剧情(旧.系统._待发送事件)?.剩余 ?? '', ctx.场景);
  if (资源.提示) 结果.提交后任务.push(() => eventEmit('人妻公寓:提示', 资源.提示));
  if (资源.线路CG) {
    const 文件 = 资源.线路CG;
    结果.提交后任务.push(() => eventEmit('人妻公寓:安若妍不必停CG', { 文件, 标题: 安若妍不必停CG标题(文件) }));
  }
  return 结果.提交后任务;
}
...
function 回合结算(
  ...
  const 本楼事件 = 本轮事件.内容;
  if (!本轮自然事件可提交(newStat, 本楼事件)) {
    保留自然剧情待续(newStat, 本楼事件);
    return { 入住预约已提交: false, 提交后任务: [] };
  }
  let 入住预约已提交 = false;
  const 提交后任务: Array<() => void | Promise<unknown>> = [];
  let 第二机位提交结果: 第二机位结果 | null = null;
  let 不再留门提交结果: 不再留门结果 | null = null;
  let 安若妍不必停提交结果: 安若妍不必停结果 | null = null;
  let 回国提交结果: 回国结果 | null = null;
  let 双重继承提交结果: 双重继承结果 | null = null;
  let 许曼君分居提交结果: 许曼君分居结果 | null = null;
  ...
  const 提交本轮事件 = () => {
    ...
    if (解析不再留门剧情事件(本楼事件)) {
      const 提交结果 = 提交不再留门剧情事件(newStat, 本楼事件, 回合场景, 楼层, 玩家行动);
      if (!提交结果?.成功) throw new Error(提交结果?.提示 || '《不再留门》当前拍未能提交。');
      不再留门提交结果 = 提交结果;
      if (提交结果.CG) {
        const 载荷 = { 文件: 提交结果.CG, 实例: newStat.系统._不再留门.实例, 聊天ID: 当前聊天ID() };
        提交后任务.push(() => eventEmit('人妻公寓:不再留门CG', 载荷));
      }
    }
    const 第二机位票 = 解析第二机位剧情事件(本楼事件);
    if (第二机位票) {
      const 提交结果 = 提交第二机位剧情事件(newStat, 本楼事件, 回合场景, 楼层);
      if (!提交结果?.成功) throw new Error(提交结果?.提示 || '第二机位剧情票未能提交。');
      第二机位提交结果 = 提交结果;
      if (提交结果.提示) 排队提交后提示(提交结果.提示);
      if (提交结果.CG) {
        const 文件 = 提交结果.CG;
        提交后任务.push(() => eventEmit('人妻公寓:第二机位CG', { 文件, 标题: 第二机位CG标题(文件) }));
      }
    }
    if (解析安若妍换掉剧情事件(本楼事件)) {
      const 演员错误 = 安若妍换掉剧情演员错误(本楼事件, 妻在场, 夫在场);
      if (演员错误) throw new Error(演员错误);
      const 结果 = 提交安若妍换掉剧情事件(newStat, 本楼事件, 回合场景, 楼层);
      if (!结果?.成功) throw new Error(结果?.提示 || '《换掉》剧情票未能提交。');
      if (结果.提示) 排队提交后提示(结果.提示);
      for (const 文件 of 结果.CG序列 ?? []) 提交后任务.push(() => eventEmit('人妻公寓:安若妍换掉CG', { 文件, 标题: '安若妍 · 换掉' }));
    }
    const 安若妍不必停票 = 解析安若妍不必停剧情事件(本楼事件);
    if (安若妍不必停票) {
      const 验收错误 =
        安若妍不必停剧情演员错误(本楼事件, 妻在场, 夫在场) ||
        (自然场景类别(本楼事件) ? '' : 安若妍不必停正文越拍原因(本楼事件, 正文));
      if (验收错误) throw new Error(`《不必停》当前剧情回合未通过验收：${验收错误}`);
      const 提交结果 = 提交安若妍不必停剧情事件(newStat, 本楼事件, 回合场景, 楼层);
      if (!提交结果?.成功) throw new Error(提交结果?.提示 || '《不必停》剧情票未能提交。');
      安若妍不必停提交结果 = 提交结果;
      if (提交结果.提示) 排队提交后提示(提交结果.提示);
      const 文件们 = 提交结果.CG序列?.length ? 提交结果.CG序列 : 提交结果.CG ? [提交结果.CG] : [];
      if (文件们.length) {
        提交后任务.push(() => {
          文件们.forEach(文件 =>
            eventEmit('人妻公寓:安若妍不必停CG', { 文件, 标题: 安若妍不必停CG标题(文件) }),
          );
        });
      }
    }
    ...
    if (许曼君分居票) {
      ...
      if (提交结果.CG) {
        const 文件 = 提交结果.CG;
        提交后任务.push(() => eventEmit('人妻公寓:许曼君分居CG', { 文件, 标题: 许曼君分居CG标题(文件) }));
      }
    }
    const 许曼君离婚票 = 解析许曼君离婚剧情事件(本楼事件);
    if (许曼君离婚票) {
      const 演员错误 = 许曼君离婚剧情演员错误(本楼事件, 妻在场, 夫在场);
      if (演员错误) throw new Error(演员错误);
      const 提交结果 = 提交许曼君离婚剧情事件(newStat, 本楼事件, 回合场景, 楼层);
      if (!提交结果?.成功) throw new Error(提交结果?.提示 || '《离婚》剧情票未能提交。');
      if (提交结果.提示) 排队提交后提示(提交结果.提示);
      if (提交结果.CG) {
        const 文件 = 提交结果.CG;
        提交后任务.push(() => eventEmit('人妻公寓:许曼君离婚CG', { 文件, 标题: 许曼君离婚CG标题(文件) }));
      }
    }
    const 许曼君离婚后日常票 = 解析许曼君离婚后日常事件(本楼事件);
    if (许曼君离婚后日常票) {
      const 演员错误 = 许曼君离婚后日常演员错误(本楼事件, 妻在场, 夫在场);
      if (演员错误) throw new Error(演员错误);
      const 正文错误 = (自然场景类别(本楼事件) ? '' : 许曼君离婚后日常正文越界原因(本楼事件, 正文));
      if (正文错误) throw new Error(正文错误);
    ...
    排入许曼君分居后续剧情(newStat, 许曼君分居提交结果);
    排入回国后续剧情(newStat, 回国提交结果);
    排入双重继承后续剧情(newStat, 双重继承提交结果);
    const 双重继承时间节点 = 同步双重继承时间节点(newStat, 楼层);
    if (双重继承时间节点.提示) 排队提交后提示(双重继承时间节点.提示);
    // H1刚提交，普通场次将在本轮资源结算中创建；不能把这个事务内窗口当成旧档失效。
    if (!安若妍不必停事件要求H1开场(本楼事件)) {
      const 不必停时间节点 = 同步安若妍不必停时间节点(newStat);
      if (不必停时间节点.提示) 排队提交后提示(不必停时间节点.提示);
    }
    结算前待发送基线 = newStat.系统._待发送事件;
  };

  if (仅提交事件 || 静音会议正式运行中(snapStat)) {
    // 隔离场只消费本轮固定事件；经济、作息、疑心、打断、入住、冷落等普通账全部暂停。
    提交本轮事件();
    return { 入住预约已提交, 提交后任务 };
  }

  // 结局完成ID旧档与日切共居状态在每次有效正文的候选账上幂等同步；回档仍由该楼stat决定。
  同步302共居状态(newStat);
  if (!安若妍不必停事件要求H1开场(本楼事件)) 同步安若妍不必停时间节点(newStat);
  if (!安若妍换掉事件要求H1开场(本楼事件)) 同步安若妍换掉时间节点(newStat);
  同步许曼君离婚完成后状态(newStat);
  const 许曼君离婚时间节点 = 同步许曼君离婚时间节点(newStat);
  if (许曼君离婚时间节点.提示) 排队提交后提示(许曼君离婚时间节点.提示);
  // 焦点户:被触碰=惰性结算生效点
  ...
  const 前台线路隔离 = 前台角色线路无关强剧情已冻结(newStat, [...焦点, ...妻在场, ...夫在场]);
  if (!前台线路隔离) {
    // 角色专属承接／结局正在前台演出时，入住、撞见、旧绿帽开线等无关强剧情全部延后；
    // 经济、资源、作息和本线路自己的确定性票据仍照常结算。
    ...
    if (
      ...
      母亲撞见风险(newStat, 撞见门牌, 回合场景, 难度表[newStat.系统._难度]?.撞见概率系数 ?? 1).概率 > 0
    ) {
  ...
  if (!newStat.系统._特殊场景.id && newStat.系统._荣耀洞拍 < 0) {
    绑定新增待发送事件到场景(newStat, 结算前待发送基线, 回合场景 || null, item => 是入住登场事件(item));
  }
  return { 入住预约已提交, 提交后任务 };
}
...
export async function 执行回合(
  ...
  选项: {
    资源计费?: boolean;
    成功结算?: (data: SchemaType) => void | Promise<void>;
    /**
     * 正式楼转正后、核心成功标记前，与 `_上次回合` 在同一个 chat updater 中提交。
     * 只供必须和本轮同成同败的聊天状态使用；抛错会清掉本轮两楼并恢复回合前聊天快照。
     */
    成功核心聊天结算?: (vars: Record<string, unknown>, data: SchemaType) => void;
  ...
  let 回合完成已广播 = false;
  let 待广播回合完成 = false;
  let 待安排数据库回合后处理: Parameters<typeof 安排数据库回合后处理>[0] | null = null;
  let 变量解析已降级 = false;
  let 变量解析降级阶段 = '';
  let 本轮静音会议 = false;
  let chat快照: Record<string, unknown> | null = null;
  let 回合基准data: SchemaType | null = null;
  let 本轮即时业务撤回: 即时业务撤回记录 | null = null;
  ...
  try {
    ...
    if (数据库状态().已装游戏模板) await 保存当前数据库恢复点();
    chat快照 = _.cloneDeep(_.pick(getVariables({ type: 'chat' }), [...回合变量键, '_rqgy数据库恢复锚']));
    ...
    const 本轮资源计费 =
      选项.资源计费 !== false && !读取活动场景剧情(data) && !data.系统._特殊场景.id && data.系统._荣耀洞拍 < 0;
    if (本轮资源计费) {
    ...
    if (普通活动场景剧情) {
      ...
      if (!当前场景剧情事务ID || 当前场景剧情事务ID !== txn.id) {
        throw new Error(
          `「${txn.标题 || '当前场景剧情'}」尚未完成，请使用场景中的“重试本段剧情”，不要用新的行动改写它。`,
        );
      }
      ...
      本轮即时业务撤回 = 读取本轮即时业务撤回(txn.id, 回合前末楼);
    } else if (
      本轮事件冻结.来源 === '待发送' &&
      本轮事件冻结.内容 &&
      !data.系统._特殊场景.id &&
    ...
    const 不再留门回应阻断 = 自然场景类别(本楼事件) ? '' : 不再留门回应错误(本楼事件, 行动);
    ...
    // 每回合重新读取一次。游戏内置解析的 API 与通道来自自己的持久权威设置；MVU 状态只
    // 决定玩家明确关闭内置解析后，是否走官方额外模型路径。正文模型始终不处理变量。
    const MVU解析 = 读取MVU解析状态();
    const 使用MVU外置解析 = MVU解析.外置模式;
    const 变量执行路径 = 选择变量解析执行路径(MVU解析, 本轮有可写演员, 本轮静音会议);
    const 变量配置握手 = 输出变量配置握手诊断({
      聊天ID: 当前聊天ID(),
      有可写演员: 本轮有可写演员,
      数据库可调用AI: 数据库状态().可调用AI,
      跳过原因: 本轮静音会议 ? 'silent-meeting' : undefined,
    });
    if (!本轮静音会议 && 本轮有可写演员 && MVU解析.内置解析) {
      console.info(
        `[人妻公寓] 变量解析门通过 selectedRoute=${变量配置握手.selectedRoute ?? 'null'} configSource=${变量配置握手.configSource}`,
      );
    }
    const 自由阶段历史 = 构造302自由阶段聊天历史(data, 回合起始场景);
    const 正文模型覆盖 = {
      chat_history: {
    ...
    const 自然观察请求 = 构造自然场景请求(data, 本楼事件, 行动, '', 当前聊天ID(), 本回合消息令牌) ??
      构造安抚观察请求(data, 本轮余波目标, 行动, '', 当前聊天ID(), 本回合消息令牌);
    const 自然提示 = 自然场景生成提示(自然观察请求, data);
    if (自然提示) injects.push({ role: 'system', content: 自然提示, position: 'in_chat', depth: 0, should_scan: false });
    if (本轮数据库已安装) {
    ...
    const 临时用户消息ID原值 = (临时用户消息引用 as { message_id?: unknown } | undefined)?.message_id;
    const 数据库Plot消息锚: 数据库剧情规划消息锚 = {
      chatId: 当前聊天ID(),
      messageIndex: 临时用户楼层,
      messageId:
        typeof 临时用户消息ID原值 === 'string' || typeof 临时用户消息ID原值 === 'number'
          ? 临时用户消息ID原值
          : 临时用户楼层,
      turnToken: 本回合消息令牌,
    };
      ...
      原文 = await 等待正文生成(
        ...
        },
        {
          ...
          规划输入: 构造数据库剧情规划输入(行动, {
            ...
            时段: 当前时段(演出data),
            地点: 回合起始场景 || '公寓公共区域',
            当前角色: _.uniq([...妻在场.map(m => 户静态表[m]?.妻名), ...夫在场.map(m => 户静态表[m]?.夫名)]).filter(
              (名字): 名字 is string => Boolean(名字),
            ),
    ...
    const 焦点妻门牌 = 焦点妻们[0];
    const 阶段表 = Object.fromEntries(焦点妻们.map(m => [m, 亲密场景许可阶段(data, m, 本楼事件)])) as Partial<
      Record<门牌, number>
    >;
    // 脚本自己导演的晋阶/特殊正戏已经有独立许可，不再被普通阶段上限二次拦截。
    const 正戏免检 =
      /【特殊场景·|【转折正戏】|【药物首夜】|【早饭桌】|【破墙】/.test(本楼事件) ||
      第二机位事件要求真实开录(本楼事件) ||
      安若妍不必停事件要求H1开场(本楼事件) ||
      许曼君分居事件要求真实亲密(本楼事件);
    const 当前拍正文 = 提取可提交正文(原文);
    if (是提供方拒答正文(当前拍正文)) {
      失败残稿 = 提取正文舞台文本(最终显示原文);
      throw new Error('AI服务返回了拒答说明，本轮正文、剧情票和资源均未提交；请重试或更换模型线路。');
    }
    const 拍摄尺度契约 = 读取拍摄剧情尺度契约(本楼事件);
    let 稽查: 稽查结果 = 输出稽查(原文, 焦点妻们, 阶段表, 尺度模式, 正戏免检, 当前拍正文, 拍摄尺度契约);
    const 第二机位票 = 解析第二机位剧情事件(本楼事件);
    const 安若妍不必停票 = 解析安若妍不必停剧情事件(本楼事件);
    const 许曼君分居票 = 解析许曼君分居剧情事件(本楼事件);
    const 许曼君离婚票 = 解析许曼君离婚剧情事件(本楼事件);
    ...
    const 回国票 = 解析回国剧情事件(本楼事件);
    const 双重继承票 = 解析双重继承剧情事件(本楼事件);
    const 不再留门票 = 解析不再留门剧情事件(本楼事件);
    const 专属节拍错误 =
      (自然场景类别(本楼事件) ? '' : 不再留门正文越拍原因(本楼事件, 当前拍正文)) ||
      第二机位正文越拍原因(本楼事件, 当前拍正文) ||
      (自然场景类别(本楼事件) ? '' : 安若妍不必停正文越拍原因(本楼事件, 当前拍正文)) ||
      (自然场景类别(本楼事件) ? '' : 许曼君分居正文越拍原因(本楼事件, 当前拍正文)) ||
      (自然场景类别(本楼事件) ? '' : 许曼君离婚正文越拍原因(本楼事件, 当前拍正文)) ||
      (自然场景类别(本楼事件) ? '' : 许曼君离婚后日常正文越界原因(本楼事件, 当前拍正文)) ||
      回国正文越拍原因(本楼事件, 当前拍正文) ||
      (自然场景类别(本楼事件) ? '' : 双重继承正文越拍原因(本楼事件, 当前拍正文));
    const 首稿重写原因 = 专属节拍错误 || (稽查.状态 === '需重写' ? 稽查.原因 : '');

    if (
      首稿重写原因 &&
      (不再留门票 || 第二机位票 || 安若妍不必停票 || 许曼君分居票 || 许曼君离婚票 || 许曼君离婚后日常票 || 回国票 || 双重继承票 || 焦点妻门牌)
    ) {
      // 首稿已经作废，重写稿只重新生成故事与尺度判定；三条多拍剧情还要严格留在当前拍。
      console.warn(`[人妻公寓·稽查] 首稿需静默重写：${首稿重写原因}`);
      eventEmit('人妻公寓:运行阶段', '正在校准角色反应');
      const 校准令 =
        `${快照}${行动锚}${选项.系统注入?.trim() ? `\n${选项.系统注入}\n` : ''}\n` +
        `【本轮重写硬裁决】上一稿作废。脚本复核发现：${首稿重写原因}。` +
        (不再留门票 || 第二机位票 || 安若妍不必停票 || 许曼君分居票 || 许曼君离婚票 || 许曼君离婚后日常票 || 回国票 || 双重继承票
      ...
      const 重写正文 = 提取可提交正文(重写);
      if (是提供方拒答正文(重写正文)) {
        失败残稿 = 提取正文舞台文本(重写显示原文);
        throw new Error('AI服务在重写时返回了拒答说明，本轮正文、剧情票和资源均未提交；请重试或更换模型线路。');
      }
      const 重写节拍错误 =
        (自然场景类别(本楼事件) ? '' : 不再留门正文越拍原因(本楼事件, 重写正文)) ||
        第二机位正文越拍原因(本楼事件, 重写正文) ||
        (自然场景类别(本楼事件) ? '' : 安若妍不必停正文越拍原因(本楼事件, 重写正文)) ||
        (自然场景类别(本楼事件) ? '' : 许曼君分居正文越拍原因(本楼事件, 重写正文)) ||
        (自然场景类别(本楼事件) ? '' : 许曼君离婚正文越拍原因(本楼事件, 重写正文)) ||
        (自然场景类别(本楼事件) ? '' : 许曼君离婚后日常正文越界原因(本楼事件, 重写正文)) ||
        回国正文越拍原因(本楼事件, 重写正文) ||
        (自然场景类别(本楼事件) ? '' : 双重继承正文越拍原因(本楼事件, 重写正文));
      if (重写稽查.状态 === '通过' && !重写节拍错误) {
        原文 = 重写;
        最终显示原文 = 重写显示原文;
        稽查 = 重写稽查;
      } else if (
        ...
        if (不再留门票) throw new Error(`《不再留门》当前拍未能通过验收：${重写节拍错误 || 重写稽查.原因}。可重试或暂缓。`);
        if (第二机位票) {
          throw new Error(
        ...
        if (许曼君分居票) {
          throw new Error(
            `《分居》当前剧情回合两次未能停在正确节点：${重写节拍错误 || 重写稽查.原因}。本拍保留，可直接重试。`,
          );
        }
        if (许曼君离婚票) {
          throw new Error(
            `《离婚》当前剧情回合两次未能停在正确节点：${重写节拍错误 || 重写稽查.原因}。本拍保留，可直接重试。`,
          );
        }
        if (许曼君离婚后日常票) {
          throw new Error(
            `201新日常当前回合两次未能停在D1/D2边界：${重写节拍错误 || 重写稽查.原因}。检查点保留，可直接重试。`,
          );
        }
        if (回国票) {
          throw new Error(
            `《回国》当前剧情回合两次未能停在正确节点：${重写节拍错误 || 重写稽查.原因}。本拍保留，可直接重试。`,
          );
        }
        if (双重继承票?.动作 === '终幕后收束') {
          // 最后一幕没有后续分支。弱模型连续两次被视频上下文拉回时，使用固定安全正文
          // 完成玩家已经亲手点击的钥匙动作，避免整个结局永久卡死；其他多拍剧情仍失败关闭。
          原文 = 双重继承终幕后安全收束正文();
          最终显示原文 = 原文;
          稽查 = {
            状态: '通过',
            原因: '终幕后二次生成越界，使用确定性安全收束',
            模式: 尺度模式,
            角色: {},
            最高实际等级: 0,
          };
          console.warn('[人妻公寓·双重继承] 终幕后正文连续两次越界，已使用固定安全收束。');
        } else {
          throw new Error(
            `《双重继承》当前剧情回合两次未能停在正确节点：${重写节拍错误 || 重写稽查.原因}。本拍保留，可直接重试。`,
          );
        }
      } else {
        // 第二次仍不可靠：普通场景本地收束为角色拒绝。专属多拍剧情绝不走此兜底推进剧情。
        console.warn(`[人妻公寓·稽查] 重写仍未通过，启用无处罚兜底：${重写稽查.原因}`);
        if (!焦点妻门牌) throw new Error(`正文重写仍未通过：${重写稽查.原因}`);
        原文 = 无处罚拒绝正文(焦点妻门牌);
        最终显示原文 = 原文;
        使用无处罚拒绝兜底 = true;
        稽查 = {
    ...
    const 本轮数据库结果摘要 = 本轮数据库已安装 ? (提取回合事件摘要(原文) ?? 脚本保守回合摘要(行动)) : '';
          ...
          [
            {
              ...
              extra: {
                ...
                [数据库事件元数据键]: {
                  ...
                  时间: 格式化游戏内时间(data),
                  地点: 回合起始场景 || '公寓公共区域',
                  参与者: [...妻在场.map(m => 户静态表[m]?.妻名), ...夫在场.map(m => 户静态表[m]?.夫名)].filter(
    ...
    if (变量执行路径 === '游戏内置') {
      console.info('[人妻公寓] 内置变量解析启动(每轮读取游戏权威 API 配置)');
    ...
    const 自然观察 = await 观察自然场景(
      构造自然场景请求(data, 本楼事件, 行动, 可提交正文, 当前聊天ID(), 本回合消息令牌) ??
        构造安抚观察请求(data, 本轮余波目标, 行动, 可提交正文, 当前聊天ID(), 本回合消息令牌),
      原文,
      () => { try { 确认本轮事务有效(); return true; } catch { return false; } },
    );
    确认本轮事务有效();
    if (自然观察) {
      自然观察.提交上下文 = { 楼层: 生成楼层, 场景: 回合起始场景, 时段: newStat.系统._绝对时段,
        事务ID: newStat.系统._场景剧情事务.id, 请求世代: newStat.系统._场景剧情事务.请求世代,
        妻在场: [...妻在场], 夫在场: [...夫在场], 事件: 本轮事件冻结, 日常提交锚,
        实际尺度: Object.fromEntries(Object.entries(稽查.角色).map(([门牌号, 项]) => [门牌号, 项?.实际 ?? 0])), 资源计费: 本轮资源计费 };
      保存自然对话记录(newStat, 自然观察);
    }
    ...
    const 自然场景待续 = !!自然场景类别(本楼事件) && !本轮自然事件可提交(newStat, 本楼事件);
    if (自然场景待续) 本楼事件 = '';
    ...
    if (!本轮静音会议) {
      ...
      if (本轮余波目标 && newStat.户[本轮余波目标] && !角色线路无关打断已停用(newStat, 本轮余波目标)) {
        推进余波安抚(newStat.户[本轮余波目标]!.妻, {
          正文楼: 生成楼层,
          当前绝对时段,
          成功主线当面楼: Boolean(可提交正文) && 妻在场.includes(本轮余波目标),
          玩家有效回应: 自然观察?.请求.类别 === '冷落安抚' && 自然对话可提交(自然观察),
        });
      }
      if (变量重生成派生票据) {
    ...
    if (变量重生成派生票据) {
      变量重生成派生票据.不可逆身体增长资格 = 现场楼身体增长依赖(newStat, 本轮结算基准, {
        ...
        实际尺度: 资源实际尺度,
        尺度判定: 稽查.角色,
        资源计费: 本轮资源计费 && Boolean(可提交正文) && !使用无处罚拒绝兜底,
      });
    }
    const 资源结算 = 结算成功现场楼(newStat, 本轮结算基准, {
      ...
      资源计费: 本轮资源计费 && Boolean(可提交正文) && !使用无处罚拒绝兜底,
      跳过亲密结算: 使用无处罚拒绝兜底 || 自然场景待续,
      记录攻略风闻调用: (门牌号, 档) => 变量重生成后续风闻调用.push({ 门牌: 门牌号, 档 }),
    });
    if (不再留门事件要求真实开录(本楼事件) && (!资源结算.性爱开始 || 资源结算.性爱结束 || !不再留门真实录制已绑定(newStat))) {
      throw new Error('《不再留门》开录没有创建唯一绑定的202普通场次，本拍未提交。');
    }
    if (
      第二机位事件要求真实开录(本楼事件) &&
      (!资源结算.性爱开始 || 资源结算.性爱结束 || !第二机位真实录制已绑定(newStat))
    ) {
      throw new Error('第二机位开录正文没有建立可继续游玩的真实102亲密场次，本轮不提交。');
    }
    if (
      许曼君分居事件要求真实亲密(本楼事件) &&
      (!资源结算.性爱开始 || 资源结算.性爱结束 || !许曼君分居真实亲密已绑定(newStat))
    ) {
      throw new Error('《分居》亲密开场没有建立精确绑定、可继续游玩的真实201亲密场次，本轮不提交。');
    }
    if (安若妍换掉事件要求H1开场(本楼事件) && (!资源结算.性爱开始 || 资源结算.性爱结束 || !安若妍换掉真实亲密已绑定(newStat))) {
      throw new Error('《换掉》开场未建立可继续的唯一301场次，本轮不提交。');
    }
    if (
      安若妍不必停事件要求H1开场(本楼事件) &&
      (!资源结算.性爱开始 || 资源结算.性爱结束 || !安若妍不必停真实亲密已绑定(newStat))
    ) {
      throw new Error('《不必停》H1没有建立精确绑定、可继续游玩的真实301亲密场次，本轮不提交。');
    }
    if (资源结算.线路CG) {
      const 文件 = 资源结算.线路CG;
      回合提交后任务.push(() => eventEmit('人妻公寓:安若妍不必停CG', { 文件, 标题: 安若妍不必停CG标题(文件) }));
    }
    ...
    确认本轮事务有效();
    await 选项.成功结算?.(newStat);
    // 回合结算之后，丈夫多拍、特殊主线接线或成功回调仍可能产生下一张强剧情。
    // 普通场景下把这些后置新增逐项绑定当前地点，确保下一楼只处理一张票。
    if (!newStat.系统._特殊场景.id && newStat.系统._荣耀洞拍 < 0) {
      绑定新增待发送事件到场景(newStat, 回合结算后待发送基线, 回合起始场景 || null, item => 是入住登场事件(item));
    }
    ...
    const 提交最终整表 = () =>
      排队父亲通话整表写(async () => {
        ...
        确认本轮事务有效();
        if (内置解析变量块已就绪 && !变量解析已降级) {
          console.info('[人妻公寓·变量请求] 变量块已解析、通过写权守护并应用到本轮助手楼');
        }
        捕获保护快照(newStat);
      });
    ...
    const 持久转正本轮临时楼 = async (): Promise<void> => {
      ...
      await 持久写入转正标记(构造转正更新负载(消息表, 命中), setChatMessages, 读取立即持久保存宿主聊天());
      确认本轮事务有效();
      ...
      if (复核命中.length !== 2 || 复核命中.some(项 => 复核表[项.楼层]?.extra?.[临时楼标记键] !== false)) {
        throw new Error('转正失败：宿主持久保存后临时标记复核不通过');
      }
    };
    const 写成功回合聊天状态 = (vars: Record<string, unknown>): Record<string, unknown> => {
      _.set(vars, '_上次回合', {
        行动,
        回合前末楼,
        chat快照: chat快照!,
      ...
      if (本轮即时业务撤回) delete vars[即时业务撤回键];
    ...
    // 正式楼与必要聊天状态已经同成同败；此后的世界书、视图和广播都是可选后处理。
    临时用户已转正 = true;
    ...
    await 同步全部角色阶段世界书(newStat, 本轮事务仍有效);
    确认本轮事务有效();
    ...
    // 带核心聊天结算的动作已经在“成功标记”前同一 updater 中完成，不能再写第二次。
    if (!成功聊天状态已提交) {
      确认本轮事务有效();
      await updateVariablesWith(
        vars => {
          确认本轮事务有效();
          return 写成功回合聊天状态(vars);
        },
        { type: 'chat' },
      );
      确认本轮事务有效();
    ...
    // 先记录成功，不在共享前台租约与回合事务仍持有时解锁客户端。真正完成广播统一放到
    // finally 的所有运行期门释放之后，避免客户端 pull／场景检查重入时误报“内容正在生成”。
    待广播回合完成 = true;
    ...
    // 成功与失败广播都必须放在释放之后：客户端 pull、同步重试和后续场景检查进入时，
    // `回合进行中()` 与共享前台槽必须已经同时为空。
    前台租约.释放();
    if (待安排数据库回合后处理) {
      try {
        // 这里只登记下一任务拍，不同步执行数据库 SQL。先把已提交回合的后处理可靠排入，
        // 再以回合完成作为客户端与手机看到的最后开放信号。
        安排数据库回合后处理({ ...待安排数据库回合后处理 });
      } catch (e) {
        console.error('[人妻公寓] 回合已成功提交，但数据库后处理未能安排:', e);
      }
    }
    if (待广播回合完成 && !回合完成已广播) {
      回合完成已广播 = true;
      try {
        ...
        console.error('[人妻公寓] 回合已成功提交，但完成通知监听器执行失败:', e);
      }
    }
    if (待广播失败原因 !== null) {
...
export interface 即时业务撤回运行端口 {
  备份?: () => Promise<boolean>;
  捕获: (data: SchemaType) => 即时业务撤回准备;
  登记: (
    准备: 即时业务撤回准备,
    场景事务ID: string,
    业务后候选: SchemaType,
  ) => Promise<即时业务撤回记录>;
  确认: (记录: 即时业务撤回记录) => Promise<即时业务撤回记录>;
  收口未落地: (记录: 即时业务撤回记录) => Promise<'未提交' | '已提交'>;
}

/**
 * `即时开演` 被多组独立验收夹具按函数体提取。把新事务能力挂在它本来就依赖的
 * `执行回合` 函数上：真实模块加载时端口必定存在；旧夹具没有端口时维持原测试范围，
 * 不要求几十个无关夹具复制本事务的全部宿主依赖。
 */
export const 即时业务撤回端口: 即时业务撤回运行端口 = {
  备份: 保存当前数据库恢复点,
  捕获: 捕获即时业务撤回准备,
  登记: 登记即时业务撤回准备,
  确认: 确认即时业务撤回已提交,
...
(执行回合 as typeof 执行回合 & { 即时业务撤回?: 即时业务撤回运行端口 }).即时业务撤回 = 即时业务撤回端口;
...
export async function 重掷回合(): Promise<void> {
  ...
  try {
    await 排队MVU操作(async () => {
      ...
      标记数据库时间线将变更(记录.回合前末楼, '重掷回合', { 恢复点: 记录.chat快照?._rqgy数据库恢复锚 });
  ...
  if (恢复后 && 活动剧情) {
    ...
    try {
      ...
      try {
        ...
        if (!重演成功 && 重掷仍有效()) {
          const 失败后 = 读取最近有效();
          if (失败后 && 标记场景剧情待重试(失败后.data, 重试.事务.id, 重试.事务.请求世代)) {
            await 排队MVU操作(async () => {
              确认重掷仍有效();
...
function 准备恢复即时业务撤回(
  ...
  if (上次回合.回合前末楼 !== 目标楼层 || record.锚楼 !== 目标楼层) {
    throw new Error('本回合的即时业务撤回锚点与目标楼层不一致，已在删楼前停止。');
  }
  if (!即时业务撤回聊天快照完整(record, 回合变量键)) {
    throw new Error('本回合的即时业务撤回聊天快照不完整，已在删楼前停止。');
  }
  const 锚数据 = 读取即时业务锚数据(record.锚楼);
  if (!锚数据) throw new Error('本回合的即时业务原锚楼变量无法读取，已在删楼前停止。');
  const 判定 = 核验即时业务撤回记录(record, {
    当前聊天ID: 当前聊天ID(),
    当前锚楼: record.锚楼,
    当前锚消息: SillyTavern.chat?.[record.锚楼],
  ...
  if (!判定.有效) throw new Error(`本回合的即时业务撤回票已经失效：${判定.原因}`);
  return 判定.记录;
}
...
export async function 回档至(楼层: number): Promise<void> {
  ...
  let 已发生物理删楼 = false;
  let 回档已提交 = false;
  // 赋值发生在串行队列回调内；用可变槽避免 TypeScript 把 catch 中的外层 let 错窄化为永远 null。
  const 完整撤回记录槽 = { 当前: null as 即时业务撤回记录 | null };
  try {
    await 排队MVU操作(async () => {
      确认回档仍有效();
      // 撤回快照必须在删楼前读取并验证；完整即时业务票若损坏，必须在数据库栅栏和物理删楼前停止。
      const 上次回合 = 读上次回合();
      const 撤回快照 = 上次回合 && 上次回合.回合前末楼 === 楼层 ? 上次回合.chat快照 : undefined;
      完整撤回记录槽.当前 = 上次回合 && 上次回合.回合前末楼 === 楼层
        ? 准备恢复即时业务撤回(上次回合, 楼层)
        : null;
      const 数据库恢复锚 = 完整撤回记录槽.当前?.业务前聊天._rqgy数据库恢复锚?.值 ?? 撤回快照?._rqgy数据库恢复锚;
      // 接入旧档时，先把仍可读取的已打开裂缝纳入现有镜像；不从已删除历史猜测进度。
      const 回档前数据 = 读取最近有效()?.data;
      if (回档前数据) 捕获保护快照(回档前数据);
      await 等待晋阶镜像写入();
      确认回档仍有效();
      标记数据库时间线将变更(楼层, `回档至${楼层}楼`, { 恢复点: 数据库恢复锚 });
      ...
      const 回档前场景 = (() => {
        if (撤回快照 || 完整撤回记录槽.当前) return null; // 快照路径自行恢复
        try {
      ...
      await 协调已删时间线(
        ...
        完整撤回记录槽.当前
          ...
          : 撤回快照
            ? { 恢复回合变量: 撤回快照, 清上次回合: true, 作废晋阶镜像: true }
            : { 作废晋阶镜像: true },
        回档仍有效,
      );
      确认回档仍有效();
    ...
    if (!回档仍有效()) {
      ...
      if (已发生物理删楼) {
        ...
        const 可恢复完整记录 =
          完整撤回记录槽.当前?.锚楼 === 实际末楼 ? 完整撤回记录槽.当前 : null;
        let 已恢复到存活时间线 = false;
        try {
          await 协调已删时间线(
            实际末楼,
    ...
    if (回档已提交 && 回档仍有效()) {
      try {
        ...
        console.error('[人妻公寓] 回档已经提交，但完成通知监听器执行失败:', 通知错误);
      }
    }
  }
}
...
export async function 开始新游戏(难度: string): Promise<boolean> {
  ...
  const 开局仍属原聊天 = () => 开局时间线世代 === 当前时间线切换世代() && 开局聊天ID === 当前聊天ID();
  const 确认开局仍有效 = () => {
  ...
  try {
    ...
    if (开局已提交 && 开局仍属原聊天()) {
      try {
        ...
        console.error('[人妻公寓] 序章已经提交，但完成通知监听器执行失败:', 通知错误);
      }
    }
  }
}
...
export async function 重开一局(): Promise<void> {
  ...
  try {
    await 排队MVU操作(async () => {
      ...
      // 只剩 0 楼也必须清场；超时保持数据库门关闭，由后台接续官方持久导入。
      await 等待数据库时间线就绪(8000);
      确认重开仍有效();
      ...
      await updateVariablesWith(
        vars => {
          ...
          // 时间守卫按 own-property 判断恢复记录是否存在。新局不能把旧键只置为 null，
          // 否则出厂 stat 已写回后仍会被永久判成“时间操作尚未完成恢复”。
          delete vars[时间推进事务键];
          return vars;
        },
        { type: 'chat' },
      ...
      确认重开仍有效();
      作废全部角色阶段世界书缓存();
      await 同步全部角色阶段世界书(出厂, 重开仍有效, true);
      确认重开仍有效();
    ...
    if (重开核心已提交 && 重开仍有效()) {
      try {
        ...
        console.error('[人妻公寓] 新局核心已经提交，但重载通知监听器执行失败:', 通知错误);
      }
    }
  }
}

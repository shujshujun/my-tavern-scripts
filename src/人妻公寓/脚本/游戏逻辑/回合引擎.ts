...
/**
 * 移动类选项拦截(2026-07-17 用户反馈):移动是纯 UI 操作不产回合,AI 给出"离开/去某房"
 * 类选项被点击后=人没动戏演走了(位置失同步)。提示词已禁,这里兜底过滤。
 * 只拦明确句式(开头移动动词/指名去往某地点),不误伤"去摸她的手"这类就地行动。
 */
const 移动选项句式 = [
  /^(离开|前往|回到|返回|走出|出门|上楼|下楼|下去|上去|去往)/,
  /[去回到往](101|102|201|202|301|302|天台|垃圾房|管理员室|大堂|信箱区|楼梯间|楼道)/,
];

/** 行动选项(AI 隐藏块 → 客户端可点即发的按钮;最多 4 条,闭合缺失就吃到文末,选项不丢) */
function 提取行动选项(原文: string): string[] {
  const m = 原文.match(/<options>([\s\S]*?)(?:<\/options>|$)/);
  if (!m) return [];
  return m[1]
    .split('\n')
    .map(s => s.replace(/^[-·•*\d.、()\s]+/, '').trim())
    .filter(Boolean)
    .filter(条 => !移动选项句式.some(re => re.test(条)))
    .slice(0, 4);
}
...
export async function 执行回合(行动: string): Promise<void> {
  ...
  try {
    ...
    const 稽查 = 输出稽查(原文, 焦点阶段, false /* 正戏楼免检 P5 场景引擎接入 */);
    if (稽查.违规 && 焦点门牌) {
...
const 序章行动选项 = [
  '去信箱区看看这个月的租约单子',
  '带上工具箱去 101,把报修的水管看一眼',
  '去 102 收这个月的房租',
  '先喝了妈盛的汤,在管理员室把钥匙和账本理一遍',
];

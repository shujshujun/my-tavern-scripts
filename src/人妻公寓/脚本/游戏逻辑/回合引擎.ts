...
import { 选择变量解析通道, 读取MVU外置模型配置, 读取MVU解析状态, 读取变量解析通道 } from '../../MVU解析模式';
// (难度表兼供撞见概率系数查表)
...
function 含可改变量命令(可解析文本: string): boolean {
  ...
  const 裸补丁 = 提取末尾裸JSON补丁(可解析文本.replace(/(?:<\/(?:UpdateVariable|json_?patch)\s*>\s*)+$/gi, ''));
  if (裸补丁 !== null) {
    try {
...
export async function 执行回合(
  ...
  try {
    ...
    // 从最终采用的原始模型输出提取事件摘要机器块。必须在此处提取：后面的补模型变量结算/通用兜底
    // 结算会把清洗后的正文（摘要块已被清洗删除）当作基底再拼变量块，错过这里就拿不到摘要了。
    // 漏块/块无效时由基于玩家行动的保守短句兜底，不追加第二次 AI 请求；摘要块随后在所有清洗路径
    ...
    // v0.74 两段式写权：正文模型与 MVU 外置路线都按最终被采纳的稽查逐角色结果扩展
    // 精确范围，不再只限正文路线。本地拒绝兜底的角色:{}、实际 0、无有效正文都不新增
    ...
    let 已补结算 = false;
    if (!本轮静音会议 && !使用MVU外置解析 && 本轮有可写演员 && (是DeepSeek || 是Gemini) && 二次变量结算开启()) {
    ...
    if (!本轮静音会议 && !使用MVU外置解析 && !有可用变量命令(流式兜底变量块 ?? 原文) && 本轮有可写演员) {
    ...
    let 变量块 = 本轮静音会议 || 使用MVU外置解析 || !本轮有可写演员 ? '' : (流式兜底变量块 ?? 取变量块(原文));
          ...
          [
            {
              ...
              extra: { [回合令牌键]: 本回合消息令牌, [回合角色键]: 'assistant', [临时楼标记键]: true },
            },
          ],
          { refresh: 'none' },
        );
    ...
    if (!本轮静音会议 && 使用MVU外置解析 && MVU解析.内置解析 && 本轮有可写演员) {
      ...
      try {
        ...
        if (内置变量块) {
          ...
          // 失败链:已重试一次仍无果→保留正文与旧值,本轮不更新,绝不降级成随AI输出,
    ...
    if (本轮静音会议) {
      ...
      try {
        ...
        await 降级AI变量解析(使用MVU外置解析 ? (内置解析变量块已就绪 ? '内置变量' : 'MVU外置模型') : '正文模型', e);

...
async function 协调已删时间线(
  ...
  if (存活 && (不必停检查点修复 || 换掉检查点修复 || 换掉当前票修复)) {
    确认提交仍有效();
  ...
  作废301换掉阶段世界书同步缓存();
  作废302阶段世界书同步缓存();
  await 同步201离婚阶段世界书(当前真值, 提交校验, true);
  await 同步301换掉阶段世界书(当前真值, 提交校验, true);
  确认提交仍有效();
  await 同步302阶段世界书(当前真值, 提交校验, true);
  确认提交仍有效();
...
function 回合结算(
  ...
  const 提交本轮事件 = () => {
    ...
    if (解析不再留门剧情事件(本楼事件)) {
      ...
      不再留门提交结果 = 提交结果;
      if (提交结果.CG) 提交后任务.push(() => eventEmit('人妻公寓:不再留门CG', { 文件: 提交结果.CG, 实例: newStat.系统._不再留门.实例 }));
    }
...
export async function 执行回合(
  ...
  try {
    ...
    await 同步201离婚阶段世界书(newStat, 本轮事务仍有效);
    await 同步301换掉阶段世界书(newStat, 本轮事务仍有效);
    确认本轮事务有效();
    await 同步302阶段世界书(newStat, 本轮事务仍有效);
    确认本轮事务有效();
...
export async function 重开一局(): Promise<void> {
  ...
  try {
    await 排队MVU操作(async () => {
      ...
      作废301换掉阶段世界书同步缓存();
      作废302阶段世界书同步缓存();
      await 同步201离婚阶段世界书(出厂, 重开仍有效, true);
      await 同步301换掉阶段世界书(出厂, 重开仍有效, true);
      确认重开仍有效();
      await 同步302阶段世界书(出厂, 重开仍有效, true);
      确认重开仍有效();

...
const data = {
  ...
  mes_example: '',
  creator_notes: `人妻公寓 v${版本}。支持继承 v0.80～v0.91 存档；需安装最新版酒馆助手、MVU 与数据库插件，并把数据库模式设为 SQLite；首次进入按引导安装／更新五张游戏记忆表。v0.91.2 修复卡内重掷、回档与失败清理触发内部删楼时，数据库跨界面的通用删除事件覆盖精确时间线原因、造成长期记忆恢复等待异常的问题；真实玩家删楼仍保留原有安全栅栏。v0.91 完成电脑与手机辅助面板统一收起，横屏矮窗口采用正文与操作分栏；加入全员结局后的通关纪念、首次成绩和探索评级，整合角色阶段、数据库恢复、快照与微信上下文修复。v0.90.4 修复启动整表写回尚未排空时第一次操作被误判失败、原生正文在 prompt 阶段失败后前台生成槽不释放的问题；扩大微信近期原文与结构化进展容量，保留中文全角标点并继续在折叠副本上拦截伪装指令；RQ 剧情摘要放宽为 1～3 句、最多 120 字，写长时按完整句末收口，同楼真摘要可覆盖保守兜底；侦探渠道显示统一改称朋友圈，同时保留旧档死路计数键。v0.90.3 为微信恢复镜像增加 localStorage 跨关闭页面与酒馆重启兜底，同时保留 sessionStorage 刷新恢复；三路镜像按聊天身份、时间线、分支指纹和单调修订合并，存储通道失败时互相降级。回档、删楼、swipe、切聊天和重开墓碑仍按时间线隔离，正式 chat 变量仍是权威真值。v0.90.2 为微信写入增加同聊天单调修订与刷新恢复镜像，修复客户端或网页刷新后上下文回退、角色重发初始私聊；RQ 剧情摘要改由本楼正文同轮产出并以 floor_no + event_code 双键精确写入；剧情事件与社交轨迹从数据库自动／手动通用填表中排除，重要朋友圈改由稳定 RQP 事件键脚本写入。v0.90.1 修复强制剧情首次生成、亲密底栏遮挡、监控／荣耀洞数据库空响应及生成等待；v0.90 收口多预设输出、移动端前台决策与完整提示词查看。游戏只采用酒馆显示正则后的纯文字，不复制预设 HTML、CSS、动画或折叠面板；真正未完成的输出显示为未结算残稿，不推进强制剧情、任务、资源、变量或数据库回合。变量解析已自动配置（默认走数据库代发），无需手动调整 MVU 面板；如需切换解析通道，进入游戏【设置 → 变量解析】调整即可。本作不再兼容智脑。开局：0楼进入游戏界面，选难度→接父亲电话进入到任首日；输入使用游戏内输入框。`,
  system_prompt: '',

...
const data = {
  ...
  mes_example: '',
  creator_notes: `人妻公寓 v${版本}。支持继承 v0.80～v0.89 存档；需安装最新版酒馆助手、MVU 与数据库插件，并把数据库模式设为 SQLite；首次进入按引导安装五张游戏记忆表。v0.86 实装夏乔「借种」结局及专属 CG。v0.89 修复 RQ_剧情事件物理表升级与 UPSERT 重绑定，并重构正文输出边界。v0.90 继续收口多预设兼容：识别常见思维链与正文封套，避免睡眠等独立事件已经生成正文却无法推进；微信同楼刷新只容忍宿主改写正文，不会把真正切换分支误当成原分支。移动端监控判断和静音会议散会选择改为前台决策模式，正文保持可读，答案区域独立滚动；普通行动建议改为可收起抽屉。提示词按钮改为高对比显示，并可查看该楼生成时实际发送的完整提示词，包括当时生效的预设、世界书、角色卡、历史和注入内容。游戏只采用酒馆显示正则后的纯文字，不复制预设 HTML、CSS、动画或折叠面板；真正未完成的输出显示为未结算残稿，不会推进强制剧情、任务、资源、变量或数据库回合。变量解析已自动配置（默认走数据库代发），无需手动调整 MVU 面板；如需切换解析通道，进入游戏【设置 → 变量解析】调整即可。本作不再兼容智脑。开局：0楼进入游戏界面，选难度→接父亲电话进入到任首日；输入使用游戏内输入框。`,
  system_prompt: '',

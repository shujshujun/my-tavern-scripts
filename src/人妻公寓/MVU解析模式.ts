...
/**
 * 旧偏好迁移（0.74 前存档可能把通道存成 `正文`，或存在其他非法值）：
 * 统一规范为 `自动`。`正文` 通道已删除，读取时不得让 UI 出现无选中态。
 */
...
/** 0.74 版本化初始化键：换新键让旧用户在 0.74 首次启动也迁移一次（旧键 MVU外置默认已初始化 不再使用）。 */
const MVU外置默认V074初始化键 = 'MVU外置默认V074已初始化';

/**
 * 启动一次性初始化（2026-08-04 用户拍板，0.74 版本化迁移）：本卡默认走"额外模型解析"，
 * 玩家装好 MVU 什么都不用设。只有确认 MVU 当前已是外置、或 写入MVU设置 确认成功后，
 * 才记录 V074 初始化标记；拿不到 extensionSettings 或写入失败时不得提前写标记，
 * 下一次启动必须可重试。成功迁移一次后完全尊重玩家在游戏设置页（或 MVU 面板）的
 * 选择，绝不每次启动强改回去。返回 true 表示本次确实把更新方式写成了外置。
 */
export function 确保MVU默认外置解析(): boolean {
  try {
    if (读界面偏好()[MVU外置默认V074初始化键] === true) return false;
    const 设置 = 读MVU设置();
    if (设置?.更新方式 === '额外模型解析') {
      写界面偏好({ [MVU外置默认V074初始化键]: true });
      return false;
    }
    const 成功 = 写入MVU设置({ 更新方式: '额外模型解析' });
    if (成功) 写界面偏好({ [MVU外置默认V074初始化键]: true });
    return 成功;
  } catch {
    return false;
  }

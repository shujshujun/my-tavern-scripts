...
import { 录像带结局后亲密可用, type 录像带结局角色, type 录像带结局后开场选择 } from '../../../脚本/游戏逻辑/录像带结局后相处';
/**
...
export interface 房间动作事件 {
  ...
  录像带结局后亲密: (门牌号: 录像带结局角色, 选择: 录像带结局后开场选择) => void;
...
export function useRoomActions(options: 房间动作选项) {
  ...
  function 房间动作(id: string | null): 卡动作[] {
    ...
    // H7是必须先做出的线路硬选择：手机与桌面都只保留“继续／暂缓”，
    // 不让管理任务、其他角色线或普通房间动作挤进同一决策面板。
    if (data.value.系统._安若妍不必停.阶段 === '待H7决定') {
      添加安若妍不必停动作(动作, id);
      return 动作;
    }
    if (当前房间.value === id) {
      if ((id === '102' || id === '202') && 录像带结局后亲密可用(data.value, id, id)) 动作.push({
        kicker: id, icon: 'heart', 文案: '和她亲密', 做: () => undefined,
        选项: (['由我开始', '让她开始'] as const).map(选择 => ({ kicker: id, icon: 'heart', 文案: 选择,
          做: () => { if (!发送中.value && 当前房间.value === id && 录像带结局后亲密可用(data.value, id, id)) 事件.录像带结局后亲密(id, 选择); } })),
      });
  ...
  /** 《不必停》在301、大堂／信箱区与管理员室之间推进；不可执行原因直接显示在瓷砖上。 */
  function 添加安若妍不必停动作(动作: 卡动作[], 地点: string): void {
    if (当前房间.value !== 地点) return;
    for (const 候选 of 安若妍不必停地点动作(data.value, 地点)) {
      动作.push({
        kicker: 候选.kicker,
        icon: 候选.icon,
        文案: 候选.文案,

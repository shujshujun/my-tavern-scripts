...
import { 录像带V4录像带可购买, 录像带V4贞操锁可购买数量, 录像带V4已经使用, 录像带V4使用阻断, 录像带沿用旧流程 } from '../../脚本/游戏逻辑/录像带V4状态';
...
function 背景图(房间id: string | null): string {
  ...
  if (房间id) {
    ...
    const 第二机位地址 = 第二机位文件 ? 第二机位图片(第二机位文件) : '';
    if (第二机位地址) return 第二机位地址;
  }
...
const { 房间动作, 当前房间动作, 普通房间动作, 确认已到达动作地点 } = useRoomActions({
  ...
  事件: {
    ...
    录像带结局后亲密: (门牌号, 选择) => void 提交界面事务(() => eventEmit('人妻公寓:录像带结局后亲密', { 门牌: 门牌号, 选择 })),
...
@media (max-width: 540px) and (max-height: 700px) {
  .page { padding-top: 4px; padding-bottom: 4px; }
  .avatar-row { margin-top: 2px; margin-bottom: 2px; }
  .avatar-glyph, .avatar > img { width: 32px; height: 32px; }
  .stage-tools { flex-wrap: nowrap; }
  .stage-tools > :deep(.aux-disclosure), .stage-tools > :deep(.in-room-acts), .stage-tools > :deep(.option-drawer) { flex: 1 1 0; }
  .stage-tools :deep(.aux-handle), .stage-tools :deep(.drawer-handle), .stage-tools :deep(.option-drawer-handle) { gap: 4px; padding: 6px; }
  .stage-tools :deep(.aux-handle .ic), .stage-tools :deep(.drawer-handle .handle-arrow), .stage-tools :deep(.option-drawer-handle .ic), .stage-tools :deep(.aux-handle small) { display: none; }
  .stage-tools :deep(.option-drawer-handle span) { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .stage-tools > .peep-card { flex: 1 1 100%; }
}
</style>

...
test('正文舞台把 HTML/CSS/动画收敛为纯文字，不复制视觉层或折叠状态', () => {
  ...
  assert.equal(
    转为正文舞台纯文本('<thinking>未完成的分析仍供上游协议门判断'),
    '<thinking>未完成的分析仍供上游协议门判断',
    '纯文本转换不越权识别外部预设协议；真实消费者会先走正文输出边界',
  );
  assert.equal(转为正文舞台纯文本('<style>@keyframes 未闭合的视觉代码'), '');
  assert.equal(转为正文舞台纯文本('<p>第一段<br>第二段 &amp; 继续</p>'), '第一段\n第二段 & 继续');
});

test('纯文本转换只移除真实标签，普通比较符与未知尖括号文字必须完整保留', () => {
  assert.equal(转为正文舞台纯文本('温度 < 20°C，心率 > 100。'), '温度 < 20°C，心率 > 100。');
  assert.equal(转为正文舞台纯文本('门牌上写着<请勿打扰>，旁边画了<3。'), '门牌上写着<请勿打扰>，旁边画了<3。');
  assert.equal(转为正文舞台纯文本('x<y>z，但<strong>重点</strong>保留。'), 'x<y>z，但重点保留。');
});

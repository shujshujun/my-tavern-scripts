...
test('夏乔家庭计划第一胎无视疑心门槛走感谢，两拍有效收尾只奖励一次1200金币', () => {
  const data = 建数据(['101']);
  公开孕情(data, '101', 0);
  data.户['101'].妻._怀孕.已曝光 = false;
  Object.assign(data.户['101'].妻._生产, { 家庭计划知情: true, 本胎序号: 1, 状态: '孕期' });
  assert.equal(解析丈夫特殊登门变体(data, '101'), '夏乔感谢', 'v0.85 已冻结的家庭计划知情旧档必须保持原丈夫语义');
  assert.deepEqual(同步丈夫登门排期(data), ['101']);
  assert.equal(安眠药可圆场(data, '101'), false, '夏乔专属变体不能被安眠药改写');
  const 第一拍 = 准备睡前丈夫登门(data, '管理员室');
  assert.match(第一拍.事件, /【丈夫登门:101:夏乔感谢:1】/);
  assert.match(第一拍.事件, /家庭计划.*道谢/);
  data.系统._待发送事件 = '';
  const 第二拍 = 推进丈夫登门(data, 第一拍.事件);
  assert.match(第二拍.事件, /1200金币补偿/);
  data.系统._待发送事件 = '';
  const 原现金 = data.现金;
  const 收尾 = 推进丈夫登门(data, 第二拍.事件);
  assert.equal(data.现金, 原现金 + 1200);
  assert.match(收尾.提示, /1200/);
  assert.equal(推进丈夫登门(data, 第二拍.事件), null);
  assert.equal(data.现金, 原现金 + 1200, '迟到旧正文不得重复奖励');
});

test('夏乔第二胎带酒认命无奖惩，第三胎不再排期；其他角色继续复用通用路线', () => {
  const 二胎 = 建数据(['101']);
  公开孕情(二胎, '101', 0);
  二胎.户['101'].妻._怀孕.已曝光 = false;
  Object.assign(二胎.户['101'].妻._生产, { 家庭计划知情: true, 本胎序号: 2, 状态: '孕期' });
  标记借种结局完成(二胎);
  assert.deepEqual(同步丈夫登门排期(二胎), ['101']);
  const 第一拍 = 准备睡前丈夫登门(二胎, '管理员室');
  assert.match(第一拍.事件, /【丈夫登门:101:夏乔带酒:1】/);
  assert.match(第一拍.事件, /嫉妒、失落.*认命/);
  二胎.系统._待发送事件 = '';
  const 第二拍 = 推进丈夫登门(二胎, 第一拍.事件);
  二胎.系统._待发送事件 = '';
  const 快照 = { 现金: 二胎.现金, 疑心: 二胎.户['101'].夫.疑心值, 信任: 二胎.户['101'].夫.信任值, 风闻: 二胎.风闻 };
  推进丈夫登门(二胎, 第二拍.事件);
  assert.deepEqual(
    { 现金: 二胎.现金, 疑心: 二胎.户['101'].夫.疑心值, 信任: 二胎.户['101'].夫.信任值, 风闻: 二胎.风闻 },
    快照,
  );

  const 三胎 = 建数据(['101']);
  公开孕情(三胎, '101', 100);
  Object.assign(三胎.户['101'].妻._生产, { 家庭计划知情: true, 本胎序号: 3, 状态: '孕期' });
  标记借种结局完成(三胎);
  assert.deepEqual(同步丈夫登门排期(三胎), []);
  assert.equal(丈夫登门药物窗口已开启(三胎), false);
  assert.equal(准备睡前丈夫登门(三胎, '管理员室'), null);

  const 其他 = 建数据(['102']);
  公开孕情(其他, '102', 50);
  assert.deepEqual(同步丈夫登门排期(其他), ['102']);
  assert.match(准备睡前丈夫登门(其他, '管理员室').事件, /:对质:1】/);
});
...
test('姐妹群孕情话题沿用阶段3双成员门，自动多气泡且玩家插话后围绕玩家', () => {
  ...
  assert.match(孕情段, /账\?\.状态 !== '已告知'/u);
  assert.match(孕情段, /账\.已曝光 \|\| 借种孕情可进入姐妹群\(data, 门牌号\)/u);
  assert.match(孕情段, /专场AI气泡下限 = 5/);
  assert.match(孕情段, /输出\$\{专场AI气泡下限\}~\$\{专场AI气泡上限\}行/);
  assert.match(
    孕情段,
    /孕情专场 \|\| 生产专场 \|\| 住院专场 \|\| 借种产后合照专场 \? 专场AI气泡上限 : 4/,
  );
  assert.match(孕情段, /合法消息们\.length < 专场AI气泡下限/, '孕产专场不足五只气泡必须整批拒绝并等待重试');
  assert.match(孕情段, /连续说了/);
  assert.match(孕情段, /程序只提供主题、硬事实、角色数值与记忆/);
  assert.match(孕情段, /逐角色父亲认知与此前确认胎次是硬事实/);
  assert.doesNotMatch(孕情段, /未知者可以震惊/);
  assert.match(孕情段, /本轮孕产角色数据/);

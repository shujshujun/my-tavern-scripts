...
test('失败行动缓存按聊天与时间线锚恢复，清理一个聊天不影响另一个', () => {
  ...
  assert.equal(
    读取回合恢复记录(存储, { 聊天ID: 'chat-a', 锚楼: 12, 锚签名: 'anchor-a' }, 记录时间 + 100)?.行动,
    '敲门询问情况',
  );
  assert.equal(
    读取回合恢复记录(存储, { 聊天ID: 'chat-a', 锚楼: 13, 锚签名: 'anchor-a' }, 记录时间 + 100),
    null,
    '楼层变化后不得把旧行动带到新时间线',
  );
  assert.equal(
    读取回合恢复记录(存储, { 聊天ID: 'chat-a', 锚楼: 12, 锚签名: 'other' }, 记录时间 + 100),
    null,
    '同楼不同分支不得恢复旧行动',
  );

  清除回合恢复记录(存储, 'chat-a');
  assert.equal(读取回合恢复记录(存储, { 聊天ID: 'chat-a', 锚楼: 12, 锚签名: 'anchor-a' }, 记录时间 + 100), null);
  assert.equal(
    读取回合恢复记录(存储, { 聊天ID: 'chat-b', 锚楼: 8, 锚签名: 'anchor-b' }, 记录时间 + 100)?.行动,
    '查看信箱',
  );
});
...
test('过期、空行动与损坏缓存失败关闭，不制造跨局重试入口', () => {
  ...
  assert.equal(读取回合恢复记录(存储, { 聊天ID: 'chat-a', 锚楼: 1, 锚签名: 'anchor' }, 记录时间), null);
  ...
  assert.equal(
    读取回合恢复记录(存储, { 聊天ID: 'chat-a', 锚楼: 1, 锚签名: 'anchor' }, 记录时间 + 24 * 60 * 60 * 1000 + 1),
    null,
    '超过一天的恢复记录必须过期',
  );

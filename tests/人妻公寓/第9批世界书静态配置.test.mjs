...
test('五户夫妻姓名、年龄、婚龄与职业在妻条目、丈夫条目和 stageConfig 三方一致', () => {
  ...
  for (const 项 of 配对) {
    ...
    assert.equal(去初始事实前缀(妻档.婚龄), 项.婚龄);
    assert.match(去初始事实前缀(妻档.丈夫), new RegExp(`^${项.夫}\\(${项.夫龄},${项.职业}`));
    assert.equal(夫档.姓名, 项.夫);
    assert.equal(夫档.年龄, 项.夫龄);
    assert.match(去初始事实前缀(夫档.身份), new RegExp(`^${项.门牌}室(?:的)?男主人[,、]${项.职业}`));
    assert.match(去初始事实前缀(夫档.妻子), new RegExp(`^${项.妻}\\(${项.妻龄},结婚${项.婚龄}\\)$`));
  }

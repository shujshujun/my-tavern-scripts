...
test('节拍引擎真实拥有手机节拍/重入状态/七入口/共享姐妹群拍与水位裁剪，内核与旧水位文件无重复所有者', () => {
  // 新所有者真实声明
  ...
  for (const 入口 of ['荣耀洞专属动态', '母亲共居事件朋友圈', '朋友圈近期流', '主动私聊', '楼务群自动消息', '仅你可见动态', '姐妹群主动拍']) {
...
test('七入口由主循环按原顺序逐一调用，而不是只定义不用', () => {
  const 主循环 = 截源(节拍引擎源码, 'const 上下文: 节拍上下文 =', '// 只有内容成功入库');
  assert.match(
    主循环,
    /for \(const 入口 of \[\s*荣耀洞专属动态,\s*母亲共居事件朋友圈,\s*朋友圈近期流,\s*主动私聊,\s*楼务群自动消息,\s*仅你可见动态,\s*姐妹群主动拍,?\s*\]\)/,
    '七入口必须按原顺序进入拍循环，共居硬反馈位于普通随机朋友圈之前',
  );
...
test('增量基线、节拍差量、已发私聊图差量、余波同事务与成功后 UI 注册表刷新保持', () => {
  ...
  // 只有写入成功且本批含玩家可见内容才刷新 UI；隐藏的跨容器收据仍硬保存，但不得制造红点。
  const 刷新保存 = 截源(收尾, 'const 刷新并保存本拍 =', '// 刷新或上一拍');
  assert.match(刷新保存, /if \(!时间线仍有效\(\)\) return false;/);
  assert.match(刷新保存, /startsWith\(双重继承群聊余波收据消息键前缀\)/);
  assert.match(刷新保存, /写入统计\.实际插入消息数 > 实际隐藏收据数/);
  assert.match(刷新保存, /写入统计\.实际插入朋友圈数 \?\? 0\) > 0/);
  assert.match(
    刷新保存,
    /if \(有新可见内容\) \{\s*请求刷新手机红点\(\);\s*if \(!时间线仍有效\(\)\) return false;\s*请求手机重绘\(\);\s*\}\s*if \(!时间线仍有效\(\)\) return false;\s*await 立即持久保存手机聊天变量\(/,
  );
  assert.match(刷新保存, /return 时间线仍有效\(\);/);
  for (const 统计 of ['必达写入统计', '交接写入统计', '写入统计']) {
    assert.match(收尾, new RegExp(`if \\(!已写(?:必达群)?\\) return;\\s*if \\(!\\(await 刷新并保存本拍\\(${统计}\\)\\)\\) return;`));
  }
...
test('荣耀洞失败仍推进事件去重水位，普通内容失败不推进对应冷却水位', () => {
  ...
  const 去重位 = 荣耀.lastIndexOf('库.节拍[荣耀键] = 钟;');
...
test('姐妹群一拍导出并由交互模块 import，攻略私聊提示只有生成引擎一个定义', () => {
  ...
  assert.match(
    交互源码,
    /await 姐妹群一拍\(data, 库, 楼, 起因, 控制(?:, \{ 玩家刚发言: true[^}]*\})?\)/,
    '手动群接话仍消费共享姐妹群一拍，并可声明玩家刚发言',
  );

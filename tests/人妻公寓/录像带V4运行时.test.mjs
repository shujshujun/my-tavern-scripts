...
test('逐幕正文硬校验拒绝拒答、空泛首幕、串房、界面串词、现场接触和提前完成，但不误判明确否定句', () => {
  const 第一幕 = 读取录像带V4镜头卡('102', 1);
  ...
  assert.ok(第一幕 && 第二幕 && 第四幕 && 第八幕 && 第十五幕 && 第十六幕 && 第十七幕 && 第十八幕 && 第十九幕);

  assert.equal(
    录像带V4正文越拍原因(第一幕, '顾国栋独坐等待，锁具证据窗显示装置仍处于锁定；周小满尚未出现。'),
    '',
  );
  assert.equal(
    录像带V4正文越拍原因(
      第一幕,
      '屏幕里，玩家曾与沈静仪发生性关系。现实中顾国栋独坐等待，锁具证据窗显示装置仍处于锁定；周小满尚未出现。',
    ),
    '',
    '屏幕中的既往玩家行为不能被误判成玩家进入远端当前房间',
  );
  assert.match(录像带V4正文越拍原因(第一幕, '抱歉，我无法继续这项请求。'), /AI服务返回了拒答说明/u);
  assert.match(
    录像带V4正文越拍原因(第一幕, '玩家人在302，却与突然出现的陌生女人发生性关系，屏幕中的当前镜头没有被描写。'),
    /玩家只在302观看/u,
  );
  assert.match(
    录像带V4正文越拍原因(第一幕, '顾国栋坐在房里。'),
    /第1幕没有同时写出/u,
  );

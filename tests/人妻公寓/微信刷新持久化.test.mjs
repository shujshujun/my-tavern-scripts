...
const 时间写入门源码 = readFileSync(new URL('../时间事务写入门.ts', 手机目录), 'utf8');
...
test('聊天变量硬保存：串行等待宿主 saveMetadata，同一聊天才执行，不能只等防抖定时器', async () => {
  ...
  const 片段 = 时间写入门源码 + '\n' + 截源(数据层源码, 'function 手机时间写入允许', '/** 玩家名') + '\n' +
    截源(数据层源码, 'type 手机宿主保存接口', '/**\n * 宿主明确报告删楼');
  let 聊天ID = 'chat-a';
  const 调用 = [];
  const 放行 = [];
  const 直接上下文 = {
...
test('宿主保存没有成功回执时：微信先写刷新恢复日志，并安排静默期补存，不能再把 Promise resolve 当成落盘', () => {
  ...
  assert.match(保存段, /记录当前微信刷新恢复副本\(预期聊天ID, 时间事务校验, 允许保存\)/, '请求宿主保存前必须先用同一授权和时间线校验同步留下刷新恢复副本');
  assert.match(
    保存段,
    /localStorage[\s\S]*sessionStorage|sessionStorage[\s\S]*localStorage/,
    '恢复副本必须同时覆盖关闭页面后的 localStorage 与同 tab 的 sessionStorage',
  );
  assert.match(保存段, /安排手机聊天变量延迟补存\(预期聊天ID, 允许保存\)/, '宿主保存占用或吞错时必须有同时间线的静默期补存');
...
test('时间线事务收口：回档裁剪、删楼协调、重开与失败补偿都会更新镜像并请求宿主保存', () => {
  ...
  assert.match(裁剪段, /推进微信持久修订\(库 as Record<string, unknown>, 手机聊天ID, 目标钟, vars\)/,
    '裁剪后的微信与预约必须在同一个回调冻结关联修订');
  ...
  assert.match(补偿片段, /await 确认当前微信为刷新真值\(预期聊天ID, undefined, 时间事务校验\)/, '精确快照恢复必须带事务授权升修订覆盖旧镜像');
  assert.match(补偿片段, /await 立即持久保存手机聊天变量\(预期聊天ID, 时间事务校验\)/, '补偿后的权威快照必须带事务授权补存宿主');
});
...
test('微信可见事务提交：发送、整批回复、邀约、撤回和父亲通话都在事务边界硬保存', () => {
  ...
  const 落库位 = 发送段.indexOf('已成功落库 = 核对消息已接受();');
  const 重绘位 = 发送段.indexOf('请求手机重绘();', 落库位);
  const 保存位 = 发送段.indexOf('if (已成功落库) 保存已接受手机消息(发送租约);', 落库位);
  assert.ok(落库位 >= 0 && 重绘位 > 落库位 && 保存位 > 重绘位, '玩家气泡先显示，再独立请求本聊天硬保存');
  const 保存段 = 截源(交互源码, 'function 保存已接受手机消息(', 'async function 发消息(');
  assert.match(保存段, /void 立即持久保存手机聊天变量\(发送租约\.聊天ID, undefined, \(\) => 手机发送租约仍有效\(发送租约\)\)/,
    '保存必须实际排队并沿用发送时间线；pending不占住已接受草稿/回复批次');

  assert.match(
    批次段,
    /if \(已生成 && 手机发送租约仍有效\(上下文\.发送租约\)\) \{\s*保存已接受手机消息\(上下文\.发送租约\);\s*\}/,
...
test('自动通知、必达群聊、普通节拍、冷落预警与实时已读都在各自批次边界硬保存一次', () => {
  ...
  assert.match(
    节拍源码,
    /if \(必达结果 === '有新'\)[\s\S]*?if \(!已写必达群\) return;\s*if \(!\(await 刷新并保存本拍\(必达写入统计\)\)\) return;/,
    '孕产必达群批次必须只在整批成功后保存一次',
  );
  assert.match(
    节拍源码,
    /if \(有新 \|\| Object\.keys\(节拍改\)\.length \|\| 已发私聊图改\)[\s\S]*?if \(!已写\) return;\s*if \(!\(await 刷新并保存本拍\(写入统计\)\)\) return;/,
    '普通自动节拍必须只在最终增量成功后保存一次；隐藏跨容器收据不刷新UI但仍持久保存',
  );
  const 本拍保存 = 截源(节拍源码, 'const 刷新并保存本拍 =', '// 刷新或上一拍');
  assert.equal((本拍保存.match(/await 立即持久保存手机聊天变量\(/g) ?? []).length, 1);
  assert.match(本拍保存, /if \(有新可见内容\)[\s\S]*?请求手机重绘\(\);\s*\}\s*if \(!时间线仍有效\(\)\) return false;\s*await 立即持久保存手机聊天变量\(时间线租约\.聊天标识, undefined, 时间线仍有效\);/);
  assert.match(
    楼务通知源码,
    /const 已写 = await 写库增量\([\s\S]*?if \(!已写 \|\| !时间线仍有效\(\)\) return false;[\s\S]*?const 已持久 = await 立即持久保存手机聊天变量\(时间线租约\.聊天标识\);[\s\S]*?if \(!已持久 \|\| !时间线仍有效\(\)\) return false;/,
    '楼务与201确定性通知只有在增量、硬保存和同聊天租约都有效后才确认提交',
  );
  assert.match(
    冷落预警源码,

...
test('App 与时间事务按是否真正产生正文更新幕归属', () => {
  ...
  assert.match(
    时间段,
    /回合完成选项\s*=\s*\{ 更新正文幕: Boolean\(时间反馈草稿\) \}/,
    '时间事务必须先冻结本次是否真实产生正文，不能在退出事务后重新猜测',
  );
  assert.match(
    时间段,
    /_时间推进中 = false;[\s\S]*?eventEmit\('人妻公寓:回合完成', 回合完成选项\)/,
    '小憩/晨跑/健身/普通推进无草稿时不得激活旧正文；只有睡眠草稿在时间锁释放后保留新演出',
  );
  ...
  assert.match(
    撤销段,
    /回合完成选项\s*=\s*\{ 更新正文幕: false \}/,
    '撤销必须冻结“不更新正文幕”的完成选项',
  );
  assert.match(
    撤销段,
    /_时间推进中 = false;[\s\S]*?eventEmit\('人妻公寓:回合完成', 回合完成选项\)/,
    '撤销只刷新状态，并且只能在时间锁释放后通知客户端，不能重新认领其他房间旧正文',
  );
});

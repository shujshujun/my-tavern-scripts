...
test('孕情微信真实已读后的公开与孕期起算不破坏撤销点，其他孕产改写仍会令其失效', () => {
  ...
  微信确认后.户['101'].妻._生产.状态 = '孕期';
  微信确认后.户['101'].妻._生产.确认已读绝对时段 = 42;
  微信确认后.户['101'].妻._生产.预产绝对时段 = 168;
  ...
  const 预产锚被改写 = structuredClone(微信确认后);
  预产锚被改写.户['101'].妻._生产.预产绝对时段 += 1;
  assert.equal(判定(基线.点, 预产锚被改写, 基线.后聊天).有效, false);
});

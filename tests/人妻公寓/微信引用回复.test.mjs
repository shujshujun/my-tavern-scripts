...
test('AI 引用默认只接受同会话真实逐字原文，继续保护群聊与阶段验收', () => {
  const 历史 = [
...
test('显式引用失配与遗漏“引用”关键字都不会吞气泡或重复玩家原话', () => {
  const 历史 = [
    {
      楼: 4,
      时: 4,
  ...
  const 原始回复 = [
    '夏乔:「引用 斩言：之前给你按摩的时候，就感觉你的脚……确实挺好看的……」算你说了句实话！',
    '夏乔:「斩言：我也不敢想...怕误会和自作多情...」傻瓜……',
    '夏乔:行啦，别躲了。',
  ].join('\n');
  const 解析结果 = 解析微信私聊气泡(原始回复, '夏乔', 150, 5)
    .map(回复 =>
      解析微信AI引用前缀(回复, 历史, '101', '斩言', '夏乔', {
        引用失配时保留回复: true,
        识别无标记引用: true,
      }),
    )
  ...
  assert.equal(解析结果[0].引用已降级, true);
  assert.equal(解析结果[1].引用已降级, true);
  assert.equal(解析结果[2].引用已降级, undefined);
  assert.equal(
    解析结果.some(回复 => /斩言|按摩|自作多情/u.test(回复.正文)),
    false,
    '玩家原话不再混入角色气泡',
  );
});

test('遗漏“引用”二字但逐字命中真实消息时兼容生成引用卡，普通标签引文保持原文', () => {
  const 历史 = [{ 楼: 5, 时: 5, 会话: '101', 发: '我', 文: '今晚见吗', 标识: 'player-meet' }];
  const 宽容私聊 = { 引用失配时保留回复: true, 识别无标记引用: true };
  assert.deepEqual(解析微信AI引用前缀('「斩言：今晚见吗」我晚点告诉你。', 历史, '101', '斩言', '夏乔', 宽容私聊), {
    正文: '我晚点告诉你。',
    引用: { 标识: 'player-meet' },
  });
  assert.deepEqual(解析微信AI引用前缀('「提醒：明天带伞」知道了。', 历史, '101', '斩言', '夏乔', 宽容私聊), {
    正文: '「提醒：明天带伞」知道了。',
  });
  assert.deepEqual(解析微信AI引用前缀('「引用 斩言 今晚见吗」我晚点告诉你。', 历史, '101', '斩言', '夏乔', 宽容私聊), {
    正文: '我晚点告诉你。',
    引用已降级: true,
  });
  assert.deepEqual(解析微信AI引用前缀('「斩言：历史里暂时没有这条」仍然回答。', [], '101', '斩言', '夏乔', 宽容私聊), {
    正文: '仍然回答。',
    引用已降级: true,
  });
});

test('引用角色必答在前，概率跟聊角色随后出现；指定角色缺失时整批拒绝而不伪造发言', () => {
  const 约束 = {
    必答角色: '林悦',
    跟聊角色: '夏乔',
    必答画像: '林悦当前阶段3，好感62；性格温柔克制。',
    跟聊画像: '夏乔当前阶段4，好感75；性格直接。',
  };
  assert.deepEqual(确保群聊指定角色发言(['周诗雨:先等等', '夏乔:我也想说', '林悦:我来说'], 约束, 4), [
    '林悦:我来说',
    '夏乔:我也想说',
    '周诗雨:先等等',
  ]);
  assert.deepEqual(确保群聊指定角色发言(['林悦:我来说'], 约束, 4), [], '缺跟聊角色时不冒充她发言');
  assert.deepEqual(确保群聊指定角色发言(['夏乔:我也想说'], 约束, 4), [], '缺必答角色时不冒充她发言');
  assert.deepEqual(确保群聊指定角色发言([], 约束, 4), [], '模型零条合法输出时保持待回复');
});
...
test('接线包含长按引用、瞬态清理、单次请求、低概率上限与最近十条硬冷却', () => {
  ...
  assert.match(交互源码, /引用失配时保留回复:\s*true/);
  assert.match(交互源码, /识别无标记引用:\s*true/);

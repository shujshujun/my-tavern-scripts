...
test('cg3 亲密场景五阶段清单结构：1070张普通CG与378张孕肚CG严格分线', () => {
  ...
  assert.equal(
    ...
    true,
    '孕肚 CG 必须使用门牌/pregnancy/亲密场景五阶段的公开路径',
  );
});

test('进行中只认前后穴进入：口交/乳交/未插入玩具一律算深度前戏', () => {
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密({ 当前行为: '阴道插入', 当前接触部位: '小屄' }) })), 'active');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密({ 当前行为: '肛门插入', 当前接触部位: '屁穴' }) })), 'active');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密({ 当前行为: '玩具', 当前接触部位: '小屄' }) })), 'active');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密({ 当前行为: '玩具', 当前接触部位: '屁穴' }) })), 'active');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密({ 当前行为: '口交', 当前接触部位: '嘴' }) })), 'deep_foreplay');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密({ 当前行为: '乳交', 当前接触部位: '胸部' }) })), 'deep_foreplay');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密({ 当前行为: '玩具', 当前接触部位: '其他' }) })), 'deep_foreplay');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密(), 正文: '她含着你的阴茎吞吐。' })), 'deep_foreplay');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密(), 正文: '她用双乳夹住你的肉棒。' })), 'deep_foreplay');
});

test('正文进行中词只认插入；普通接触与开场词分级判定', () => {
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密(), 正文: '你挺入她的小屄，抽送起来。' })), 'active');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密(), 正文: '她主动亲吻并拥抱了你。' })), 'light_contact');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密(), 正文: '她轻轻揉捏你的手。' })), 'light_contact');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密(), 正文: '她当着你的面脱下裙子。' })), 'intro_no_contact');
});

test('首楼开场：空闲进入进行中的首个成功楼优先显示开场图', () => {
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密({ 本楼开始: true }) })), 'intro_no_contact');
  // 后续楼不再开场
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密({ 当前行为: '阴道插入', 当前接触部位: '小屄' }) })), 'active');
});

test('场外文本与尺度都不是成人场景准入证据：普通脱衣、邀请、接触与暧昧不得出图', () => {
  assert.equal(判定亲密场景CG阶段(信号({ 正文: '她脱下外套换上居家服。', 行为等级: 2 })), null);
  assert.equal(判定亲密场景CG阶段(信号({ 正文: '她笑着示意你过去坐近一点。', 行为等级: 2 })), null);
  assert.equal(判定亲密场景CG阶段(信号({ 正文: '她主动亲吻你的脸颊，又轻轻拥抱了你。', 行为等级: 2 })), null);
  assert.equal(
    选择成人CG(信号({ 正文: '她暧昧地依偎过来，牵住了你的手。', 行为等级: 5 }), new Set()),
    null,
    '即使 AI 虚报高尺度，场景外的肢体接触也不能绕过结构化亲密账本解锁 CG',
  );
  assert.equal(判定亲密场景CG阶段(信号({ 行动: '强行脱她的衣服', 正文: '她明确拒绝并推开你。' })), null);
});

test('亲密场景 CG 五段只读取本轮亲密事实：角色关系阶段编号不得决定 CG 段位', () => {
  const 最低关系阶段 = 0;
  assert.equal(
    判定亲密场景CG阶段(
      信号({
  ...
  assert.equal(
    判定亲密场景CG阶段(
      信号({ 角色阶段: 最低关系阶段, 行为等级: 2, 正文: '她主动亲吻并拥抱了你。', 亲密: 场内亲密() }),
    ),
    'light_contact',
  );
  assert.equal(
...
test('尺度1的日常暧昧不是亲密场景 CG 五段：普通邀请和拆包装不得靠文本兜底出CG', () => {
  assert.equal(
    判定亲密场景CG阶段(
      信号({
        角色阶段: 5,
        行为等级: 1,
        正文: '她解开礼盒包装，笑着邀请你周末一起去看演出。',
      }),
    ),
    null,
  );
...
test('关系晋阶不是 CG 开场：只有明确成人正戏能用事件触发亲密场景 CG 五段', () => {
  assert.equal(
    判定亲密场景CG阶段(
      信号({
        角色阶段: 2,
        行为等级: 1,
        正文: '她第一次承认自己开始在意你的目光，随后把话题岔开。',
        事件: '【转折正戏】她刚进入「动摇」，演出关系越过心坎的过程。',
      }),
    ),
    null,
    'L1→L2的关系转折没有亲密行为，不得强制点亮CG开场',
  );
  assert.equal(
    判定亲密场景CG阶段(
      信号({
        角色阶段: 3,
        行为等级: 3,
        事件: '【转折正戏】她刚进入「越界」。这是她的第一夜——第一次真正背叛婚姻的性。',
      }),
    ),
    'intro_no_contact',
    '明确写明第一夜的成人转折仍可触发亲密开场',
  );
  assert.equal(
    判定亲密场景CG阶段(信号({ 行为等级: 3, 事件: '【阶段性癖票:101:露出】【性癖开幕·露出】对象:夏乔。' })),
    'intro_no_contact',
    '带主题后缀的性癖开幕仍是明确成人事件',
  );
});

test('关系0→1开门剧情与亲密场景 CG 五段隔离：礼物措辞不得触发亲密无接触CG', () => {
  const 开门礼信号 = 信号({
    门牌: '101',
    角色阶段: 1,
    行为等级: 1,
    正文: '夏乔解开装着livehouse双人票的信封，红着眼邀请你有空一起去看现场。',
    行动: '当面把livehouse双人票递给夏乔，停下来等她回应。',
    事件: '【开门正戏】这份礼物只让夏乔重新感到自己还是夏乔。',
  });
  assert.equal(判定亲密场景CG阶段(开门礼信号), null, 'L1开门礼是情绪破冰，不是成人开场');
  assert.equal(选择成人CG(开门礼信号, new Set()), null, '开门礼不得解锁或展示成人CG');
  assert.equal(
    判定亲密场景CG阶段(
      信号({
        门牌: '302',
        角色阶段: 1,
        行为等级: 1,
        正文: '她解开包装，脱下旧外套，把新衣服换上。',
        事件: '【破墙】第一次有人把她当女人而不是当妈看。',
      }),
    ),
    null,
    '母亲关系0→1的破墙赠衣也属于同一非亲密逻辑族',
  );
  assert.equal(
    判定亲密场景CG阶段(信号({ 事件: '【开门正戏】旧事件残片', 亲密: 场内亲密({ 本楼开始: true }) })),
    'intro_no_contact',
    '已经由结构化状态确认开始的亲密场景仍按自身 CG 五阶段流程运行',
  );
});

test('自然否定句不得被亲密场景阶段关键词反向触发，但同句后续真实行为仍可识别', () => {
  const 高尺度场内 = 正文 => 信号({ 正文, 角色阶段: 5, 行为等级: 5, 亲密: 场内亲密() });
  assert.equal(判定亲密场景CG阶段(高尺度场内('她没有真的进行口交，而是摇头拒绝。')), null);
  assert.equal(判定亲密场景CG阶段(高尺度场内('她明确说不接受肛交。')), null);
  assert.equal(判定亲密场景CG阶段(高尺度场内('她不愿继续口交。')), null);
  assert.equal(判定亲密场景CG阶段(高尺度场内('她拒绝进行插入。')), null);
  assert.equal(
    判定亲密场景CG阶段(高尺度场内('她拒绝进行肛交，却低头为你口交。')),
    'deep_foreplay',
    '不能因为前一分句是否定就吞掉后一分句真实发生的行为',
  );
  const 转换行为 = 高尺度场内('她拒绝口交，转而用双乳夹住你的肉棒。');
  assert.equal(判定亲密场景CG阶段(转换行为), 'deep_foreplay');
  assert.equal(判定CG动作(转换行为, 'deep_foreplay'), 'paizuri', '动作池也必须忽略已拒绝的前一行为');
});

test('口交与乳交绝不判定为 active，插入仍是 active', () => {
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密(), 正文: '她低头为你口交。' })), 'deep_foreplay');
  assert.equal(判定亲密场景CG阶段(信号({ 亲密: 场内亲密(), 正文: '她继续承受你的插入。' })), 'active');
  assert.equal(
    判定亲密场景CG阶段(
      信号({
        行动: '把玩具插入她的小屄',
...
test('冻结主焦点本楼独立退出时，CG 改跟仍实际继续的新焦点且不沿用退出者旧图', () => {
  const 旧值 = Schema.parse({ 户: { 101: 创建户节点(3), 102: 创建户节点(3) } });
  旧值.系统._性爱场景 = {
    状态: '进行中',
    场次标识: 'focus-exit-scene',
    开始楼层: 8,
    有效楼数: 2,
    本场等级加成: 0,
    当前接触部位: '小屄',
    ...
    参与者: {
      ...
      102: { 满意度: 0, 满意目标: 4, 偏好命中: [], 等级加成已用: true },
    },
  };
  const 新值 = structuredClone(旧值);
  新值.系统._性爱场景.参与者['101'].已退出 = true;
  新值.系统._性爱场景.参与者['101'].退出方式 = '角色中止';
  新值.系统._性爱场景.主焦点门牌 = '102';

  const 亲密 = 构造CG亲密上下文(旧值, 新值, false);
  assert.equal(亲密.主焦点门牌, '102');
  const 图 = 选择成人CG(信号({ 门牌: 亲密.主焦点门牌, 亲密, 楼层: 12 }), new Set());
  assert.ok(图);
  assert.equal(图.door, '102');
});

test('手动切换主焦点后，下一楼 CG 只从当前对象的图库选择', () => {
  const 旧值 = Schema.parse({ 户: { 101: 创建户节点(3), 102: 创建户节点(3) } });
  旧值.系统._性爱场景 = {
    状态: '进行中',
    场次标识: 'manual-focus-102',
    开始楼层: 8,
    有效楼数: 2,
    当前接触部位: '小屄',
    当前行为: '阴道插入',
    ...
    参与者: {
      ...
      102: { 满意度: 1, 满意目标: 3, 偏好命中: [], 等级加成已用: true },
    },
  };
  const 新值 = structuredClone(旧值);
  const 亲密 = 构造CG亲密上下文(旧值, 新值, false);
...
test('场外普通楼不保留旧CG，委婉对话楼保留当前CG', () => {
  ...
  assert.equal(判定亲密场景CG阶段(信号({ 正文: '她进入厨房，结束后清理并擦拭了桌面。' })), null, '进入、结束后、清理是场外弱词');

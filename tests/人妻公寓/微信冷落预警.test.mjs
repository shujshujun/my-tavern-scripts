...
test('手机侧只通过冷落系统稳定API取档、取唯一方向、验语义与余波资格', () => {
  assert.match(手机源, /from '\.\.\/\.\.\/冷落系统'/);
  assert.match(手机源, /计算妻冷落消息档/);
  assert.match(手机源, /冷落私聊方向/);
  assert.match(手机源, /冷落语义指纹/);
  assert.match(手机源, /余波有冻结效力/);
  assert.match(冷落预警源, /余波有冻结效力/);
});

test('预警统一调用冷落系统构造周期键，只把当前分支活消息当成已发', () => {
  const 预警 = 截到尾(冷落预警源, 'type 冷落指纹');
  assert.match(
    预警,
    /构造冷落预警去重键\(门牌号, 指纹\.成长轮次, 档, 指纹\.冷落周期锚\)/,
  );
  assert.match(预警, /const 兼容旧键 = 指纹\.冷落周期锚 === 上次成长锚/);
  assert.match(预警, /消息\.键 === 键 \|\| \(兼容旧键 !== '' && 消息\.键 === 兼容旧键\)/);
...
test('玩家在冷落期回信只获得当前档方向，并明确需要当面解决', () => {
  ...
  assert.match(回信, /余波有冻结效力\(门牌号,\s*节点\.妻,\s*data\.系统\._母亲入列,\s*data\)/);

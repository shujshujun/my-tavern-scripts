...
test('v7 新局契约移除系统操作中字段，并明确拒绝旧版存档', () => {
  assert.equal(当前MVU数据版本, 7);
  assert.equal(Object.hasOwn(initvar.系统, '_系统操作中'), false);
  assert.throws(() => 验证当前MVU存档版本({ 系统: { _数据版本: 6, _系统操作中: true } }), /不兼容旧存档/);
  const 新局 = Schema.parse(initvar);
  assert.equal(Object.hasOwn(新局.系统, '_系统操作中'), false);
  assert.equal(新局.系统._数据版本, 7);
});

test('存档版本守卫与 Schema 一样拒绝字符串版本号', () => {
  assert.throws(() => 验证当前MVU存档版本({ 系统: { _数据版本: '7' } }), /不兼容旧存档/);
  assert.throws(() => Schema.safeParse({ 系统: { _数据版本: '7' } }), /不兼容旧存档/);
});

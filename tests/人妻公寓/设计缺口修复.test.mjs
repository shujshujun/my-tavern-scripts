...
test('新档不再上架旧录像带，历史实现仍为未完成旧存档续接保留，另外六场保持只读占位', () => {
  const data = 建数据('101', '102', '201', '202', '301', '302');
  data.户['101'].妻.当前阶段 = 3;
  const 场景页 = 取货架(data).find(页 => 页.页签 === '特殊场景');
  const 十一场 = Object.keys(特殊场景表);
  const 保持实装 = ['录像带', '静音会议', '借种', '肉偿账本', '双重继承'];

  assert.ok(场景页);
  assert.equal(十一场.length, 11);
  assert.deepEqual(
    十一场.filter(id => 场景页.商品.some(商品 => 商品.id === id)).sort(),
    十一场.filter(id => id !== '录像带' && id !== '双重继承').sort(),
    '旧录像带已经由双承接共享结局替代，新档不能再从普通特殊剧情货架开启旧单线',
  );
  assert.deepEqual([...特殊场景占位ID列表].sort(), ['大扫除日', '部位评比', '合拍', '家宴', '制服夜巡', '衣柜'].sort());

  const 未完成旧档 = 建数据('102', '202');
  未完成旧档.户['102'].妻.当前阶段 = 5;
  未完成旧档.户['202'].妻.当前阶段 = 5;
  未完成旧档.系统._特殊场景前置.push('录像带:102');
  assert.ok(
    取货架(未完成旧档)
      .find(页 => 页.页签 === '特殊场景')
      ?.商品.some(商品 => 商品.id === '录像带'),
    '带有旧录像带前置的未完成历史存档必须仍可续接',
  );

  未完成旧档.系统._已完成特殊场景.push('录像带');
  assert.equal(
    取货架(未完成旧档)
      .find(页 => 页.页签 === '特殊场景')
      ?.商品.some(商品 => 商品.id === '录像带'),
    false,
    '已经完成旧录像带的历史存档不能因遗留前置再次上架',
  );

...
test('跨聊天重载留下的时间半事务会在挂监听前按持久记录恢复', () => {
  ...
  assert.match(恢复段, /await 脚本写入\(恢复raw, _.cloneDeep\(记录\.推进前数据\)/);
  assert.match(恢复段, /Mvu\.getMvuData\(\{ type: 'message', message_id: -1 \}\)/);
  assert.match(恢复段, /恢复精确聊天快照\(当前变量, 记录\.推进前聊天, 时间推进事务恢复聊天键\)/);
  // s4t8：删除已委托给身份受控写口；仍要求真实删除存在，并且晚于全部恢复。
  assert.match(恢复段, /const 写口 = 创建时间事务写口\(记录,/);
  assert.match(恢复段, /校验归属: 恢复仍有效/);
  assert.match(恢复段, /登记MVU提交校验\(恢复仍有效\)/);
  assert.match(恢复段, /取消恢复校验\(\)/);
  assert.match(恢复段, /记录\.方向 === '撤销'/);
  assert.match(恢复段, /读取时间事务恢复键\(记录\)/);
  断言先后(恢复段, '恢复精确聊天快照(当前变量', 'await 写口.清理()', '聊天恢复后才能清理恢复记录');
  断言先后(恢复段, 'await 立即持久保存手机聊天变量(聊天ID, 写口.校验)', 'await 写口.清理()', '持有本事务授权的反向手机保存完成后才能清理恢复记录');
  assert.match(恢复段, /时间事务校验: 写口\.校验/);
  断言先后(Index源, 'await 恢复中断时间推进()', 'await 恢复微信刷新恢复副本(启动聊天ID)', '时间备份先于普通镜像恢复');
  const 清理段 = 截段(撤销源, 'async 清理(): Promise<void>', 'export interface 时间推进双存储提交参数');
  断言先后(清理段, '校验(vars)', 'delete vars[时间推进事务键]', '清理必须先证明本事务归属');
...
test('撤销恢复完整 MVU 与版本2隔离日志快照，并复用手机双轴裁剪后销毁单槽撤销点', () => {
  ...
  assert.match(撤销段, /回合完成选项 = \{ 更新正文幕: false \}/);
  assert.match(撤销段, /eventEmit\('人妻公寓:回合完成', 回合完成选项\)/);
  断言先后(
    撤销段,
    '_时间推进中 = false;',
    "eventEmit('人妻公寓:回合完成', 回合完成选项)",
    '撤销成功必须在清除时间忙旗后才通知客户端完成',
  );
});
...
test('推进与撤销的每次 stat 改写都在正确边界作废手机租约世代', () => {
  ...
  // s4t8：反向事务复用双存储控制器，不再靠局部布尔旗标组织补偿。
  const 撤销写入 = 截段(撤销段, '写推进状态: async () => {', '写撤销点: async () => {');
  const 撤销补偿 = 截段(撤销段, '恢复推进前状态: async () => {', '恢复推进前聊天: async () => {');
  assert.match(撤销段, /方向: '撤销'/);
  assert.match(撤销段, /准备补偿: 撤销写口\.准备补偿/);
  assert.match(撤销段, /提交完成: 撤销写口\.清理/);
  断言先后(撤销段, 'vars[时间推进事务键] = _.cloneDeep(撤销事务)', 'await 执行时间推进双存储提交', '反向恢复记录必须先于任何双存储修改');
  断言先后(撤销写入, 'await 作废晋阶镜像时间线()', 'await 脚本写入(raw, 推进前数据', '反向提交仍先等待镜像世代作废');

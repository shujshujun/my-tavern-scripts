...
test('内核 re-export 精确收口为 18 个真实外部符号，门面外部路径不变', () => {
  ...
  assert.match(内核源码, /export \{ 按消息重建已发私聊图, 隔离当前手机分支 \} from '\.\/数据层'/);
  // P8:门面导出精确收口——18 个真实消费者符号显式 re-export，10 个无人使用符号不再泄露。
  for (const 符号 of [
  ...
  for (const 符号 of [
    ...
    '编译楼务群公开风闻摘要', '编译管理任务微信通知', '同步管理任务微信',
  ]) {
    assert.doesNotMatch(内核源码, new RegExp(`export[^\\n]{0,120}${符号}`), `内核不得再 re-export:${符号}`);
  }

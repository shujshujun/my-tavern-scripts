...
function 载入手机生成行为(data, 当前场景 = '垃圾房') {
  ...
  const 提示 = [];
  const stubs = {
    '../手机事件进度': load('../../src/人妻公寓/脚本/游戏逻辑/手机事件进度.ts'),
    './母亲称呼规则': load('../../src/人妻公寓/脚本/游戏逻辑/手机/母亲称呼规则.ts'),
...
test('强剧情只读门先于玩家微信发送、邀约与撤回的首次持久写，邀约先占手机生成租约', () => {
  ...
  const 发送受控等待 = 发送入口.indexOf('await 创建受控生成等待(');
  const 发送首次写 = 发送入口.indexOf('写库增量(', 发送受控等待);
  assert.ok(发送受控等待 >= 0 && 发送首次写 > 发送受控等待, '玩家消息写入由受控等待管理超时与取消');
...
test('全量扫描强剧情生产者：新增直接写入点必须进入事务、回合绑定或独立特殊场景白名单', () => {
  ...
  const allowed = [
    'index.ts',
    '不再留门系统.ts', // 只精确删除旧票内两条未经授权尺度句，不生产新票或改写事务身份
    '商店系统.ts',
    '场景剧情事务.ts',
    '安若妍不必停系统.ts',
    '安若妍换掉系统.ts', // 只按本线路标记裁除失效等待票，保留其他剧情队列
    '打断系统.ts',
    '时间推进系统.ts',
    '特殊场景系统.ts',
    '父亲通话写租约.ts',
    '线路待演打断.ts', // 仅删除来源与角色资格均已确认失效的候选，不生产剧情或修改活动事务
    '结算系统.ts',
    '阶段线路系统.ts',
  ].sort();

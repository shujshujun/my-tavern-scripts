...
test('编译近期微信胶囊：超长分段整体跳过，后续分段保留、标签闭合', () => {
  // 分段长度必须真正越过 最大胶囊长度(4000)：16 条 × 300 字符 ≈ 4800。
  const 超长 = Array.from({ length: 16 }, () => ({ 楼: 8, 时: 3, 会话: '101', 发: '对方', 文: '啊'.repeat(300) }));
  const 胶囊 = 编译近期微信胶囊(
  ...
  assert.ok([...胶囊].length <= 4000, '总长不得超过 最大胶囊长度');
});
...
test('三个胶囊调用点共用胶囊预算纯函数', () => {
  ...
  assert.match(进展段, /胶囊预算选择\(开头, 结尾, lines, 4800\)/, '私有微信进展由预算函数限制总长');

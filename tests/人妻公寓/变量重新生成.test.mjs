...
test('变量重生成冻结原回合的微信联系，后来消息不会倒灌进过去，原有联系也不会丢失', () => {
  const 回合前 = 建数据();
  回合前.系统._绝对时段 = 12;
  回合前.户['101'].妻.堕落值 = 70;
  回合前.户['101'].妻._成长账 = { 上次有效成长钟楼: 0, 成长轮次: 0, 已结算冷落日: 0 };
  const 原AI = lodash.cloneDeep(回合前);
  const 原派生后 = lodash.cloneDeep(回合前);
  const 快照 = 提取变量重生成AI结果(原AI, 范围);
  const 基础票据 = {
    当前绝对时段: 12,
    母亲入列: false,
    妻: {
      101: {
        回合前: lodash.cloneDeep(回合前.户['101'].妻),
        派生前: lodash.cloneDeep(回合前.户['101'].妻),
        原派生后: lodash.cloneDeep(原派生后.户['101'].妻),
        独立合法正候选: [],
        余波冻结: false,
      },
  ...
  const 原回合有联系 = 重算变量重生成派生(快照, 快照, {
    ...基础票据,
    微信联系保护: { 101: 12 },
  });
  const 原回合无联系 = 重算变量重生成派生(快照, 快照, 基础票据);

  assert.equal(原回合有联系.妻['101'].堕落值, 70, '原回合已有微信联系时重生成不得补扣冷落');
  assert.equal(原回合无联系.妻['101'].堕落值, 65, '后来才发送的微信不能倒灌到无联系的旧回合');
});
...
test('变量重生成只接受结构完整的持久 stat，不能让 Schema 默认值把空壳伪装成可提交真值', () => {
  const 引擎 = 读('src/人妻公寓/脚本/游戏逻辑/回合引擎.ts');
  const 状态起点 = 引擎.indexOf('export function 读取变量重生成状态');
  const 状态终点 = 引擎.indexOf('function 广播变量重生成状态', 状态起点);
  const 执行起点 = 引擎.indexOf('export async function 重新生成最近回合变量');
  const 执行终点 = 引擎.indexOf('/** 楼层尾部 + 本次行动', 执行起点);
  const 状态段 = 引擎.slice(状态起点, 状态终点);
  const 执行段 = 引擎.slice(执行起点, 执行终点);

  assert.match(状态段, /验证可继续MVU存档结构\(当前stat原始\)/, '按钮可用前必须拒绝未加载或截断的持久快照');
  assert.match(执行段, /验证可继续MVU存档结构\(当前stat原始\)/, '最终三方合并前必须再次拒绝损坏当前真值');
});
...
test('引擎与界面契约：独立模型直算、末楼身份门、成功一次标记和常驻快捷按钮', () => {
  ...
  assert.match(引擎, /版本: 3/, '新版上下文带原回合结算资格，旧版安全降级');

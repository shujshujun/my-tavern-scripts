...
const { 解析报孕生成资料 } = require('../../src/人妻公寓/脚本/游戏逻辑/孕产叙事系统.ts');
const {
  孕态服装立绘图,
  孕态服装白名单,
  角色立绘候选,
...
function 建数据(门牌 = '101', 绝对时段 = 0, 借种已完成 = 门牌 === '101') {
  const 节点 = 创建户节点(0);
  节点.妻.当前阶段 = 3;
  const data = Schema.parse({ 户: { [门牌]: 节点 }, 系统: { _绝对时段: 绝对时段 } });
  data.系统._家庭计划.阶段 = '已完成';
  if (门牌 === '101' && 借种已完成) data.系统._已完成特殊场景.push('借种');
  return data;
}
...
test('夏乔首胎在家庭计划与借种结局完成前不能被普通场景抢先受孕，结局完成后恢复；其他角色不受影响', () => {
  const 未完成阶段 = [
    '未开始',
    '待安装',
    '待投资料',
  ...
  for (const 阶段 of 未完成阶段) {
    const data = 建数据('101', 0, false);
    data.系统._家庭计划.阶段 = 阶段;
    data.户['101'].妻._怀孕.连续未中次数 = 2;
    const 怀孕基线 = structuredClone(data.户['101'].妻._怀孕);
    assert.equal(判定受孕(data, 有效输入({ 场次标识: `locked-${阶段}` })), '不符合');
    assert.deepEqual(data.户['101'].妻._怀孕, 怀孕基线, `${阶段} 不得占用当天机会或累积失败次数`);
  }

  const 仅家庭计划完成 = 建数据('101', 0, false);
  仅家庭计划完成.户['101'].妻._怀孕.连续未中次数 = 2;
  const 普通受孕基线 = structuredClone(仅家庭计划完成.户['101'].妻._怀孕);
  assert.equal(判定受孕(仅家庭计划完成, 有效输入({ 场次标识: 'family-plan-only' })), '不符合');
  assert.deepEqual(仅家庭计划完成.户['101'].妻._怀孕, 普通受孕基线, '商品开放后、结局完成前仍不得抢跑首胎');

  const 借种已完成 = 建数据();
  借种已完成.户['101'].妻._怀孕.连续未中次数 = 2;
  assert.equal(判定受孕(借种已完成, 有效输入({ 场次标识: 'post-borrow-future-pregnancy' })), '已受孕');

  const 其他角色 = 建数据('102', 6);
  其他角色.系统._家庭计划.阶段 = '未开始';
  其他角色.户['102'].妻._怀孕.连续未中次数 = 2;
  assert.equal(
    判定受孕(其他角色, 有效输入({ 场次标识: 'other-role', 收尾对象门牌: '102' })),
    '已受孕',
    '家庭计划只锁夏乔，不能污染其他角色',
  );
});
...
test('亲密收尾冻结当前主焦点为多人场景的唯一受孕对象，切换焦点后对象随之切换', () => {
  for (const 主焦点 of ['101', '102']) {
    const data = Schema.parse({
      户: { 101: 创建户节点(0), 102: 创建户节点(0) },
      系统: { _绝对时段: 主焦点 === '101' ? 0 : 6 },
    });
    data.户['101'].妻.当前阶段 = 4;
    data.户['102'].妻.当前阶段 = 4;
    data.户['101'].妻._怀孕.连续未中次数 = 2;
    ...
    const 另一人 = 主焦点 === '101' ? '102' : '101';
    assert.equal(data.系统._上次性爱结果.收尾对象门牌, 主焦点);
    assert.equal(data.户[主焦点].妻._怀孕.状态, '已受孕');
    assert.equal(data.户[另一人].妻._怀孕.状态, '未孕');
  }
});
...
test('满一周只进入待告知，微信落库确认后才公开立绘与AI认知', () => {
  ...
  assert.equal(应使用怀孕CG(data, '101'), false, '隐藏孕期不得提前展示孕肚CG');

  data.系统._绝对时段 = 42;
  assert.deepEqual(推进怀孕告知(data), ['101']);
  assert.equal(data.户['101'].妻._怀孕.状态, '待告知');
  assert.equal(怀孕已公开(data, '101'), false);
  assert.equal(应使用怀孕CG(data, '101'), false, '消息待送达仍不得展示孕肚CG');
  assert.doesNotMatch(妻状态包('101', data), /怀孕/);
  assert.equal(data.户['101'].妻._怀孕.丈夫登门.状态, '无', '微信落库前丈夫不得获知或排期');

  const 消息 = 编译怀孕微信通知(data, 20, 42);
  assert.equal(消息.length, 0, '结构化报孕资料不能被当成玩家可见台词直接入库');
  const 报孕资料 = 解析报孕生成资料(data.户['101'].妻._怀孕.告知文案);
  assert.equal(报孕资料?.胎次, 1);
  assert.equal(报孕资料?.母亲.当前阶段, 3);

  assert.deepEqual(确认怀孕微信已送达(data, [{ 门牌: '101', 场次标识: 'wrong' }]), []);
  assert.equal(怀孕已公开(data, '101'), false);
  assert.equal(应使用怀孕CG(data, '101'), false, '错误送达凭据不得提前打开孕肚图库');
  ...
  assert.equal(应使用怀孕CG(data, '101'), true, '正确微信送达确认后才打开孕肚图库');
...
test('告知到期冻结阶段、关系、性格、胎次和风险数据，高风闻仍接入既有危机坏结局链', () => {
  const data = 建数据();
  data.风闻 = 75;
  data.户['101'].妻.当前阶段 = 5;
  data.户['101'].妻.好感值 = 85;
  ...
  const 报孕资料 = 解析报孕生成资料(data.户['101'].妻._怀孕.告知文案);
  assert.equal(报孕资料?.胎次, 1);
  assert.equal(报孕资料?.母亲.当前阶段, 5);
  assert.equal(报孕资料?.母亲.好感值, 85);
  assert.equal(报孕资料?.母亲.堕落值, 80);
  assert.equal(typeof 报孕资料?.母亲.性格, 'string');
  assert.equal(报孕资料?.风险.风闻值, 75);
  assert.equal(报孕资料?.风险.达到盯防, true);
  assert.doesNotMatch(data.户['101'].妻._怀孕.告知文案, /认真谈谈|楼里的风声/);
...
test('孕情初见迟到提交必须重新验证公开状态与观察者资格，不能消费出生后的旧票', () => {
  const data = Schema.parse({ 户: { 101: 创建户节点(0), 102: 创建户节点(0) } });
  Object.assign(data.户['101'].妻._怀孕, { 状态: '已告知', 受孕场次标识: 'stale-first-seen' });
  const 计划 = 规划孕情初见(data, ['101', '102'], 40, () => false);
  assert.equal(计划.配对.length, 1);

  data.户['101'].妻._怀孕.状态 = '未孕';
  assert.deepEqual(
    提交孕情初见评价(data, 计划.提示, 40),
    [],
    '生产完成或孕情失效后到达的旧正文回调不得再消费首次看见票',
  );
  assert.deepEqual(data.系统._孕情初见评价楼, {});

  Object.assign(data.户['101'].妻._怀孕, { 状态: '已告知', 受孕场次标识: 'stale-first-seen' });
  data.户['102'].妻._怀孕.状态 = '已受孕';
  assert.deepEqual(
    提交孕情初见评价(data, 计划.提示, 40),
    [],
    '隐藏孕期观察者已经不属于可见反应资格，迟到票同样必须关闭',
  );
});
...
test('主界面与档案卡共用孕态服装解析器，且不新增孕情信息卡', () => {
  ...
  assert.match(
    通知源码,
    /export function 怀孕确认微信已读凭据[\s\S]*?消息已读\(凭据\.门牌, 怀孕微信键/,
    '消息落库不再等于告知完成，只有真实会话已读水位才能生成确认凭据',
  );
  assert.doesNotMatch(通知源码, /通知孕情已送达/);
  assert.match(
    入口源码,
    /eventOn\('人妻公寓:同步孕产微信已读'[\s\S]*?确认怀孕微信已送达\(data, 怀孕确认微信已读凭据\(data\)\)[\s\S]*?脚本写入/,
    '孕期锚点必须由真实已读凭据触发，不能仅凭消息写入数据库就开始计时',
  );

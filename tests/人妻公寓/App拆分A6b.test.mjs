...
test('App 仍持有进入/同步/提示/时间推进/撤销/剧情启动，options 接线完整且八类业务回调保持原事件名与载荷', () => {
  ...
  const 回调对 = [
    [/对饮: id => eventEmit\('人妻公寓:对饮', id\)/, /事件\.对饮\(id\)/],
    [
      /丈夫礼物: \(\{ 门牌, 道具id \}\) => eventEmit\('人妻公寓:丈夫礼物', \{ 门牌, 道具id \}\)/,
      /事件\.丈夫礼物\(\{ 门牌: id, 道具id: 礼物 \}\)/,
    ],
    [/催租: \(\{ 门牌, 选择 \}\) => eventEmit\('人妻公寓:催租', \{ 门牌, 选择 \}\)/, /事件\.催租\(\{ 门牌: id, 选择 \}\)/],
    [/空房偷窃: id => eventEmit\('人妻公寓:空房偷窃', id\)/, /事件\.空房偷窃\(id\)/],
    [/打听: m => eventEmit\('人妻公寓:打听', m\)/, /事件\.打听\(m\)/],
    [/荣耀洞: \(\) => eventEmit\('人妻公寓:荣耀洞'\)/, /事件\.荣耀洞\(\)/],
    [/捡金币: id => eventEmit\('人妻公寓:捡金币', id\)/, /事件\.捡金币\(id\)/],
    [
...
test('破门计时器由 composable 作用域清理；App unmount 不再引用破门计时且其他清理不动', () => {
  ...
  assert.match(卸载, /clearTimeout\(转场计时\)/, '转场 timer 清理保留');
  assert.match(卸载, /clearTimeout\(提示timer\)/, '提示 timer 清理保留');
  assert.match(卸载, /clearTimeout\(性爱结果timer\)/, '性爱结果 timer 清理保留');
});

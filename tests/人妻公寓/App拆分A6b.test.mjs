...
test('全部分支与关键动作契约保持：外部/晨跑/健身/住户/302/管理员室/公共区、训练门、撤销门、现场门、6 连点', () => {
  ...
  assert.match(
    住户段,
    /if \(当前房间\.value === id\) \{[\s\S]{0,100}添加借种动作\(动作, id\);[\s\S]{0,80}添加地点线路动作\(动作, id\);[\s\S]{0,120}if \(id === '201'\)/,
    '住户借种动作与原线路动作仍只在玩家真实位于该户时添加，随后才扩展201条件式留宿',
  );
  assert.match(合成源码, /添加许曼君分居动作\(动作, id\)/, '《分居》地点硬动作进入统一房间动作合成器');
  ...
  assert.match(
    管理员室段,
    /kicker: 'GO',[\s\S]{0,60}icon: 'arrow',[\s\S]{0,60}文案: '走过去'[\s\S]{0,180}添加地点线路动作\(动作, id\)/,
    '管理员室走过去后仍按原顺序追加地点线路',
  );

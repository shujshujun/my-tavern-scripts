...
test('回合只缓存正文请求流，并在正文清洗前完成流式兜底且不采纳正文变量协议', () => {
  ...
  assert.match(
    回合源码,
    /const 流式边界 = 当前预设流式边界\(\);[\s\S]{0,180}正文流式等待思维闭标签 = 流式边界\.等待思维闭标签/,
    '每个回合开始时必须冻结当前预设封套',
  );
  assert.match(
    回合源码,
    /更新有效流式正文\(正文流式原文,\s*文本,\s*候选 =>[\s\S]{0,260}期望正文标签: 正文流式预期标签,[\s\S]{0,100}等待思维闭标签: 正文流式等待思维闭标签,[\s\S]{0,80}流式: true/,
    '回合缓存必须按冻结封套隐藏正文开标签前的裸思维链',
  );
  assert.match(
    回合源码,
  ...
  assert.match(回合源码, /const 最终尺度块 = 取尺度判定块\(最终控制尾段\)/, '流式兜底只补回尺度控制块');
  assert.match(回合源码, /let 变量块 = '';/, '正文模型偶然输出的变量协议不得成为候选变量块');
  assert.match(回合源码, /function 取变量块\(文本: string\)[\s\S]{0,900}提取末尾裸JSON补丁\(/);
  assert.match(
    回合源码,
    /let 内置解析变量块已就绪 = false;[\s\S]{0,1800}if \(结果\.结果 === '成功'\) 内置变量块 = 结果\.变量块/,
    '变量块只能来自当前外置解析请求',
  );
  ...
  assert.match(
    客户端源码,
    /const 流式边界 = 当前预设流式边界\(\)[\s\S]{0,5000}提取正文舞台文本\(文本, \{[\s\S]{0,180}期望正文标签: 流式预期正文标签,[\s\S]{0,100}等待思维闭标签: 流式等待思维闭标签,[\s\S]{0,80}流式: true/,
    '客户端流式显示必须与回合缓存共用同一封套门',
  );
  assert.match(客户端源码, /eventOn\('人妻公寓:生成开始',[\s\S]{0,800}流式段\.value = \[\]/);
  assert.doesNotMatch(客户端源码, /<UpdateVariable\\b|<json_\?patch\\b|<尺度判定/, '客户端不得维护机器协议正则副本');
  assert.match(
    输出边界源码,
    /const 外部正文 = 提取外部预设正文原文\(原文, 选项\);[\s\S]{0,120}const 纯文本 = 转为正文舞台纯文本\(清除游戏机器协议\(外部正文\)\)/,
    '共享输出边界必须先隔离外部思维链，再隔离游戏协议并转纯文本',
  );

...
test('回合只缓存正文请求流，并在正文清洗前完成流式兜底', () => {
  ...
  assert.match(
    回合源码,
    /function 含可改变量命令\(可解析文本: string\)[\s\S]{0,900}提取末尾裸JSON补丁\(/,
    '裸补丁判定收敛在 含可改变量命令 中，且必须真解析 JSON',
  );
  assert.match(
    回合源码,
    /function 有可用变量命令\(文本: string\)[\s\S]{0,300}含可改变量命令\(可解析文本\)/,
    '2026-08-04:空 [] 补丁等于没写，不得算作有效结算而短路兜底二次请求',
  );

...
test('L3 基础性交只能保护内收尾或体外收尾，无保护阴道内射属于 L4', () => {
  assert.match(阶段接受上限[3], /安全套.*体外|体外.*安全套/);
  assert.match(阶段接受上限[3], /无保护.*(?:阴道)?内射.*4级|4级.*无保护.*(?:阴道)?内射/);
  assert.match(阶段接受上限[4], /无保护.*(?:阴道)?内射/);
});

...
const { 生成本期管理任务, 结算管理任务, 结算管理任务逾期 } = require('../../src/人妻公寓/脚本/游戏逻辑/管理任务系统.ts');
const { 合并最新父亲通话 } = require('../../src/人妻公寓/脚本/游戏逻辑/父亲通话写租约.ts');
const {
  使用答谢会降低风闻,
  使用聚餐降低风闻,
  尝试转入风闻投诉,
...
test('302与母亲普通聊天不形成风闻，真实亲密与正式晋阶仍保留母亲额外风险', () => {
  for (const 母亲入列 of [false, true]) {
    const 普通聊天 = 建数据({ 门牌: ['302'] });
    普通聊天.系统._母亲入列 = 母亲入列;

    const result = 登记攻略风闻(普通聊天, '302', '普通');

    assert.equal(result.变动, false, `母亲入列=${母亲入列}`);
    assert.equal(result.实际增加, 0, `母亲入列=${母亲入列}`);
    assert.equal(result.事件ID, '', `母亲入列=${母亲入列}`);
    assert.equal(普通聊天.风闻, 0, `母亲入列=${母亲入列}`);
    assert.equal(普通聊天.系统._风闻账.最近事件.length, 0, `母亲入列=${母亲入列}`);
  }

  const 真实越界 = 建数据({ 门牌: ['302', '101'] });
  真实越界.系统._母亲入列 = true;
  assert.equal(登记攻略风闻(真实越界, '302', '亲密').实际增加, 3, '亲密档保留基础二点和母亲额外一点');
  assert.equal(登记攻略风闻(真实越界, '302', '晋阶').实际增加, 1, '正式晋阶补满同户四点目标');
  assert.equal(登记攻略风闻(真实越界, '101', '普通').实际增加, 0, '全楼同时段四点上限仍生效');
  assert.equal(真实越界.风闻, 4);
});
...
test('正文与父亲分支并发修改同一风闻事件时保留更强事实与已计责终态', () => {
  const 基准 = 建数据({ 门牌: ['101', '102'] });
  登记风闻事件(基准, {
    id: '并发同事件',
    类型: '正式投诉',
    目标增量: 1,
  ...
  标记父亲风闻责任(最新持久态, '并发同事件', '已计责');

  合并最新父亲通话(正文候选, 最新持久态, 基准);

  const 合并事件 = 正文候选.系统._风闻账.最近事件.find(event => event.id === '并发同事件');
  assert.equal(合并事件?.迹象, '硬证据');
  assert.equal(合并事件?.目标增量, 2);
  assert.equal(合并事件?.父亲责任, '已计责');
  assert.equal(风闻事件已即时计责(正文候选, '并发同事件'), true);
  assert.equal(
    正文候选.系统._风闻账.去重票据.filter(ticket => ticket.includes('并发同事件')).length,
    1,
    '同一稳定 ID 最终只能保留一张结构化票据',
  );

  const 原胜任 = 正文候选.胜任度;
  正文候选.系统._绝对时段 = 20;
  const 逾期 = 结算管理任务逾期(正文候选, 20);
  assert.equal(逾期.扣分, 0, '已由父亲计责的同一危机投诉逾期只能转补办，不得再次扣分');
  assert.equal(正文候选.胜任度, 原胜任);
});

...
test('设置页只保留单路线变量解析、游戏内置解析、自定义API与MVU恢复按钮', () => {
  ...
  assert.match(设置弹窗, /变量解析：独立模型（默认）/, '只读独立模型状态说明保持');
  assert.match(设置弹窗, /恢复 MVU 官方外置解析/, '关闭游戏内置解析且MVU未在外置模式时保留恢复按钮');
  assert.match(设置弹窗, /await 写入MVU设置\(\{ 更新方式: '额外模型解析' \}\)/, '恢复按钮只异步写回MVU官方外置路线，不提供正文变量按钮');
...
test('v0.80默认外置初始化用当前版本键，只在已是外置或写入成功后记录，失败可重试', () => {
  ...
  assert.match(状态, /const 成功 = await 写入MVU设置\(\{ 更新方式: '额外模型解析' \}\)/, '等待宿主设置真正保存后再判断');
...
test('v0.80让正文模型始终退出变量处理：官方桥已落地结果不重复应用，未落地标准块与内置块各本地应用一次', () => {
  ...
  assert.match(引擎, /const 变量执行路径 = 选择变量解析执行路径\(MVU解析, 本轮有可写演员, 本轮静音会议\)/);
  assert.match(
    引擎,
    /变量执行路径 === 'MVU官方自动'[\s\S]*?await eventEmit\('人妻公寓:MVU外置模型重试'\)/,
    '只有单路线总闸选中MVU官方自动时才触发官方重试桥',
  );
  ...
  // 官方桥分支：通常已自带“生成变量块 → parse → 写回该楼”；只有规范块最终值尚未落地时才标记本地幂等应用
  ...
  assert.match(官方桥分支, /标准变量块需要本地应用\(变量块, 外置stat\)/, '官方桥逐项核对 replace 最终值');
  assert.match(官方桥分支, /官方外置变量块需本地应用 = Boolean\(/, '只有未落地标准块才取得本地应用资格');

  // 最终本地解析段只允许“内置解析的新块”或“官方桥未落地的标准 replace 块”进入；降级一律禁止。
  assert.match(
    最终本地解析段,
    /if \(\(内置解析变量块已就绪 \|\| 官方外置变量块需本地应用\) && !变量解析已降级\)[\s\S]{0,260}const 候选基准 = _\.cloneDeep\(解析基准\)[\s\S]{0,260}await Mvu\.parseMessage\(可重处理楼层正文, 候选基准\)/,
    '已落地官方结果不重复应用；内置块或官方未落地标准块只本地应用一次；降级不得进入',
  );
  ...
  assert.equal(
    ...
    1,
    '本地 Mvu.parseMessage 只有一个调用点，任一路线至多应用一次',
  );

...
test('关系线路票：查看摄像头 入口只读准备调查演出，只有结果带 线路动作成功 且准备成功才升级隔离', () => {
  const 监控段 = 事件段('查看摄像头');
  const 查看位置 = 监控段.indexOf('const 结果 = 查看摄像头(data');
  const 准备位置 = 监控段.indexOf(
    "准备调查演出事件(data, { 类型: '调查', 门牌: 门牌号, 标识: '查看摄像头' })",
  );
  assert.ok(查看位置 >= 0 && 准备位置 > 查看位置, '必须先在取得 查看摄像头 结果后只读准备调查演出票');
  assert.match(监控段, /已确认有效查看/, '必须用 线路动作成功 判断是否为已确认有效查看');
...
test('写核心：关系线路票严格提交冻结节点，正文已生成但票据失效时抛错让事务回滚', () => {
  ...
  assert.match(写核心段, /上报阶段线路事件\(data, \{/, '普通裂缝拍仍保持现有上报语义');
  const 生成位置 = 监控段.indexOf('await 生成隔离事件草稿({');
...
test('关系线路票线程按门牌、目标阶段与节点区分，普通监控拍沿用 监控:门牌:拍', () => {
  const 监控段 = 事件段('查看摄像头');
  const 票线程 = 监控段.indexOf('监控线路:${门牌号}:${调查票.目标阶段!}:${调查票.节点!}');
  const 普通线程 = 监控段.indexOf('监控:${门牌号}:${(结果 as { 拍: number }).拍}');
  assert.ok(票线程 >= 0, '关系票线程必须按 门牌:目标阶段:节点 区分，不得写成同一拍历史');
  assert.ok(普通线程 >= 0, '普通监控拍仍沿用 监控:门牌:拍');
  assert.ok(票线程 < 普通线程, '关系票线程是三元 true 分支，必须先于普通监控拍出现');
});

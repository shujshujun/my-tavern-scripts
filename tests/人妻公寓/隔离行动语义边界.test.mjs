...
test('确认隔离事务无需隔离：删除同一事务记录但保留本点击合法写入的 _侦探', async () => {
  const 事务 = 创建隔离事件事务记录({
    聊天ID: 'chat-a',
    提交前数据,
    提交前聊天: 捕获精确聊天快照({}, 隔离恢复聊天键),
  ...
  await 确认隔离事务无需隔离({ 事务, 身份, 操作仍有效: () => true });
  assert.equal(Object.hasOwn(当前聊天变量, '_隔离事件事务'), false);
  assert.deepEqual(当前聊天变量._侦探, { 偷窥待选: null, 偷窥上次: { 101: 9 } }, '软计数必须保留并继续线路落地');
});

test('确认隔离事务无需隔离：身份变化/事务键缺失/记录损坏/ID不同均抛错且零写入', async () => {
  const 事务 = 创建隔离事件事务记录({
    聊天ID: 'chat-a',
    提交前数据,
    提交前聊天: 捕获精确聊天快照({}, 隔离恢复聊天键),
  ...
  for (const 案例 of 场景) {
    挂载提交(案例.初始);
    await assert.rejects(
      确认隔离事务无需隔离({ 事务, 身份, 操作仍有效: 案例.操作仍有效 }),
      案例.错误,
    );

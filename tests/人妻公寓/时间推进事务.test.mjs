...
const { 清理旧版电话软事件, 执行时间推进事务 } = require('../../src/人妻公寓/脚本/游戏逻辑/时间推进系统.ts');
...
test('预期时段冲突、坏结局和强事件都失败关闭且深度零变更', () => {
  const 场景们 = [
    ...
    {
      data: Schema.parse({ 系统: { _绝对时段: 4, _待发送事件: '【时间流逝】旧版遗留票' } }),
      请求: { 方式: '推进一时段', 预期绝对时段: 4, 当前消息楼: 20, 当前地点: '管理员室' },
      提示: /强制事件/,
    },
    {
...
test('任何已有强制 pending 都阻断时间事务，单一旧版时间票也不能被覆盖', () => {
...
test('运行时不设电话白名单：残留旧电话软项同真实强剧情一样阻塞时间推进', () => {
  测试聊天变量 = { _场景: { 房间id: '管理员室' }, _粘滞: null, _赴约: null };
  const 旧时间事件 = '【时间流逝】{{user}}沿河畔公园完成了一次晨跑，让时间向前走过一个时段。';
  const data = Schema.parse({
    系统: {
      _绝对时段: 0,
      _待发送事件:
        '【来电回流】{{user}}刚挂了爸的电话，只抽象带过心绪|【母亲裂缝·父亲来电】一条已经在电话里取得的线索',
      _已注入事件: { 楼层: 20, 内容: 旧时间事件 },
    },
  });
  const 原始 = structuredClone(data);
  ...
  assert.equal(结果.成功, false, '迁移在启动时清理旧软项，运行时不做“来电可略过”白名单');
  assert.match(结果.提示, /强制事件/);
  assert.equal(结果.推进时段数, 0);
  assert.deepEqual(data, 原始);
});

test('队列中任何残留事件（含旧电话软项）都按强事件阻断时间推进', () => {
  for (const 待发送事件 of [
    '【来电回流】抽象心绪|【转折正戏】这一幕必须先完成',
    '【来电回流】抽象心绪【转折正戏】缺少分隔符也不能被吞掉',
    '【母亲裂缝·父亲来电】已取得线索【转折正戏】缺少分隔符也不能被吞掉',
  ]) {
...
test('迁移：旧队列只有两个电话软项时清理为空', () => {
  const 原 =
    '【来电回流】{{user}}刚挂了爸的电话，只抽象带过心绪|【母亲裂缝·父亲来电】一条已经在电话里取得的线索';
  assert.equal(清理旧版电话软事件(原), '');
});

test('迁移：软项与强剧情并存时只删软项，强剧情保留且迁移后仍阻塞时间推进', () => {
  const 原 = '【来电回流】抽象心绪|【转折正戏】这一幕必须先完成';
  const 迁移后 = 清理旧版电话软事件(原);
  assert.equal(迁移后, '【转折正戏】这一幕必须先完成');

  const data = Schema.parse({ 系统: { _绝对时段: 0, _待发送事件: 迁移后 } });
  const 原始 = structuredClone(data);
  const 结果 = 推进(data, '推进一时段', 20, '管理员室');
  assert.equal(结果.成功, false, '保留的强剧情仍要阻塞时间推进');
  assert.match(结果.提示, /强制事件/);
  assert.deepEqual(data, 原始);
});

test('迁移：软标签与强标签黏在同一项时整项保留，防止误吞剧情', () => {
  for (const 原 of [
    '【来电回流】抽象心绪【转折正戏】缺少分隔符也不能被吞掉',
    '【母亲裂缝·父亲来电】已取得线索【转折正戏】缺少分隔符也不能被吞掉',
    '前文【来电回流】软标签不在开头的旧项也不清理',
    '【来电回流】同一项含两个软标签【来电回流】也不清理',
  ]) {
    assert.equal(清理旧版电话软事件(原), 原, `不应清理：${原}`);
  }
});

test('迁移：不含目标软项的普通事件（含首尾空格或空分隔）逐字不变，启动层不得误判为已清理', () => {
  for (const 原 of [
    '【转折正戏】这一幕必须先完成',
    '  【转折正戏】带首尾空格的强事件  ',
    '【转折正戏】A||【转折正戏】B',
    ...
    assert.equal(清理旧版电话软事件(原), 原, `无迁移时应逐字返回原字符串：${JSON.stringify(原)}`);
  }
});

test('迁移：混合软项与带空格强事件时只删软项，强事件正文不改写', () => {
  const 原 = '【来电回流】抽象心绪|  【转折正戏】这一幕必须先完成  ';
  assert.equal(清理旧版电话软事件(原), '  【转折正戏】这一幕必须先完成  ', '强事件段的首尾空格不得被 trim');

  const 空分隔 = '【来电回流】抽象心绪||【转折正戏】这一幕必须先完成';
  assert.equal(清理旧版电话软事件(空分隔), '【转折正戏】这一幕必须先完成', '删除软项产生的空分隔可清理');
});

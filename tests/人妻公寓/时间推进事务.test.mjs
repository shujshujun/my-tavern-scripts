...
test('连续锁场的后续拍即使玩家已经离开目标地点也冻结世界时间，普通远处预约仍放行', () => {
  const 连续现场 = Schema.parse({ 系统: { _绝对时段: 8 } });
  追加等待场景剧情(
    连续现场,
    '【场景剧情连续锁场】【场景剧情需回应】【回国提交:B:待收纳:8:2】继续302现场',
    '302',
    '回国 · 收纳私人物件 2/3',
  );
  const 原始 = structuredClone(连续现场);
  const 结果 = 推进(连续现场, '推进一时段', 20, '管理员室');
  assert.equal(结果.成功, false);
  assert.match(结果.提示, /强制事件/);
  assert.deepEqual(连续现场, 原始, '连续现场只能回目标地点续演，不能靠离场让时间戳票过期');

  const 普通预约 = Schema.parse({ 系统: { _绝对时段: 8 } });
  追加等待场景剧情(普通预约, '【普通预约】稍后去302处理', '302', '普通预约');
  assert.equal(推进(普通预约, '推进一时段', 20, '管理员室').成功, true, '不带连续锁场语义的远处预约继续保持原放行策略');
});

...
test('同一期生成确定、不会重掷，且最多两项与同地点不重复', () => {
...
test('所有难度与入住规模下每次普通生成最多一件报修，各档确有报修期样本', () => {
  const 场景 = [
    { 难度: '轻松', 户数: 1 },
    { 难度: '轻松', 户数: 5 },
    { 难度: '标准', 户数: 1 },
  ...
  for (const 配置 of 场景) {
    ...
    for (let 期号 = 0; 期号 < 64; 期号 += 1) {
      ...
      assert.ok(报修数 <= 1, `${配置.难度}/${配置.户数}户第${期号}期报修超过 1 件`);
      if (报修数 === 1) 有过报修 = true;
    }
    assert.equal(实际最大, 1, `${配置.难度}/${配置.户数}户应保留恰好一件报修的确定性期样本`);
    assert.equal(有过报修, true, `${配置.难度}/${配置.户数}户应至少有一期生成报修`);
  }
});

test('本期任务数矩阵：0 户为 0，轻松/标准最多 1 件，严苛 1～2 户为 1、3～5 户为 2', () => {
  for (const 难度 of ['轻松', '标准', '严苛']) {
    for (let 户数 = 0; 户数 <= 5; 户数 += 1) {
      const 期望数 = 户数 === 0 ? 0 : 难度 === '严苛' ? (户数 < 3 ? 1 : 2) : 1;
      const data = 建数据({ 难度, 户数 });
      const tasks = 生成本期管理任务(data, 2, 12);
      assert.equal(tasks.length, 期望数, `${难度}/${户数}户应生成 ${期望数} 件普通任务`);
    }
  }
});

test('已有一件活跃报修时本期不再新增报修，公共与投诉仍可继续选择', () => {
  const data = 建数据({ 难度: '严苛', 户数: 5 });
  ...
  const tasks = 生成本期管理任务(data, 1, 9);

  assert.equal(tasks.filter(task => task.类型 === '报修').length, 0, '已有活跃报修时本期不得新增报修');
  assert.equal(data.系统._管理考核.活跃任务.filter(task => task.类型 === '报修').length, 1);
  assert.ok(tasks.length >= 2, '报修被过滤后应继续从候选池选择公共或普通投诉补足配额');
});

test('序章引导：标准难度 101/102 第 0 期仍只生成一件 101 报修', () => {
  const data = 建数据({ 难度: '标准', 户数: 2 });
  const tasks = 生成本期管理任务(data, 0, 12);

  assert.equal(tasks.length, 1, '标准 2 户第 0 期只应生成一件普通任务');
  assert.equal(tasks[0].类型, '报修', '第 0 期唯一任务应为报修');
  assert.equal(tasks[0].门牌, '101', '开局软引导的报修落在 101 住户');
});
...
test('同模板冷却两个完整考核期，换门牌也不得在第二期提前复用', () => {
  ...
  let 冷却结束后复现 = false;
  for (let 期号 = 3; 期号 <= 30; 期号 += 1) {
    const 当期 = 生成本期管理任务(data, 期号, 期号 * 9);
    if (当期.some(task => firstTemplates.has(task.模板))) 冷却结束后复现 = true;
    data.系统._管理考核.活跃任务 = [];
  }
  assert.equal(
    冷却结束后复现,
    true,
    '冷却结束后后续期可再次出现第 0 期模板，无需恰好 N+3 抽中',
  );
});

test('任一普通报修后两个完整考核期全楼不再生成报修，N+3 冷却到期可恢复', () => {
  const data = 建数据({ 难度: '严苛', 户数: 5 });
  const 报修期 = [];
  for (let 期号 = 0; 期号 < 300; 期号 += 1) {
    const 当期 = 生成本期管理任务(data, 期号, 期号 * 9);
    if (当期.some(task => task.类型 === '报修')) 报修期.push(期号);
    data.系统._管理考核.活跃任务 = [];
  }

  assert.ok(报修期.length >= 8, '足够长的确定性样本中报修应周期性复现');
  for (let i = 1; i < 报修期.length; i += 1) {
    assert.ok(
      报修期[i] - 报修期[i - 1] >= 3,
      `第${报修期[i - 1]}期生成报修后未满两个完整考核期即再次生成`,
    );
  }
  assert.equal(
    报修期.some((期号, i) => 报修期[i + 1] === 期号 + 3),
    true,
    '冷却到期后的 N+3 期应能找到恰好恢复的确定性样本',
  );
});

...
test('v0.90.1 发布统一版本、标签与客户端构建标记', () => {
  ...
  assert.match(组卡源, /const 版本 = '0\.90\.1'/);
  assert.match(组卡源, /const TAG = 'rq0\.90\.1'/);
  assert.match(组卡源, /校验发布版本一致\(\{ 版本, 标签: TAG \}\)/);
  assert.match(组卡源, /校验客户端构建版本\(readFileSync\(客户端构建路径, 'utf8'\), 版本\)/);
});

test('0.90 玩家能检测到 0.90.1；本地与官方同为 0.90.1 时不重复提示', () => {
  const { 比较稳定版本 } = 载入TypeScript('src/人妻公寓/脚本/游戏逻辑/依赖版本.ts');
  const 首次准备源 = 读('src/人妻公寓/界面/客户端/components/首次准备.vue');
  assert.equal(比较稳定版本('0.90', '0.90.1'), '当前较旧');
  assert.equal(比较稳定版本('0.90.1', '0.90.1'), '相同');
...
test('组卡门禁拒绝旧客户端、缺失标记、多版本混包和标签不一致', () => {
  const 当前标记 = 客户端构建版本标记('0.90.1');
  assert.equal(当前标记, 'RQGY_GAME_VERSION:0.90.1');
  assert.equal(校验发布版本一致({ 版本: '0.90.1', 标签: 'rq0.90.1' }), 当前标记);
  assert.equal(校验客户端构建版本(`<script>${当前标记}</script>`, '0.90.1'), 当前标记);

  assert.throws(() => 校验发布版本一致({ 版本: '0.90.1', 标签: 'rq0.90' }), /标签.*不一致/);
  assert.throws(() => 校验客户端构建版本('<script>RQGY_GAME_VERSION:0.90</script>', '0.90.1'), /客户端构建版本不一致/);
  assert.throws(() => 校验客户端构建版本('<script>没有版本标记</script>', '0.90.1'), /未找到构建标记/);
  assert.throws(
    () => 校验客户端构建版本('<script>RQGY_GAME_VERSION:0.90 RQGY_GAME_VERSION:0.90.1</script>', '0.90.1'),
    /客户端构建版本不一致/,
  );
});

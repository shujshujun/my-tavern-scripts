...
function 建样本记录() {
  ...
  return { 业务前, 业务后, 消息, 业务前聊天, 准备, 准备记录, 已提交记录 };
}
...
test('捕获阶段准备票与完整撤回记录都能核对业务前锚，快照篡改仍失败关闭', () => {
  const { 业务前, 准备, 准备记录 } = 建样本记录();
  assert.equal(读取即时业务撤回记录(准备), null, '准备票尚未分配事务 ID，不能冒充持久撤回记录');
  assert.equal(即时业务锚仍是业务前状态(准备, 业务前), true, '正常入口准备票必须通过业务前锚核对');
  assert.equal(即时业务锚仍是业务前状态(准备记录, 业务前), true, '登记后的完整记录仍必须通过同一核对');

  const 已变化锚 = structuredClone(业务前);
  已变化锚.现金 -= 1;
  assert.equal(即时业务锚仍是业务前状态(准备, 已变化锚), false);

  const 被篡改准备 = structuredClone(准备);
  被篡改准备.业务前数据.现金 -= 1;
  assert.equal(即时业务锚仍是业务前状态(被篡改准备, 业务前), false);
});
...
test('生产接线保持捕获→登记→核心落地→确认→正文，普通重掷不退款，完整撤回先验票再删楼', () => {
  ...
  assert.match(
    engine,
    /const 准备 = 创建即时业务撤回准备\([\s\S]*?if \(!即时业务锚仍是业务前状态\(准备, data\)\)/,
    '即时业务入口必须用捕获阶段准备票核对当前数据与持久锚楼',
  );
});

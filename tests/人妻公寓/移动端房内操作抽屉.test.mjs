...
test('App 导入并常驻接线抽屉组件；垃圾选择弹窗留在组件外；普通动作与两类门控保持', () => {
  ...
  // 原回调与门控语义保持；App 统一过滤动作数组与计数，动作执行与录像带门控仍在组件。
  assert.match(
    App源码,
  ...
  assert.match(
    ...
    '普通场景锁期间动作数组归零，最终总钥匙继续走既有精确例外',
  );
  assert.match(
    App源码,
    /const 普通动作可见数 = computed\(\(\) => \(录像带任一中\.value \? 0 : 抽屉普通房间动作\.value\.length\)\)/,
    '普通动作数量与抽屉实际渲染数组来自同一过滤结果',
  );
  assert.match(
    App源码,
    /const 房内操作抑制 = computed\([\s\S]{0,260}发送中\.value[\s\S]{0,80}静音会议正式中\.value[\s\S]{0,100}\(移动端\.value && 键盘打开\.value\)[\s\S]{0,120}\(场景剧情锁定\.value && !双重继承最终收束锁\.value\)/,
    '统一抑制保留发送、静音会议和移动端键盘门，并增加普通场景锁可见性门',
  );
...
test('组件自身：消费 普通房间动作、触发动作收起并直调原回调、垃圾入口只发事件、无反向依赖', () => {
  ...
  assert.match(模板段, /v-if="实际展开"/, '训练与其他动作都服从真实收起状态');
  assert.match(抽屉源码, /collapsible: true/, '桌面接入同一展示状态机');
  assert.match(抽屉源码, /forcedOpen\?: boolean/);
  assert.match(抽屉源码, /const 实际展开 = computed\(\(\) => 状态\.展开\)/);
  assert.match(抽屉源码, /有可见动作 && \(!suppressed \|\| forcedOpen\)/);
  assert.doesNotMatch(抽屉源码, /function 切换\(\): void \{\s*if \(props\.forcedOpen\) return;/);
  assert.match(
    抽屉源码,
...
test('双端把手至少44px；面板向上覆盖并限高滚动', () => {
  const 模板段 = 提取模板(抽屉源码);
  assert.match(模板段, /ref="把手"[\s\S]*?class="drawer-handle"/, '双端都渲染同一把手');
  ...
  assert.match(模板段, /role="region"/, '手机面板 role=region');
  assert.match(模板段, /aria-label="当前房间可执行操作"/, '手机面板有明确 aria-label');
  assert.match(
    抽屉源码,
...
test('双端展开后两列，302只增加同层二选一；窄屏两列不横溢', () => {
  const 模板段 = 提取模板(抽屉源码);
  assert.match(
    模板段,
    /v-if="实际展开"/,

...
test('游戏当前版本固定为 0.86，最新版只接受官方 rq 稳定标签', () => {
  ...
  assert.equal(当前游戏版本, '0.86', '下一次发布必须显式更新游戏本体版本');
  assert.equal(当前游戏版本, 组卡版本, '游戏检测版本必须与角色卡展示版本一致');
  assert.equal(当前游戏版本, 组卡标签, '游戏检测版本必须与角色卡资源标签一致');
  assert.equal(
    选择最新游戏稳定版本({
      versions: ['xdy0.99', 'v9.0', '0.90', 'rq0.83', 'rq0.84', 'rq0.85', 'rq0.86', 'rq0.87-beta', 'rq0.9'],
    }),
    '0.86',
  );
  assert.equal(
  ...
  assert.equal(选择最新游戏稳定版本({ versions: ['xdy0.99', 'v0.86', 'rq0.86-beta'] }), '');
});

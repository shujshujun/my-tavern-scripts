...
test('游戏当前版本固定为 0.90.3，最新版只接受官方 rq 稳定标签', () => {
  ...
  assert.equal(当前游戏版本, '0.90.3', '下一次发布必须显式更新游戏本体版本');
  assert.equal(当前游戏版本, 组卡版本, '游戏检测版本必须与角色卡展示版本一致');
  assert.equal(当前游戏版本, 组卡标签, '游戏检测版本必须与角色卡资源标签一致');
  assert.equal(
    选择最新游戏稳定版本({
      versions: ['xdy0.99', 'v9.0', '0.90.1', 'rq0.83', 'rq0.84', 'rq0.85', 'rq0.86', 'rq0.87', 'rq0.88', 'rq0.89', 'rq0.90', 'rq0.90.1', 'rq0.90.2', 'rq0.90.3', 'rq0.91-beta', 'rq0.9'],
    }),
    '0.90.3',
  );
  assert.equal(
...
test('稳定版本解析严格区分正式版、预发布版与当前高于远端缓存的状态', () => {
  ...
  assert.equal(比较稳定版本('0.90.3', '0.90.3'), '相同', '0.90.3 玩家不得重复提示同版更新');

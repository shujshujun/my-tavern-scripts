...
test('游戏当前版本固定为 0.91，最新版只接受官方 rq 稳定标签', () => {
  ...
  assert.equal(当前游戏版本, '0.91', '下一次发布必须显式更新游戏本体版本');
  assert.equal(当前游戏版本, 组卡版本, '游戏检测版本必须与角色卡展示版本一致');
  assert.equal(当前游戏版本, 组卡标签, '游戏检测版本必须与角色卡资源标签一致');
  assert.equal(
    选择最新游戏稳定版本({
      versions: ['xdy0.99', 'v9.0', '0.90.1', 'rq0.83', 'rq0.84', 'rq0.85', 'rq0.86', 'rq0.87', 'rq0.88', 'rq0.89', 'rq0.90', 'rq0.90.1', 'rq0.90.2', 'rq0.90.3', 'rq0.90.4', 'rq0.91-beta', 'rq0.9'],
    }),
    '0.90.4',
  );
  assert.equal(
...
test('稳定版本解析严格区分正式版、预发布版与当前高于远端缓存的状态', () => {
  ...
  assert.equal(比较稳定版本('0.91', '0.91'), '相同', '0.91 玩家不得重复提示同版更新');
  assert.equal(比较稳定版本('0.91', '0.90.4'), '当前较新', '远端缓存落后时不得把 0.91 玩家误判为需要降级');

...
test('母亲首夜持续场景在真实阶段2期间临时沿用阶段3亲密边界，收尾后不永久晋阶', () => {
  const data = Schema.parse({ 户: { 302: 创建户节点(0) }, 系统: { _母亲入列: true, _母亲首夜第二幕: true } });
  data.户['302'].妻.当前阶段 = 2;
  设置场景('302', ['302']);
  设置亲密(data, ['302'], '302');

  const 快照 = 组公寓快照([{ role: 'user', content: '继续当前亲密互动。' }], data, 10);
  assert.match(快照, /【亲密场景·持续】/);
  assert.match(快照, /【母亲的界线】[^\n]*做,但心虚/);
  assert.doesNotMatch(快照, /【母亲的界线】[^\n]*到吻为止/);
  assert.doesNotMatch(快照, /当前尚未进入成人关系可发生的阶段/);
  assert.equal(data.户['302'].妻.当前阶段, 2, '临时许可只影响演出，不得提前修改真实阶段');
});

test('母亲药物首夜开场楼从第一句起就按阶段3反应与行为边界演出', () => {
  const data = Schema.parse({ 户: { 302: 创建户节点(0) }, 系统: { _母亲入列: true } });
  data.户['302'].妻.当前阶段 = 2;
  设置场景('302', ['302']);
  const 首夜事件 = '【事件在场妻:302】【药物首夜】这一楼发生母亲第一夜。';

  const 快照 = 组公寓快照([{ role: 'user', content: '开始这一夜。' }], data, 10, 首夜事件);
  assert.match(快照, /◆ 母亲\(302室\)\|本人在场\|阶段:越界/);
  assert.match(快照, /【母亲的界线】[^\n]*做,但心虚/);
  assert.doesNotMatch(快照, /【母亲的界线】[^\n]*到吻为止|当前尚未进入成人关系可发生的阶段/);
  assert.equal(data.户['302'].妻.当前阶段, 2, '首夜开场只改变本轮演出许可，不提前提交真实阶段');
});

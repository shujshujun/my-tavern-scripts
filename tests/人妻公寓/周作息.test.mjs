...
test('界面只从系统绝对时段显示周历，妻子基础位置不再读取消息楼种子', () => {
  ...
  assert.doesNotMatch(
    界面源,
    /_时段偏移楼|钟楼号|当前时段\(|当前天数\(|妻位置推算|位置种子|Math\.floor\(Math\.max\(0,[^)]+\) \/ 18\)/,
  );
  ...
  const 基础 = 位置函数.indexOf('妻基础位置');
  assert.ok(特殊 >= 0 && 特殊 < 赴约 && 赴约 < 连续 && 连续 < 基础, '覆盖优先级应为特殊场景→赴约→连续场景→基础作息');
});

test('管理员室只提供小憩和睡眠两个时间按钮，并携带防双击预期水位', () => {

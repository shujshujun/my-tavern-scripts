...
const { 包装场景剧情内容 } = require('../../src/人妻公寓/脚本/游戏逻辑/场景剧情事务.ts');
...
test('主回合与时间撤销的入住世界书同步携带动态时间线校验，旧分支不得迟到改全局条目', () => {
  assert.match(回合源, /await 同步入住世界书条目\(newStat, 本轮事务仍有效\)/);
  const 撤销起 = 索引源.indexOf('function 处理撤销时间推进');
  const 撤销止 = 索引源.indexOf("eventOn('人妻公寓:撤销时间推进'", 撤销起);
  const 撤销段 = 索引源.slice(撤销起, 撤销止);
  assert.ok(撤销起 >= 0 && 撤销止 > 撤销起, '必须能定位时间撤销事务');
  assert.match(撤销段, /await 同步入住世界书条目\(推进前数据, 操作仍有效\)/);
});
...
test('新活动事务优先于同楼旧注入重放，首次强剧情不得冻结到上一张票', () => {
  const 新活动票 = '【场景剧情:v1:new:101:阶段预约】新活动事务正文';
  const 冻结 = 选择本轮事件({
    楼层: 12,
    已注入: { 楼层: 12, 内容: '上一张已经完成的同楼事件' },
    待发送: 新活动票,
    活动事务内容: 新活动票,
    入住场景可用: true,
  });
  assert.deepEqual(冻结, { 楼层: 12, 内容: 新活动票, 来源: '待发送', 待发送快照: 新活动票 });
});
...
test('入住提交只要求冻结事件仍是队首，后续独立等待票不会让当前入住丢失', () => {
  const 入住头 = 包装场景剧情内容(新户事件, { id: 'move-in-201', 标题: '201新住户搬入', 目标场景: '大堂' });
  const 后续票 = 包装场景剧情内容('【门缝那一眼】另一段剧情', {
    id: 'later-story',
    标题: '另一段剧情',
    目标场景: '102',
  });
  const data = Schema.parse({
    户: { 101: 创建户节点(0) },
    系统: { _绝对时段: 3, _待发送事件: `${入住头}|${后续票}` },
  });

  const 演出态 = 构造入住登场演出态(data, 入住头, 12);
  assert.ok(演出态.户['201'], '队首入住仍应获得预演节点');
  assert.deepEqual(提交入住登场(data, 入住头, 12), { 类型: '住户', 门牌: '201' });
  assert.ok(data.户['201']);
  assert.equal(提交入住登场(data, 后续票, 12), null, '队尾事件不能冒充当前入住票');
});
...
test('母亲封顶暗账从预演克隆转入正式提交后仍只刷新一次成长账', () => {
  ...
  // 冷落从阶段2参与：资格外（阶段1）只校准成长账、不记成长轮次，成长账刷新用例必须用合格妻。
  原态.户['302'].妻.当前阶段 = 2;
  原态.户['302'].妻.堕落值 = 100;
...
test('主回合对空确定性剧情拒绝推进，并在失败 finally 恢复 chat 快照', () => {
  assert.match(
    回合源,
    /const 正文判定 = 判定正文提交\(最终显示原文, 提取正文舞台文本, 提取可提交正文, 原文\)[\s\S]{0,360}if \(!正文判定\.可提交\)[\s\S]{0,300}事件必须有正文\(本楼事件\)/,
  );
  ...
  const 恢复快照位置 = 主回合区.indexOf('恢复回合变量快照(chat快照, 本轮时间线仍有效)', 未转正分支位置);

  assert.ok(finally位置 >= 0, '主回合 finally 清理区缺失');
  assert.ok(未转正分支位置 >= 0, 'finally 清理区缺少 未转正分支(if (!临时用户已转正))');
  assert.ok(恢复快照位置 >= 0, '未转正分支后缺少带动态时间线校验的恢复回合变量快照调用');
  assert.ok(finally位置 < 未转正分支位置, '顺序错：未转正分支必须位于 finally 之内');
  assert.ok(未转正分支位置 < 恢复快照位置, '顺序错：恢复回合变量快照必须位于未转正分支之后');
...
test('在场与粘滞使用等待完成的整值替换，空数组能清掉旧丈夫和离场楼戳', () => {
  const 主路径写入 = 回合源.slice(
    ...
    回合源.indexOf('async function 广播生成完成事件'),
  );

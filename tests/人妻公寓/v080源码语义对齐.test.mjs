...
test('v0.80运行时只允许外置变量解析，不保留正文变量直出或二次结算入口', () => {
  ...
  assert.match(回合引擎, /if \(内置解析变量块已就绪 && !变量解析已降级\)/, '本地只解析内置外置模型的新变量块');
});
...
test('当前存档错误明确迁移 v0.80-v0.82，并且维护说明不再声称正文变量路线仍需覆盖', () => {
  assert.match(存档结构, /v0\.83 仅兼容数据版本 7、8 和 9/);
  assert.match(存档结构, /v0\.80\/v0\.81\/v0\.82 存档可直接继承/);

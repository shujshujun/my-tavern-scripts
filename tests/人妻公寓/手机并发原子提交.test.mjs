...
test('发送入口冻结聊天ID与原时间线，切档后不得重捕获新档继续回复', () => {
  ...
  assert.match(发送段, /const 已写 = await 创建受控生成等待\(\s*写库增量\([\s\S]*?\(\) => 允许本次写入 && 手机发送租约仍有效\(发送租约\),/);
  assert.match(发送段, /请求停止: \(\) => \{ 允许本次写入 = false; \}/, '到期或切档必须撤销宿主迟到回调的写入资格');
...
test('跨妻邀约全局串行，接受回复与同批共同计划在同一变量回调 CAS', async () => {
  ...
  // 接受回复与 `_手机邀约计划` 在同一变量回调 CAS；同批同锚可追加多人，
  // 旧 `_赴约` 以及不同批次/不同地点仍失败关闭。
  ...
  assert.match(增量写, /合并手机邀约计划/);
  assert.match(增量写, /合并后邀约计划/);
  assert.match(邀约段, /手机邀约计划可并入/);
});

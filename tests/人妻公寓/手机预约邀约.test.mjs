...
test('客户端地图在目标时段把妻位置显示为约定地点，不返回当前房间；待赴约不提前现身', () => {
  ...
  assert.match(索引源码, /读赴约\(楼, 场\.房间id, data\.系统\._绝对时段, data\)\?\.m === m/);
});
...
test('快照系统接线：读赴约统一返回当前现场视图，待赴约不得返回现场', () => {
  ...
  assert.match(
    快照源码,
    /房间id\?: string,[\s\S]*?绝对时段\?: number,[\s\S]*?data\?: SchemaType,?\s*\): \{ m: 门牌; 地点\?: string; 至楼: number \} \| null/,
  );
  ...
  assert.match(
    快照源码,
    /赴约人数 = 读赴约\(楼层, 房间id, 取绝对时段\(data\), data\) \? 1 : 0/,
    '入住登场只把现场赴约算作持续人物',
  );

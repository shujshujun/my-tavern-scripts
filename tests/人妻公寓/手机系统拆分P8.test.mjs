...
test('P8 交互模块真实拥有全部交互业务与模块级状态，两个注册各一次，内核无重复声明', () => {
  ...
  for (const 符号 of [
    ...
    '排队手机邀约', '读赴约条', '约出来', '约多人出来', '楼务群一拍', '手动群接话', '发消息',
    '执行待回复批次', '执行批次聊天回复',
  ]) {
    assert.ok(交互源码.includes(`function ${符号}`), `交互模块应真实声明:${符号}`);
  }
  // 两个注册各一次
  assert.match(交互源码, /注册手机渲染业务端口\(\{\s*绑定玩家微信撤回,\s*读赴约条,\s*约多人出来,\s*发消息,\s*\}/);
...
test('门面导出精确收口：22 个真实消费者符号保留，无人使用导出不再泄露，无 export * 泄漏', () => {
  const 保留 = [
    ...
    '手机AI生成中', '手机节拍进行中', '同步管理任务微信', '当前微信联系保护表',
  ];
  const 移除 = [
...
test('新交互模块不 import 内核/门面/渲染调度器/红点实现，改经 UI刷新 注册表请求重绘与红点', () => {
  ...
  // v0.80:点"约出来"只进安排邀约页(WeUI 设置页),发送改由安排页经业务端口发起。
  assert.match(渲染chat源码, /名: 'invite'/);
  ...
  assert.doesNotMatch(渲染invite源码, /for \(const 门牌号 of 邀请成员\)/, 'UI 不拥有跨聊天的异步批次循环');
});
...
test('邀约不变量：聊天ID+世代串行、先占输入锁、每 await 后复核、概率公式与同批多人 CAS 保留', () => {
  ...
  // v0.80 可选数据库不阻塞核心：核心重绘/红点刷新先于 fire-and-forget 的 同步社交轨迹。
  const 邀约核心位 = 邀约段.indexOf('请求刷新手机红点()');
  const 邀约数据库位 = 邀约段.indexOf('void 同步社交轨迹(');
  assert.ok(邀约核心位 >= 0 && 邀约数据库位 > 邀约核心位, '核心刷新必须先于可选数据库同步');
  assert.doesNotMatch(邀约段, /await 同步社交轨迹/, '邀约不得 await 同步社交轨迹');
  assert.match(邀约段, /void 同步社交轨迹\(/);
  assert.match(邀约段, /let 率 = \[0\.35, 0\.55, 0\.75, 0\.9, 0\.98\]/);
  assert.match(邀约段, /节点\.妻\.好感值 >= 70/);
  // v0.80:时段名与丈夫在否都按目标绝对时段裁定;防刷随机含目标绝对时段、门牌、地点。
  assert.match(邀约段, /丈夫在楼\(节点, m, 计划\.目标绝对时段\) !== '外出'/);
  assert.match(邀约段, /seededRandom\(计划\.目标绝对时段, m, 计划\.地点, '赴约'\) < 率/);
  assert.match(
    邀约段,
    /当前赴约条\?\.来源 !== '旧即时' && 手机邀约计划可并入\(读手机邀约计划\(\), 计划, 钟, 楼\)/,
  );
  assert.match(邀约段, /邀约计划提交: \{\s*m,\s*创建楼: 楼,\s*创建绝对时段: 钟,\s*目标绝对时段: 计划\.目标绝对时段,\s*地点: 计划\.地点/);
  assert.match(交互源码, /共同受邀者/);
  assert.match(交互源码, /共同邀约只代表见面/);
  ...
  assert.match(邀约段, /事件键: 邀约社交轨迹事件键\(m, 楼, 钟\)/);
});
...
test('群聊不变量：楼务公开边界、姐妹群复用共享一拍、逐气泡延迟与每气泡增量提交保留', () => {
  ...
  assert.doesNotMatch(手动段, /创建手机已读时锚/);
});
...
test('妻回复不变量：楼务有效性、会议只读记忆与知识边界、冷落双语义租约、纪律与逐气泡增量写保留', () => {
  ...
  assert.match(回复段, /\$\{家庭事实\(门牌号, data\)\}/);
  ...
  // v0.80 私聊回复不再无条件写已读：已读由 chat 页前台渲染确认。
  assert.doesNotMatch(回复段, /读到改/);
  assert.doesNotMatch(回复段, /创建手机已读时锚/);
  // v0.80 部分气泡统一收尾：取消/写失败/抛错都走收尾，摘要依据已写数与硬发送时间线。
  assert.match(回复段, /if \(已写回复\.length && 手机发送租约仍有效\(发送租约\)\) \{/);
  assert.match(回复段, /写会场私聊摘要\(门牌号, 已写回复\.join\('\\n'\), 发送租约\.会场摘要租约\)/);
  assert.match(回复段, /排队刷新微信进展摘要\(门牌号\)/);
  // 冷落档与语义指纹只经冷落系统三个稳定 API 取，并共用本批已落库玩家私聊形成的联系保护。
  assert.match(回复段, /const 微信联系保护 = 构造微信联系保护表\(库\.消息, 回复钟\)/);
  assert.match(回复段, /计算妻冷落消息档\(data, 门牌号, 微信联系保护\)/);
  assert.match(回复段, /冷落语义指纹\(data, 门牌号, 微信联系保护\)/);

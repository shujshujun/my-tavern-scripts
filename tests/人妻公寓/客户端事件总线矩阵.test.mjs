...
test('客户端发出的全部静态业务事件都有脚本消费者，客户端监听的全部事件都有生产者', () => {
  ...
  assert.equal(客户端发出.length, 72, '新增或删除客户端事件时必须重新审查生产—消费矩阵');
  ...
  assert.equal(客户端监听.length, 32, '新增或删除客户端监听时必须重新审查生命周期所有者');
  ...
  assert.ok(客户端监听.includes('人妻公寓:录像带双承接CG'), '录像带双承接CG必须作为显式客户端监听纳入生命周期审计');

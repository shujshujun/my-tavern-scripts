...
test('客户端发出的全部静态业务事件都有脚本消费者，客户端监听的全部事件都有生产者', () => {
  ...
  assert.equal(客户端发出.length, 78, '新增或删除客户端事件时必须重新审查生产—消费矩阵');
  assert.ok(客户端发出.includes('人妻公寓:重新识别已保存对话'), '原回复补取通过受控脚本入口提交');
  assert.ok(客户端发出.includes('人妻公寓:使用承接剧情票'), '承接票开始由脚本安全操作校验与提交');

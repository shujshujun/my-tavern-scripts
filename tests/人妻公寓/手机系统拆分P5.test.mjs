...
test('内核正确 re-export 既有公共 API（P8 精确收口），旧门面仍是纯 re-export', () => {
  ...
  assert.match(交互源码, /import \{[\s\S]*获取会议会话禁用原因,[\s\S]*获取静音会议手机状态,[\s\S]*取静音会议正文记忆,[\s\S]*创建会场私聊摘要租约,[\s\S]*写会场私聊摘要,[\s\S]*\} from '\.\.\/静音会议旁路';/);
  ...
  assert.match(节拍引擎源码, /import \{[\s\S]*微信好友,[\s\S]*编译楼务群公开风闻摘要,[\s\S]*同步管理任务微信,?[\s\S]*\} from '\.\/通知桥';/);
  assert.ok(
    (节拍引擎源码.match(/微信好友\(/g) ?? []).length >= 1,
    '节拍引擎应在 import 之外调用 微信好友()',
  );
  assert.match(冷落预警源码, /import \{ 微信好友 \} from '\.\/通知桥';/);
  assert.ok(
    (冷落预警源码.match(/微信好友\(/g) ?? []).length >= 1,
    '冷落预警应在 import 之外调用 微信好友()',
  );
  assert.match(渲染chats源码, /import \{ 微信好友 \} from '\.\.\/\.\.\/通知桥';/);
  assert.ok(
    (渲染chats源码.match(/微信好友\(/g) ?? []).length >= 1,
    '渲染chats 应在 import 之外调用 微信好友()',
  );
  ...
  assert.doesNotMatch(内核源码, /export \{ 微信好友, 编译楼务群公开风闻摘要, 编译管理任务微信通知, 同步管理任务微信 \}/);
  assert.doesNotMatch(内核源码, /export[^\n]{0,120}获取会议会话禁用原因/);
  assert.doesNotMatch(内核源码, /export[^\n]{0,120}取静音会议正文记忆/);
  assert.match(内核源码, /export \{[\s\S]*设置静音会议手机生成中,[\s\S]*获取静音会议手机状态,[\s\S]*取会场私聊摘要提示[\s\S]*\} from '\.\/静音会议旁路';/);
...
test('通知键 `楼务:${任务.id}`、公开风闻脱敏和好友规则仍在', () => {
  ...
  // 同步成功写入后经 UI 刷新注册表请求红点与重绘，不直调内核实现。
  assert.match(通知桥源码, /if \(已写\) \{\s*请求刷新手机红点\(\);\s*请求手机重绘\(\);\s*\}/);
  assert.doesNotMatch(通知桥源码, /刷新红点\(\)|渲染\(/);
});

test('冷落语义双重租约、重入补跑、一次一户和 `冷落:` 幂等键仍在', () => {
  assert.match(冷落预警源码, /键 = `冷落:\$\{门牌号\}:\$\{指纹\.成长轮次\}:\$\{档\}`/);
  assert.match(冷落预警源码, /已发 = 库\.消息\.some\(消息 => 消息\.键 === 键 && 消息\.楼 <= 楼\)/);
...
test('父亲回复/收尾幂等键、MVU→父亲双锁顺序、完成消息键与社交轨迹事件键仍在', () => {
  assert.match(父亲通话源码, /const 生成键 = `\$\{预期聊天ID\}\|\$\{回复请求时间线世代\}\|\$\{通话\.标识\}:\$\{序号\}`/);
...
test('父亲模块通过端口导航且业务模块刷新均走 UI 注册表', () => {
  ...
  for (const [名称, 源码] of [
    ...
    assert.match(源码, /import \{[^}]*请求刷新手机红点[^}]*\} from '\.\.?\/UI刷新'/, `${名称}应经 UI 刷新注册表请求红点`);
  }
});

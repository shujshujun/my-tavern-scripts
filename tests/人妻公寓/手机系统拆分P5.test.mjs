...
test('通知键 `楼务:${任务.id}`、公开风闻脱敏和好友规则仍在', () => {
  ...
  // 消息落库刷新 UI 并在同一批次硬保存；孕情仍必须等会话页真实已读水位覆盖后才返回确认凭据。
  assert.match(
    通知桥源码,
    /const 已写 = await 写库增量\([\s\S]*?if \(!已写 \|\| !时间线仍有效\(\)\) return false;[\s\S]*?请求刷新手机红点\(\);[\s\S]*?请求手机重绘\(\);/,
    '成功落库且租约仍有效后才刷新红点与手机界面',
  );
  assert.match(
    通知桥源码,
    /const 已持久 = await 立即持久保存手机聊天变量\(时间线租约\.聊天标识\);[\s\S]*?if \(!已持久 \|\| !时间线仍有效\(\)\) return false;/,
    '当前聊天批次硬保存必须真实成功且租约仍有效',
  );
...
test('冷落语义双重租约、重入补跑、一次一户和带联系周期的 `冷落:` 幂等键仍在', () => {
  ...
  // 成功写入后经 UI 刷新注册表请求红点与重绘，并在批次边界硬保存，不直调内核实现。
  assert.match(
    冷落预警源码,
    /if \(!已写\) return;\s*请求刷新手机红点\(\);\s*请求手机重绘\(\);\s*await 立即持久保存手机聊天变量\(时间线租约\.聊天标识\);/,
  );
...
test('父亲回复/收尾幂等键、MVU→父亲双锁顺序、完成消息键与社交轨迹事件键仍在', () => {
  ...
  assert.match(父亲通话源码, /事件键: `RQP-来电-\$\{通话\.标识\}`/);
...
test('三个共享提示纪律只有一个定义，文本和调用点未弱化', () => {
  ...
  assert.match(生成引擎源码, /玩家名叫"\$\{玩家名\(\)\}"/);
  assert.match(生成引擎源码, /严禁臆造别的姓氏或称呼/);
  ...
  assert.match(生成引擎源码, /普通住户称呼他可用姓名或"管理员"，关系近了可用由这个名字自然衍生的昵称/);
  assert.match(生成引擎源码, /\$\{母亲称呼纪律\}/);

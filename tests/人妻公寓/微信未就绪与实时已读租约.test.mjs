...
test('数据层实时已读入口：冻结时间线租约，租约失败不写，复核函数传到 写库增量', async () => {
  let 聊天ID = 'chat-a';
  const 楼 = 0;
  let 时段 = 4;
  ...
  // 复核函数真实复用同一租约：世代作废 / 时段变化 / 切聊都会令提交失效。
  时段 = 4;
  assert.equal(await 写实时手机已读({ 会话: '101' }), true);
  const 复核 = 写库调用.允许写入;
  作废当前手机时间线租约世代();
...
test('chats/共享 UI 只走数据层实时入口，不再裸拼已读锚；入口经数据层导出', () => {
  assert.match(数据层源码, /export async function 写实时手机已读/);
  assert.match(渲染chats源码, /写实时手机已读\(\{ 会话:/);
  assert.doesNotMatch(渲染chats源码, /创建手机已读时锚/, '会话页不得再裸拼已读锚');
  assert.match(渲染共享源码, /写实时手机已读\(\{ 朋友圈: true \}\)/);
  assert.doesNotMatch(渲染共享源码, /创建手机已读时锚/, '朋友圈不得再裸拼已读锚');
});

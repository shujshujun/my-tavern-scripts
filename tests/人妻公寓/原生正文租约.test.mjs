...
const 作废等待票借由结束事件 = (票, 消息数) => {
  // 复刻 index.ts GENERATION_ENDED 等待分支的精确身份校验与作废路径，供“释放前台 token”回归。
  // 上层已校验 chat/时间线/用户楼层/引用相符；此处直接调纯判据包裹作废。
  if (等待票匹配结束事件(票, 消息数)) 作废原生正文租约();
};
...
test('等待票匹配结束事件：精确结束或 prompt 阶段提前失败都匹配本等待票，跨预期楼层的旧/错结束事件不命中', () => {
  重置原生正文租约();
  const 票 = 建票(); // user 12 → 预期助手楼层 13
  assert.equal(票.用户楼层, 12);
  assert.equal(票.预期助手楼层, 13);
  // 旧结束事件 messageCount = 12（= 用户楼层，低于预期助手楼层）：这是本等待票自己的 prompt
  // 阶段失败/中断结束，必须匹配以释放随票持有的共享前台生成槽（修复「内容正在生成」长期闩死）。
  assert.equal(等待票匹配结束事件(读原生正文开始票(), 12), true, 'prompt 阶段失败的提前结束 messageCount=12 必须匹配释放');
  assert.equal(读原生正文开始票()?.序号, 票.序号, '匹配的是本票自己');
  // 等待票自己的正常结束事件 messageCount = 13（= 预期助手楼层 = user 尾楼时的 chat.length）：匹配。
  assert.equal(等待票匹配结束事件(读原生正文开始票(), 13), true, '本票正常结束事件 messageCount=13 匹配');
  // 缺失/非整数/大于等于预期助手楼层的旧/错结束事件不命中，避免误清合法新票。
  ...
  // 0 等越界值：虽 < 预期助手楼层，但 messageCount=0 不可能对应本等待票（user 尾楼至少为 12），
  // 放宽判据只消“提前失败”语义而 0 不属于任何真实等待票的结束，仍不得误命中。
  assert.equal(等待票匹配结束事件(读原生正文开始票(), 14), false, '大于预期值不得匹配');
  assert.equal(等待票匹配结束事件(读原生正文开始票(), 0), false, '0 不得匹配（非本票结束）');
  assert.equal(等待票匹配结束事件(null, 13), false, '无票不得匹配');
  // messageCount 等于用户楼层（= 预期助手楼层 - 1）的提前失败结束必须匹配，这是关键回归点。
  assert.equal(等待票匹配结束事件(读原生正文开始票(), 13 - 1), true, '失败结束 messageCount=用户楼层 必须匹配释放');
  ...
  assert.equal(等待票匹配结束事件(读原生正文开始票(), 20), true, '新票的 prompt 阶段失败结束 messageCount=20 匹配');
  assert.equal(等待票匹配结束事件(读原生正文开始票(), 21), true, '新票自己的正常结束事件 21 匹配');
...
test('API 失败（GENERATION_ENDED 在 prompt 阶段携带 messageCount=用户楼层）经等待分支作废释放前台 token', () => {
  清空生成租约();
  重置原生正文租约();
  const 票 = 建票(); // user 12 → 预期助手楼层 13，登记同时持有共享前台 token
  assert.equal(前台生成租约持有中(), true, '登记开始票必须持有共享前台生成槽');
  // 模拟服务商在 prompt 阶段失败：只发 GENERATION_ENDED(messageCount=12=用户楼层)，不发 STOPPED。
  作废等待票借由结束事件(票, 12);
  assert.equal(读原生正文开始票(), null, 'prompt 阶段失败的等待票必须作废');
  assert.equal(前台生成租约持有中(), false, '作废必须交还共享前台生成槽，否则安全操作入口被长期闩死');
  assert.equal(读当前租约owner(), null);
});

test('等待分支的提前失败清票不得误清合法新票（messageCount >= 预期助手楼层 不命中）', () => {
  清空生成租约();
  重置原生正文租约();
  const 旧票 = 建票(); // user 12 → 预期助手楼层 13
  assert.equal(前台生成租约持有中(), true);
  // 一张旧结束事件 messageCount=13（等于旧票预期助手楼层但属于更早一轮的延迟结束）迟到：
  // 放宽判据只认 消息数 < 预期助手楼层 的提前失败，13 不在范围内，不得误清旧票。
  assert.equal(等待票匹配结束事件(读原生正文开始票(), 13), true, '13 属正常结束仍匹配（属本票自身）');
  assert.equal(读原生正文开始票()?.序号, 旧票.序号, '本票自身匹配后仍在');
  // 更关键：模拟“新票已替换旧票、旧结束事件 messageCount=13 迟到”的场景——此时新票预期助手楼层
  // 是 21，旧结束事件 13 < 21 看似命中，但上层已先对 chat/时间线/用户楼层/引用做校验过滤，旧票
  // 引用校验不过会直接 return，不会走到此处；此处仅验证判据本身对“>= 预期助手楼层”严格不命中。
  重置原生正文租约();
  const 新票 = 建票({ 用户楼层: 20 }); // → 预期助手楼层 21
  assert.equal(等待票匹配结束事件(读原生正文开始票(), 21 + 1), false, '超过预期助手楼层绝不命中');
});

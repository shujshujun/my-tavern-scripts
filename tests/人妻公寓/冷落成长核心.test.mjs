...
test('余波楼数按减半公式：6时段=3楼、18时段=5楼、超长封顶9楼', () => {
  assert.equal(计算余波所需楼数(6), 3);
  assert.equal(计算余波所需楼数(12), 4);
  assert.equal(计算余波所需楼数(18), 5);
  assert.equal(计算余波所需楼数(24), 6);
  assert.equal(计算余波所需楼数(36), 8);
  assert.equal(计算余波所需楼数(999), 9);
});

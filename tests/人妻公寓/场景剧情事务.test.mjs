...
test('连续锁场是结构化状态语义：等待后续拍在目标地点必须硬锁移动，普通等待票仍可确认离开', () => {
  const { 追加等待场景剧情, 读取场景剧情状态, 场景剧情连续锁场 } = 载入纯函数();
  const 连续 = 建数据();
  追加等待场景剧情(连续, '【场景剧情连续锁场】【场景剧情需回应】继续本段', '302', '连续现场');
  const 连续状态 = 读取场景剧情状态(连续);
  assert.equal(场景剧情连续锁场('【场景剧情连续锁场】继续本段'), true);
  assert.equal(连续状态?.连续锁场, true);
  assert.equal(连续状态?.需要玩家回应, true);

  const 普通 = 建数据();
  追加等待场景剧情(普通, '【普通预约】稍后处理', '302', '普通预约');
  assert.equal(读取场景剧情状态(普通)?.连续锁场, false);

  assert.match(App源码, /const 场景剧情连续锁场 = computed/);
  assert.match(App源码, /\|\|\s*场景剧情连续锁场\.value/);
  assert.match(App源码, /连续现场尚未完成|连续剧情尚未完成/);
});
...
test('旧存档积压时只把唯一匹配的活动事务移回队首，其他失败票和后续票原顺序保留', () => {
  const {
    激活新增场景剧情,
    追加等待场景剧情,
    恢复活动场景剧情队首,
  ...
  assert.equal(激活.成功, true);
  const 活动内容 = 激活.事务.内容;
  const 后续一 = 追加等待场景剧情(data, '【阶段预约】后来的预约', '101', '阶段预约').内容;
  const 后续二 = 追加等待场景剧情(data, '【丈夫登门】更晚的剧情', '101', '丈夫登门').内容;
  data.系统._待发送事件 = [后续一, 活动内容, 后续二].join('|');

  assert.equal(恢复活动场景剧情队首(data), '已恢复');
  assert.equal(读取队首场景剧情(data.系统._待发送事件)?.id, 激活.事务.id);
  assert.deepEqual(读取待发送事件队列(data.系统._待发送事件), [
    ...读取待发送事件队列(活动内容),
    ...读取待发送事件队列(后续一),
    ...读取待发送事件队列(后续二),
  ]);
...
test('活动事务正文已经遗失或出现多个冲突副本时拒绝猜测，不删除任何积压票', () => {
  const { 激活新增场景剧情, 追加等待场景剧情, 恢复活动场景剧情队首 } = 载入纯函数();
  const data = 建数据();
  const 激活 = 激活新增场景剧情(data, {
    标题: '遗失活动票',
    目标场景: '101',
  ...
  assert.equal(激活.成功, true);
  const 其他 = 追加等待场景剧情(data, '【其他剧情】不能冒充', '101', '其他剧情').内容;
  data.系统._待发送事件 = 其他;
  const 原队列 = data.系统._待发送事件;
  assert.equal(恢复活动场景剧情队首(data), '不可恢复');
  assert.equal(data.系统._待发送事件, 原队列);
});
